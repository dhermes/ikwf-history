-- Copyright (c) 2025 - Present. IKWF History. All rights reserved.

-- Generated by `write_sql.py`

PRAGMA foreign_keys = ON;
PRAGMA encoding = 'UTF-8';
PRAGMA integrity_check;

--------------------------------------------------------------------------------

----------------------------------------
-- 815 Stateline WC

UPDATE
  team
SET
  name_normalized = '815 Stateline WC',
  url_path_slug = '815-stateline'
WHERE
  id = 13350;

UPDATE
  tournament_team
SET
  team_id = 13350
WHERE
  id IN (13350, 13620, 13890, 14160, 14430, 14700, 14970, 15240);

DELETE FROM
  team
WHERE
  id IN (13620, 13890, 14160, 14430, 14700, 14970, 15240);

----------------------------------------
-- A & M Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'A & M Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 7302;

UPDATE
  tournament_team
SET
  team_id = 7302
WHERE
  id IN (7302, 7490, 8058, 8245, 8432, 8623);

DELETE FROM
  team
WHERE
  id IN (7490, 8058, 8245, 8432, 8623);

----------------------------------------
-- AJ Junior Wildcats WC

UPDATE
  team
SET
  name_normalized = 'AJ Junior Wildcats WC',
  url_path_slug = 'aj-jr-wildcats'
WHERE
  id = 3569;

UPDATE
  tournament_team
SET
  team_id = 3569
WHERE
  id IN (3569, 4300, 4468, 5254, 5400, 6194, 6378, 6562, 6744, 7303, 7491, 7678, 7868, 8059, 8246, 8433, 8624, 8814, 9005, 9196, 9371, 9546, 9713, 9880, 10057, 10234, 10405, 10576, 10759, 10942, 11104, 11590, 11803, 12016, 12229, 12442, 12669, 12896, 13123, 13351, 13621, 13891, 14161, 14431, 14701, 14971, 15241);

DELETE FROM
  team
WHERE
  id IN (4300, 4468, 5254, 5400, 6194, 6378, 6562, 6744, 7303, 7491, 7678, 7868, 8059, 8246, 8433, 8624, 8814, 9005, 9196, 9371, 9546, 9713, 9880, 10057, 10234, 10405, 10576, 10759, 10942, 11104, 11590, 11803, 12016, 12229, 12442, 12669, 12896, 13123, 13351, 13621, 13891, 14161, 14431, 14701, 14971, 15241);

----------------------------------------
-- Academy Grapplers

UPDATE
  team
SET
  name_normalized = 'Academy Grapplers',
  url_path_slug = NULL
WHERE
  id = 6195;

UPDATE
  tournament_team
SET
  team_id = 6195
WHERE
  id IN (6195, 6379, 6563, 6745);

DELETE FROM
  team
WHERE
  id IN (6379, 6563, 6745);

----------------------------------------
-- Aces Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Aces Wrestling Club',
  url_path_slug = 'aces-wc'
WHERE
  id = 3685;

UPDATE
  tournament_team
SET
  team_id = 3685
WHERE
  id IN (3685, 4044, 4168, 4301, 4469, 5255, 5550, 5690, 5836, 6015, 6196, 6380, 6564, 6746, 6926, 7114, 7304, 7492, 7679, 7869, 8060, 8247, 8434, 8625, 8816, 9007, 9197, 9372, 9547, 9714, 9881, 10058, 10943, 11105, 11591, 11804, 12017, 12230, 12443, 12670, 12897, 13124, 13352, 13622, 13892, 14162, 14432, 14702, 14972, 15242);

DELETE FROM
  team
WHERE
  id IN (4044, 4168, 4301, 4469, 5255, 5550, 5690, 5836, 6015, 6196, 6380, 6564, 6746, 6926, 7114, 7304, 7492, 7679, 7869, 8060, 8247, 8434, 8625, 8816, 9007, 9197, 9372, 9547, 9714, 9881, 10058, 10943, 11105, 11591, 11804, 12017, 12230, 12443, 12670, 12897, 13124, 13352, 13622, 13892, 14162, 14432, 14702, 14972, 15242);

----------------------------------------
-- Addams Junior High WC

UPDATE
  team
SET
  name_normalized = 'Addams Junior High WC',
  url_path_slug = 'addams-jh'
WHERE
  id = 1416;

UPDATE
  tournament_team
SET
  team_id = 1416
WHERE
  id IN (1416, 1541, 1668);

DELETE FROM
  team
WHERE
  id IN (1541, 1668);

----------------------------------------
-- Addison Indian Trail

UPDATE
  team
SET
  name_normalized = 'Addison Indian Trail',
  url_path_slug = NULL
WHERE
  id = 172;

UPDATE
  tournament_team
SET
  team_id = 172
WHERE
  id IN (172, 1800, 2505);

DELETE FROM
  team
WHERE
  id IN (1800, 2505);

----------------------------------------
-- Addison Street WC

UPDATE
  team
SET
  name_normalized = 'Addison Street WC',
  url_path_slug = NULL
WHERE
  id = 10577;

UPDATE
  tournament_team
SET
  team_id = 10577
WHERE
  id IN (10577, 10760);

DELETE FROM
  team
WHERE
  id IN (10760);

----------------------------------------
-- Alber Athletics Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Alber Athletics Wrestling Club',
  url_path_slug = 'alber-athletics'
WHERE
  id = 10944;

UPDATE
  tournament_team
SET
  team_id = 10944
WHERE
  id IN (10944, 11106, 11266, 11428, 11592, 11805, 12018, 12231, 12444, 12671, 12898, 13125, 13353, 13623, 13893, 14163, 14433, 14703, 14973, 15243);

DELETE FROM
  team
WHERE
  id IN (11106, 11266, 11428, 11592, 11805, 12018, 12231, 12444, 12671, 12898, 13125, 13353, 13623, 13893, 14163, 14433, 14703, 14973, 15243);

----------------------------------------
-- Aledo Bear Country WC

UPDATE
  team
SET
  name_normalized = 'Aledo Bear Country WC',
  url_path_slug = 'aledo-bear-country'
WHERE
  id = 3688;

UPDATE
  tournament_team
SET
  team_id = 3688
WHERE
  id IN (3688, 4045, 4170, 4636, 4764, 4907, 5080, 5256, 5551, 6197, 6381, 7685, 7875);

DELETE FROM
  team
WHERE
  id IN (4045, 4170, 4636, 4764, 4907, 5080, 5256, 5551, 6197, 6381, 7685, 7875);

----------------------------------------
-- Alleman Jr Pioneer WC

UPDATE
  team
SET
  name_normalized = 'Alleman Jr Pioneer WC',
  url_path_slug = 'alleman'
WHERE
  id = 4765;

UPDATE
  tournament_team
SET
  team_id = 4765
WHERE
  id IN (4765, 5257, 5691, 5837, 6016, 6198, 6382);

DELETE FROM
  team
WHERE
  id IN (5257, 5691, 5837, 6016, 6198, 6382);

----------------------------------------
-- Alton Little Redbirds WC

UPDATE
  team
SET
  name_normalized = 'Alton Little Redbirds WC',
  url_path_slug = 'alton-redbirds'
WHERE
  id = 6565;

UPDATE
  tournament_team
SET
  team_id = 6565
WHERE
  id IN (6565, 6747, 10578, 10761, 10945, 11107, 11593, 11806, 12019, 12232, 13354, 13624, 13894, 14164, 14434, 14704, 14974, 15244);

DELETE FROM
  team
WHERE
  id IN (6747, 10578, 10761, 10945, 11107, 11593, 11806, 12019, 12232, 13354, 13624, 13894, 14164, 14434, 14704, 14974, 15244);

----------------------------------------
-- Animal WC

UPDATE
  team
SET
  name_normalized = 'Animal WC',
  url_path_slug = NULL
WHERE
  id = 5838;

UPDATE
  tournament_team
SET
  team_id = 5838
WHERE
  id IN (5838, 6017, 6199, 6383, 6566, 6748);

DELETE FROM
  team
WHERE
  id IN (6017, 6199, 6383, 6566, 6748);

----------------------------------------
-- Animals WC

UPDATE
  team
SET
  name_normalized = 'Animals WC',
  url_path_slug = NULL
WHERE
  id = 4908;

UPDATE
  tournament_team
SET
  team_id = 4908
WHERE
  id IN (4908, 5081, 5258, 5401, 5692);

DELETE FROM
  team
WHERE
  id IN (5081, 5258, 5401, 5692);

----------------------------------------
-- Ano Kato Fs.Gr.

UPDATE
  team
SET
  name_normalized = 'Ano Kato Fs.Gr.',
  url_path_slug = NULL
WHERE
  id = 10235;

UPDATE
  tournament_team
SET
  team_id = 10235
WHERE
  id IN (10235, 10406);

DELETE FROM
  team
WHERE
  id IN (10406);

----------------------------------------
-- Antioch Predators WC

UPDATE
  team
SET
  name_normalized = 'Antioch Predators WC',
  url_path_slug = NULL
WHERE
  id = 11594;

UPDATE
  tournament_team
SET
  team_id = 11594
WHERE
  id IN (11594, 11807, 12020, 12233, 12445, 12672, 12899, 13126, 13355, 13625, 13895, 14165, 14435, 14705, 14975, 15245);

DELETE FROM
  team
WHERE
  id IN (11807, 12020, 12233, 12445, 12672, 12899, 13126, 13355, 13625, 13895, 14165, 14435, 14705, 14975, 15245);

----------------------------------------
-- Antioch Upper Grade School

UPDATE
  team
SET
  name_normalized = 'Antioch Upper Grade School',
  url_path_slug = 'antioch-upper'
WHERE
  id = 336;

UPDATE
  tournament_team
SET
  team_id = 336
WHERE
  id IN (336, 442, 899, 1011, 1417, 1671);

DELETE FROM
  team
WHERE
  id IN (442, 899, 1011, 1417, 1671);

----------------------------------------
-- Apex Stallions

UPDATE
  team
SET
  name_normalized = 'Apex Stallions',
  url_path_slug = NULL
WHERE
  id = 10236;

UPDATE
  tournament_team
SET
  team_id = 10236
WHERE
  id IN (10236, 10407);

DELETE FROM
  team
WHERE
  id IN (10407);

----------------------------------------
-- Argenta-Oreana Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Argenta-Oreana Kids Wrestling Club',
  url_path_slug = 'argenta-oreana'
WHERE
  id = 1418;

UPDATE
  tournament_team
SET
  team_id = 1418
WHERE
  id IN (1418, 1544, 1672, 1946, 3570, 3802, 3928, 4043, 4167, 4302, 4470, 4637, 4766, 4909, 5082, 5402, 5693, 5839, 6018, 6927, 7115, 9882, 10059, 11267, 11429);

DELETE FROM
  team
WHERE
  id IN (1544, 1672, 1946, 3570, 3802, 3928, 4043, 4167, 4302, 4470, 4637, 4766, 4909, 5082, 5402, 5693, 5839, 6018, 6927, 7115, 9882, 10059, 11267, 11429);

----------------------------------------
-- Arlington Cardinals Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Arlington Cardinals Wrestling Club',
  url_path_slug = 'arlington-cardinals'
WHERE
  id = 1012;

UPDATE
  tournament_team
SET
  team_id = 1012
WHERE
  id IN (1012, 1128, 1352, 1419, 1545, 1673, 4046, 4303, 4471, 4638, 4767, 4910, 5083, 5259, 5403, 5552, 5694, 5840, 6019, 6200, 6384, 6567, 6749, 6928, 7116, 7305, 7493, 7680, 7870, 8061, 8248, 8435, 8626, 8817, 9008, 9198, 9373, 9548, 9715, 9883, 10060, 10237, 10408, 10579, 10762, 10946, 11108, 11268, 11430, 11595, 11808, 12021, 12234, 12446, 12673, 12900, 13127, 13356, 13626, 13896, 14166, 14436, 14706, 14976, 15246);

DELETE FROM
  team
WHERE
  id IN (1128, 1352, 1419, 1545, 1673, 4046, 4303, 4471, 4638, 4767, 4910, 5083, 5259, 5403, 5552, 5694, 5840, 6019, 6200, 6384, 6567, 6749, 6928, 7116, 7305, 7493, 7680, 7870, 8061, 8248, 8435, 8626, 8817, 9008, 9198, 9373, 9548, 9715, 9883, 10060, 10237, 10408, 10579, 10762, 10946, 11108, 11268, 11430, 11595, 11808, 12021, 12234, 12446, 12673, 12900, 13127, 13356, 13626, 13896, 14166, 14436, 14706, 14976, 15246);

----------------------------------------
-- Assumption Elite WC

UPDATE
  team
SET
  name_normalized = 'Assumption Elite WC',
  url_path_slug = NULL
WHERE
  id = 11596;

UPDATE
  tournament_team
SET
  team_id = 11596
WHERE
  id IN (11596, 11809, 12022, 12235, 12447, 12674, 12901, 13128, 13357, 13627, 13897, 14167, 14437, 14707, 14977, 15247);

DELETE FROM
  team
WHERE
  id IN (11809, 12022, 12235, 12447, 12674, 12901, 13128, 13357, 13627, 13897, 14167, 14437, 14707, 14977, 15247);

----------------------------------------
-- Astro Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Astro Wrestling Club',
  url_path_slug = 'astro'
WHERE
  id = 8062;

UPDATE
  tournament_team
SET
  team_id = 8062
WHERE
  id IN (8062, 8249, 8818, 9009, 10238, 10409, 10580, 10763, 10947, 11109, 11597, 11810, 12023, 12236, 12448, 12675, 12902, 13129, 13358, 13628, 13898, 14168, 14438, 14708, 14978, 15248);

DELETE FROM
  team
WHERE
  id IN (8249, 8818, 9009, 10238, 10409, 10580, 10763, 10947, 11109, 11597, 11810, 12023, 12236, 12448, 12675, 12902, 13129, 13358, 13628, 13898, 14168, 14438, 14708, 14978, 15248);

----------------------------------------
-- Attwarhawks Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Attwarhawks Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8815;

UPDATE
  tournament_team
SET
  team_id = 8815
WHERE
  id IN (8815, 9006);

DELETE FROM
  team
WHERE
  id IN (9006);

----------------------------------------
-- Aurora J-Hawks

UPDATE
  team
SET
  name_normalized = 'Aurora J-Hawks',
  url_path_slug = 'aurora-j-hawks'
WHERE
  id = 1353;

UPDATE
  tournament_team
SET
  team_id = 1353
WHERE
  id IN (1353, 1420, 1546, 1674);

DELETE FROM
  team
WHERE
  id IN (1420, 1546, 1674);

----------------------------------------
-- Aurora Simmons

UPDATE
  team
SET
  name_normalized = 'Aurora Simmons',
  url_path_slug = NULL
WHERE
  id = 145;

UPDATE
  tournament_team
SET
  team_id = 145
WHERE
  id IN (145, 205);

DELETE FROM
  team
WHERE
  id IN (205);

----------------------------------------
-- Aurora Tomcats

UPDATE
  team
SET
  name_normalized = 'Aurora Tomcats',
  url_path_slug = 'aurora-tomcats'
WHERE
  id = 901;

UPDATE
  tournament_team
SET
  team_id = 901
WHERE
  id IN (901, 1117, 1223, 1338, 1407, 1525, 1653, 1927, 2063, 2190, 2486, 2625, 2919, 3231, 4451, 4619, 4747, 5059, 5233, 5383, 5822, 5993, 6172, 6352, 6536, 6722, 6904, 7090, 7278, 7468, 7656, 8222, 8409, 8598, 8789, 9696, 9863, 10036, 10213, 10387, 10558, 10738, 10921);

DELETE FROM
  team
WHERE
  id IN (1117, 1223, 1338, 1407, 1525, 1653, 1927, 2063, 2190, 2486, 2625, 2919, 3231, 4451, 4619, 4747, 5059, 5233, 5383, 5822, 5993, 6172, 6352, 6536, 6722, 6904, 7090, 7278, 7468, 7656, 8222, 8409, 8598, 8789, 9696, 9863, 10036, 10213, 10387, 10558, 10738, 10921);

----------------------------------------
-- Aurora WC

UPDATE
  team
SET
  name_normalized = 'Aurora WC',
  url_path_slug = NULL
WHERE
  id = 10948;

UPDATE
  tournament_team
SET
  team_id = 10948
WHERE
  id IN (10948, 11110, 11269, 11431, 11598, 11811, 12024, 12237, 13359, 13629, 13899, 14169, 14439, 14709, 14979, 15249);

DELETE FROM
  team
WHERE
  id IN (11110, 11269, 11431, 11598, 11811, 12024, 12237, 13359, 13629, 13899, 14169, 14439, 14709, 14979, 15249);

----------------------------------------
-- Backyard Brawlers Midwest

UPDATE
  team
SET
  name_normalized = 'Backyard Brawlers Midwest',
  url_path_slug = NULL
WHERE
  id = 11599;

UPDATE
  tournament_team
SET
  team_id = 11599
WHERE
  id IN (11599, 11812, 12025, 12238, 12449, 12676, 12903, 13130, 13365, 13635, 13905, 14175, 14445, 14715, 14985, 15255);

DELETE FROM
  team
WHERE
  id IN (11812, 12025, 12238, 12449, 12676, 12903, 13130, 13365, 13635, 13905, 14175, 14445, 14715, 14985, 15255);

----------------------------------------
-- Backyard Brawlers North Wrestling

UPDATE
  team
SET
  name_normalized = 'Backyard Brawlers North Wrestling',
  url_path_slug = NULL
WHERE
  id = 12450;

UPDATE
  tournament_team
SET
  team_id = 12450
WHERE
  id IN (12450, 12677, 12904, 13131, 13366, 13636, 13906, 14176, 14446, 14716, 14986, 15256);

DELETE FROM
  team
WHERE
  id IN (12677, 12904, 13131, 13366, 13636, 13906, 14176, 14446, 14716, 14986, 15256);

----------------------------------------
-- Backyard Brawlers West Wrestling

UPDATE
  team
SET
  name_normalized = 'Backyard Brawlers West Wrestling',
  url_path_slug = NULL
WHERE
  id = 12451;

UPDATE
  tournament_team
SET
  team_id = 12451
WHERE
  id IN (12451, 12678, 12905, 13132);

DELETE FROM
  team
WHERE
  id IN (12678, 12905, 13132);

----------------------------------------
-- Bad Bass Wrestling

UPDATE
  team
SET
  name_normalized = 'Bad Bass Wrestling',
  url_path_slug = NULL
WHERE
  id = 8063;

UPDATE
  tournament_team
SET
  team_id = 8063
WHERE
  id IN (8063, 8250);

DELETE FROM
  team
WHERE
  id IN (8250);

----------------------------------------
-- Badger WC

UPDATE
  team
SET
  name_normalized = 'Badger WC',
  url_path_slug = NULL
WHERE
  id = 3803;

UPDATE
  tournament_team
SET
  team_id = 3803
WHERE
  id IN (3803, 3929, 4047, 4171, 4304, 4472, 4639, 4768, 4911, 5084, 5553, 5841, 6020, 6201, 6385, 6568, 6750, 6929, 7117, 7306, 7494, 7681, 7871, 8064, 8251, 8436, 8627, 9199, 9374, 9884, 10061, 10239, 10410, 10581, 10764, 11270, 11432, 11600, 11813, 12026, 12239, 12452, 12679, 12906, 13133, 13367, 13637, 13907, 14177, 14447, 14717, 14987, 15257);

DELETE FROM
  team
WHERE
  id IN (3929, 4047, 4171, 4304, 4472, 4639, 4768, 4911, 5084, 5553, 5841, 6020, 6201, 6385, 6568, 6750, 6929, 7117, 7306, 7494, 7681, 7871, 8064, 8251, 8436, 8627, 9199, 9374, 9884, 10061, 10239, 10410, 10581, 10764, 11270, 11432, 11600, 11813, 12026, 12239, 12452, 12679, 12906, 13133, 13367, 13637, 13907, 14177, 14447, 14717, 14987, 15257);

----------------------------------------
-- Badger Wc/B.E.T.A.

UPDATE
  team
SET
  name_normalized = 'Badger Wc/B.E.T.A.',
  url_path_slug = NULL
WHERE
  id = 9549;

UPDATE
  tournament_team
SET
  team_id = 9549
WHERE
  id IN (9549, 9716);

DELETE FROM
  team
WHERE
  id IN (9716);

----------------------------------------
-- Badger Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Badger Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 5260;

UPDATE
  tournament_team
SET
  team_id = 5260
WHERE
  id IN (5260, 5404, 8819, 9010);

DELETE FROM
  team
WHERE
  id IN (5404, 8819, 9010);

----------------------------------------
-- Barrington

UPDATE
  team
SET
  name_normalized = 'Barrington',
  url_path_slug = NULL
WHERE
  id = 443;

UPDATE
  tournament_team
SET
  team_id = 443
WHERE
  id IN (443, 461, 685, 795);

DELETE FROM
  team
WHERE
  id IN (461, 685, 795);

----------------------------------------
-- Barrington Broncos Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Barrington Broncos Wrestling Club',
  url_path_slug = 'barrington-broncos'
WHERE
  id = 902;

UPDATE
  tournament_team
SET
  team_id = 902
WHERE
  id IN (902, 4769, 4912, 5085, 5261, 5405, 5842, 6021, 6202, 6386, 6569, 6751, 6930, 7118, 7307, 7495, 7682, 7872, 8065, 8252, 8437, 8628, 8820, 9011, 9200, 9375, 9550, 9717, 9885, 10062, 10240, 10411, 10582, 10765, 10949, 11111, 11271, 11433, 11601, 11814, 12027, 12240, 12453, 12680, 12907, 13134, 13368, 13638, 13908, 14178, 14448, 14718, 14988, 15258);

DELETE FROM
  team
WHERE
  id IN (4769, 4912, 5085, 5261, 5405, 5842, 6021, 6202, 6386, 6569, 6751, 6930, 7118, 7307, 7495, 7682, 7872, 8065, 8252, 8437, 8628, 8820, 9011, 9200, 9375, 9550, 9717, 9885, 10062, 10240, 10411, 10582, 10765, 10949, 11111, 11271, 11433, 11601, 11814, 12027, 12240, 12453, 12680, 12907, 13134, 13368, 13638, 13908, 14178, 14448, 14718, 14988, 15258);

----------------------------------------
-- Bartlett Hawk WC

UPDATE
  team
SET
  name_normalized = 'Bartlett Hawk WC',
  url_path_slug = NULL
WHERE
  id = 3930;

UPDATE
  tournament_team
SET
  team_id = 3930
WHERE
  id IN (3930, 4305, 4473, 4770);

DELETE FROM
  team
WHERE
  id IN (4305, 4473, 4770);

----------------------------------------
-- Bartlett Jr Hawks

UPDATE
  team
SET
  name_normalized = 'Bartlett Jr Hawks',
  url_path_slug = NULL
WHERE
  id = 4913;

UPDATE
  tournament_team
SET
  team_id = 4913
WHERE
  id IN (4913, 5086, 6931, 7119, 7308, 7496, 7683, 7873);

DELETE FROM
  team
WHERE
  id IN (5086, 6931, 7119, 7308, 7496, 7683, 7873);

----------------------------------------
-- Batavia Pinners

UPDATE
  team
SET
  name_normalized = 'Batavia Pinners',
  url_path_slug = NULL
WHERE
  id = 3572;

UPDATE
  tournament_team
SET
  team_id = 3572
WHERE
  id IN (3572, 3687, 3804, 3931, 4048, 4172, 4306, 4474, 4640, 4771, 4914, 5087, 5263, 5406, 5695, 5843, 6022, 6203, 6387, 6570, 6752, 6932, 7120, 7309, 7497, 7684, 7874, 8066, 8253);

DELETE FROM
  team
WHERE
  id IN (3687, 3804, 3931, 4048, 4172, 4306, 4474, 4640, 4771, 4914, 5087, 5263, 5406, 5695, 5843, 6022, 6203, 6387, 6570, 6752, 6932, 7120, 7309, 7497, 7684, 7874, 8066, 8253);

----------------------------------------
-- Batavia Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Batavia Wrestling Club',
  url_path_slug = 'batavia-wc'
WHERE
  id = 462;

UPDATE
  tournament_team
SET
  team_id = 462
WHERE
  id IN (462, 903, 1014, 1947, 5554, 8438, 8629, 8821, 9012, 9201, 9376, 9551, 9718, 9886, 10063, 10241, 10412, 10583, 10766, 10950, 11112, 11272, 11434, 11602, 11815, 12028, 12241, 12454, 12681, 12908, 13135, 13369, 13639, 13909, 14179, 14449, 14719, 14989, 15259);

DELETE FROM
  team
WHERE
  id IN (903, 1014, 1947, 5554, 8438, 8629, 8821, 9012, 9201, 9376, 9551, 9718, 9886, 10063, 10241, 10412, 10583, 10766, 10950, 11112, 11272, 11434, 11602, 11815, 12028, 12241, 12454, 12681, 12908, 13135, 13369, 13639, 13909, 14179, 14449, 14719, 14989, 15259);

----------------------------------------
-- Beat The Streets Chicago-Avondale

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Avondale',
  url_path_slug = NULL
WHERE
  id = 11273;

UPDATE
  tournament_team
SET
  team_id = 11273
WHERE
  id IN (11273, 11435, 11603, 11816, 12029, 12242, 12455, 12682, 12909, 13136);

DELETE FROM
  team
WHERE
  id IN (11435, 11603, 11816, 12029, 12242, 12455, 12682, 12909, 13136);

----------------------------------------
-- Beat The Streets Chicago-Bellwood

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Bellwood',
  url_path_slug = NULL
WHERE
  id = 11274;

UPDATE
  tournament_team
SET
  team_id = 11274
WHERE
  id IN (11274, 11436);

DELETE FROM
  team
WHERE
  id IN (11436);

----------------------------------------
-- Beat The Streets Chicago-Hyde Park

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Hyde Park',
  url_path_slug = NULL
WHERE
  id = 11275;

UPDATE
  tournament_team
SET
  team_id = 11275
WHERE
  id IN (11275, 11437, 11604, 11817, 12030, 12243);

DELETE FROM
  team
WHERE
  id IN (11437, 11604, 11817, 12030, 12243);

----------------------------------------
-- Beat The Streets Chicago-Midway

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Midway',
  url_path_slug = NULL
WHERE
  id = 11276;

UPDATE
  tournament_team
SET
  team_id = 11276
WHERE
  id IN (11276, 11438, 11605, 11818, 12031, 12244, 12456, 12683, 12910, 13137);

DELETE FROM
  team
WHERE
  id IN (11438, 11605, 11818, 12031, 12244, 12456, 12683, 12910, 13137);

----------------------------------------
-- Beat The Streets Chicago-Oak Park

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Oak Park',
  url_path_slug = NULL
WHERE
  id = 11277;

UPDATE
  tournament_team
SET
  team_id = 11277
WHERE
  id IN (11277, 11439, 11606, 11819, 12032, 12245, 12457, 12684, 12911, 13138);

DELETE FROM
  team
WHERE
  id IN (11439, 11606, 11819, 12032, 12245, 12457, 12684, 12911, 13138);

----------------------------------------
-- Beat The Streets Chicago-Tri Taylor

UPDATE
  team
SET
  name_normalized = 'Beat The Streets Chicago-Tri Taylor',
  url_path_slug = NULL
WHERE
  id = 11607;

UPDATE
  tournament_team
SET
  team_id = 11607
WHERE
  id IN (11607, 11820, 12033, 12246);

DELETE FROM
  team
WHERE
  id IN (11820, 12033, 12246);

----------------------------------------
-- Beat The Streets-Chicago

UPDATE
  team
SET
  name_normalized = 'Beat The Streets-Chicago',
  url_path_slug = NULL
WHERE
  id = 9552;

UPDATE
  tournament_team
SET
  team_id = 9552
WHERE
  id IN (9552, 9719, 9887, 10064, 10242, 10413, 10584, 10767, 10951, 11113);

DELETE FROM
  team
WHERE
  id IN (9719, 9887, 10064, 10242, 10413, 10584, 10767, 10951, 11113);

----------------------------------------
-- Belleville Junior Hit Squad

UPDATE
  team
SET
  name_normalized = 'Belleville Junior Hit Squad',
  url_path_slug = NULL
WHERE
  id = 6933;

UPDATE
  tournament_team
SET
  team_id = 6933
WHERE
  id IN (6933, 7121, 7310, 7498);

DELETE FROM
  team
WHERE
  id IN (7121, 7310, 7498);

----------------------------------------
-- Belleville Little Devils Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Belleville Little Devils Wrestling Club',
  url_path_slug = 'belleville-devils'
WHERE
  id = 175;

UPDATE
  tournament_team
SET
  team_id = 175
WHERE
  id IN (175, 796, 904, 1015, 1130, 1355, 1421, 1602, 1675, 1868, 2282, 2360, 2643, 2941, 3093, 3574, 3805, 3932, 4049, 4173, 4307, 4475, 4641, 4772, 4915, 5088, 5264, 5407, 5555, 5696, 5844, 6023, 6204, 6388, 6571, 6753, 6934, 7122, 7311, 7499, 8822, 9013, 9202, 9377, 9553, 9720, 9888, 10065, 10243, 10414, 10585, 10768, 10952, 11114, 11278, 11440, 11608, 11821, 12034, 12247, 12458, 12685, 12912, 13139, 13370, 13640, 13910, 14180, 14450, 14720, 14990, 15260);

DELETE FROM
  team
WHERE
  id IN (796, 904, 1015, 1130, 1355, 1421, 1602, 1675, 1868, 2282, 2360, 2643, 2941, 3093, 3574, 3805, 3932, 4049, 4173, 4307, 4475, 4641, 4772, 4915, 5088, 5264, 5407, 5555, 5696, 5844, 6023, 6204, 6388, 6571, 6753, 6934, 7122, 7311, 7499, 8822, 9013, 9202, 9377, 9553, 9720, 9888, 10065, 10243, 10414, 10585, 10768, 10952, 11114, 11278, 11440, 11608, 11821, 12034, 12247, 12458, 12685, 12912, 13139, 13370, 13640, 13910, 14180, 14450, 14720, 14990, 15260);

----------------------------------------
-- Belvidere Bandits Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Belvidere Bandits Wrestling Club',
  url_path_slug = 'belividere-bandits'
WHERE
  id = 905;

UPDATE
  tournament_team
SET
  team_id = 905
WHERE
  id IN (905, 1016, 1131, 1415, 1422, 1548, 1676, 1948, 2644, 3094, 3575, 3690, 3806, 3933, 4050, 4174, 4308, 4476, 4642, 4916, 5089, 5408, 5556, 5845, 6024, 6205, 6389, 6572, 6754, 6935, 7123, 7312, 7500, 7686, 7876, 8823, 9014, 9203, 9378, 9554, 9721, 9889, 10066, 10244, 10415, 10586, 10769, 10953, 11115, 11279, 11441, 11609, 11822, 12035, 12248, 12459, 12686, 12913, 13140, 13371, 13641, 13911, 14181, 14451, 14721, 14991, 15261);

DELETE FROM
  team
WHERE
  id IN (1016, 1131, 1415, 1422, 1548, 1676, 1948, 2644, 3094, 3575, 3690, 3806, 3933, 4050, 4174, 4308, 4476, 4642, 4916, 5089, 5408, 5556, 5845, 6024, 6205, 6389, 6572, 6754, 6935, 7123, 7312, 7500, 7686, 7876, 8823, 9014, 9203, 9378, 9554, 9721, 9889, 10066, 10244, 10415, 10586, 10769, 10953, 11115, 11279, 11441, 11609, 11822, 12035, 12248, 12459, 12686, 12913, 13140, 13371, 13641, 13911, 14181, 14451, 14721, 14991, 15261);

----------------------------------------
-- Belvidere WC

UPDATE
  team
SET
  name_normalized = 'Belvidere WC',
  url_path_slug = NULL
WHERE
  id = 10245;

UPDATE
  tournament_team
SET
  team_id = 10245
WHERE
  id IN (10245, 10416, 10587, 10770);

DELETE FROM
  team
WHERE
  id IN (10416, 10587, 10770);

----------------------------------------
-- Bensenville

UPDATE
  team
SET
  name_normalized = 'Bensenville',
  url_path_slug = NULL
WHERE
  id = 465;

UPDATE
  tournament_team
SET
  team_id = 465
WHERE
  id IN (465, 682, 798, 906);

DELETE FROM
  team
WHERE
  id IN (682, 798, 906);

----------------------------------------
-- Benton WC

UPDATE
  team
SET
  name_normalized = 'Benton WC',
  url_path_slug = 'benton'
WHERE
  id = 3807;

UPDATE
  tournament_team
SET
  team_id = 3807
WHERE
  id IN (3807, 4051, 4175, 4309, 4477, 4643, 4773, 4917, 5090, 5409, 5557, 5697, 5846, 6025, 6206, 6390, 6573, 6755, 6936, 7124, 7313, 7501, 7687, 7877, 9204, 9379, 9555, 9722, 9890, 10067, 10246, 10417, 10588, 10771, 10954, 11116, 11610, 11823, 12036, 12249, 12460, 12687, 12914, 13141, 13372, 13642, 13912, 14182, 14452, 14722, 14992, 15262);

DELETE FROM
  team
WHERE
  id IN (4051, 4175, 4309, 4477, 4643, 4773, 4917, 5090, 5409, 5557, 5697, 5846, 6025, 6206, 6390, 6573, 6755, 6936, 7124, 7313, 7501, 7687, 7877, 9204, 9379, 9555, 9722, 9890, 10067, 10246, 10417, 10588, 10771, 10954, 11116, 11610, 11823, 12036, 12249, 12460, 12687, 12914, 13141, 13372, 13642, 13912, 14182, 14452, 14722, 14992, 15262);

----------------------------------------
-- Berta Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Berta Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 8439;

UPDATE
  tournament_team
SET
  team_id = 8439
WHERE
  id IN (8439, 8630);

DELETE FROM
  team
WHERE
  id IN (8630);

----------------------------------------
-- Bethalto Bulls WC

UPDATE
  team
SET
  name_normalized = 'Bethalto Bulls WC',
  url_path_slug = 'bethalto-bulls'
WHERE
  id = 799;

UPDATE
  tournament_team
SET
  team_id = 799
WHERE
  id IN (799, 907, 1018, 1133, 1356, 1423, 1549, 1677, 1805, 1949, 3577, 3692, 3808, 3934, 4052, 4176, 4310, 4478, 4644, 4774, 4918, 5091, 5265, 5410, 5558, 5698, 5847, 6026);

DELETE FROM
  team
WHERE
  id IN (907, 1018, 1133, 1356, 1423, 1549, 1677, 1805, 1949, 3577, 3692, 3808, 3934, 4052, 4176, 4310, 4478, 4644, 4774, 4918, 5091, 5265, 5410, 5558, 5698, 5847, 6026);

----------------------------------------
-- Bismarck-Henning Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Bismarck-Henning Youth Wrestling Club',
  url_path_slug = 'bismarck-henning'
WHERE
  id = 908;

UPDATE
  tournament_team
SET
  team_id = 908
WHERE
  id IN (908, 1019, 1134, 1357, 1424, 1550, 3693, 3809, 3935, 4053, 4177, 4311, 4479, 4775, 5092, 5266, 5699, 5848, 6027, 6207, 6391, 6574, 6756, 6937, 7125, 7314, 7502, 7688, 7878, 8067, 8254, 8440, 8631, 8824, 9015, 9556, 9723, 9891, 10068, 10247, 10418, 10589, 10772, 11611, 11824, 12037, 12250, 12461, 12688, 12915, 13142, 13373, 13643, 13913, 14183, 14453, 14723, 14993, 15263);

DELETE FROM
  team
WHERE
  id IN (1019, 1134, 1357, 1424, 1550, 3693, 3809, 3935, 4053, 4177, 4311, 4479, 4775, 5092, 5266, 5699, 5848, 6027, 6207, 6391, 6574, 6756, 6937, 7125, 7314, 7502, 7688, 7878, 8067, 8254, 8440, 8631, 8824, 9015, 9556, 9723, 9891, 10068, 10247, 10418, 10589, 10772, 11611, 11824, 12037, 12250, 12461, 12688, 12915, 13142, 13373, 13643, 13913, 14183, 14453, 14723, 14993, 15263);

----------------------------------------
-- Bison WC

UPDATE
  team
SET
  name_normalized = 'Bison WC',
  url_path_slug = NULL
WHERE
  id = 1288;

UPDATE
  tournament_team
SET
  team_id = 1288
WHERE
  id IN (1288, 4312, 4480, 4645, 4776, 4920, 5093, 5267, 5411, 5559, 5700, 5849, 6028, 6208, 6392, 6938, 7126, 7315, 7503, 7689, 7879, 8068, 8255, 8441, 8632, 11612, 11825, 12038, 12251);

DELETE FROM
  team
WHERE
  id IN (4312, 4480, 4645, 4776, 4920, 5093, 5267, 5411, 5559, 5700, 5849, 6028, 6208, 6392, 6938, 7126, 7315, 7503, 7689, 7879, 8068, 8255, 8441, 8632, 11612, 11825, 12038, 12251);

----------------------------------------
-- Black Kats WC

UPDATE
  team
SET
  name_normalized = 'Black Kats WC',
  url_path_slug = NULL
WHERE
  id = 5412;

UPDATE
  tournament_team
SET
  team_id = 5412
WHERE
  id IN (5412, 5850, 6029);

DELETE FROM
  team
WHERE
  id IN (5850, 6029);

----------------------------------------
-- Blackhawk Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Blackhawk Wrestling Club',
  url_path_slug = 'blackhawk'
WHERE
  id = 909;

UPDATE
  tournament_team
SET
  team_id = 909
WHERE
  id IN (909, 1021, 1135, 1425, 1551, 1679, 2649, 4313, 4481, 4646, 4777, 4921, 5094, 5268, 5413, 5560, 5701, 5851, 6030, 6209, 6393, 6575, 6757, 6939, 7127, 7316, 7504, 7690, 7880, 8069, 8256, 8442, 8633, 8825, 9016, 9557, 9724, 9892, 10069, 10248, 10419, 10590, 10773, 10955, 11117, 11280, 11442, 11613, 11826, 12039, 12252, 13374, 13644, 13914, 14184, 14454, 14724, 14994, 15264);

DELETE FROM
  team
WHERE
  id IN (1021, 1135, 1425, 1551, 1679, 2649, 4313, 4481, 4646, 4777, 4921, 5094, 5268, 5413, 5560, 5701, 5851, 6030, 6209, 6393, 6575, 6757, 6939, 7127, 7316, 7504, 7690, 7880, 8069, 8256, 8442, 8633, 8825, 9016, 9557, 9724, 9892, 10069, 10248, 10419, 10590, 10773, 10955, 11117, 11280, 11442, 11613, 11826, 12039, 12252, 13374, 13644, 13914, 14184, 14454, 14724, 14994, 15264);

----------------------------------------
-- Blazer Kids

UPDATE
  team
SET
  name_normalized = 'Blazer Kids',
  url_path_slug = NULL
WHERE
  id = 4778;

UPDATE
  tournament_team
SET
  team_id = 4778
WHERE
  id IN (4778, 4922, 5095);

DELETE FROM
  team
WHERE
  id IN (4922, 5095);

----------------------------------------
-- Bloomingdale Bears WC

UPDATE
  team
SET
  name_normalized = 'Bloomingdale Bears WC',
  url_path_slug = NULL
WHERE
  id = 13375;

UPDATE
  tournament_team
SET
  team_id = 13375
WHERE
  id IN (13375, 13645, 13915, 14185, 14455, 14725, 14995, 15265);

DELETE FROM
  team
WHERE
  id IN (13645, 13915, 14185, 14455, 14725, 14995, 15265);

----------------------------------------
-- Bloomington JHS

UPDATE
  team
SET
  name_normalized = 'Bloomington JHS',
  url_path_slug = NULL
WHERE
  id = 1023;

UPDATE
  tournament_team
SET
  team_id = 1023
WHERE
  id IN (1023, 1136);

DELETE FROM
  team
WHERE
  id IN (1136);

----------------------------------------
-- Bloomington Raider WC

UPDATE
  team
SET
  name_normalized = 'Bloomington Raider WC',
  url_path_slug = NULL
WHERE
  id = 4314;

UPDATE
  tournament_team
SET
  team_id = 4314
WHERE
  id IN (4314, 4482, 4647, 4923, 5096, 5269, 5414, 5561, 6576, 6758, 8070, 8257, 8443, 8634);

DELETE FROM
  team
WHERE
  id IN (4482, 4647, 4923, 5096, 5269, 5414, 5561, 6576, 6758, 8070, 8257, 8443, 8634);

----------------------------------------
-- Blue Crew WC

UPDATE
  team
SET
  name_normalized = 'Blue Crew WC',
  url_path_slug = NULL
WHERE
  id = 1129;

UPDATE
  tournament_team
SET
  team_id = 1129
WHERE
  id IN (1129, 1354, 1426, 1552, 7767, 7957, 9959, 10136, 10317, 10488, 10659, 10842, 11020, 11182, 11281, 11443, 11614, 11827, 12040, 12253, 12462, 12689, 12916, 13143);

DELETE FROM
  team
WHERE
  id IN (1354, 1426, 1552, 7767, 7957, 9959, 10136, 10317, 10488, 10659, 10842, 11020, 11182, 11281, 11443, 11614, 11827, 12040, 12253, 12462, 12689, 12916, 13143);

----------------------------------------
-- Blue Line Training Academy

UPDATE
  team
SET
  name_normalized = 'Blue Line Training Academy',
  url_path_slug = NULL
WHERE
  id = 10956;

UPDATE
  tournament_team
SET
  team_id = 10956
WHERE
  id IN (10956, 11118, 11282, 11444, 11615, 11828, 12041, 12254, 12463, 12690, 12917, 13144, 13376, 13646, 13916, 14186, 14456, 14726, 14996, 15266);

DELETE FROM
  team
WHERE
  id IN (11118, 11282, 11444, 11615, 11828, 12041, 12254, 12463, 12690, 12917, 13144, 13376, 13646, 13916, 14186, 14456, 14726, 14996, 15266);

----------------------------------------
-- Bolingbrook Junior Raiders WC

UPDATE
  team
SET
  name_normalized = 'Bolingbrook Junior Raiders WC',
  url_path_slug = 'bolingbrook'
WHERE
  id = 6940;

UPDATE
  tournament_team
SET
  team_id = 6940
WHERE
  id IN (6940, 7128, 7317, 7505, 7691, 7881, 8071, 8258, 8444, 8635, 8826, 9017, 9205, 9380, 9558, 9725, 9893, 10070, 10249, 10420, 10591, 10774, 10957, 11119, 11283, 11445, 11616, 11829, 12042, 12255, 13377, 13647, 13917, 14187, 14457, 14727, 14997, 15267);

DELETE FROM
  team
WHERE
  id IN (7128, 7317, 7505, 7691, 7881, 8071, 8258, 8444, 8635, 8826, 9017, 9205, 9380, 9558, 9725, 9893, 10070, 10249, 10420, 10591, 10774, 10957, 11119, 11283, 11445, 11616, 11829, 12042, 12255, 13377, 13647, 13917, 14187, 14457, 14727, 14997, 15267);

----------------------------------------
-- Bolingbrook Ward

UPDATE
  team
SET
  name_normalized = 'Bolingbrook Ward',
  url_path_slug = NULL
WHERE
  id = 330;

UPDATE
  tournament_team
SET
  team_id = 330
WHERE
  id IN (330, 436);

DELETE FROM
  team
WHERE
  id IN (436);

----------------------------------------
-- Boys & Girls Club Of Pekin

UPDATE
  team
SET
  name_normalized = 'Boys & Girls Club Of Pekin',
  url_path_slug = 'bg-pekin'
WHERE
  id = 911;

UPDATE
  tournament_team
SET
  team_id = 911
WHERE
  id IN (911, 1024, 1505, 1632, 2888, 3936, 4178, 4315, 4483, 5270, 5415, 6685, 6867, 7426, 7614, 7802, 7992, 8178, 8365, 8553, 8744, 8938, 9129, 9306, 9481, 9656, 9823, 9992, 10169, 10347, 10518, 10691, 10874, 11048, 11210, 11377, 11539, 11734, 11947, 12160, 12373, 12595, 12822, 13049, 13276, 13526, 13796, 14066, 14336, 14606, 14876, 15146, 15416);

DELETE FROM
  team
WHERE
  id IN (1024, 1505, 1632, 2888, 3936, 4178, 4315, 4483, 5270, 5415, 6685, 6867, 7426, 7614, 7802, 7992, 8178, 8365, 8553, 8744, 8938, 9129, 9306, 9481, 9656, 9823, 9992, 10169, 10347, 10518, 10691, 10874, 11048, 11210, 11377, 11539, 11734, 11947, 12160, 12373, 12595, 12822, 13049, 13276, 13526, 13796, 14066, 14336, 14606, 14876, 15146, 15416);

----------------------------------------
-- Bradley Bourbonnais Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Bradley Bourbonnais Wrestling Club',
  url_path_slug = 'bradley-bourbonnais'
WHERE
  id = 800;

UPDATE
  tournament_team
SET
  team_id = 800
WHERE
  id IN (800, 912, 1427, 1553, 1953, 2369, 3100, 3579, 3694, 3810, 3937);

DELETE FROM
  team
WHERE
  id IN (912, 1427, 1553, 1953, 2369, 3100, 3579, 3694, 3810, 3937);

----------------------------------------
-- Brawlers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Brawlers Wrestling Club',
  url_path_slug = 'brawlers'
WHERE
  id = 4054;

UPDATE
  tournament_team
SET
  team_id = 4054
WHERE
  id IN (4054, 4179, 4316, 4484, 4648, 4779, 4924, 5097, 5271, 5416, 5562, 5852, 6031, 6210, 6394, 6577, 6759, 6941, 7129, 7318, 7506, 7692, 7882, 8072, 8259, 8445, 8636, 8827, 9018, 9206, 9381, 9559, 9726, 9894, 10071, 10250, 10421, 10592, 10775, 10958, 11120, 11617, 11830, 12043, 12256, 12464, 12691, 12918, 13145, 13378, 13648, 13918, 14188, 14458, 14728, 14998, 15268);

DELETE FROM
  team
WHERE
  id IN (4179, 4316, 4484, 4648, 4779, 4924, 5097, 5271, 5416, 5562, 5852, 6031, 6210, 6394, 6577, 6759, 6941, 7129, 7318, 7506, 7692, 7882, 8072, 8259, 8445, 8636, 8827, 9018, 9206, 9381, 9559, 9726, 9894, 10071, 10250, 10421, 10592, 10775, 10958, 11120, 11617, 11830, 12043, 12256, 12464, 12691, 12918, 13145, 13378, 13648, 13918, 14188, 14458, 14728, 14998, 15268);

----------------------------------------
-- Broadview Park District

UPDATE
  team
SET
  name_normalized = 'Broadview Park District',
  url_path_slug = NULL
WHERE
  id = 6578;

UPDATE
  tournament_team
SET
  team_id = 6578
WHERE
  id IN (6578, 6760);

DELETE FROM
  team
WHERE
  id IN (6760);

----------------------------------------
-- Bronco Wrestling

UPDATE
  team
SET
  name_normalized = 'Bronco Wrestling',
  url_path_slug = NULL
WHERE
  id = 3811;

UPDATE
  tournament_team
SET
  team_id = 3811
WHERE
  id IN (3811, 4055, 4317, 4485, 5853, 6032, 6211, 6395, 6579, 6761, 6942, 7130);

DELETE FROM
  team
WHERE
  id IN (4055, 4317, 4485, 5853, 6032, 6211, 6395, 6579, 6761, 6942, 7130);

----------------------------------------
-- Bts Chicago-Avondale

UPDATE
  team
SET
  name_normalized = 'Bts Chicago-Avondale',
  url_path_slug = NULL
WHERE
  id = 13360;

UPDATE
  tournament_team
SET
  team_id = 13360
WHERE
  id IN (13360, 13630, 13900, 14170, 14440, 14710, 14980, 15250);

DELETE FROM
  team
WHERE
  id IN (13630, 13900, 14170, 14440, 14710, 14980, 15250);

----------------------------------------
-- Bts Chicago-Midway

UPDATE
  team
SET
  name_normalized = 'Bts Chicago-Midway',
  url_path_slug = NULL
WHERE
  id = 13361;

UPDATE
  tournament_team
SET
  team_id = 13361
WHERE
  id IN (13361, 13631, 13901, 14171, 14441, 14711, 14981, 15251);

DELETE FROM
  team
WHERE
  id IN (13631, 13901, 14171, 14441, 14711, 14981, 15251);

----------------------------------------
-- Bts Chicago-Oak Park

UPDATE
  team
SET
  name_normalized = 'Bts Chicago-Oak Park',
  url_path_slug = NULL
WHERE
  id = 13362;

UPDATE
  tournament_team
SET
  team_id = 13362
WHERE
  id IN (13362, 13632, 13902, 14172, 14442, 14712, 14982, 15252);

DELETE FROM
  team
WHERE
  id IN (13632, 13902, 14172, 14442, 14712, 14982, 15252);

----------------------------------------
-- Bts Chicago-Roseland

UPDATE
  team
SET
  name_normalized = 'Bts Chicago-Roseland',
  url_path_slug = NULL
WHERE
  id = 13363;

UPDATE
  tournament_team
SET
  team_id = 13363
WHERE
  id IN (13363, 13633, 13903, 14173, 14443, 14713, 14983, 15253);

DELETE FROM
  team
WHERE
  id IN (13633, 13903, 14173, 14443, 14713, 14983, 15253);

----------------------------------------
-- Bts Chicago-Tri Taylor

UPDATE
  team
SET
  name_normalized = 'Bts Chicago-Tri Taylor',
  url_path_slug = NULL
WHERE
  id = 13364;

UPDATE
  tournament_team
SET
  team_id = 13364
WHERE
  id IN (13364, 13634, 13904, 14174, 14444, 14714, 14984, 15254);

DELETE FROM
  team
WHERE
  id IN (13634, 13904, 14174, 14444, 14714, 14984, 15254);

----------------------------------------
-- Built By Brunson Wrestling

UPDATE
  team
SET
  name_normalized = 'Built By Brunson Wrestling',
  url_path_slug = 'built-by-brunson'
WHERE
  id = 11284;

UPDATE
  tournament_team
SET
  team_id = 11284
WHERE
  id IN (11284, 11446, 11618, 11831, 12044, 12257, 12465, 12692, 12919, 13146, 13379, 13649, 13919, 14189, 14459, 14729, 14999, 15269);

DELETE FROM
  team
WHERE
  id IN (11446, 11618, 11831, 12044, 12257, 12465, 12692, 12919, 13146, 13379, 13649, 13919, 14189, 14459, 14729, 14999, 15269);

----------------------------------------
-- Bulldog Elite WC

UPDATE
  team
SET
  name_normalized = 'Bulldog Elite WC',
  url_path_slug = NULL
WHERE
  id = 10959;

UPDATE
  tournament_team
SET
  team_id = 10959
WHERE
  id IN (10959, 11121, 11285, 11447, 11619, 11832, 12045, 12258, 12466, 12693, 12920, 13147, 13380, 13650, 13920, 14190, 14460, 14730, 15000, 15270);

DELETE FROM
  team
WHERE
  id IN (11121, 11285, 11447, 11619, 11832, 12045, 12258, 12466, 12693, 12920, 13147, 13380, 13650, 13920, 14190, 14460, 14730, 15000, 15270);

----------------------------------------
-- Bulldog WC

UPDATE
  team
SET
  name_normalized = 'Bulldog WC',
  url_path_slug = NULL
WHERE
  id = 6212;

UPDATE
  tournament_team
SET
  team_id = 6212
WHERE
  id IN (6212, 6396, 7319, 7507, 7693, 7883, 8446, 8637);

DELETE FROM
  team
WHERE
  id IN (6396, 7319, 7507, 7693, 7883, 8446, 8637);

----------------------------------------
-- Bulldog Youth Sports Wrestling

UPDATE
  team
SET
  name_normalized = 'Bulldog Youth Sports Wrestling',
  url_path_slug = NULL
WHERE
  id = 12467;

UPDATE
  tournament_team
SET
  team_id = 12467
WHERE
  id IN (12467, 12694, 12921, 13148, 13381, 13651, 13921, 14191, 14461, 14731, 15001, 15271);

DELETE FROM
  team
WHERE
  id IN (12694, 12921, 13148, 13381, 13651, 13921, 14191, 14461, 14731, 15001, 15271);

----------------------------------------
-- Bulls WC

UPDATE
  team
SET
  name_normalized = 'Bulls WC',
  url_path_slug = NULL
WHERE
  id = 6213;

UPDATE
  tournament_team
SET
  team_id = 6213
WHERE
  id IN (6213, 6397, 6580, 6762, 6943, 7131, 7320, 7508, 7694, 7884, 8073, 8260, 8447, 8638, 9207, 9382, 9560, 9727, 9895, 10072, 10251, 10422, 10593, 10776, 10960, 11122, 11286, 11448, 11620, 11833, 12046, 12259, 12468, 12695, 12922, 13149, 13382, 13652, 13922, 14192, 14462, 14732, 15002, 15272);

DELETE FROM
  team
WHERE
  id IN (6397, 6580, 6762, 6943, 7131, 7320, 7508, 7694, 7884, 8073, 8260, 8447, 8638, 9207, 9382, 9560, 9727, 9895, 10072, 10251, 10422, 10593, 10776, 10960, 11122, 11286, 11448, 11620, 11833, 12046, 12259, 12468, 12695, 12922, 13149, 13382, 13652, 13922, 14192, 14462, 14732, 15002, 15272);

----------------------------------------
-- Bulls Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Bulls Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8828;

UPDATE
  tournament_team
SET
  team_id = 8828
WHERE
  id IN (8828, 9019);

DELETE FROM
  team
WHERE
  id IN (9019);

----------------------------------------
-- Bur-Oak Wildcats WC

UPDATE
  team
SET
  name_normalized = 'Bur-Oak Wildcats WC',
  url_path_slug = NULL
WHERE
  id = 5854;

UPDATE
  tournament_team
SET
  team_id = 5854
WHERE
  id IN (5854, 6033, 6214, 6398);

DELETE FROM
  team
WHERE
  id IN (6033, 6214, 6398);

----------------------------------------
-- Burbank Titans

UPDATE
  team
SET
  name_normalized = 'Burbank Titans',
  url_path_slug = NULL
WHERE
  id = 8448;

UPDATE
  tournament_team
SET
  team_id = 8448
WHERE
  id IN (8448, 8639);

DELETE FROM
  team
WHERE
  id IN (8639);

----------------------------------------
-- Cahokia WC

UPDATE
  team
SET
  name_normalized = 'Cahokia WC',
  url_path_slug = NULL
WHERE
  id = 1027;

UPDATE
  tournament_team
SET
  team_id = 1027
WHERE
  id IN (1027, 1138);

DELETE FROM
  team
WHERE
  id IN (1138);

----------------------------------------
-- Callan Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Callan Wrestling Academy',
  url_path_slug = 'callan'
WHERE
  id = 10961;

UPDATE
  tournament_team
SET
  team_id = 10961
WHERE
  id IN (10961, 11123, 11287, 11449, 11622, 11835, 12048, 12261, 12469, 12696, 12923, 13150, 13383, 13653, 13923, 14193, 14463, 14733, 15003, 15273);

DELETE FROM
  team
WHERE
  id IN (11123, 11287, 11449, 11622, 11835, 12048, 12261, 12469, 12696, 12923, 13150, 13383, 13653, 13923, 14193, 14463, 14733, 15003, 15273);

----------------------------------------
-- Calumet City

UPDATE
  team
SET
  name_normalized = 'Calumet City',
  url_path_slug = NULL
WHERE
  id = 445;

UPDATE
  tournament_team
SET
  team_id = 445
WHERE
  id IN (445, 475, 574, 687, 1955);

DELETE FROM
  team
WHERE
  id IN (475, 574, 687, 1955);

----------------------------------------
-- Calumet Memorial Park District Wolverines

UPDATE
  team
SET
  name_normalized = 'Calumet Memorial Park District Wolverines',
  url_path_slug = 'calumet-wolverines'
WHERE
  id = 1028;

UPDATE
  tournament_team
SET
  team_id = 1028
WHERE
  id IN (1028, 1428, 1554, 1686, 5272, 5417, 5563, 5855, 6034, 6215, 6399, 6581, 6763, 6944, 7132, 7321, 7509, 8074, 8261, 8449, 8640, 8829, 9020);

DELETE FROM
  team
WHERE
  id IN (1428, 1554, 1686, 5272, 5417, 5563, 5855, 6034, 6215, 6399, 6581, 6763, 6944, 7132, 7321, 7509, 8074, 8261, 8449, 8640, 8829, 9020);

----------------------------------------
-- Camp Point Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Camp Point Youth Wrestling',
  url_path_slug = 'camp-point'
WHERE
  id = 4056;

UPDATE
  tournament_team
SET
  team_id = 4056
WHERE
  id IN (4056, 4318, 4486, 6216, 6400, 12470, 12697, 12924, 13151, 13384, 13654, 13924, 14194, 14464, 14734, 15004, 15274);

DELETE FROM
  team
WHERE
  id IN (4318, 4486, 6216, 6400, 12470, 12697, 12924, 13151, 13384, 13654, 13924, 14194, 14464, 14734, 15004, 15274);

----------------------------------------
-- Caravan Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Caravan Kids Wrestling Club',
  url_path_slug = 'caravan-kids'
WHERE
  id = 8830;

UPDATE
  tournament_team
SET
  team_id = 8830
WHERE
  id IN (8830, 9021, 9208, 9383, 10594, 10777, 11288, 11450, 11623, 11836, 12049, 12262, 12471, 12698, 12925, 13152, 13385, 13655, 13925, 14195, 14465, 14735, 15005, 15275);

DELETE FROM
  team
WHERE
  id IN (9021, 9208, 9383, 10594, 10777, 11288, 11450, 11623, 11836, 12049, 12262, 12471, 12698, 12925, 13152, 13385, 13655, 13925, 14195, 14465, 14735, 15005, 15275);

----------------------------------------
-- Carbondale Junior Spirits

UPDATE
  team
SET
  name_normalized = 'Carbondale Junior Spirits',
  url_path_slug = NULL
WHERE
  id = 5273;

UPDATE
  tournament_team
SET
  team_id = 5273
WHERE
  id IN (5273, 5418);

DELETE FROM
  team
WHERE
  id IN (5418);

----------------------------------------
-- Carbondale Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Carbondale Wrestling Club',
  url_path_slug = 'carbondale-wc'
WHERE
  id = 914;

UPDATE
  tournament_team
SET
  team_id = 914
WHERE
  id IN (914, 1029, 1358, 1429, 4057, 4319, 4487, 4649, 4780, 4925, 5098, 5564, 5702, 5856, 6035, 6217, 6401, 6582, 6764, 6945, 7133, 7322, 7510, 7695, 7885, 8075, 8262, 8450, 8641, 8831, 9022, 9209, 9384, 9561, 9728, 9896, 10073, 10252, 10423, 10595, 10778, 12472, 12699, 12926, 13153, 13386, 13656, 13926, 14196, 14466, 14736, 15006, 15276);

DELETE FROM
  team
WHERE
  id IN (1029, 1358, 1429, 4057, 4319, 4487, 4649, 4780, 4925, 5098, 5564, 5702, 5856, 6035, 6217, 6401, 6582, 6764, 6945, 7133, 7322, 7510, 7695, 7885, 8075, 8262, 8450, 8641, 8831, 9022, 9209, 9384, 9561, 9728, 9896, 10073, 10252, 10423, 10595, 10778, 12472, 12699, 12926, 13153, 13386, 13656, 13926, 14196, 14466, 14736, 15006, 15276);

----------------------------------------
-- Cardinals WC

UPDATE
  team
SET
  name_normalized = 'Cardinals WC',
  url_path_slug = NULL
WHERE
  id = 1030;

UPDATE
  tournament_team
SET
  team_id = 1030
WHERE
  id IN (1030, 12473, 12700, 12927, 13154, 13387, 13657, 13927, 14197, 14467, 14737, 15007, 15277);

DELETE FROM
  team
WHERE
  id IN (12473, 12700, 12927, 13154, 13387, 13657, 13927, 14197, 14467, 14737, 15007, 15277);

----------------------------------------
-- Carlinville Kids WC

UPDATE
  team
SET
  name_normalized = 'Carlinville Kids WC',
  url_path_slug = NULL
WHERE
  id = 4058;

UPDATE
  tournament_team
SET
  team_id = 4058
WHERE
  id IN (4058, 4320, 4488, 4650, 4926, 5099, 5274, 5857, 6036, 7323, 7511);

DELETE FROM
  team
WHERE
  id IN (4320, 4488, 4650, 4926, 5099, 5274, 5857, 6036, 7323, 7511);

----------------------------------------
-- Carlinville WC

UPDATE
  team
SET
  name_normalized = 'Carlinville WC',
  url_path_slug = NULL
WHERE
  id = 7696;

UPDATE
  tournament_team
SET
  team_id = 7696
WHERE
  id IN (7696, 7886, 8076, 8263, 8451, 8642, 9210, 9385, 9562, 9729, 9897, 10074, 11289, 11451, 11624, 11837, 12050, 12263, 12474, 12701, 12928, 13155, 13388, 13658, 13928, 14198, 14468, 14738, 15008, 15278);

DELETE FROM
  team
WHERE
  id IN (7886, 8076, 8263, 8451, 8642, 9210, 9385, 9562, 9729, 9897, 10074, 11289, 11451, 11624, 11837, 12050, 12263, 12474, 12701, 12928, 13155, 13388, 13658, 13928, 14198, 14468, 14738, 15008, 15278);

----------------------------------------
-- Carlinville Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Carlinville Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8832;

UPDATE
  tournament_team
SET
  team_id = 8832
WHERE
  id IN (8832, 9023);

DELETE FROM
  team
WHERE
  id IN (9023);

----------------------------------------
-- Carlyle WC

UPDATE
  team
SET
  name_normalized = 'Carlyle WC',
  url_path_slug = NULL
WHERE
  id = 9563;

UPDATE
  tournament_team
SET
  team_id = 9563
WHERE
  id IN (9563, 9730);

DELETE FROM
  team
WHERE
  id IN (9730);

----------------------------------------
-- Carmi Bulldog WC

UPDATE
  team
SET
  name_normalized = 'Carmi Bulldog WC',
  url_path_slug = NULL
WHERE
  id = 8833;

UPDATE
  tournament_team
SET
  team_id = 8833
WHERE
  id IN (8833, 9024, 9211, 9386);

DELETE FROM
  team
WHERE
  id IN (9024, 9211, 9386);

----------------------------------------
-- Carol Stream

UPDATE
  team
SET
  name_normalized = 'Carol Stream',
  url_path_slug = NULL
WHERE
  id = 18;

UPDATE
  tournament_team
SET
  team_id = 18
WHERE
  id IN (18, 446, 805);

DELETE FROM
  team
WHERE
  id IN (446, 805);

----------------------------------------
-- Carterville Jr WC

UPDATE
  team
SET
  name_normalized = 'Carterville Jr WC',
  url_path_slug = NULL
WHERE
  id = 9212;

UPDATE
  tournament_team
SET
  team_id = 9212
WHERE
  id IN (9212, 9387);

DELETE FROM
  team
WHERE
  id IN (9387);

----------------------------------------
-- Carthage WC

UPDATE
  team
SET
  name_normalized = 'Carthage WC',
  url_path_slug = NULL
WHERE
  id = 13389;

UPDATE
  tournament_team
SET
  team_id = 13389
WHERE
  id IN (13389, 13659, 13929, 14199, 14469, 14739, 15009, 15279);

DELETE FROM
  team
WHERE
  id IN (13659, 13929, 14199, 14469, 14739, 15009, 15279);

----------------------------------------
-- Cary-Grove Matmen

UPDATE
  team
SET
  name_normalized = 'Cary-Grove Matmen',
  url_path_slug = 'cary-grove-matmen'
WHERE
  id = 3812;

UPDATE
  tournament_team
SET
  team_id = 3812
WHERE
  id IN (3812, 3938, 4059, 4180, 4321, 4489, 4781, 4927, 5100, 5275, 5565, 5703, 5858, 6037, 6218, 6402, 6583, 6765, 6946, 7134, 7324, 7512, 7697, 7887, 8452, 8643, 8834, 9025, 9213, 9388, 9898, 10075);

DELETE FROM
  team
WHERE
  id IN (3938, 4059, 4180, 4321, 4489, 4781, 4927, 5100, 5275, 5565, 5703, 5858, 6037, 6218, 6402, 6583, 6765, 6946, 7134, 7324, 7512, 7697, 7887, 8452, 8643, 8834, 9025, 9213, 9388, 9898, 10075);

----------------------------------------
-- Catlin WC

UPDATE
  team
SET
  name_normalized = 'Catlin WC',
  url_path_slug = NULL
WHERE
  id = 916;

UPDATE
  tournament_team
SET
  team_id = 916
WHERE
  id IN (916, 1141);

DELETE FROM
  team
WHERE
  id IN (1141);

----------------------------------------
-- Celtic Elite

UPDATE
  team
SET
  name_normalized = 'Celtic Elite',
  url_path_slug = NULL
WHERE
  id = 8077;

UPDATE
  tournament_team
SET
  team_id = 8077
WHERE
  id IN (8077, 8264, 8453, 8644, 8835, 9026, 9214, 9389, 9564, 9731);

DELETE FROM
  team
WHERE
  id IN (8264, 8453, 8644, 8835, 9026, 9214, 9389, 9564, 9731);

----------------------------------------
-- Celtic Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Celtic Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 13390;

UPDATE
  tournament_team
SET
  team_id = 13390
WHERE
  id IN (13390, 13660, 13930, 14200, 14470, 14740, 15010, 15280);

DELETE FROM
  team
WHERE
  id IN (13660, 13930, 14200, 14470, 14740, 15010, 15280);

----------------------------------------
-- Celtics WC

UPDATE
  team
SET
  name_normalized = 'Celtics WC',
  url_path_slug = NULL
WHERE
  id = 9899;

UPDATE
  tournament_team
SET
  team_id = 9899
WHERE
  id IN (9899, 10076, 10253, 10424, 10596, 10779);

DELETE FROM
  team
WHERE
  id IN (10076, 10253, 10424, 10596, 10779);

----------------------------------------
-- Central Elite Rebels

UPDATE
  team
SET
  name_normalized = 'Central Elite Rebels',
  url_path_slug = NULL
WHERE
  id = 5859;

UPDATE
  tournament_team
SET
  team_id = 5859
WHERE
  id IN (5859, 6038, 6219, 6403, 6584, 6766, 6947, 7135, 7325, 7513, 7698, 7888, 8078, 8265, 8454, 8645, 8836, 9027);

DELETE FROM
  team
WHERE
  id IN (6038, 6219, 6403, 6584, 6766, 6947, 7135, 7325, 7513, 7698, 7888, 8078, 8265, 8454, 8645, 8836, 9027);

----------------------------------------
-- Central Illinois Academy Of Wrestling

UPDATE
  team
SET
  name_normalized = 'Central Illinois Academy Of Wrestling',
  url_path_slug = NULL
WHERE
  id = 10254;

UPDATE
  tournament_team
SET
  team_id = 10254
WHERE
  id IN (10254, 10425);

DELETE FROM
  team
WHERE
  id IN (10425);

----------------------------------------
-- Central Illinois Bulldogs WC

UPDATE
  team
SET
  name_normalized = 'Central Illinois Bulldogs WC',
  url_path_slug = NULL
WHERE
  id = 11625;

UPDATE
  tournament_team
SET
  team_id = 11625
WHERE
  id IN (11625, 11838, 12051, 12264, 12475, 12702, 12929, 13156, 13391, 13661, 13931, 14201, 14471, 14741, 15011, 15281);

DELETE FROM
  team
WHERE
  id IN (11838, 12051, 12264, 12475, 12702, 12929, 13156, 13391, 13661, 13931, 14201, 14471, 14741, 15011, 15281);

----------------------------------------
-- Central Illinois Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Central Illinois Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 7699;

UPDATE
  tournament_team
SET
  team_id = 7699
WHERE
  id IN (7699, 7889, 8079, 8266, 8455, 8646, 8837, 9028, 9215, 9390);

DELETE FROM
  team
WHERE
  id IN (7889, 8079, 8266, 8455, 8646, 8837, 9028, 9215, 9390);

----------------------------------------
-- Central Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Central Wrestling Club',
  url_path_slug = 'central-wc'
WHERE
  id = 4322;

UPDATE
  tournament_team
SET
  team_id = 4322
WHERE
  id IN (4322, 4490, 4928, 5101, 5566, 5860, 6039);

DELETE FROM
  team
WHERE
  id IN (4490, 4928, 5101, 5566, 5860, 6039);

----------------------------------------
-- Centralia WC

UPDATE
  team
SET
  name_normalized = 'Centralia WC',
  url_path_slug = 'centralia'
WHERE
  id = 1142;

UPDATE
  tournament_team
SET
  team_id = 1142
WHERE
  id IN (1142, 1432, 3699, 3939, 4181, 5419, 6948, 7136, 7326, 7514, 10255, 10426, 11626, 11839, 12052, 12265, 13392, 13662, 13932, 14202, 14472, 14742, 15012, 15282);

DELETE FROM
  team
WHERE
  id IN (1432, 3699, 3939, 4181, 5419, 6948, 7136, 7326, 7514, 10255, 10426, 11626, 11839, 12052, 12265, 13392, 13662, 13932, 14202, 14472, 14742, 15012, 15282);

----------------------------------------
-- Champaign Kids Wrestlings Club

UPDATE
  team
SET
  name_normalized = 'Champaign Kids Wrestlings Club',
  url_path_slug = 'champaign-kids'
WHERE
  id = 349;

UPDATE
  tournament_team
SET
  team_id = 349
WHERE
  id IN (349, 807, 917, 1032, 1959, 3700, 3813, 3940, 4060, 4182, 4323, 4491, 4651, 4782, 4929, 5102, 5277, 5420, 5567, 5704, 5861, 6040, 6220, 6404, 6585, 6767, 6949, 7137, 7327, 7515, 7700, 7890, 8080, 8267, 8456, 8647, 8838, 9029, 9216, 9391, 9565, 9732, 9900, 10077, 10256, 10427, 10597, 10780, 10962, 11124, 11290, 11452, 11627, 11840, 12053, 12266, 12476, 12703, 12930, 13157, 13393, 13663, 13933, 14203, 14473, 14743, 15013, 15283);

DELETE FROM
  team
WHERE
  id IN (807, 917, 1032, 1959, 3700, 3813, 3940, 4060, 4182, 4323, 4491, 4651, 4782, 4929, 5102, 5277, 5420, 5567, 5704, 5861, 6040, 6220, 6404, 6585, 6767, 6949, 7137, 7327, 7515, 7700, 7890, 8080, 8267, 8456, 8647, 8838, 9029, 9216, 9391, 9565, 9732, 9900, 10077, 10256, 10427, 10597, 10780, 10962, 11124, 11290, 11452, 11627, 11840, 12053, 12266, 12476, 12703, 12930, 13157, 13393, 13663, 13933, 14203, 14473, 14743, 15013, 15283);

----------------------------------------
-- Champbuilders WC

UPDATE
  team
SET
  name_normalized = 'Champbuilders WC',
  url_path_slug = 'champbuilders'
WHERE
  id = 6586;

UPDATE
  tournament_team
SET
  team_id = 6586
WHERE
  id IN (6586, 6768, 7328, 7516, 7701, 7891, 8081, 8268, 8457, 8648, 8839, 9030, 9217, 9392, 9566, 9733, 9901, 10078, 10257, 10428, 10598, 10781, 11291, 11453, 11628, 11841, 12054, 12267, 12477, 12704, 12931, 13158);

DELETE FROM
  team
WHERE
  id IN (6768, 7328, 7516, 7701, 7891, 8081, 8268, 8457, 8648, 8839, 9030, 9217, 9392, 9566, 9733, 9901, 10078, 10257, 10428, 10598, 10781, 11291, 11453, 11628, 11841, 12054, 12267, 12477, 12704, 12931, 13158);

----------------------------------------
-- Champions WC

UPDATE
  team
SET
  name_normalized = 'Champions WC',
  url_path_slug = NULL
WHERE
  id = 6587;

UPDATE
  tournament_team
SET
  team_id = 6587
WHERE
  id IN (6587, 6769, 6950, 7138, 7329, 7517, 7702, 7892, 8082, 8269, 8840, 9031, 9218, 9393, 12478, 12705, 12932, 13159, 13394, 13664, 13934, 14204, 14474, 14744, 15014, 15284);

DELETE FROM
  team
WHERE
  id IN (6769, 6950, 7138, 7329, 7517, 7702, 7892, 8082, 8269, 8840, 9031, 9218, 9393, 12478, 12705, 12932, 13159, 13394, 13664, 13934, 14204, 14474, 14744, 15014, 15284);

----------------------------------------
-- Charger WC

UPDATE
  team
SET
  name_normalized = 'Charger WC',
  url_path_slug = NULL
WHERE
  id = 7330;

UPDATE
  tournament_team
SET
  team_id = 7330
WHERE
  id IN (7330, 7518, 8083, 8270, 8841, 9032, 10258, 10429, 10599, 10782);

DELETE FROM
  team
WHERE
  id IN (7518, 8083, 8270, 8841, 9032, 10258, 10429, 10599, 10782);

----------------------------------------
-- Chargers WC

UPDATE
  team
SET
  name_normalized = 'Chargers WC',
  url_path_slug = NULL
WHERE
  id = 8458;

UPDATE
  tournament_team
SET
  team_id = 8458
WHERE
  id IN (8458, 8649);

DELETE FROM
  team
WHERE
  id IN (8649);

----------------------------------------
-- Charleston Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Charleston Wrestling Club',
  url_path_slug = 'charleston'
WHERE
  id = 1815;

UPDATE
  tournament_team
SET
  team_id = 1815
WHERE
  id IN (1815, 4183, 4324, 4492, 4783, 4930, 5103, 5278, 5705, 5862, 6041, 6221, 6405, 6951, 7139, 8084, 8271, 8459, 8650, 8842, 9033, 9219, 9394, 9567, 9734, 9902, 10079, 10259, 10430, 10963, 11125, 11292, 11454, 11629, 11842, 12055, 12268, 13395, 13665, 13935, 14205, 14475, 14745, 15015, 15285);

DELETE FROM
  team
WHERE
  id IN (4183, 4324, 4492, 4783, 4930, 5103, 5278, 5705, 5862, 6041, 6221, 6405, 6951, 7139, 8084, 8271, 8459, 8650, 8842, 9033, 9219, 9394, 9567, 9734, 9902, 10079, 10259, 10430, 10963, 11125, 11292, 11454, 11629, 11842, 12055, 12268, 13395, 13665, 13935, 14205, 14475, 14745, 15015, 15285);

----------------------------------------
-- Chatham WC

UPDATE
  team
SET
  name_normalized = 'Chatham WC',
  url_path_slug = NULL
WHERE
  id = 9220;

UPDATE
  tournament_team
SET
  team_id = 9220
WHERE
  id IN (9220, 9395, 9568, 9735, 13396, 13666, 13936, 14206, 14476, 14746, 15016, 15286);

DELETE FROM
  team
WHERE
  id IN (9395, 9568, 9735, 13396, 13666, 13936, 14206, 14476, 14746, 15016, 15286);

----------------------------------------
-- Chenoa Mat Cats

UPDATE
  team
SET
  name_normalized = 'Chenoa Mat Cats',
  url_path_slug = NULL
WHERE
  id = 3814;

UPDATE
  tournament_team
SET
  team_id = 3814
WHERE
  id IN (3814, 3941, 4061, 4184, 4325, 4493, 4652, 4784, 4931, 5104);

DELETE FROM
  team
WHERE
  id IN (3941, 4061, 4184, 4325, 4493, 4652, 4784, 4931, 5104);

----------------------------------------
-- Chicago Crusaders

UPDATE
  team
SET
  name_normalized = 'Chicago Crusaders',
  url_path_slug = NULL
WHERE
  id = 5568;

UPDATE
  tournament_team
SET
  team_id = 5568
WHERE
  id IN (5568, 5863, 6042, 7331, 7519);

DELETE FROM
  team
WHERE
  id IN (5863, 6042, 7331, 7519);

----------------------------------------
-- Chicago Crusaders WC

UPDATE
  team
SET
  name_normalized = 'Chicago Crusaders WC',
  url_path_slug = NULL
WHERE
  id = 8085;

UPDATE
  tournament_team
SET
  team_id = 8085
WHERE
  id IN (8085, 8272);

DELETE FROM
  team
WHERE
  id IN (8272);

----------------------------------------
-- Chicago Park District

UPDATE
  team
SET
  name_normalized = 'Chicago Park District',
  url_path_slug = NULL
WHERE
  id = 8086;

UPDATE
  tournament_team
SET
  team_id = 8086
WHERE
  id IN (8086, 8273, 9221, 9396, 9569, 9736, 9903, 10080, 10260, 10431, 10600, 10783);

DELETE FROM
  team
WHERE
  id IN (8273, 9221, 9396, 9569, 9736, 9903, 10080, 10260, 10431, 10600, 10783);

----------------------------------------
-- Chicago Ridge PD

UPDATE
  team
SET
  name_normalized = 'Chicago Ridge PD',
  url_path_slug = 'chicago-ridge'
WHERE
  id = 179;

UPDATE
  tournament_team
SET
  team_id = 179
WHERE
  id IN (179, 249, 350, 447, 478, 581, 808, 918);

DELETE FROM
  team
WHERE
  id IN (249, 350, 447, 478, 581, 808, 918);

----------------------------------------
-- Chicago Wolves Den

UPDATE
  team
SET
  name_normalized = 'Chicago Wolves Den',
  url_path_slug = NULL
WHERE
  id = 5569;

UPDATE
  tournament_team
SET
  team_id = 5569
WHERE
  id IN (5569, 5864, 6043, 6588, 6770, 6952, 7140);

DELETE FROM
  team
WHERE
  id IN (5864, 6043, 6588, 6770, 6952, 7140);

----------------------------------------
-- Chicago Wolves Den WC

UPDATE
  team
SET
  name_normalized = 'Chicago Wolves Den WC',
  url_path_slug = NULL
WHERE
  id = 8087;

UPDATE
  tournament_team
SET
  team_id = 8087
WHERE
  id IN (8087, 8274);

DELETE FROM
  team
WHERE
  id IN (8274);

----------------------------------------
-- Chilli Dawgs WC

UPDATE
  team
SET
  name_normalized = 'Chilli Dawgs WC',
  url_path_slug = 'chilli-dawgs'
WHERE
  id = 1033;

UPDATE
  tournament_team
SET
  team_id = 1033
WHERE
  id IN (1033, 1144, 1433, 3702, 3815, 3942, 4062, 4185, 4326, 4494, 4653, 4786, 4932, 5105, 5279, 5706, 5865, 6044, 6222, 6406, 6589, 6771, 6953, 7141, 7332, 7520, 7703, 7893, 8088, 8275);

DELETE FROM
  team
WHERE
  id IN (1144, 1433, 3702, 3815, 3942, 4062, 4185, 4326, 4494, 4653, 4786, 4932, 5105, 5279, 5706, 5865, 6044, 6222, 6406, 6589, 6771, 6953, 7141, 7332, 7520, 7703, 7893, 8088, 8275);

----------------------------------------
-- Cicc

UPDATE
  team
SET
  name_normalized = 'Cicc',
  url_path_slug = NULL
WHERE
  id = 11621;

UPDATE
  tournament_team
SET
  team_id = 11621
WHERE
  id IN (11621, 11834, 12047, 12260);

DELETE FROM
  team
WHERE
  id IN (11834, 12047, 12260);

----------------------------------------
-- Clinton WC

UPDATE
  team
SET
  name_normalized = 'Clinton WC',
  url_path_slug = NULL
WHERE
  id = 6590;

UPDATE
  tournament_team
SET
  team_id = 6590
WHERE
  id IN (6590, 6772, 8460, 8651, 10261, 10432, 10601, 10784, 12479, 12706, 12933, 13160, 13397, 13667, 13937, 14207, 14477, 14747, 15017, 15287);

DELETE FROM
  team
WHERE
  id IN (6772, 8460, 8651, 10261, 10432, 10601, 10784, 12479, 12706, 12933, 13160, 13397, 13667, 13937, 14207, 14477, 14747, 15017, 15287);

----------------------------------------
-- Clinton Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Clinton Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8843;

UPDATE
  tournament_team
SET
  team_id = 8843
WHERE
  id IN (8843, 9034);

DELETE FROM
  team
WHERE
  id IN (9034);

----------------------------------------
-- Clipper WC

UPDATE
  team
SET
  name_normalized = 'Clipper WC',
  url_path_slug = 'clipper'
WHERE
  id = 5280;

UPDATE
  tournament_team
SET
  team_id = 5280
WHERE
  id IN (5280, 6223, 6407, 6591, 6773, 6954, 7142, 7333, 7521, 10602, 10785, 10964, 11126, 11293, 11455, 11630, 11843, 12056, 12269, 12480, 12707, 12934, 13161, 13398, 13668, 13938, 14208, 14478, 14748, 15018, 15288);

DELETE FROM
  team
WHERE
  id IN (6223, 6407, 6591, 6773, 6954, 7142, 7333, 7521, 10602, 10785, 10964, 11126, 11293, 11455, 11630, 11843, 12056, 12269, 12480, 12707, 12934, 13161, 13398, 13668, 13938, 14208, 14478, 14748, 15018, 15288);

----------------------------------------
-- Club Attack

UPDATE
  team
SET
  name_normalized = 'Club Attack',
  url_path_slug = NULL
WHERE
  id = 6955;

UPDATE
  tournament_team
SET
  team_id = 6955
WHERE
  id IN (6955, 7143);

DELETE FROM
  team
WHERE
  id IN (7143);

----------------------------------------
-- Cogs WC

UPDATE
  team
SET
  name_normalized = 'Cogs WC',
  url_path_slug = NULL
WHERE
  id = 13399;

UPDATE
  tournament_team
SET
  team_id = 13399
WHERE
  id IN (13399, 13669, 13939, 14209, 14479, 14749, 15019, 15289);

DELETE FROM
  team
WHERE
  id IN (13669, 13939, 14209, 14479, 14749, 15019, 15289);

----------------------------------------
-- Collinsville Raider WC

UPDATE
  team
SET
  name_normalized = 'Collinsville Raider WC',
  url_path_slug = NULL
WHERE
  id = 7704;

UPDATE
  tournament_team
SET
  team_id = 7704
WHERE
  id IN (7704, 7894, 8089, 8276);

DELETE FROM
  team
WHERE
  id IN (7894, 8089, 8276);

----------------------------------------
-- Collinsville Raiders

UPDATE
  team
SET
  name_normalized = 'Collinsville Raiders',
  url_path_slug = NULL
WHERE
  id = 5281;

UPDATE
  tournament_team
SET
  team_id = 5281
WHERE
  id IN (5281, 5422, 5570, 5707, 5866, 6045, 6224, 6408, 6592, 6774, 6956, 7144, 7334, 7522);

DELETE FROM
  team
WHERE
  id IN (5422, 5570, 5707, 5866, 6045, 6224, 6408, 6592, 6774, 6956, 7144, 7334, 7522);

----------------------------------------
-- Collinsville WC

UPDATE
  team
SET
  name_normalized = 'Collinsville WC',
  url_path_slug = NULL
WHERE
  id = 11294;

UPDATE
  tournament_team
SET
  team_id = 11294
WHERE
  id IN (11294, 11456, 11631, 11844, 12057, 12270, 12481, 12708, 12935, 13162, 13400, 13670, 13940, 14210, 14480, 14750, 15020, 15290);

DELETE FROM
  team
WHERE
  id IN (11456, 11631, 11844, 12057, 12270, 12481, 12708, 12935, 13162, 13400, 13670, 13940, 14210, 14480, 14750, 15020, 15290);

----------------------------------------
-- Colts WC

UPDATE
  team
SET
  name_normalized = 'Colts WC',
  url_path_slug = NULL
WHERE
  id = 920;

UPDATE
  tournament_team
SET
  team_id = 920
WHERE
  id IN (920, 1034, 1145);

DELETE FROM
  team
WHERE
  id IN (1034, 1145);

----------------------------------------
-- Combative Sports Athletic Center

UPDATE
  team
SET
  name_normalized = 'Combative Sports Athletic Center',
  url_path_slug = NULL
WHERE
  id = 10262;

UPDATE
  tournament_team
SET
  team_id = 10262
WHERE
  id IN (10262, 10433, 10603, 10786, 10965, 11127);

DELETE FROM
  team
WHERE
  id IN (10433, 10603, 10786, 10965, 11127);

----------------------------------------
-- Combative Sports Athletic Center Wrestling

UPDATE
  team
SET
  name_normalized = 'Combative Sports Athletic Center Wrestling',
  url_path_slug = NULL
WHERE
  id = 11295;

UPDATE
  tournament_team
SET
  team_id = 11295
WHERE
  id IN (11295, 11457, 11632, 11845, 12058, 12271, 13401, 13671, 13941, 14211, 14481, 14751, 15021, 15291);

DELETE FROM
  team
WHERE
  id IN (11457, 11632, 11845, 12058, 12271, 13401, 13671, 13941, 14211, 14481, 14751, 15021, 15291);

----------------------------------------
-- Contender Wrestling

UPDATE
  team
SET
  name_normalized = 'Contender Wrestling',
  url_path_slug = NULL
WHERE
  id = 8461;

UPDATE
  tournament_team
SET
  team_id = 8461
WHERE
  id IN (8461, 8652, 8844, 9035, 9222, 9397, 9570, 9737, 9904, 10081, 10263, 10434, 10604, 10787, 11296, 11458);

DELETE FROM
  team
WHERE
  id IN (8652, 8844, 9035, 9222, 9397, 9570, 9737, 9904, 10081, 10263, 10434, 10604, 10787, 11296, 11458);

----------------------------------------
-- Cornerstone Wrestling

UPDATE
  team
SET
  name_normalized = 'Cornerstone Wrestling',
  url_path_slug = NULL
WHERE
  id = 10605;

UPDATE
  tournament_team
SET
  team_id = 10605
WHERE
  id IN (10605, 10788);

DELETE FROM
  team
WHERE
  id IN (10788);

----------------------------------------
-- Cortese WC

UPDATE
  team
SET
  name_normalized = 'Cortese WC',
  url_path_slug = NULL
WHERE
  id = 12482;

UPDATE
  tournament_team
SET
  team_id = 12482
WHERE
  id IN (12482, 12709, 12936, 13163);

DELETE FROM
  team
WHERE
  id IN (12709, 12936, 13163);

----------------------------------------
-- Cory Clark Wrestling

UPDATE
  team
SET
  name_normalized = 'Cory Clark Wrestling',
  url_path_slug = 'cory-clark'
WHERE
  id = 11633;

UPDATE
  tournament_team
SET
  team_id = 11633
WHERE
  id IN (11633, 11846, 12059, 12272, 12483, 12710, 12937, 13164, 13402, 13672, 13942, 14212, 14482, 14752, 15022, 15292);

DELETE FROM
  team
WHERE
  id IN (11846, 12059, 12272, 12483, 12710, 12937, 13164, 13402, 13672, 13942, 14212, 14482, 14752, 15022, 15292);

----------------------------------------
-- Cps

UPDATE
  team
SET
  name_normalized = 'Cps',
  url_path_slug = NULL
WHERE
  id = 2096;

UPDATE
  tournament_team
SET
  team_id = 2096
WHERE
  id IN (2096, 2380);

DELETE FROM
  team
WHERE
  id IN (2380);

----------------------------------------
-- Crawford County WC

UPDATE
  team
SET
  name_normalized = 'Crawford County WC',
  url_path_slug = NULL
WHERE
  id = 9223;

UPDATE
  tournament_team
SET
  team_id = 9223
WHERE
  id IN (9223, 9398, 9571, 9738, 9905, 10082, 10264, 10435, 10606, 10789, 10966, 11128, 11297, 11459, 11634, 11847, 12060, 12273, 12484, 12711, 12938, 13165, 13403, 13673, 13943, 14213, 14483, 14753, 15023, 15293);

DELETE FROM
  team
WHERE
  id IN (9398, 9571, 9738, 9905, 10082, 10264, 10435, 10606, 10789, 10966, 11128, 11297, 11459, 11634, 11847, 12060, 12273, 12484, 12711, 12938, 13165, 13403, 13673, 13943, 14213, 14483, 14753, 15023, 15293);

----------------------------------------
-- Crestwood Colts WC

UPDATE
  team
SET
  name_normalized = 'Crestwood Colts WC',
  url_path_slug = 'crestwood-colts'
WHERE
  id = 1146;

UPDATE
  tournament_team
SET
  team_id = 1146
WHERE
  id IN (1146, 1434, 1557, 1691, 1963);

DELETE FROM
  team
WHERE
  id IN (1434, 1557, 1691, 1963);

----------------------------------------
-- Cross Town WC

UPDATE
  team
SET
  name_normalized = 'Cross Town WC',
  url_path_slug = NULL
WHERE
  id = 7705;

UPDATE
  tournament_team
SET
  team_id = 7705
WHERE
  id IN (7705, 7895, 8090, 8277);

DELETE FROM
  team
WHERE
  id IN (7895, 8090, 8277);

----------------------------------------
-- Crossface Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Crossface Wrestling Club',
  url_path_slug = 'crossface'
WHERE
  id = 1037;

UPDATE
  tournament_team
SET
  team_id = 1037
WHERE
  id IN (1037, 1147, 1359, 1435, 1692, 3584, 3943, 4063, 4327, 4495);

DELETE FROM
  team
WHERE
  id IN (1147, 1359, 1435, 1692, 3584, 3943, 4063, 4327, 4495);

----------------------------------------
-- Crosstown Crushers

UPDATE
  team
SET
  name_normalized = 'Crosstown Crushers',
  url_path_slug = NULL
WHERE
  id = 5282;

UPDATE
  tournament_team
SET
  team_id = 5282
WHERE
  id IN (5282, 5423, 5571, 5708);

DELETE FROM
  team
WHERE
  id IN (5423, 5571, 5708);

----------------------------------------
-- Crosstown Spartan Elite WC

UPDATE
  team
SET
  name_normalized = 'Crosstown Spartan Elite WC',
  url_path_slug = NULL
WHERE
  id = 9572;

UPDATE
  tournament_team
SET
  team_id = 9572
WHERE
  id IN (9572, 9739, 9906, 10083, 10265, 10436, 10607, 10790, 10967, 11129, 11298, 11460, 11635, 11848, 12061, 12274, 12485, 12712, 12939, 13166, 13404, 13674, 13944, 14214, 14484, 14754, 15024, 15294);

DELETE FROM
  team
WHERE
  id IN (9739, 9906, 10083, 10265, 10436, 10607, 10790, 10967, 11129, 11298, 11460, 11635, 11848, 12061, 12274, 12485, 12712, 12939, 13166, 13404, 13674, 13944, 14214, 14484, 14754, 15024, 15294);

----------------------------------------
-- Crosstown WC

UPDATE
  team
SET
  name_normalized = 'Crosstown WC',
  url_path_slug = NULL
WHERE
  id = 5867;

UPDATE
  tournament_team
SET
  team_id = 5867
WHERE
  id IN (5867, 6046, 6225, 6409, 6593, 6775, 6957, 7145, 7335, 7523);

DELETE FROM
  team
WHERE
  id IN (6046, 6225, 6409, 6593, 6775, 6957, 7145, 7335, 7523);

----------------------------------------
-- Crosstown Wrestling

UPDATE
  team
SET
  name_normalized = 'Crosstown Wrestling',
  url_path_slug = NULL
WHERE
  id = 8462;

UPDATE
  tournament_team
SET
  team_id = 8462
WHERE
  id IN (8462, 8653, 8845, 9036, 9224, 9399);

DELETE FROM
  team
WHERE
  id IN (8653, 8845, 9036, 9224, 9399);

----------------------------------------
-- Crusaders` Elite

UPDATE
  team
SET
  name_normalized = 'Crusaders` Elite',
  url_path_slug = NULL
WHERE
  id = 7706;

UPDATE
  tournament_team
SET
  team_id = 7706
WHERE
  id IN (7706, 7896, 8091, 8278, 8463, 8654, 8846, 9037, 9225, 9400, 9573, 9740);

DELETE FROM
  team
WHERE
  id IN (7896, 8091, 8278, 8463, 8654, 8846, 9037, 9225, 9400, 9573, 9740);

----------------------------------------
-- Crystal Lake Wizards WC

UPDATE
  team
SET
  name_normalized = 'Crystal Lake Wizards WC',
  url_path_slug = 'wizards'
WHERE
  id = 1559;

UPDATE
  tournament_team
SET
  team_id = 1559
WHERE
  id IN (1559, 1693, 1821, 1965, 2099, 2230, 2523, 3585, 3704, 3816, 3944, 4064, 4186, 4328, 4496, 4655, 4787, 4933, 5106, 5283, 5424, 5572, 5709, 5868, 6047, 6226, 6410, 6594, 6776, 6958, 7146, 7336, 7524, 7707, 7897, 8092, 8279, 8464, 8655, 8847, 9038, 9226, 9401, 9574, 9741, 9907, 10084, 10266, 10437, 10608, 10791, 10968, 11130, 11299, 11461, 11636, 11849, 12062, 12275, 12486, 12713, 12940, 13167, 13405, 13675, 13945, 14215, 14485, 14755, 15025, 15295);

DELETE FROM
  team
WHERE
  id IN (1693, 1821, 1965, 2099, 2230, 2523, 3585, 3704, 3816, 3944, 4064, 4186, 4328, 4496, 4655, 4787, 4933, 5106, 5283, 5424, 5572, 5709, 5868, 6047, 6226, 6410, 6594, 6776, 6958, 7146, 7336, 7524, 7707, 7897, 8092, 8279, 8464, 8655, 8847, 9038, 9226, 9401, 9574, 9741, 9907, 10084, 10266, 10437, 10608, 10791, 10968, 11130, 11299, 11461, 11636, 11849, 12062, 12275, 12486, 12713, 12940, 13167, 13405, 13675, 13945, 14215, 14485, 14755, 15025, 15295);

----------------------------------------
-- Cumberland Youth WC

UPDATE
  team
SET
  name_normalized = 'Cumberland Youth WC',
  url_path_slug = NULL
WHERE
  id = 3945;

UPDATE
  tournament_team
SET
  team_id = 3945
WHERE
  id IN (3945, 4065, 4187, 4329, 4497, 4656, 4788, 4934, 5107, 5284, 5425, 5573, 5710, 5869, 6048, 6227, 6411, 6595, 6777, 7337, 7525, 7708, 7898, 8093, 8280, 8465, 8656, 9227, 9402, 9575, 9742, 9908, 10085, 10267, 10438, 10609, 10792, 10969, 11131, 11300, 11462, 11637, 11850, 12063, 12276, 12487, 12714, 12941, 13168, 13406, 13676, 13946, 14216, 14486, 14756, 15026, 15296);

DELETE FROM
  team
WHERE
  id IN (4065, 4187, 4329, 4497, 4656, 4788, 4934, 5107, 5284, 5425, 5573, 5710, 5869, 6048, 6227, 6411, 6595, 6777, 7337, 7525, 7708, 7898, 8093, 8280, 8465, 8656, 9227, 9402, 9575, 9742, 9908, 10085, 10267, 10438, 10609, 10792, 10969, 11131, 11300, 11462, 11637, 11850, 12063, 12276, 12487, 12714, 12941, 13168, 13406, 13676, 13946, 14216, 14486, 14756, 15026, 15296);

----------------------------------------
-- Cumberland Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Cumberland Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8848;

UPDATE
  tournament_team
SET
  team_id = 8848
WHERE
  id IN (8848, 9039);

DELETE FROM
  team
WHERE
  id IN (9039);

----------------------------------------
-- Da WC

UPDATE
  team
SET
  name_normalized = 'Da WC',
  url_path_slug = NULL
WHERE
  id = 9576;

UPDATE
  tournament_team
SET
  team_id = 9576
WHERE
  id IN (9576, 9743, 9909, 10086);

DELETE FROM
  team
WHERE
  id IN (9743, 9909, 10086);

----------------------------------------
-- Dakota Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Dakota Wrestling Club',
  url_path_slug = 'dakota'
WHERE
  id = 1437;

UPDATE
  tournament_team
SET
  team_id = 1437
WHERE
  id IN (1437, 1695, 1966, 2962, 3587, 3817, 3946, 4066, 4188, 4330, 4498, 4657, 4789, 4935, 5108, 5285, 5426, 5574, 5711, 5870, 6049, 6228, 6412, 6596, 6778, 6959, 7147, 7338, 7526, 7709, 7899, 8094, 8281, 8466, 8657, 8849, 9040, 9228, 9403, 9577, 9744, 9910, 10087, 10269, 10440, 10612, 10795, 10971, 11133, 11639, 11852, 12065, 12278, 12489, 12716, 12943, 13170, 13408, 13678, 13948, 14218, 14488, 14758, 15028, 15298);

DELETE FROM
  team
WHERE
  id IN (1695, 1966, 2962, 3587, 3817, 3946, 4066, 4188, 4330, 4498, 4657, 4789, 4935, 5108, 5285, 5426, 5574, 5711, 5870, 6049, 6228, 6412, 6596, 6778, 6959, 7147, 7338, 7526, 7709, 7899, 8094, 8281, 8466, 8657, 8849, 9040, 9228, 9403, 9577, 9744, 9910, 10087, 10269, 10440, 10612, 10795, 10971, 11133, 11639, 11852, 12065, 12278, 12489, 12716, 12943, 13170, 13408, 13678, 13948, 14218, 14488, 14758, 15028, 15298);

----------------------------------------
-- Danville Broncos Youth WC

UPDATE
  team
SET
  name_normalized = 'Danville Broncos Youth WC',
  url_path_slug = NULL
WHERE
  id = 6229;

UPDATE
  tournament_team
SET
  team_id = 6229
WHERE
  id IN (6229, 6413, 6597, 6779, 6960, 7148);

DELETE FROM
  team
WHERE
  id IN (6413, 6597, 6779, 6960, 7148);

----------------------------------------
-- Danville Chargers

UPDATE
  team
SET
  name_normalized = 'Danville Chargers',
  url_path_slug = NULL
WHERE
  id = 7339;

UPDATE
  tournament_team
SET
  team_id = 7339
WHERE
  id IN (7339, 7527, 9578, 9745, 9911, 10088, 10270, 10441, 10613, 10796);

DELETE FROM
  team
WHERE
  id IN (7527, 9578, 9745, 9911, 10088, 10270, 10441, 10613, 10796);

----------------------------------------
-- Danville Chargers WC

UPDATE
  team
SET
  name_normalized = 'Danville Chargers WC',
  url_path_slug = NULL
WHERE
  id = 10972;

UPDATE
  tournament_team
SET
  team_id = 10972
WHERE
  id IN (10972, 11134, 11302, 11464, 11640, 11853, 12066, 12279, 12490, 12717, 12944, 13171, 13409, 13679, 13949, 14219, 14489, 14759, 15029, 15299);

DELETE FROM
  team
WHERE
  id IN (11134, 11302, 11464, 11640, 11853, 12066, 12279, 12490, 12717, 12944, 13171, 13409, 13679, 13949, 14219, 14489, 14759, 15029, 15299);

----------------------------------------
-- Danville Chargers Wrestling

UPDATE
  team
SET
  name_normalized = 'Danville Chargers Wrestling',
  url_path_slug = NULL
WHERE
  id = 7710;

UPDATE
  tournament_team
SET
  team_id = 7710
WHERE
  id IN (7710, 7900, 8095, 8282, 8467, 8658, 8850, 9041, 9229, 9404);

DELETE FROM
  team
WHERE
  id IN (7900, 8095, 8282, 8467, 8658, 8850, 9041, 9229, 9404);

----------------------------------------
-- Danville Youth WC

UPDATE
  team
SET
  name_normalized = 'Danville Youth WC',
  url_path_slug = NULL
WHERE
  id = 922;

UPDATE
  tournament_team
SET
  team_id = 922
WHERE
  id IN (922, 1038);

DELETE FROM
  team
WHERE
  id IN (1038);

----------------------------------------
-- Dawc

UPDATE
  team
SET
  name_normalized = 'Dawc',
  url_path_slug = NULL
WHERE
  id = 10268;

UPDATE
  tournament_team
SET
  team_id = 10268
WHERE
  id IN (10268, 10439, 10610, 10793);

DELETE FROM
  team
WHERE
  id IN (10439, 10610, 10793);

----------------------------------------
-- Dawgs WC

UPDATE
  team
SET
  name_normalized = 'Dawgs WC',
  url_path_slug = NULL
WHERE
  id = 8468;

UPDATE
  tournament_team
SET
  team_id = 8468
WHERE
  id IN (8468, 8659);

DELETE FROM
  team
WHERE
  id IN (8659);

----------------------------------------
-- Dawgs Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Dawgs Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8851;

UPDATE
  tournament_team
SET
  team_id = 8851
WHERE
  id IN (8851, 9042);

DELETE FROM
  team
WHERE
  id IN (9042);

----------------------------------------
-- Dc WC

UPDATE
  team
SET
  name_normalized = 'Dc WC',
  url_path_slug = NULL
WHERE
  id = 10611;

UPDATE
  tournament_team
SET
  team_id = 10611
WHERE
  id IN (10611, 10794, 10970, 11132, 11301, 11463, 11638, 11851, 12064, 12277, 12488, 12715, 12942, 13169, 13407, 13677, 13947, 14217, 14487, 14757, 15027, 15297);

DELETE FROM
  team
WHERE
  id IN (10794, 10970, 11132, 11301, 11463, 11638, 11851, 12064, 12277, 12488, 12715, 12942, 13169, 13407, 13677, 13947, 14217, 14487, 14757, 15027, 15297);

----------------------------------------
-- DeKalb Huntley

UPDATE
  team
SET
  name_normalized = 'DeKalb Huntley',
  url_path_slug = NULL
WHERE
  id = 35;

UPDATE
  tournament_team
SET
  team_id = 35
WHERE
  id IN (35, 120, 160, 275, 615, 694, 811, 923);

DELETE FROM
  team
WHERE
  id IN (120, 160, 275, 615, 694, 811, 923);

----------------------------------------
-- DeKalb Rosette

UPDATE
  team
SET
  name_normalized = 'DeKalb Rosette',
  url_path_slug = NULL
WHERE
  id = 1;

UPDATE
  tournament_team
SET
  team_id = 1
WHERE
  id IN (1, 812);

DELETE FROM
  team
WHERE
  id IN (812);

----------------------------------------
-- DeKalb WC

UPDATE
  team
SET
  name_normalized = 'DeKalb WC',
  url_path_slug = NULL
WHERE
  id = 1039;

UPDATE
  tournament_team
SET
  team_id = 1039
WHERE
  id IN (1039, 1149, 4331, 4499, 4658, 4790, 4936, 5109, 5575, 6230, 6414, 9912, 10089, 10271, 10442, 10614, 10797, 11641, 11854, 12067, 12280, 12491, 12718, 12945, 13172, 13410, 13680, 13950, 14220, 14490, 14760, 15030, 15300);

DELETE FROM
  team
WHERE
  id IN (1149, 4331, 4499, 4658, 4790, 4936, 5109, 5575, 6230, 6414, 9912, 10089, 10271, 10442, 10614, 10797, 11641, 11854, 12067, 12280, 12491, 12718, 12945, 13172, 13410, 13680, 13950, 14220, 14490, 14760, 15030, 15300);

----------------------------------------
-- Decatur Dawgs WC

UPDATE
  team
SET
  name_normalized = 'Decatur Dawgs WC',
  url_path_slug = NULL
WHERE
  id = 11642;

UPDATE
  tournament_team
SET
  team_id = 11642
WHERE
  id IN (11642, 11855, 12068, 12281, 12492, 12719, 12946, 13173, 13411, 13681, 13951, 14221, 14491, 14761, 15031, 15301);

DELETE FROM
  team
WHERE
  id IN (11855, 12068, 12281, 12492, 12719, 12946, 13173, 13411, 13681, 13951, 14221, 14491, 14761, 15031, 15301);

----------------------------------------
-- Decatur Lakers WC

UPDATE
  team
SET
  name_normalized = 'Decatur Lakers WC',
  url_path_slug = NULL
WHERE
  id = 13412;

UPDATE
  tournament_team
SET
  team_id = 13412
WHERE
  id IN (13412, 13682, 13952, 14222, 14492, 14762, 15032, 15302);

DELETE FROM
  team
WHERE
  id IN (13682, 13952, 14222, 14492, 14762, 15032, 15302);

----------------------------------------
-- Decatur WC

UPDATE
  team
SET
  name_normalized = 'Decatur WC',
  url_path_slug = NULL
WHERE
  id = 484;

UPDATE
  tournament_team
SET
  team_id = 484
WHERE
  id IN (484, 1040, 1361, 1439, 1562, 1697, 3947, 4189);

DELETE FROM
  team
WHERE
  id IN (1040, 1361, 1439, 1562, 1697, 3947, 4189);

----------------------------------------
-- Demolition WC

UPDATE
  team
SET
  name_normalized = 'Demolition WC',
  url_path_slug = NULL
WHERE
  id = 10973;

UPDATE
  tournament_team
SET
  team_id = 10973
WHERE
  id IN (10973, 11135, 11303, 11465, 11643, 11856, 12069, 12282, 12493, 12720, 12947, 13174, 13413, 13683, 13953, 14223, 14493, 14763, 15033, 15303);

DELETE FROM
  team
WHERE
  id IN (11135, 11303, 11465, 11643, 11856, 12069, 12282, 12493, 12720, 12947, 13174, 13413, 13683, 13953, 14223, 14493, 14763, 15033, 15303);

----------------------------------------
-- Des Plaines Gemini

UPDATE
  team
SET
  name_normalized = 'Des Plaines Gemini',
  url_path_slug = NULL
WHERE
  id = 3;

UPDATE
  tournament_team
SET
  team_id = 3
WHERE
  id IN (3, 108);

DELETE FROM
  team
WHERE
  id IN (108);

----------------------------------------
-- Dinamo Wrestling

UPDATE
  team
SET
  name_normalized = 'Dinamo Wrestling',
  url_path_slug = NULL
WHERE
  id = 9913;

UPDATE
  tournament_team
SET
  team_id = 9913
WHERE
  id IN (9913, 10090, 10615, 10798, 11304, 11466, 11644, 11857, 12070, 12283, 12494, 12721, 12948, 13175, 13414, 13684, 13954, 14224, 14494, 14764, 15034, 15304);

DELETE FROM
  team
WHERE
  id IN (10090, 10615, 10798, 11304, 11466, 11644, 11857, 12070, 12283, 12494, 12721, 12948, 13175, 13414, 13684, 13954, 14224, 14494, 14764, 15034, 15304);

----------------------------------------
-- Dixon WC

UPDATE
  team
SET
  name_normalized = 'Dixon WC',
  url_path_slug = NULL
WHERE
  id = 1150;

UPDATE
  tournament_team
SET
  team_id = 1150
WHERE
  id IN (1150, 4067, 4190, 4332, 4500, 4659, 4791, 4937, 5110, 5286, 5427, 5576, 5712, 5871, 6050, 6231, 6415, 9914, 10091, 10616, 10799, 10974, 11136, 11305, 11467, 11645, 11858, 12071, 12284, 12495, 12722, 12949, 13176, 13415, 13685, 13955, 14225, 14495, 14765, 15035, 15305);

DELETE FROM
  team
WHERE
  id IN (4067, 4190, 4332, 4500, 4659, 4791, 4937, 5110, 5286, 5427, 5576, 5712, 5871, 6050, 6231, 6415, 9914, 10091, 10616, 10799, 10974, 11136, 11305, 11467, 11645, 11858, 12071, 12284, 12495, 12722, 12949, 13176, 13415, 13685, 13955, 14225, 14495, 14765, 15035, 15305);

----------------------------------------
-- Dixon Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Dixon Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 7711;

UPDATE
  tournament_team
SET
  team_id = 7711
WHERE
  id IN (7711, 7901, 8852, 9043);

DELETE FROM
  team
WHERE
  id IN (7901, 8852, 9043);

----------------------------------------
-- Dolton Park Falcons

UPDATE
  team
SET
  name_normalized = 'Dolton Park Falcons',
  url_path_slug = NULL
WHERE
  id = 258;

UPDATE
  tournament_team
SET
  team_id = 258
WHERE
  id IN (258, 588, 815, 925, 1042, 1362, 1441, 1564, 1698, 1827, 1968, 2103, 2390, 2668, 2820, 2965);

DELETE FROM
  team
WHERE
  id IN (588, 815, 925, 1042, 1362, 1441, 1564, 1698, 1827, 1968, 2103, 2390, 2668, 2820, 2965);

----------------------------------------
-- Doom Wrestling

UPDATE
  team
SET
  name_normalized = 'Doom Wrestling',
  url_path_slug = NULL
WHERE
  id = 12496;

UPDATE
  tournament_team
SET
  team_id = 12496
WHERE
  id IN (12496, 12723, 12950, 13177, 13416, 13686, 13956, 14226, 14496, 14766, 15036, 15306);

DELETE FROM
  team
WHERE
  id IN (12723, 12950, 13177, 13416, 13686, 13956, 14226, 14496, 14766, 15036, 15306);

----------------------------------------
-- Downers Grove Cougars

UPDATE
  team
SET
  name_normalized = 'Downers Grove Cougars',
  url_path_slug = NULL
WHERE
  id = 4333;

UPDATE
  tournament_team
SET
  team_id = 4333
WHERE
  id IN (4333, 4501, 4660, 4938, 5111, 5287, 5428, 5577, 5872, 6051, 6232, 6416, 6598, 6780, 6961, 7149, 7340, 7528, 9230, 9405);

DELETE FROM
  team
WHERE
  id IN (4501, 4660, 4938, 5111, 5287, 5428, 5577, 5872, 6051, 6232, 6416, 6598, 6780, 6961, 7149, 7340, 7528, 9230, 9405);

----------------------------------------
-- Downers Grove Cougars WC

UPDATE
  team
SET
  name_normalized = 'Downers Grove Cougars WC',
  url_path_slug = NULL
WHERE
  id = 3818;

UPDATE
  tournament_team
SET
  team_id = 3818
WHERE
  id IN (3818, 7712, 7902, 8096, 8283, 8469, 8660, 8853, 9044);

DELETE FROM
  team
WHERE
  id IN (7712, 7902, 8096, 8283, 8469, 8660, 8853, 9044);

----------------------------------------
-- Downers Grove WC

UPDATE
  team
SET
  name_normalized = 'Downers Grove WC',
  url_path_slug = NULL
WHERE
  id = 9579;

UPDATE
  tournament_team
SET
  team_id = 9579
WHERE
  id IN (9579, 9746, 9915, 10092, 10272, 10443, 10617, 10800, 10975, 11137, 11306, 11468, 11646, 11859, 12072, 12285, 12497, 12724, 12951, 13178, 13417, 13687, 13957, 14227, 14497, 14767, 15037, 15307);

DELETE FROM
  team
WHERE
  id IN (9746, 9915, 10092, 10272, 10443, 10617, 10800, 10975, 11137, 11306, 11468, 11646, 11859, 12072, 12285, 12497, 12724, 12951, 13178, 13417, 13687, 13957, 14227, 14497, 14767, 15037, 15307);

----------------------------------------
-- Dragons Lair WC

UPDATE
  team
SET
  name_normalized = 'Dragons Lair WC',
  url_path_slug = NULL
WHERE
  id = 12498;

UPDATE
  tournament_team
SET
  team_id = 12498
WHERE
  id IN (12498, 12725, 12952, 13179);

DELETE FROM
  team
WHERE
  id IN (12725, 12952, 13179);

----------------------------------------
-- Driscoll Junior Highlanders

UPDATE
  team
SET
  name_normalized = 'Driscoll Junior Highlanders',
  url_path_slug = NULL
WHERE
  id = 5873;

UPDATE
  tournament_team
SET
  team_id = 5873
WHERE
  id IN (5873, 6052);

DELETE FROM
  team
WHERE
  id IN (6052);

----------------------------------------
-- Du-Pec Carnivores

UPDATE
  team
SET
  name_normalized = 'Du-Pec Carnivores',
  url_path_slug = NULL
WHERE
  id = 4792;

UPDATE
  tournament_team
SET
  team_id = 4792
WHERE
  id IN (4792, 4939, 5112);

DELETE FROM
  team
WHERE
  id IN (4939, 5112);

----------------------------------------
-- Du-Pec WC

UPDATE
  team
SET
  name_normalized = 'Du-Pec WC',
  url_path_slug = NULL
WHERE
  id = 11647;

UPDATE
  tournament_team
SET
  team_id = 11647
WHERE
  id IN (11647, 11860, 12073, 12286, 12499, 12726, 12953, 13180);

DELETE FROM
  team
WHERE
  id IN (11860, 12073, 12286, 12499, 12726, 12953, 13180);

----------------------------------------
-- Dubtown WC

UPDATE
  team
SET
  name_normalized = 'Dubtown WC',
  url_path_slug = NULL
WHERE
  id = 12500;

UPDATE
  tournament_team
SET
  team_id = 12500
WHERE
  id IN (12500, 12727, 12954, 13181, 13418, 13688, 13958, 14228, 14498, 14768, 15038, 15308);

DELETE FROM
  team
WHERE
  id IN (12727, 12954, 13181, 13418, 13688, 13958, 14228, 14498, 14768, 15038, 15308);

----------------------------------------
-- Dundee Highlanders

UPDATE
  team
SET
  name_normalized = 'Dundee Highlanders',
  url_path_slug = NULL
WHERE
  id = 180;

UPDATE
  tournament_team
SET
  team_id = 180
WHERE
  id IN (180, 209, 486, 926, 1043, 1152, 1364, 1442, 3591, 3819, 3948, 4068, 4191, 4334, 4502, 4661, 4793, 4940, 5113, 5288, 5429, 5578, 5874, 6053, 6233, 6417, 6599, 6781, 6962, 7150, 7341, 7529);

DELETE FROM
  team
WHERE
  id IN (209, 486, 926, 1043, 1152, 1364, 1442, 3591, 3819, 3948, 4068, 4191, 4334, 4502, 4661, 4793, 4940, 5113, 5288, 5429, 5578, 5874, 6053, 6233, 6417, 6599, 6781, 6962, 7150, 7341, 7529);

----------------------------------------
-- Dundee Highlanders WC

UPDATE
  team
SET
  name_normalized = 'Dundee Highlanders WC',
  url_path_slug = NULL
WHERE
  id = 1700;

UPDATE
  tournament_team
SET
  team_id = 1700
WHERE
  id IN (1700, 7713, 7903, 8098, 8285, 8470, 8661, 8856, 9047, 9232, 9407, 9581, 9748, 9917, 10094, 10273, 10444, 10618, 10801, 10976, 11138, 11648, 11861, 12074, 12287, 12501, 12728, 12955, 13182, 13419, 13689, 13959, 14229, 14499, 14769, 15039, 15309);

DELETE FROM
  team
WHERE
  id IN (7713, 7903, 8098, 8285, 8470, 8661, 8856, 9047, 9232, 9407, 9581, 9748, 9917, 10094, 10273, 10444, 10618, 10801, 10976, 11138, 11648, 11861, 12074, 12287, 12501, 12728, 12955, 13182, 13419, 13689, 13959, 14229, 14499, 14769, 15039, 15309);

----------------------------------------
-- Dungeon WC

UPDATE
  team
SET
  name_normalized = 'Dungeon WC',
  url_path_slug = NULL
WHERE
  id = 13420;

UPDATE
  tournament_team
SET
  team_id = 13420
WHERE
  id IN (13420, 13690, 13960, 14230, 14500, 14770, 15040, 15310);

DELETE FROM
  team
WHERE
  id IN (13690, 13960, 14230, 14500, 14770, 15040, 15310);

----------------------------------------
-- Dupage Tiger Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Dupage Tiger Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8854;

UPDATE
  tournament_team
SET
  team_id = 8854
WHERE
  id IN (8854, 9045);

DELETE FROM
  team
WHERE
  id IN (9045);

----------------------------------------
-- Dupage WC

UPDATE
  team
SET
  name_normalized = 'Dupage WC',
  url_path_slug = NULL
WHERE
  id = 8097;

UPDATE
  tournament_team
SET
  team_id = 8097
WHERE
  id IN (8097, 8284, 9231, 9406, 9580, 9747, 9916, 10093);

DELETE FROM
  team
WHERE
  id IN (8284, 9231, 9406, 9580, 9747, 9916, 10093);

----------------------------------------
-- Dupage Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Dupage Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8855;

UPDATE
  tournament_team
SET
  team_id = 8855
WHERE
  id IN (8855, 9046);

DELETE FROM
  team
WHERE
  id IN (9046);

----------------------------------------
-- Durand WC

UPDATE
  team
SET
  name_normalized = 'Durand WC',
  url_path_slug = NULL
WHERE
  id = 6234;

UPDATE
  tournament_team
SET
  team_id = 6234
WHERE
  id IN (6234, 6418, 6600, 6782);

DELETE FROM
  team
WHERE
  id IN (6418, 6600, 6782);

----------------------------------------
-- Durand-Pecatonica Carnivores

UPDATE
  team
SET
  name_normalized = 'Durand-Pecatonica Carnivores',
  url_path_slug = NULL
WHERE
  id = 3820;

UPDATE
  tournament_team
SET
  team_id = 3820
WHERE
  id IN (3820, 4069, 4194, 4335, 4503);

DELETE FROM
  team
WHERE
  id IN (4069, 4194, 4335, 4503);

----------------------------------------
-- Dwight WC

UPDATE
  team
SET
  name_normalized = 'Dwight WC',
  url_path_slug = NULL
WHERE
  id = 8099;

UPDATE
  tournament_team
SET
  team_id = 8099
WHERE
  id IN (8099, 8286, 8471, 8662, 10977, 11139, 11307, 11469, 12502, 12729, 12956, 13183, 13421, 13691, 13961, 14231, 14501, 14771, 15041, 15311);

DELETE FROM
  team
WHERE
  id IN (8286, 8471, 8662, 10977, 11139, 11307, 11469, 12502, 12729, 12956, 13183, 13421, 13691, 13961, 14231, 14501, 14771, 15041, 15311);

----------------------------------------
-- E/P Junior Panthers

UPDATE
  team
SET
  name_normalized = 'E/P Junior Panthers',
  url_path_slug = NULL
WHERE
  id = 6601;

UPDATE
  tournament_team
SET
  team_id = 6601
WHERE
  id IN (6601, 6783);

DELETE FROM
  team
WHERE
  id IN (6783);

----------------------------------------
-- Eagle One

UPDATE
  team
SET
  name_normalized = 'Eagle One',
  url_path_slug = NULL
WHERE
  id = 6602;

UPDATE
  tournament_team
SET
  team_id = 6602
WHERE
  id IN (6602, 6784);

DELETE FROM
  team
WHERE
  id IN (6784);

----------------------------------------
-- Eagle WC

UPDATE
  team
SET
  name_normalized = 'Eagle WC',
  url_path_slug = NULL
WHERE
  id = 1701;

UPDATE
  tournament_team
SET
  team_id = 1701
WHERE
  id IN (1701, 1829, 2238);

DELETE FROM
  team
WHERE
  id IN (1829, 2238);

----------------------------------------
-- East Alton Wood River Jr. Oilers

UPDATE
  team
SET
  name_normalized = 'East Alton Wood River Jr. Oilers',
  url_path_slug = NULL
WHERE
  id = 7714;

UPDATE
  tournament_team
SET
  team_id = 7714
WHERE
  id IN (7714, 7904, 8100, 8287, 8472, 8663);

DELETE FROM
  team
WHERE
  id IN (7904, 8100, 8287, 8472, 8663);

----------------------------------------
-- East Moline WC

UPDATE
  team
SET
  name_normalized = 'East Moline WC',
  url_path_slug = NULL
WHERE
  id = 448;

UPDATE
  tournament_team
SET
  team_id = 448
WHERE
  id IN (448, 487, 590, 927, 1044, 1153, 2672, 3118, 3593, 3710, 3821, 3949, 4070, 4195, 4336, 4504);

DELETE FROM
  team
WHERE
  id IN (487, 590, 927, 1044, 1153, 2672, 3118, 3593, 3710, 3821, 3949, 4070, 4195, 4336, 4504);

----------------------------------------
-- East Peoria Crossface WC

UPDATE
  team
SET
  name_normalized = 'East Peoria Crossface WC',
  url_path_slug = NULL
WHERE
  id = 7342;

UPDATE
  tournament_team
SET
  team_id = 7342
WHERE
  id IN (7342, 7530);

DELETE FROM
  team
WHERE
  id IN (7530);

----------------------------------------
-- East Peoria River Bandits Wrestling

UPDATE
  team
SET
  name_normalized = 'East Peoria River Bandits Wrestling',
  url_path_slug = NULL
WHERE
  id = 11308;

UPDATE
  tournament_team
SET
  team_id = 11308
WHERE
  id IN (11308, 11470, 11649, 11862, 12075, 12288, 12503, 12730, 12957, 13184, 13422, 13692, 13962, 14232, 14502, 14772, 15042, 15312);

DELETE FROM
  team
WHERE
  id IN (11470, 11649, 11862, 12075, 12288, 12503, 12730, 12957, 13184, 13422, 13692, 13962, 14232, 14502, 14772, 15042, 15312);

----------------------------------------
-- East St. Louis WC

UPDATE
  team
SET
  name_normalized = 'East St. Louis WC',
  url_path_slug = NULL
WHERE
  id = 10274;

UPDATE
  tournament_team
SET
  team_id = 10274
WHERE
  id IN (10274, 10445, 10619, 10802, 10978, 11140, 11309, 11471, 11650, 11863, 12076, 12289, 12504, 12731, 12958, 13185, 13423, 13693, 13963, 14233, 14503, 14773, 15043, 15313);

DELETE FROM
  team
WHERE
  id IN (10445, 10619, 10802, 10978, 11140, 11309, 11471, 11650, 11863, 12076, 12289, 12504, 12731, 12958, 13185, 13423, 13693, 13963, 14233, 14503, 14773, 15043, 15313);

----------------------------------------
-- East St. Louis Wrestling

UPDATE
  team
SET
  name_normalized = 'East St. Louis Wrestling',
  url_path_slug = NULL
WHERE
  id = 7343;

UPDATE
  tournament_team
SET
  team_id = 7343
WHERE
  id IN (7343, 7531, 7715, 7905, 8101, 8288, 8473, 8664, 8857, 9048, 9233, 9408, 9582, 9749, 9918, 10095);

DELETE FROM
  team
WHERE
  id IN (7531, 7715, 7905, 8101, 8288, 8473, 8664, 8857, 9048, 9233, 9408, 9582, 9749, 9918, 10095);

----------------------------------------
-- Edison Panthers

UPDATE
  team
SET
  name_normalized = 'Edison Panthers',
  url_path_slug = NULL
WHERE
  id = 3822;

UPDATE
  tournament_team
SET
  team_id = 3822
WHERE
  id IN (3822, 3950, 4196);

DELETE FROM
  team
WHERE
  id IN (3950, 4196);

----------------------------------------
-- Edwardsville

UPDATE
  team
SET
  name_normalized = 'Edwardsville',
  url_path_slug = NULL
WHERE
  id = 1365;

UPDATE
  tournament_team
SET
  team_id = 1365
WHERE
  id IN (1365, 1443, 1568);

DELETE FROM
  team
WHERE
  id IN (1443, 1568);

----------------------------------------
-- Edwardsville WC

UPDATE
  team
SET
  name_normalized = 'Edwardsville WC',
  url_path_slug = NULL
WHERE
  id = 928;

UPDATE
  tournament_team
SET
  team_id = 928
WHERE
  id IN (928, 1045, 3712, 3823, 3951, 4071, 4197, 4337, 4505, 4662, 4794, 4941, 5114, 5289, 5430, 5579, 5713, 5875, 6054, 6235, 6419, 6603, 6785, 6963, 7151, 7344, 7532, 7716, 7906, 8102, 8289, 8474, 8665, 9234, 9409, 9583, 9750, 9919, 10096, 10275, 10446, 10620, 10803, 10979, 11141, 11310, 11472, 11651, 11864, 12077, 12290, 12505, 12732, 12959, 13186, 13424, 13694, 13964, 14234, 14504, 14774, 15044, 15314);

DELETE FROM
  team
WHERE
  id IN (1045, 3712, 3823, 3951, 4071, 4197, 4337, 4505, 4662, 4794, 4941, 5114, 5289, 5430, 5579, 5713, 5875, 6054, 6235, 6419, 6603, 6785, 6963, 7151, 7344, 7532, 7716, 7906, 8102, 8289, 8474, 8665, 9234, 9409, 9583, 9750, 9919, 10096, 10275, 10446, 10620, 10803, 10979, 11141, 11310, 11472, 11651, 11864, 12077, 12290, 12505, 12732, 12959, 13186, 13424, 13694, 13964, 14234, 14504, 14774, 15044, 15314);

----------------------------------------
-- Edwardsville Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Edwardsville Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8858;

UPDATE
  tournament_team
SET
  team_id = 8858
WHERE
  id IN (8858, 9049);

DELETE FROM
  team
WHERE
  id IN (9049);

----------------------------------------
-- Effingham Junior Wrestling Team

UPDATE
  team
SET
  name_normalized = 'Effingham Junior Wrestling Team',
  url_path_slug = NULL
WHERE
  id = 5580;

UPDATE
  tournament_team
SET
  team_id = 5580
WHERE
  id IN (5580, 5714);

DELETE FROM
  team
WHERE
  id IN (5714);

----------------------------------------
-- Effingham Youth WC

UPDATE
  team
SET
  name_normalized = 'Effingham Youth WC',
  url_path_slug = NULL
WHERE
  id = 4338;

UPDATE
  tournament_team
SET
  team_id = 4338
WHERE
  id IN (4338, 4506, 4663, 4795, 4942, 5115, 7345, 7533, 9920, 10097, 10276, 10447, 10621, 10804, 10980, 11142, 11311, 11473, 11652, 11865, 12078, 12291, 12506, 12733, 12960, 13187, 13425, 13695, 13965, 14235, 14505, 14775, 15045, 15315);

DELETE FROM
  team
WHERE
  id IN (4506, 4663, 4795, 4942, 5115, 7345, 7533, 9920, 10097, 10276, 10447, 10621, 10804, 10980, 11142, 11311, 11473, 11652, 11865, 12078, 12291, 12506, 12733, 12960, 13187, 13425, 13695, 13965, 14235, 14505, 14775, 15045, 15315);

----------------------------------------
-- Eisenhower Jr. High

UPDATE
  team
SET
  name_normalized = 'Eisenhower Jr. High',
  url_path_slug = NULL
WHERE
  id = 1046;

UPDATE
  tournament_team
SET
  team_id = 1046
WHERE
  id IN (1046, 1155, 1444, 1569, 1973);

DELETE FROM
  team
WHERE
  id IN (1155, 1444, 1569, 1973);

----------------------------------------
-- El Paso / Gridley WC

UPDATE
  team
SET
  name_normalized = 'El Paso / Gridley WC',
  url_path_slug = NULL
WHERE
  id = 5290;

UPDATE
  tournament_team
SET
  team_id = 5290
WHERE
  id IN (5290, 5432, 5581, 5715, 5876, 6055, 6236, 6420);

DELETE FROM
  team
WHERE
  id IN (5432, 5581, 5715, 5876, 6055, 6236, 6420);

----------------------------------------
-- El Paso Gridley Youth WC

UPDATE
  team
SET
  name_normalized = 'El Paso Gridley Youth WC',
  url_path_slug = NULL
WHERE
  id = 6604;

UPDATE
  tournament_team
SET
  team_id = 6604
WHERE
  id IN (6604, 6786, 7717, 7907, 11312, 11474, 11653, 11866, 12079, 12292, 12507, 12734, 12961, 13188, 13426, 13696, 13966, 14236, 14506, 14776, 15046, 15316);

DELETE FROM
  team
WHERE
  id IN (6786, 7717, 7907, 11312, 11474, 11653, 11866, 12079, 12292, 12507, 12734, 12961, 13188, 13426, 13696, 13966, 14236, 14506, 14776, 15046, 15316);

----------------------------------------
-- El Paso Gridley Youth Wrestlng Club

UPDATE
  team
SET
  name_normalized = 'El Paso Gridley Youth Wrestlng Club',
  url_path_slug = NULL
WHERE
  id = 10622;

UPDATE
  tournament_team
SET
  team_id = 10622
WHERE
  id IN (10622, 10805);

DELETE FROM
  team
WHERE
  id IN (10805);

----------------------------------------
-- El Paso WC

UPDATE
  team
SET
  name_normalized = 'El Paso WC',
  url_path_slug = NULL
WHERE
  id = 3713;

UPDATE
  tournament_team
SET
  team_id = 3713
WHERE
  id IN (3713, 4664, 4943, 5116);

DELETE FROM
  team
WHERE
  id IN (4664, 4943, 5116);

----------------------------------------
-- Elevate Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Elevate Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 10623;

UPDATE
  tournament_team
SET
  team_id = 10623
WHERE
  id IN (10623, 10806, 10981, 11143, 11313, 11475);

DELETE FROM
  team
WHERE
  id IN (10806, 10981, 11143, 11313, 11475);

----------------------------------------
-- Elgin Matt Rats

UPDATE
  team
SET
  name_normalized = 'Elgin Matt Rats',
  url_path_slug = NULL
WHERE
  id = 1366;

UPDATE
  tournament_team
SET
  team_id = 1366
WHERE
  id IN (1366, 1447, 1572);

DELETE FROM
  team
WHERE
  id IN (1447, 1572);

----------------------------------------
-- Elgin WC

UPDATE
  team
SET
  name_normalized = 'Elgin WC',
  url_path_slug = NULL
WHERE
  id = 930;

UPDATE
  tournament_team
SET
  team_id = 930
WHERE
  id IN (930, 1047);

DELETE FROM
  team
WHERE
  id IN (1047);

----------------------------------------
-- Elk Grove Junior Grens

UPDATE
  team
SET
  name_normalized = 'Elk Grove Junior Grens',
  url_path_slug = NULL
WHERE
  id = 7346;

UPDATE
  tournament_team
SET
  team_id = 7346
WHERE
  id IN (7346, 7534, 7718, 7908, 8103, 8290, 8475, 8666, 8859, 9050, 9235, 9410, 9584, 9751, 9921, 10098, 10277, 10448, 10624, 10807);

DELETE FROM
  team
WHERE
  id IN (7534, 7718, 7908, 8103, 8290, 8475, 8666, 8859, 9050, 9235, 9410, 9584, 9751, 9921, 10098, 10277, 10448, 10624, 10807);

----------------------------------------
-- Elk Grove Junior Grens WC

UPDATE
  team
SET
  name_normalized = 'Elk Grove Junior Grens WC',
  url_path_slug = NULL
WHERE
  id = 10982;

UPDATE
  tournament_team
SET
  team_id = 10982
WHERE
  id IN (10982, 11144, 11314, 11476, 11654, 11867, 12080, 12293, 12508, 12735, 12962, 13189, 13427, 13697, 13967, 14237, 14507, 14777, 15047, 15317);

DELETE FROM
  team
WHERE
  id IN (11144, 11314, 11476, 11654, 11867, 12080, 12293, 12508, 12735, 12962, 13189, 13427, 13697, 13967, 14237, 14507, 14777, 15047, 15317);

----------------------------------------
-- Elmhurst Jr Dukes

UPDATE
  team
SET
  name_normalized = 'Elmhurst Jr Dukes',
  url_path_slug = NULL
WHERE
  id = 9236;

UPDATE
  tournament_team
SET
  team_id = 9236
WHERE
  id IN (9236, 9411);

DELETE FROM
  team
WHERE
  id IN (9411);

----------------------------------------
-- Elmhurst Jr Dukes WC

UPDATE
  team
SET
  name_normalized = 'Elmhurst Jr Dukes WC',
  url_path_slug = NULL
WHERE
  id = 9585;

UPDATE
  tournament_team
SET
  team_id = 9585
WHERE
  id IN (9585, 9752);

DELETE FROM
  team
WHERE
  id IN (9752);

----------------------------------------
-- Elmhurst Jr. Dukes

UPDATE
  team
SET
  name_normalized = 'Elmhurst Jr. Dukes',
  url_path_slug = NULL
WHERE
  id = 1705;

UPDATE
  tournament_team
SET
  team_id = 1705
WHERE
  id IN (1705, 4339, 4507, 4796, 4944, 5117, 5291, 5433, 5582, 5716, 6237, 6421, 6605, 6787, 6964, 7152, 7347, 7535);

DELETE FROM
  team
WHERE
  id IN (4339, 4507, 4796, 4944, 5117, 5291, 5433, 5582, 5716, 6237, 6421, 6605, 6787, 6964, 7152, 7347, 7535);

----------------------------------------
-- Elmhurst Titans Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Elmhurst Titans Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 10983;

UPDATE
  tournament_team
SET
  team_id = 10983
WHERE
  id IN (10983, 11145, 11315, 11477, 11655, 11868, 12081, 12294, 13428, 13698, 13968, 14238, 14508, 14778, 15048, 15318);

DELETE FROM
  team
WHERE
  id IN (11145, 11315, 11477, 11655, 11868, 12081, 12294, 13428, 13698, 13968, 14238, 14508, 14778, 15048, 15318);

----------------------------------------
-- Elmhurst WC

UPDATE
  team
SET
  name_normalized = 'Elmhurst WC',
  url_path_slug = NULL
WHERE
  id = 9922;

UPDATE
  tournament_team
SET
  team_id = 9922
WHERE
  id IN (9922, 10099, 10278, 10449);

DELETE FROM
  team
WHERE
  id IN (10099, 10278, 10449);

----------------------------------------
-- Englewood Live Wire WC

UPDATE
  team
SET
  name_normalized = 'Englewood Live Wire WC',
  url_path_slug = NULL
WHERE
  id = 10984;

UPDATE
  tournament_team
SET
  team_id = 10984
WHERE
  id IN (10984, 11146, 11316, 11478, 11656, 11869, 12082, 12295, 12509, 12736, 12963, 13190, 13429, 13699, 13969, 14239, 14509, 14779, 15049, 15319);

DELETE FROM
  team
WHERE
  id IN (11146, 11316, 11478, 11656, 11869, 12082, 12295, 12509, 12736, 12963, 13190, 13429, 13699, 13969, 14239, 14509, 14779, 15049, 15319);

----------------------------------------
-- Epic Wrestling

UPDATE
  team
SET
  name_normalized = 'Epic Wrestling',
  url_path_slug = NULL
WHERE
  id = 7719;

UPDATE
  tournament_team
SET
  team_id = 7719
WHERE
  id IN (7719, 7909, 8860, 9051, 9237, 9412, 9586, 9753);

DELETE FROM
  team
WHERE
  id IN (7909, 8860, 9051, 9237, 9412, 9586, 9753);

----------------------------------------
-- Erie Middle School WC

UPDATE
  team
SET
  name_normalized = 'Erie Middle School WC',
  url_path_slug = NULL
WHERE
  id = 3824;

UPDATE
  tournament_team
SET
  team_id = 3824
WHERE
  id IN (3824, 4198, 4945, 5118, 5583, 5717, 5877, 6056, 6238, 6422);

DELETE FROM
  team
WHERE
  id IN (4198, 4945, 5118, 5583, 5717, 5877, 6056, 6238, 6422);

----------------------------------------
-- Ervin WC

UPDATE
  team
SET
  name_normalized = 'Ervin WC',
  url_path_slug = NULL
WHERE
  id = 4946;

UPDATE
  tournament_team
SET
  team_id = 4946
WHERE
  id IN (4946, 5119, 5434);

DELETE FROM
  team
WHERE
  id IN (5119, 5434);

----------------------------------------
-- Eureka Kids WC

UPDATE
  team
SET
  name_normalized = 'Eureka Kids WC',
  url_path_slug = NULL
WHERE
  id = 4072;

UPDATE
  tournament_team
SET
  team_id = 4072
WHERE
  id IN (4072, 4340, 4508);

DELETE FROM
  team
WHERE
  id IN (4340, 4508);

----------------------------------------
-- Eureka WC

UPDATE
  team
SET
  name_normalized = 'Eureka WC',
  url_path_slug = NULL
WHERE
  id = 11317;

UPDATE
  tournament_team
SET
  team_id = 11317
WHERE
  id IN (11317, 11479, 11657, 11870, 12083, 12296, 12510, 12737, 12964, 13191, 13430, 13700, 13970, 14240, 14510, 14780, 15050, 15320);

DELETE FROM
  team
WHERE
  id IN (11479, 11657, 11870, 12083, 12296, 12510, 12737, 12964, 13191, 13430, 13700, 13970, 14240, 14510, 14780, 15050, 15320);

----------------------------------------
-- Evanston Black Kat WC

UPDATE
  team
SET
  name_normalized = 'Evanston Black Kat WC',
  url_path_slug = NULL
WHERE
  id = 3825;

UPDATE
  tournament_team
SET
  team_id = 3825
WHERE
  id IN (3825, 3952);

DELETE FROM
  team
WHERE
  id IN (3952);

----------------------------------------
-- Evanston School Of Wrestling

UPDATE
  team
SET
  name_normalized = 'Evanston School Of Wrestling',
  url_path_slug = NULL
WHERE
  id = 6965;

UPDATE
  tournament_team
SET
  team_id = 6965
WHERE
  id IN (6965, 7153, 8104, 8291, 8476, 8667, 8861, 9052, 9238, 9413, 9587, 9754, 10279, 10450, 10625, 10808, 10985, 11147, 11318, 11480, 11658, 11871, 12084, 12297, 12511, 12738, 12965, 13192, 13431, 13701, 13971, 14241, 14511, 14781, 15051, 15321);

DELETE FROM
  team
WHERE
  id IN (7153, 8104, 8291, 8476, 8667, 8861, 9052, 9238, 9413, 9587, 9754, 10279, 10450, 10625, 10808, 10985, 11147, 11318, 11480, 11658, 11871, 12084, 12297, 12511, 12738, 12965, 13192, 13431, 13701, 13971, 14241, 14511, 14781, 15051, 15321);

----------------------------------------
-- Evolution Wrestling Institute

UPDATE
  team
SET
  name_normalized = 'Evolution Wrestling Institute',
  url_path_slug = NULL
WHERE
  id = 9588;

UPDATE
  tournament_team
SET
  team_id = 9588
WHERE
  id IN (9588, 9755, 9923, 10100);

DELETE FROM
  team
WHERE
  id IN (9755, 9923, 10100);

----------------------------------------
-- Fairmont Rough Riders

UPDATE
  team
SET
  name_normalized = 'Fairmont Rough Riders',
  url_path_slug = NULL
WHERE
  id = 4797;

UPDATE
  tournament_team
SET
  team_id = 4797
WHERE
  id IN (4797, 5293, 5584, 5878, 6057, 6239, 6423);

DELETE FROM
  team
WHERE
  id IN (5293, 5584, 5878, 6057, 6239, 6423);

----------------------------------------
-- Falcon Elite WC

UPDATE
  team
SET
  name_normalized = 'Falcon Elite WC',
  url_path_slug = NULL
WHERE
  id = 9239;

UPDATE
  tournament_team
SET
  team_id = 9239
WHERE
  id IN (9239, 9414);

DELETE FROM
  team
WHERE
  id IN (9414);

----------------------------------------
-- Falcon WC

UPDATE
  team
SET
  name_normalized = 'Falcon WC',
  url_path_slug = NULL
WHERE
  id = 4341;

UPDATE
  tournament_team
SET
  team_id = 4341
WHERE
  id IN (4341, 4509, 4665, 4798, 5294, 5435, 5879, 6058, 6240, 6424, 6606, 6788, 6966, 7154);

DELETE FROM
  team
WHERE
  id IN (4509, 4665, 4798, 5294, 5435, 5879, 6058, 6240, 6424, 6606, 6788, 6966, 7154);

----------------------------------------
-- Falcon Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Falcon Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 4947;

UPDATE
  tournament_team
SET
  team_id = 4947
WHERE
  id IN (4947, 5120, 5585, 5718);

DELETE FROM
  team
WHERE
  id IN (5120, 5585, 5718);

----------------------------------------
-- Falcon Youth WC

UPDATE
  team
SET
  name_normalized = 'Falcon Youth WC',
  url_path_slug = NULL
WHERE
  id = 3827;

UPDATE
  tournament_team
SET
  team_id = 3827
WHERE
  id IN (3827, 3953, 4073, 4199, 4342, 4510, 4666, 4799, 4948, 5121, 5295, 5436, 5586, 5719, 5880, 6059, 6241, 6425, 6607, 6789, 6967, 7155, 7348, 7536, 10280, 10451, 10986, 11148, 11319, 11481);

DELETE FROM
  team
WHERE
  id IN (3953, 4073, 4199, 4342, 4510, 4666, 4799, 4948, 5121, 5295, 5436, 5586, 5719, 5880, 6059, 6241, 6425, 6607, 6789, 6967, 7155, 7348, 7536, 10280, 10451, 10986, 11148, 11319, 11481);

----------------------------------------
-- Falcon Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Falcon Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 12512;

UPDATE
  tournament_team
SET
  team_id = 12512
WHERE
  id IN (12512, 12739, 12966, 13193, 13432, 13702, 13972, 14242, 14512, 14782, 15052, 15322);

DELETE FROM
  team
WHERE
  id IN (12739, 12966, 13193, 13432, 13702, 13972, 14242, 14512, 14782, 15052, 15322);

----------------------------------------
-- Falcons Elite

UPDATE
  team
SET
  name_normalized = 'Falcons Elite',
  url_path_slug = NULL
WHERE
  id = 8477;

UPDATE
  tournament_team
SET
  team_id = 8477
WHERE
  id IN (8477, 8668);

DELETE FROM
  team
WHERE
  id IN (8668);

----------------------------------------
-- Falcons Elite Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Falcons Elite Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8862;

UPDATE
  tournament_team
SET
  team_id = 8862
WHERE
  id IN (8862, 9053);

DELETE FROM
  team
WHERE
  id IN (9053);

----------------------------------------
-- Falcons WC

UPDATE
  team
SET
  name_normalized = 'Falcons WC',
  url_path_slug = NULL
WHERE
  id = 7349;

UPDATE
  tournament_team
SET
  team_id = 7349
WHERE
  id IN (7349, 7537, 7720, 7910);

DELETE FROM
  team
WHERE
  id IN (7537, 7720, 7910);

----------------------------------------
-- Fenton Bison WC

UPDATE
  team
SET
  name_normalized = 'Fenton Bison WC',
  url_path_slug = NULL
WHERE
  id = 7721;

UPDATE
  tournament_team
SET
  team_id = 7721
WHERE
  id IN (7721, 7911);

DELETE FROM
  team
WHERE
  id IN (7911);

----------------------------------------
-- Fenton Jr. Bison

UPDATE
  team
SET
  name_normalized = 'Fenton Jr. Bison',
  url_path_slug = NULL
WHERE
  id = 10626;

UPDATE
  tournament_team
SET
  team_id = 10626
WHERE
  id IN (10626, 10809);

DELETE FROM
  team
WHERE
  id IN (10809);

----------------------------------------
-- Fenton Jr. Bison WC

UPDATE
  team
SET
  name_normalized = 'Fenton Jr. Bison WC',
  url_path_slug = NULL
WHERE
  id = 10987;

UPDATE
  tournament_team
SET
  team_id = 10987
WHERE
  id IN (10987, 11149, 11320, 11482);

DELETE FROM
  team
WHERE
  id IN (11149, 11320, 11482);

----------------------------------------
-- Fenton/Bbaa Jr Bison WC

UPDATE
  team
SET
  name_normalized = 'Fenton/Bbaa Jr Bison WC',
  url_path_slug = NULL
WHERE
  id = 8478;

UPDATE
  tournament_team
SET
  team_id = 8478
WHERE
  id IN (8478, 8669);

DELETE FROM
  team
WHERE
  id IN (8669);

----------------------------------------
-- Fenwick Falcons WC

UPDATE
  team
SET
  name_normalized = 'Fenwick Falcons WC',
  url_path_slug = NULL
WHERE
  id = 4667;

UPDATE
  tournament_team
SET
  team_id = 4667
WHERE
  id IN (4667, 4800, 4949, 5122, 5296, 5437, 5587, 5720, 5881, 6060, 6242, 6426);

DELETE FROM
  team
WHERE
  id IN (4800, 4949, 5122, 5296, 5437, 5587, 5720, 5881, 6060, 6242, 6426);

----------------------------------------
-- Fightin Titan WC

UPDATE
  team
SET
  name_normalized = 'Fightin Titan WC',
  url_path_slug = NULL
WHERE
  id = 4801;

UPDATE
  tournament_team
SET
  team_id = 4801
WHERE
  id IN (4801, 4950, 5123, 5588, 5721, 5882, 6061);

DELETE FROM
  team
WHERE
  id IN (4950, 5123, 5588, 5721, 5882, 6061);

----------------------------------------
-- Fighting Farmers WC

UPDATE
  team
SET
  name_normalized = 'Fighting Farmers WC',
  url_path_slug = NULL
WHERE
  id = 10988;

UPDATE
  tournament_team
SET
  team_id = 10988
WHERE
  id IN (10988, 11150, 11321, 11483, 11659, 11872, 12085, 12298, 12513, 12740, 12967, 13194, 13433, 13703, 13973, 14243, 14513, 14783, 15053, 15323);

DELETE FROM
  team
WHERE
  id IN (11150, 11321, 11483, 11659, 11872, 12085, 12298, 12513, 12740, 12967, 13194, 13433, 13703, 13973, 14243, 14513, 14783, 15053, 15323);

----------------------------------------
-- Fighting Titan

UPDATE
  team
SET
  name_normalized = 'Fighting Titan',
  url_path_slug = NULL
WHERE
  id = 6611;

UPDATE
  tournament_team
SET
  team_id = 6611
WHERE
  id IN (6611, 6793);

DELETE FROM
  team
WHERE
  id IN (6793);

----------------------------------------
-- Fighting Titans

UPDATE
  team
SET
  name_normalized = 'Fighting Titans',
  url_path_slug = NULL
WHERE
  id = 7350;

UPDATE
  tournament_team
SET
  team_id = 7350
WHERE
  id IN (7350, 7538);

DELETE FROM
  team
WHERE
  id IN (7538);

----------------------------------------
-- Fisher WC

UPDATE
  team
SET
  name_normalized = 'Fisher WC',
  url_path_slug = NULL
WHERE
  id = 931;

UPDATE
  tournament_team
SET
  team_id = 931
WHERE
  id IN (931, 1157, 3828, 4074, 4200, 4343, 4511, 4668, 4802, 4951, 5124, 5298, 5438, 5722, 5883, 6062, 6243, 6427, 6608, 6790, 6968, 7156, 7351, 7539, 8479, 8670, 9924, 10101, 10281, 10452, 10627, 10810, 11660, 11873, 12086, 12299, 12514, 12741, 12968, 13195, 13434, 13704, 13974, 14244, 14514, 14784, 15054, 15324);

DELETE FROM
  team
WHERE
  id IN (1157, 3828, 4074, 4200, 4343, 4511, 4668, 4802, 4951, 5124, 5298, 5438, 5722, 5883, 6062, 6243, 6427, 6608, 6790, 6968, 7156, 7351, 7539, 8479, 8670, 9924, 10101, 10281, 10452, 10627, 10810, 11660, 11873, 12086, 12299, 12514, 12741, 12968, 13195, 13434, 13704, 13974, 14244, 14514, 14784, 15054, 15324);

----------------------------------------
-- Fit & Fight Gym WC

UPDATE
  team
SET
  name_normalized = 'Fit & Fight Gym WC',
  url_path_slug = NULL
WHERE
  id = 13435;

UPDATE
  tournament_team
SET
  team_id = 13435
WHERE
  id IN (13435, 13705, 13975, 14245, 14515, 14785, 15055, 15325);

DELETE FROM
  team
WHERE
  id IN (13705, 13975, 14245, 14515, 14785, 15055, 15325);

----------------------------------------
-- Fitz Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Fitz Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 10989;

UPDATE
  tournament_team
SET
  team_id = 10989
WHERE
  id IN (10989, 11151, 11322, 11484);

DELETE FROM
  team
WHERE
  id IN (11151, 11322, 11484);

----------------------------------------
-- Force Elite

UPDATE
  team
SET
  name_normalized = 'Force Elite',
  url_path_slug = NULL
WHERE
  id = 10282;

UPDATE
  tournament_team
SET
  team_id = 10282
WHERE
  id IN (10282, 10453, 10628, 10811);

DELETE FROM
  team
WHERE
  id IN (10453, 10628, 10811);

----------------------------------------
-- Force Elite WC

UPDATE
  team
SET
  name_normalized = 'Force Elite WC',
  url_path_slug = NULL
WHERE
  id = 10990;

UPDATE
  tournament_team
SET
  team_id = 10990
WHERE
  id IN (10990, 11152, 11323, 11485, 11661, 11874, 12087, 12300, 12515, 12742, 12969, 13196, 13436, 13706, 13976, 14246, 14516, 14786, 15056, 15326);

DELETE FROM
  team
WHERE
  id IN (11152, 11323, 11485, 11661, 11874, 12087, 12300, 12515, 12742, 12969, 13196, 13436, 13706, 13976, 14246, 14516, 14786, 15056, 15326);

----------------------------------------
-- Force WC

UPDATE
  team
SET
  name_normalized = 'Force WC',
  url_path_slug = NULL
WHERE
  id = 7722;

UPDATE
  tournament_team
SET
  team_id = 7722
WHERE
  id IN (7722, 7912, 8105, 8292, 8480, 8671, 9240, 9415, 9589, 9756, 9925, 10102);

DELETE FROM
  team
WHERE
  id IN (7912, 8105, 8292, 8480, 8671, 9240, 9415, 9589, 9756, 9925, 10102);

----------------------------------------
-- Force Wrestling

UPDATE
  team
SET
  name_normalized = 'Force Wrestling',
  url_path_slug = NULL
WHERE
  id = 5884;

UPDATE
  tournament_team
SET
  team_id = 5884
WHERE
  id IN (5884, 6063, 6244, 6428, 6609, 6791, 6969, 7157, 7352, 7540);

DELETE FROM
  team
WHERE
  id IN (6063, 6244, 6428, 6609, 6791, 6969, 7157, 7352, 7540);

----------------------------------------
-- Force Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Force Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8863;

UPDATE
  tournament_team
SET
  team_id = 8863
WHERE
  id IN (8863, 9054);

DELETE FROM
  team
WHERE
  id IN (9054);

----------------------------------------
-- Ford Heights Falcons

UPDATE
  team
SET
  name_normalized = 'Ford Heights Falcons',
  url_path_slug = NULL
WHERE
  id = 4344;

UPDATE
  tournament_team
SET
  team_id = 4344
WHERE
  id IN (4344, 4512, 4803);

DELETE FROM
  team
WHERE
  id IN (4512, 4803);

----------------------------------------
-- Foreman WC

UPDATE
  team
SET
  name_normalized = 'Foreman WC',
  url_path_slug = NULL
WHERE
  id = 822;

UPDATE
  tournament_team
SET
  team_id = 822
WHERE
  id IN (822, 932, 1158, 1367, 1448, 1573);

DELETE FROM
  team
WHERE
  id IN (932, 1158, 1367, 1448, 1573);

----------------------------------------
-- Fox Lake Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Fox Lake Wrestling Club',
  url_path_slug = 'fox-lake-wc'
WHERE
  id = 6970;

UPDATE
  tournament_team
SET
  team_id = 6970
WHERE
  id IN (6970, 7158, 7353, 7541, 7723, 7913, 8106, 8293, 8864, 9055, 9241, 9416, 9590, 9757, 9926, 10103, 10283, 10454, 10629, 10812, 10991, 11153, 11324, 11486, 11662, 11875, 12088, 12301, 12516, 12743, 12970, 13197, 13437, 13707, 13977, 14247, 14517, 14787, 15057, 15327);

DELETE FROM
  team
WHERE
  id IN (7158, 7353, 7541, 7723, 7913, 8106, 8293, 8864, 9055, 9241, 9416, 9590, 9757, 9926, 10103, 10283, 10454, 10629, 10812, 10991, 11153, 11324, 11486, 11662, 11875, 12088, 12301, 12516, 12743, 12970, 13197, 13437, 13707, 13977, 14247, 14517, 14787, 15057, 15327);

----------------------------------------
-- Fox Valley Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Fox Valley Wrestling Club',
  url_path_slug = 'fox-valley'
WHERE
  id = 3272;

UPDATE
  tournament_team
SET
  team_id = 3272
WHERE
  id IN (3272, 3378, 3600, 3717, 3829, 3954, 4075, 4202, 4345, 4513, 4669, 4804, 4952, 5125, 5299, 5439, 5589, 5723, 5885, 6064, 6245, 6429, 6610, 6792, 6971, 7159, 7354, 7542, 7724, 7914, 8107, 8294, 8481, 8672, 8865, 9056, 9242, 9417, 9591, 9758, 9927, 10104, 10284, 10455, 10630, 10813, 10992, 11154, 11325, 11487, 11663, 11876, 12089, 12302, 12517, 12744, 12971, 13198, 13438, 13708, 13978, 14248, 14518, 14788, 15058, 15328, 15551, 15696, 15834, 15986, 16229);

DELETE FROM
  team
WHERE
  id IN (3378, 3600, 3717, 3829, 3954, 4075, 4202, 4345, 4513, 4669, 4804, 4952, 5125, 5299, 5439, 5589, 5723, 5885, 6064, 6245, 6429, 6610, 6792, 6971, 7159, 7354, 7542, 7724, 7914, 8107, 8294, 8481, 8672, 8865, 9056, 9242, 9417, 9591, 9758, 9927, 10104, 10284, 10455, 10630, 10813, 10992, 11154, 11325, 11487, 11663, 11876, 12089, 12302, 12517, 12744, 12971, 13198, 13438, 13708, 13978, 14248, 14518, 14788, 15058, 15328, 15551, 15696, 15834, 15986, 16229);

----------------------------------------
-- Frankfort

UPDATE
  team
SET
  name_normalized = 'Frankfort',
  url_path_slug = NULL
WHERE
  id = 596;

UPDATE
  tournament_team
SET
  team_id = 596
WHERE
  id IN (596, 704, 823);

DELETE FROM
  team
WHERE
  id IN (704, 823);

----------------------------------------
-- Frankfort Falcons

UPDATE
  team
SET
  name_normalized = 'Frankfort Falcons',
  url_path_slug = NULL
WHERE
  id = 933;

UPDATE
  tournament_team
SET
  team_id = 933
WHERE
  id IN (933, 1050, 1159);

DELETE FROM
  team
WHERE
  id IN (1050, 1159);

----------------------------------------
-- Frankfort Gladiator Wrestling

UPDATE
  team
SET
  name_normalized = 'Frankfort Gladiator Wrestling',
  url_path_slug = NULL
WHERE
  id = 9928;

UPDATE
  tournament_team
SET
  team_id = 9928
WHERE
  id IN (9928, 10105, 10285, 10456, 10631, 10814, 11664, 11877, 12090, 12303, 12518, 12745, 12972, 13199, 13439, 13709, 13979, 14249, 14519, 14789, 15059, 15329);

DELETE FROM
  team
WHERE
  id IN (10105, 10285, 10456, 10631, 10814, 11664, 11877, 12090, 12303, 12518, 12745, 12972, 13199, 13439, 13709, 13979, 14249, 14519, 14789, 15059, 15329);

----------------------------------------
-- Frankfort Square Wildcats Wrestling

UPDATE
  team
SET
  name_normalized = 'Frankfort Square Wildcats Wrestling',
  url_path_slug = NULL
WHERE
  id = 7725;

UPDATE
  tournament_team
SET
  team_id = 7725
WHERE
  id IN (7725, 7915, 8108, 8295, 8482, 8673);

DELETE FROM
  team
WHERE
  id IN (7915, 8108, 8295, 8482, 8673);

----------------------------------------
-- Frankfort Wildcats Wrestling

UPDATE
  team
SET
  name_normalized = 'Frankfort Wildcats Wrestling',
  url_path_slug = NULL
WHERE
  id = 9929;

UPDATE
  tournament_team
SET
  team_id = 9929
WHERE
  id IN (9929, 10106, 10286, 10457, 10632, 10815, 10993, 11155, 11326, 11488, 11665, 11878, 12091, 12304, 12519, 12746, 12973, 13200, 13440, 13710, 13980, 14250, 14520, 14790, 15060, 15330);

DELETE FROM
  team
WHERE
  id IN (10106, 10286, 10457, 10632, 10815, 10993, 11155, 11326, 11488, 11665, 11878, 12091, 12304, 12519, 12746, 12973, 13200, 13440, 13710, 13980, 14250, 14520, 14790, 15060, 15330);

----------------------------------------
-- Franklin Park

UPDATE
  team
SET
  name_normalized = 'Franklin Park',
  url_path_slug = NULL
WHERE
  id = 106;

UPDATE
  tournament_team
SET
  team_id = 106
WHERE
  id IN (106, 161, 211, 362, 824);

DELETE FROM
  team
WHERE
  id IN (161, 211, 362, 824);

----------------------------------------
-- Franklin Park Raiders

UPDATE
  team
SET
  name_normalized = 'Franklin Park Raiders',
  url_path_slug = 'franklin-park-raiders'
WHERE
  id = 934;

UPDATE
  tournament_team
SET
  team_id = 934
WHERE
  id IN (934, 1051, 1160, 1368, 1449, 1575, 1708, 1979, 3830, 3955, 4076, 4203);

DELETE FROM
  team
WHERE
  id IN (1051, 1160, 1368, 1449, 1575, 1708, 1979, 3830, 3955, 4076, 4203);

----------------------------------------
-- Friar Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Friar Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8866;

UPDATE
  tournament_team
SET
  team_id = 8866
WHERE
  id IN (8866, 9057);

DELETE FROM
  team
WHERE
  id IN (9057);

----------------------------------------
-- Fuel WC

UPDATE
  team
SET
  name_normalized = 'Fuel WC',
  url_path_slug = NULL
WHERE
  id = 9243;

UPDATE
  tournament_team
SET
  team_id = 9243
WHERE
  id IN (9243, 9418);

DELETE FROM
  team
WHERE
  id IN (9418);

----------------------------------------
-- Fulton County Stingers

UPDATE
  team
SET
  name_normalized = 'Fulton County Stingers',
  url_path_slug = NULL
WHERE
  id = 6972;

UPDATE
  tournament_team
SET
  team_id = 6972
WHERE
  id IN (6972, 7160, 10633, 10816);

DELETE FROM
  team
WHERE
  id IN (7160, 10633, 10816);

----------------------------------------
-- G2 WC

UPDATE
  team
SET
  name_normalized = 'G2 WC',
  url_path_slug = NULL
WHERE
  id = 9592;

UPDATE
  tournament_team
SET
  team_id = 9592
WHERE
  id IN (9592, 9759, 9930, 10107, 10287, 10458, 10994, 11156, 11327, 11489);

DELETE FROM
  team
WHERE
  id IN (9759, 9930, 10107, 10287, 10458, 10994, 11156, 11327, 11489);

----------------------------------------
-- Galesburg Jr. Streaks

UPDATE
  team
SET
  name_normalized = 'Galesburg Jr. Streaks',
  url_path_slug = NULL
WHERE
  id = 3832;

UPDATE
  tournament_team
SET
  team_id = 3832
WHERE
  id IN (3832, 3957, 4204, 4346, 4514, 4670, 4953, 5126);

DELETE FROM
  team
WHERE
  id IN (3957, 4204, 4346, 4514, 4670, 4953, 5126);

----------------------------------------
-- Gators Elite WC

UPDATE
  team
SET
  name_normalized = 'Gators Elite WC',
  url_path_slug = NULL
WHERE
  id = 13441;

UPDATE
  tournament_team
SET
  team_id = 13441
WHERE
  id IN (13441, 13711, 13981, 14251, 14521, 14791, 15061, 15331);

DELETE FROM
  team
WHERE
  id IN (13711, 13981, 14251, 14521, 14791, 15061, 15331);

----------------------------------------
-- Gators WC

UPDATE
  team
SET
  name_normalized = 'Gators WC',
  url_path_slug = NULL
WHERE
  id = 6246;

UPDATE
  tournament_team
SET
  team_id = 6246
WHERE
  id IN (6246, 6430);

DELETE FROM
  team
WHERE
  id IN (6430);

----------------------------------------
-- Gc Jr Warriors

UPDATE
  team
SET
  name_normalized = 'Gc Jr Warriors',
  url_path_slug = NULL
WHERE
  id = 5724;

UPDATE
  tournament_team
SET
  team_id = 5724
WHERE
  id IN (5724, 5886, 6065, 6247, 6431);

DELETE FROM
  team
WHERE
  id IN (5886, 6065, 6247, 6431);

----------------------------------------
-- Generals

UPDATE
  team
SET
  name_normalized = 'Generals',
  url_path_slug = NULL
WHERE
  id = 3833;

UPDATE
  tournament_team
SET
  team_id = 3833
WHERE
  id IN (3833, 4077, 4347, 4515);

DELETE FROM
  team
WHERE
  id IN (4077, 4347, 4515);

----------------------------------------
-- Geneseo Spider WC

UPDATE
  team
SET
  name_normalized = 'Geneseo Spider WC',
  url_path_slug = NULL
WHERE
  id = 5590;

UPDATE
  tournament_team
SET
  team_id = 5590
WHERE
  id IN (5590, 5725, 5887, 6066, 6248, 6432, 6612, 6794, 6973, 7161, 7355, 7543, 7726, 7916, 8109, 8296, 8483, 8674);

DELETE FROM
  team
WHERE
  id IN (5725, 5887, 6066, 6248, 6432, 6612, 6794, 6973, 7161, 7355, 7543, 7726, 7916, 8109, 8296, 8483, 8674);

----------------------------------------
-- Geneseo Spider Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Geneseo Spider Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8867;

UPDATE
  tournament_team
SET
  team_id = 8867
WHERE
  id IN (8867, 9058);

DELETE FROM
  team
WHERE
  id IN (9058);

----------------------------------------
-- Geneseo WC

UPDATE
  team
SET
  name_normalized = 'Geneseo WC',
  url_path_slug = NULL
WHERE
  id = 825;

UPDATE
  tournament_team
SET
  team_id = 825
WHERE
  id IN (825, 937, 1052, 1161, 1369, 1450, 1577, 1709, 1983, 3721, 3834, 3958, 4078, 4206, 4348, 4516, 4671, 4805, 4954, 5127, 5301, 5441);

DELETE FROM
  team
WHERE
  id IN (937, 1052, 1161, 1369, 1450, 1577, 1709, 1983, 3721, 3834, 3958, 4078, 4206, 4348, 4516, 4671, 4805, 4954, 5127, 5301, 5441);

----------------------------------------
-- Geneva Junior Vikings

UPDATE
  team
SET
  name_normalized = 'Geneva Junior Vikings',
  url_path_slug = NULL
WHERE
  id = 8868;

UPDATE
  tournament_team
SET
  team_id = 8868
WHERE
  id IN (8868, 9059, 9244, 9419, 9931, 10108, 10288, 10459, 10634, 10817, 13442, 13712, 13982, 14252, 14522, 14792, 15062, 15332);

DELETE FROM
  team
WHERE
  id IN (9059, 9244, 9419, 9931, 10108, 10288, 10459, 10634, 10817, 13442, 13712, 13982, 14252, 14522, 14792, 15062, 15332);

----------------------------------------
-- Geneva WC

UPDATE
  team
SET
  name_normalized = 'Geneva WC',
  url_path_slug = NULL
WHERE
  id = 4349;

UPDATE
  tournament_team
SET
  team_id = 4349
WHERE
  id IN (4349, 4517, 4806, 4955, 5128);

DELETE FROM
  team
WHERE
  id IN (4517, 4806, 4955, 5128);

----------------------------------------
-- Genoa-Kingston WC

UPDATE
  team
SET
  name_normalized = 'Genoa-Kingston WC',
  url_path_slug = NULL
WHERE
  id = 6249;

UPDATE
  tournament_team
SET
  team_id = 6249
WHERE
  id IN (6249, 6433, 6974, 7162);

DELETE FROM
  team
WHERE
  id IN (6433, 6974, 7162);

----------------------------------------
-- Georgetown

UPDATE
  team
SET
  name_normalized = 'Georgetown',
  url_path_slug = NULL
WHERE
  id = 938;

UPDATE
  tournament_team
SET
  team_id = 938
WHERE
  id IN (938, 1053, 1270);

DELETE FROM
  team
WHERE
  id IN (1053, 1270);

----------------------------------------
-- Gibson City Youth

UPDATE
  team
SET
  name_normalized = 'Gibson City Youth',
  url_path_slug = NULL
WHERE
  id = 939;

UPDATE
  tournament_team
SET
  team_id = 939
WHERE
  id IN (939, 1054, 1164);

DELETE FROM
  team
WHERE
  id IN (1054, 1164);

----------------------------------------
-- Gilbert Grappling

UPDATE
  team
SET
  name_normalized = 'Gilbert Grappling',
  url_path_slug = NULL
WHERE
  id = 8484;

UPDATE
  tournament_team
SET
  team_id = 8484
WHERE
  id IN (8484, 8675);

DELETE FROM
  team
WHERE
  id IN (8675);

----------------------------------------
-- Gillespie Eagles

UPDATE
  team
SET
  name_normalized = 'Gillespie Eagles',
  url_path_slug = NULL
WHERE
  id = 6250;

UPDATE
  tournament_team
SET
  team_id = 6250
WHERE
  id IN (6250, 6434);

DELETE FROM
  team
WHERE
  id IN (6434);

----------------------------------------
-- Gladiator Elite

UPDATE
  team
SET
  name_normalized = 'Gladiator Elite',
  url_path_slug = NULL
WHERE
  id = 9593;

UPDATE
  tournament_team
SET
  team_id = 9593
WHERE
  id IN (9593, 9760, 9932, 10109, 10289, 10460, 10635, 10818);

DELETE FROM
  team
WHERE
  id IN (9760, 9932, 10109, 10289, 10460, 10635, 10818);

----------------------------------------
-- Gladiator Elite WC

UPDATE
  team
SET
  name_normalized = 'Gladiator Elite WC',
  url_path_slug = NULL
WHERE
  id = 10995;

UPDATE
  tournament_team
SET
  team_id = 10995
WHERE
  id IN (10995, 11157, 11328, 11490, 11666, 11879, 12092, 12305, 12520, 12747, 12974, 13201, 13443, 13713, 13983, 14253, 14523, 14793, 15063, 15333);

DELETE FROM
  team
WHERE
  id IN (11157, 11328, 11490, 11666, 11879, 12092, 12305, 12520, 12747, 12974, 13201, 13443, 13713, 13983, 14253, 14523, 14793, 15063, 15333);

----------------------------------------
-- Gladiators

UPDATE
  team
SET
  name_normalized = 'Gladiators',
  url_path_slug = NULL
WHERE
  id = 4350;

UPDATE
  tournament_team
SET
  team_id = 4350
WHERE
  id IN (4350, 4518, 4673, 4807, 5302, 5444, 5726, 5888, 6067, 6251, 6435, 6613, 6795, 6975, 7163, 7356, 7544);

DELETE FROM
  team
WHERE
  id IN (4518, 4673, 4807, 5302, 5444, 5726, 5888, 6067, 6251, 6435, 6613, 6795, 6975, 7163, 7356, 7544);

----------------------------------------
-- Gladiators WC

UPDATE
  team
SET
  name_normalized = 'Gladiators WC',
  url_path_slug = NULL
WHERE
  id = 7727;

UPDATE
  tournament_team
SET
  team_id = 7727
WHERE
  id IN (7727, 7917);

DELETE FROM
  team
WHERE
  id IN (7917);

----------------------------------------
-- Glen Ellyn Dungeon WC

UPDATE
  team
SET
  name_normalized = 'Glen Ellyn Dungeon WC',
  url_path_slug = NULL
WHERE
  id = 3835;

UPDATE
  tournament_team
SET
  team_id = 3835
WHERE
  id IN (3835, 4079, 4207, 4351, 4519, 5591, 7728, 7918, 12521, 12748, 12975, 13202);

DELETE FROM
  team
WHERE
  id IN (4079, 4207, 4351, 4519, 5591, 7728, 7918, 12521, 12748, 12975, 13202);

----------------------------------------
-- Glenbard East Jr. Rams WC

UPDATE
  team
SET
  name_normalized = 'Glenbard East Jr. Rams WC',
  url_path_slug = NULL
WHERE
  id = 3959;

UPDATE
  tournament_team
SET
  team_id = 3959
WHERE
  id IN (3959, 4208, 4352, 4520, 4674, 4808, 4956, 5129, 5303, 5445, 5727, 5889, 6068, 6252, 6436, 6614, 6796, 6976, 7164, 7729, 7919, 8110, 8297, 8485, 8676, 8869, 9060, 9245, 9420, 9594, 9761, 10636, 10819, 10996, 11158, 11329, 11491, 12522, 12749, 12976, 13203, 13444, 13714, 13984, 14254, 14524, 14794, 15064, 15334);

DELETE FROM
  team
WHERE
  id IN (4208, 4352, 4520, 4674, 4808, 4956, 5129, 5303, 5445, 5727, 5889, 6068, 6252, 6436, 6614, 6796, 6976, 7164, 7729, 7919, 8110, 8297, 8485, 8676, 8869, 9060, 9245, 9420, 9594, 9761, 10636, 10819, 10996, 11158, 11329, 11491, 12522, 12749, 12976, 13203, 13444, 13714, 13984, 14254, 14524, 14794, 15064, 15334);

----------------------------------------
-- Glenwood

UPDATE
  team
SET
  name_normalized = 'Glenwood',
  url_path_slug = NULL
WHERE
  id = 69;

UPDATE
  tournament_team
SET
  team_id = 69
WHERE
  id IN (69, 449, 605);

DELETE FROM
  team
WHERE
  id IN (449, 605);

----------------------------------------
-- Gmo Eagles Kids Wrestling

UPDATE
  team
SET
  name_normalized = 'Gmo Eagles Kids Wrestling',
  url_path_slug = NULL
WHERE
  id = 6615;

UPDATE
  tournament_team
SET
  team_id = 6615
WHERE
  id IN (6615, 6797);

DELETE FROM
  team
WHERE
  id IN (6797);

----------------------------------------
-- Golden Eagles

UPDATE
  team
SET
  name_normalized = 'Golden Eagles',
  url_path_slug = NULL
WHERE
  id = 5304;

UPDATE
  tournament_team
SET
  team_id = 5304
WHERE
  id IN (5304, 5446, 5592, 5728, 5890, 6069, 7730, 7920);

DELETE FROM
  team
WHERE
  id IN (5446, 5592, 5728, 5890, 6069, 7730, 7920);

----------------------------------------
-- Golden Eagles WC

UPDATE
  team
SET
  name_normalized = 'Golden Eagles WC',
  url_path_slug = NULL
WHERE
  id = 6977;

UPDATE
  tournament_team
SET
  team_id = 6977
WHERE
  id IN (6977, 7165, 7357, 7545, 8111, 8298, 8486, 8677, 9246, 9421, 9595, 9762, 9933, 10110, 10290, 10461, 10637, 10820, 10997, 11159, 11667, 11880, 12093, 12306, 13445, 13715, 13985, 14255, 14525, 14795, 15065, 15335);

DELETE FROM
  team
WHERE
  id IN (7165, 7357, 7545, 8111, 8298, 8486, 8677, 9246, 9421, 9595, 9762, 9933, 10110, 10290, 10461, 10637, 10820, 10997, 11159, 11667, 11880, 12093, 12306, 13445, 13715, 13985, 14255, 14525, 14795, 15065, 15335);

----------------------------------------
-- Golden Eagles Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Golden Eagles Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8870;

UPDATE
  tournament_team
SET
  team_id = 8870
WHERE
  id IN (8870, 9061);

DELETE FROM
  team
WHERE
  id IN (9061);

----------------------------------------
-- Gomez Wrestling Acad.

UPDATE
  team
SET
  name_normalized = 'Gomez Wrestling Acad.',
  url_path_slug = NULL
WHERE
  id = 7358;

UPDATE
  tournament_team
SET
  team_id = 7358
WHERE
  id IN (7358, 7546);

DELETE FROM
  team
WHERE
  id IN (7546);

----------------------------------------
-- Gomez Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Gomez Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 5593;

UPDATE
  tournament_team
SET
  team_id = 5593
WHERE
  id IN (5593, 5729, 5891, 6070, 6253, 6437, 6616, 6798, 6978, 7166, 7731, 7921, 8112, 8299, 8487, 8678, 8871, 9062, 9247, 9422, 9596, 9763, 9934, 10111, 10291, 10462);

DELETE FROM
  team
WHERE
  id IN (5729, 5891, 6070, 6253, 6437, 6616, 6798, 6978, 7166, 7731, 7921, 8112, 8299, 8487, 8678, 8871, 9062, 9247, 9422, 9596, 9763, 9934, 10111, 10291, 10462);

----------------------------------------
-- Gomez Wrestling Rtc

UPDATE
  team
SET
  name_normalized = 'Gomez Wrestling Rtc',
  url_path_slug = NULL
WHERE
  id = 13446;

UPDATE
  tournament_team
SET
  team_id = 13446
WHERE
  id IN (13446, 13716, 13986, 14256, 14526, 14796, 15066, 15336);

DELETE FROM
  team
WHERE
  id IN (13716, 13986, 14256, 14526, 14796, 15066, 15336);

----------------------------------------
-- Granite City Coolidge

UPDATE
  team
SET
  name_normalized = 'Granite City Coolidge',
  url_path_slug = NULL
WHERE
  id = 1583;

UPDATE
  tournament_team
SET
  team_id = 1583
WHERE
  id IN (1583, 1712, 2546);

DELETE FROM
  team
WHERE
  id IN (1712, 2546);

----------------------------------------
-- Granite City Jr Warriors

UPDATE
  team
SET
  name_normalized = 'Granite City Jr Warriors',
  url_path_slug = NULL
WHERE
  id = 4353;

UPDATE
  tournament_team
SET
  team_id = 4353
WHERE
  id IN (4353, 4521, 4675, 4809, 4957, 5130);

DELETE FROM
  team
WHERE
  id IN (4521, 4675, 4809, 4957, 5130);

----------------------------------------
-- Granite City WC

UPDATE
  team
SET
  name_normalized = 'Granite City WC',
  url_path_slug = NULL
WHERE
  id = 6617;

UPDATE
  tournament_team
SET
  team_id = 6617
WHERE
  id IN (6617, 6799, 6979, 7167, 7359, 7547, 7732, 7922, 8113, 8300, 8488, 8679, 8872, 9063);

DELETE FROM
  team
WHERE
  id IN (6799, 6979, 7167, 7359, 7547, 7732, 7922, 8113, 8300, 8488, 8679, 8872, 9063);

----------------------------------------
-- Granite City Wrestling Association

UPDATE
  team
SET
  name_normalized = 'Granite City Wrestling Association',
  url_path_slug = NULL
WHERE
  id = 9248;

UPDATE
  tournament_team
SET
  team_id = 9248
WHERE
  id IN (9248, 9423, 9597, 9764, 9935, 10112, 10292, 10463, 10638, 10821, 10998, 11160, 11330, 11492, 11668, 11881, 12094, 12307, 12523, 12750, 12977, 13204);

DELETE FROM
  team
WHERE
  id IN (9423, 9597, 9764, 9935, 10112, 10292, 10463, 10638, 10821, 10998, 11160, 11330, 11492, 11668, 11881, 12094, 12307, 12523, 12750, 12977, 13204);

----------------------------------------
-- Granite City-Grigsby

UPDATE
  team
SET
  name_normalized = 'Granite City-Grigsby',
  url_path_slug = NULL
WHERE
  id = 943;

UPDATE
  tournament_team
SET
  team_id = 943
WHERE
  id IN (943, 1056, 1166);

DELETE FROM
  team
WHERE
  id IN (1056, 1166);

----------------------------------------
-- Grapplin' Devils WC

UPDATE
  team
SET
  name_normalized = 'Grapplin'' Devils WC',
  url_path_slug = NULL
WHERE
  id = 3836;

UPDATE
  tournament_team
SET
  team_id = 3836
WHERE
  id IN (3836, 4209, 4676, 4810, 5892, 6071, 6254, 6438, 6618, 6800, 6980, 7168, 8114, 8301, 8489, 8680);

DELETE FROM
  team
WHERE
  id IN (4209, 4676, 4810, 5892, 6071, 6254, 6438, 6618, 6800, 6980, 7168, 8114, 8301, 8489, 8680);

----------------------------------------
-- Grayslake WC

UPDATE
  team
SET
  name_normalized = 'Grayslake WC',
  url_path_slug = NULL
WHERE
  id = 8490;

UPDATE
  tournament_team
SET
  team_id = 8490
WHERE
  id IN (8490, 8681, 9249, 9424, 9936, 10113, 10293, 10464);

DELETE FROM
  team
WHERE
  id IN (8681, 9249, 9424, 9936, 10113, 10293, 10464);

----------------------------------------
-- Grayslake Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Grayslake Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8873;

UPDATE
  tournament_team
SET
  team_id = 8873
WHERE
  id IN (8873, 9064);

DELETE FROM
  team
WHERE
  id IN (9064);

----------------------------------------
-- Grayslake Youth WC

UPDATE
  team
SET
  name_normalized = 'Grayslake Youth WC',
  url_path_slug = NULL
WHERE
  id = 13447;

UPDATE
  tournament_team
SET
  team_id = 13447
WHERE
  id IN (13447, 13717, 13987, 14257, 14527, 14797, 15067, 15337);

DELETE FROM
  team
WHERE
  id IN (13717, 13987, 14257, 14527, 14797, 15067, 15337);

----------------------------------------
-- Greg Gomez Trained Wrestling

UPDATE
  team
SET
  name_normalized = 'Greg Gomez Trained Wrestling',
  url_path_slug = NULL
WHERE
  id = 8115;

UPDATE
  tournament_team
SET
  team_id = 8115
WHERE
  id IN (8115, 8302, 8491, 8682, 8874, 9065, 9250, 9425, 9598, 9765, 9937, 10114, 10294, 10465, 10639, 10822, 11331, 11493, 11669, 11882, 12095, 12308, 12524, 12751, 12978, 13205, 13448, 13718, 13988, 14258, 14528, 14798, 15068, 15338);

DELETE FROM
  team
WHERE
  id IN (8302, 8491, 8682, 8874, 9065, 9250, 9425, 9598, 9765, 9937, 10114, 10294, 10465, 10639, 10822, 11331, 11493, 11669, 11882, 12095, 12308, 12524, 12751, 12978, 13205, 13448, 13718, 13988, 14258, 14528, 14798, 15068, 15338);

----------------------------------------
-- Guerrero`S Garage Wrestling

UPDATE
  team
SET
  name_normalized = 'Guerrero`S Garage Wrestling',
  url_path_slug = NULL
WHERE
  id = 10295;

UPDATE
  tournament_team
SET
  team_id = 10295
WHERE
  id IN (10295, 10466, 10640, 10823, 10999, 11161);

DELETE FROM
  team
WHERE
  id IN (10466, 10640, 10823, 10999, 11161);

----------------------------------------
-- HPL Wrestling

UPDATE
  team
SET
  name_normalized = 'HPL Wrestling',
  url_path_slug = NULL
WHERE
  id = 11000;

UPDATE
  tournament_team
SET
  team_id = 11000
WHERE
  id IN (11000, 11162);

DELETE FROM
  team
WHERE
  id IN (11162);

----------------------------------------
-- Hampshire WC

UPDATE
  team
SET
  name_normalized = 'Hampshire WC',
  url_path_slug = NULL
WHERE
  id = 7360;

UPDATE
  tournament_team
SET
  team_id = 7360
WHERE
  id IN (7360, 7548, 7733, 7923);

DELETE FROM
  team
WHERE
  id IN (7548, 7733, 7923);

----------------------------------------
-- Harlem Cougars WC

UPDATE
  team
SET
  name_normalized = 'Harlem Cougars WC',
  url_path_slug = NULL
WHERE
  id = 1843;

UPDATE
  tournament_team
SET
  team_id = 1843
WHERE
  id IN (1843, 1986, 2409, 2547, 2841, 3605, 3723, 3837, 3960, 4082, 4210, 4354, 4522, 4677, 4811, 4958, 5132, 5305, 5447, 5594, 5731, 5893, 6072, 6255, 6439, 6619, 6801, 7734, 7924, 8116, 8303, 8493, 8684, 8876, 9067, 9252, 9427, 9599, 9766, 9939, 10116, 10296, 10467, 10641, 10824);

DELETE FROM
  team
WHERE
  id IN (1986, 2409, 2547, 2841, 3605, 3723, 3837, 3960, 4082, 4210, 4354, 4522, 4677, 4811, 4958, 5132, 5305, 5447, 5594, 5731, 5893, 6072, 6255, 6439, 6619, 6801, 7734, 7924, 8116, 8303, 8493, 8684, 8876, 9067, 9252, 9427, 9599, 9766, 9939, 10116, 10296, 10467, 10641, 10824);

----------------------------------------
-- Harlem Huskies WC

UPDATE
  team
SET
  name_normalized = 'Harlem Huskies WC',
  url_path_slug = NULL
WHERE
  id = 7361;

UPDATE
  tournament_team
SET
  team_id = 7361
WHERE
  id IN (7361, 7549, 11670, 11883, 12096, 12309, 12525, 12752, 12979, 13206, 13449, 13719, 13989, 14259, 14529, 14799, 15069, 15339);

DELETE FROM
  team
WHERE
  id IN (7549, 11670, 11883, 12096, 12309, 12525, 12752, 12979, 13206, 13449, 13719, 13989, 14259, 14529, 14799, 15069, 15339);

----------------------------------------
-- Harlem Renegades

UPDATE
  team
SET
  name_normalized = 'Harlem Renegades',
  url_path_slug = NULL
WHERE
  id = 5894;

UPDATE
  tournament_team
SET
  team_id = 5894
WHERE
  id IN (5894, 6073, 6256, 6440, 6620, 6802);

DELETE FROM
  team
WHERE
  id IN (6073, 6256, 6440, 6620, 6802);

----------------------------------------
-- Harlem Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Harlem Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 6981;

UPDATE
  tournament_team
SET
  team_id = 6981
WHERE
  id IN (6981, 7169);

DELETE FROM
  team
WHERE
  id IN (7169);

----------------------------------------
-- Harrisburg Jr. Bulldogs

UPDATE
  team
SET
  name_normalized = 'Harrisburg Jr. Bulldogs',
  url_path_slug = NULL
WHERE
  id = 7735;

UPDATE
  tournament_team
SET
  team_id = 7735
WHERE
  id IN (7735, 7925);

DELETE FROM
  team
WHERE
  id IN (7925);

----------------------------------------
-- Harrisburg Junior Bulldogs

UPDATE
  team
SET
  name_normalized = 'Harrisburg Junior Bulldogs',
  url_path_slug = NULL
WHERE
  id = 8117;

UPDATE
  tournament_team
SET
  team_id = 8117
WHERE
  id IN (8117, 8304, 8494, 8685, 8877, 9068);

DELETE FROM
  team
WHERE
  id IN (8304, 8494, 8685, 8877, 9068);

----------------------------------------
-- Harvard Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Harvard Wrestling Club',
  url_path_slug = 'harvard'
WHERE
  id = 270;

UPDATE
  tournament_team
SET
  team_id = 270
WHERE
  id IN (270, 368, 499, 609, 716, 831, 944, 1059, 1169, 1277, 1459, 1586, 1715, 1844, 1987, 2123, 2257, 2410, 2548, 2842, 2983, 3137, 3280, 3387, 3606, 3724, 10297, 10468, 11332, 11494, 11671, 11884, 12097, 12310, 12526, 12753, 12980, 13207, 13450, 13720, 13990, 14260, 14530, 14800, 15070, 15340);

DELETE FROM
  team
WHERE
  id IN (368, 499, 609, 716, 831, 944, 1059, 1169, 1277, 1459, 1586, 1715, 1844, 1987, 2123, 2257, 2410, 2548, 2842, 2983, 3137, 3280, 3387, 3606, 3724, 10297, 10468, 11332, 11494, 11671, 11884, 12097, 12310, 12526, 12753, 12980, 13207, 13450, 13720, 13990, 14260, 14530, 14800, 15070, 15340);

----------------------------------------
-- Harvey Twisters

UPDATE
  team
SET
  name_normalized = 'Harvey Twisters',
  url_path_slug = 'twisters'
WHERE
  id = 832;

UPDATE
  tournament_team
SET
  team_id = 832
WHERE
  id IN (832, 945, 1060, 1170, 1278, 1372, 1460, 1587, 1716, 1845, 1988, 2124, 2411, 2549, 2687, 2984, 3138, 4355, 4523, 4678, 4812, 4959, 5133, 5306, 5448, 5595, 5732, 5895, 6074, 6257, 6441, 6621, 6803, 6982, 7170, 7362, 7550, 7736, 7926, 8118, 8305, 8495, 8686, 8878, 9069, 9253, 9428, 9600, 9767, 9940, 10117, 10298, 10469, 10642, 10825, 11001, 11163, 11333, 11495, 11672, 11885, 12098, 12311, 12527, 12754, 12981, 13208, 13451, 13721, 13991, 14261, 14531, 14801, 15071, 15341);

DELETE FROM
  team
WHERE
  id IN (945, 1060, 1170, 1278, 1372, 1460, 1587, 1716, 1845, 1988, 2124, 2411, 2549, 2687, 2984, 3138, 4355, 4523, 4678, 4812, 4959, 5133, 5306, 5448, 5595, 5732, 5895, 6074, 6257, 6441, 6621, 6803, 6982, 7170, 7362, 7550, 7736, 7926, 8118, 8305, 8495, 8686, 8878, 9069, 9253, 9428, 9600, 9767, 9940, 10117, 10298, 10469, 10642, 10825, 11001, 11163, 11333, 11495, 11672, 11885, 12098, 12311, 12527, 12754, 12981, 13208, 13451, 13721, 13991, 14261, 14531, 14801, 15071, 15341);

----------------------------------------
-- Hawk WC

UPDATE
  team
SET
  name_normalized = 'Hawk WC',
  url_path_slug = NULL
WHERE
  id = 7737;

UPDATE
  tournament_team
SET
  team_id = 7737
WHERE
  id IN (7737, 7927, 8119, 8306, 8496, 8687, 9601, 9768, 9941, 10118, 10299, 10470, 10643, 10826, 11002, 11164, 11334, 11496, 11673, 11886, 12099, 12312, 12528, 12755, 12982, 13209, 13452, 13722, 13992, 14262, 14532, 14802, 15072, 15342);

DELETE FROM
  team
WHERE
  id IN (7927, 8119, 8306, 8496, 8687, 9601, 9768, 9941, 10118, 10299, 10470, 10643, 10826, 11002, 11164, 11334, 11496, 11673, 11886, 12099, 12312, 12528, 12755, 12982, 13209, 13452, 13722, 13992, 14262, 14532, 14802, 15072, 15342);

----------------------------------------
-- Hawk WC Ltd.

UPDATE
  team
SET
  name_normalized = 'Hawk WC Ltd.',
  url_path_slug = NULL
WHERE
  id = 9254;

UPDATE
  tournament_team
SET
  team_id = 9254
WHERE
  id IN (9254, 9429);

DELETE FROM
  team
WHERE
  id IN (9429);

----------------------------------------
-- Hawk Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Hawk Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 7363;

UPDATE
  tournament_team
SET
  team_id = 7363
WHERE
  id IN (7363, 7551);

DELETE FROM
  team
WHERE
  id IN (7551);

----------------------------------------
-- Hawk Wrestling Club Ltd.

UPDATE
  team
SET
  name_normalized = 'Hawk Wrestling Club Ltd.',
  url_path_slug = NULL
WHERE
  id = 8879;

UPDATE
  tournament_team
SET
  team_id = 8879
WHERE
  id IN (8879, 9070);

DELETE FROM
  team
WHERE
  id IN (9070);

----------------------------------------
-- Hazel Crest

UPDATE
  team
SET
  name_normalized = 'Hazel Crest',
  url_path_slug = NULL
WHERE
  id = 162;

UPDATE
  tournament_team
SET
  team_id = 162
WHERE
  id IN (162, 213, 610);

DELETE FROM
  team
WHERE
  id IN (213, 610);

----------------------------------------
-- Hazel Crest Park District Thunder WC

UPDATE
  team
SET
  name_normalized = 'Hazel Crest Park District Thunder WC',
  url_path_slug = NULL
WHERE
  id = 10300;

UPDATE
  tournament_team
SET
  team_id = 10300
WHERE
  id IN (10300, 10471);

DELETE FROM
  team
WHERE
  id IN (10471);

----------------------------------------
-- Headlock Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Headlock Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 7738;

UPDATE
  tournament_team
SET
  team_id = 7738
WHERE
  id IN (7738, 7928, 8120, 8307, 8497, 8688, 8880, 9071, 9255, 9430, 9602, 9769, 9942, 10119, 10301, 10472, 10644, 10827, 11335, 11497, 11674, 11887, 12100, 12313, 12529, 12756, 12983, 13210, 13453, 13723, 13993, 14263, 14533, 14803, 15073, 15343);

DELETE FROM
  team
WHERE
  id IN (7928, 8120, 8307, 8497, 8688, 8880, 9071, 9255, 9430, 9602, 9769, 9942, 10119, 10301, 10472, 10644, 10827, 11335, 11497, 11674, 11887, 12100, 12313, 12529, 12756, 12983, 13210, 13453, 13723, 13993, 14263, 14533, 14803, 15073, 15343);

----------------------------------------
-- Hecox Team Benaiah

UPDATE
  team
SET
  name_normalized = 'Hecox Team Benaiah',
  url_path_slug = NULL
WHERE
  id = 5733;

UPDATE
  tournament_team
SET
  team_id = 5733
WHERE
  id IN (5733, 5896, 6075, 6258, 6442, 6622, 6804, 6983, 7171, 7364, 7552, 7739, 7929, 9256, 9431);

DELETE FROM
  team
WHERE
  id IN (5896, 6075, 6258, 6442, 6622, 6804, 6983, 7171, 7364, 7552, 7739, 7929, 9256, 9431);

----------------------------------------
-- Herrin Jr WC

UPDATE
  team
SET
  name_normalized = 'Herrin Jr WC',
  url_path_slug = NULL
WHERE
  id = 7740;

UPDATE
  tournament_team
SET
  team_id = 7740
WHERE
  id IN (7740, 7930, 9603, 9770, 9943, 10120);

DELETE FROM
  team
WHERE
  id IN (7930, 9603, 9770, 9943, 10120);

----------------------------------------
-- Herrin Junior WC

UPDATE
  team
SET
  name_normalized = 'Herrin Junior WC',
  url_path_slug = NULL
WHERE
  id = 5596;

UPDATE
  tournament_team
SET
  team_id = 5596
WHERE
  id IN (5596, 5734, 5897, 6076, 6259, 6443, 6623, 6805, 6984, 7172, 7365, 7553, 11003, 11165, 11336, 11498);

DELETE FROM
  team
WHERE
  id IN (5734, 5897, 6076, 6259, 6443, 6623, 6805, 6984, 7172, 7365, 7553, 11003, 11165, 11336, 11498);

----------------------------------------
-- Herrin Tiger Wrestling

UPDATE
  team
SET
  name_normalized = 'Herrin Tiger Wrestling',
  url_path_slug = NULL
WHERE
  id = 11675;

UPDATE
  tournament_team
SET
  team_id = 11675
WHERE
  id IN (11675, 11888, 12101, 12314, 13454, 13724, 13994, 14264, 14534, 14804, 15074, 15344);

DELETE FROM
  team
WHERE
  id IN (11888, 12101, 12314, 13454, 13724, 13994, 14264, 14534, 14804, 15074, 15344);

----------------------------------------
-- Herrin WC

UPDATE
  team
SET
  name_normalized = 'Herrin WC',
  url_path_slug = NULL
WHERE
  id = 4083;

UPDATE
  tournament_team
SET
  team_id = 4083
WHERE
  id IN (4083, 4813, 4960, 5134, 5307, 5449);

DELETE FROM
  team
WHERE
  id IN (4813, 4960, 5134, 5307, 5449);

----------------------------------------
-- Hf Elite WC

UPDATE
  team
SET
  name_normalized = 'Hf Elite WC',
  url_path_slug = NULL
WHERE
  id = 9938;

UPDATE
  tournament_team
SET
  team_id = 9938
WHERE
  id IN (9938, 10115);

DELETE FROM
  team
WHERE
  id IN (10115);

----------------------------------------
-- Hf Spartan Elite WC

UPDATE
  team
SET
  name_normalized = 'Hf Spartan Elite WC',
  url_path_slug = NULL
WHERE
  id = 8492;

UPDATE
  tournament_team
SET
  team_id = 8492
WHERE
  id IN (8492, 8683, 8875, 9066, 9251, 9426);

DELETE FROM
  team
WHERE
  id IN (8683, 8875, 9066, 9251, 9426);

----------------------------------------
-- Hickory Hills Park District

UPDATE
  team
SET
  name_normalized = 'Hickory Hills Park District',
  url_path_slug = NULL
WHERE
  id = 450;

UPDATE
  tournament_team
SET
  team_id = 450
WHERE
  id IN (450, 833, 948, 1173, 1373, 1588, 1718, 1989, 2261);

DELETE FROM
  team
WHERE
  id IN (833, 948, 1173, 1373, 1588, 1718, 1989, 2261);

----------------------------------------
-- Highland Bulldogs Jr. WC

UPDATE
  team
SET
  name_normalized = 'Highland Bulldogs Jr. WC',
  url_path_slug = NULL
WHERE
  id = 3838;

UPDATE
  tournament_team
SET
  team_id = 3838
WHERE
  id IN (3838, 3961, 4084, 4211, 4356, 4524, 4679, 4814, 4961, 5135, 5308, 5450, 5597, 5735, 5898, 6077, 6260, 6444, 6624, 6806, 6985, 7173, 7366, 7554, 7741, 7931, 8121, 8308, 9944, 10121, 10302, 10473, 10645, 10828, 11004, 11166, 11337, 11499, 11676, 11889, 12102, 12315, 12530, 12757, 12984, 13211, 13455, 13725, 13995, 14265, 14535, 14805, 15075, 15345);

DELETE FROM
  team
WHERE
  id IN (3961, 4084, 4211, 4356, 4524, 4679, 4814, 4961, 5135, 5308, 5450, 5597, 5735, 5898, 6077, 6260, 6444, 6624, 6806, 6985, 7173, 7366, 7554, 7741, 7931, 8121, 8308, 9944, 10121, 10302, 10473, 10645, 10828, 11004, 11166, 11337, 11499, 11676, 11889, 12102, 12315, 12530, 12757, 12984, 13211, 13455, 13725, 13995, 14265, 14535, 14805, 15075, 15345);

----------------------------------------
-- Highland Park Little Giants

UPDATE
  team
SET
  name_normalized = 'Highland Park Little Giants',
  url_path_slug = NULL
WHERE
  id = 1478;

UPDATE
  tournament_team
SET
  team_id = 1478
WHERE
  id IN (1478, 1603, 3839, 3962, 4104, 4984, 5158, 5309, 5451, 5598, 5736, 11698, 11911, 12124, 12337, 12556, 12783, 13010, 13237, 13487, 13757, 14027, 14297, 14567, 14837, 15107, 15377);

DELETE FROM
  team
WHERE
  id IN (1603, 3839, 3962, 4104, 4984, 5158, 5309, 5451, 5598, 5736, 11698, 11911, 12124, 12337, 12556, 12783, 13010, 13237, 13487, 13757, 14027, 14297, 14567, 14837, 15107, 15377);

----------------------------------------
-- Hill Trailblazers

UPDATE
  team
SET
  name_normalized = 'Hill Trailblazers',
  url_path_slug = NULL
WHERE
  id = 3840;

UPDATE
  tournament_team
SET
  team_id = 3840
WHERE
  id IN (3840, 3963);

DELETE FROM
  team
WHERE
  id IN (3963);

----------------------------------------
-- Hillsboro Jr. Toppers WC

UPDATE
  team
SET
  name_normalized = 'Hillsboro Jr. Toppers WC',
  url_path_slug = NULL
WHERE
  id = 3841;

UPDATE
  tournament_team
SET
  team_id = 3841
WHERE
  id IN (3841, 4085, 4212, 4357, 4525, 4680, 4962, 5136, 5310, 5452, 6261, 6445, 6986, 7174, 7367, 7555, 8122, 8309, 8498, 8689, 8881, 9072, 9604, 9771, 10303, 10474, 11005, 11167, 11677, 11890, 12103, 12316, 13456, 13726, 13996, 14266, 14536, 14806, 15076, 15346);

DELETE FROM
  team
WHERE
  id IN (4085, 4212, 4357, 4525, 4680, 4962, 5136, 5310, 5452, 6261, 6445, 6986, 7174, 7367, 7555, 8122, 8309, 8498, 8689, 8881, 9072, 9604, 9771, 10303, 10474, 11005, 11167, 11677, 11890, 12103, 12316, 13456, 13726, 13996, 14266, 14536, 14806, 15076, 15346);

----------------------------------------
-- Hilltoppers WC

UPDATE
  team
SET
  name_normalized = 'Hilltoppers WC',
  url_path_slug = NULL
WHERE
  id = 4358;

UPDATE
  tournament_team
SET
  team_id = 4358
WHERE
  id IN (4358, 4526, 5599, 5899, 6078, 6262, 6446, 6625, 6807);

DELETE FROM
  team
WHERE
  id IN (4526, 5599, 5899, 6078, 6262, 6446, 6625, 6807);

----------------------------------------
-- Hinsdale Falcon WC

UPDATE
  team
SET
  name_normalized = 'Hinsdale Falcon WC',
  url_path_slug = NULL
WHERE
  id = 8123;

UPDATE
  tournament_team
SET
  team_id = 8123
WHERE
  id IN (8123, 8310, 8499, 8690);

DELETE FROM
  team
WHERE
  id IN (8310, 8499, 8690);

----------------------------------------
-- Hinsdale Red Devil WC

UPDATE
  team
SET
  name_normalized = 'Hinsdale Red Devil WC',
  url_path_slug = NULL
WHERE
  id = 1463;

UPDATE
  tournament_team
SET
  team_id = 1463
WHERE
  id IN (1463, 1589, 1720, 3842, 4213, 4359, 4527, 4681, 4815, 4963, 5137, 5311, 5453, 5600, 5737, 5900, 6079, 6626, 6808, 6987, 7175, 7368, 7556, 7742, 7932, 8882, 9073, 9257, 9432, 9605, 9772, 9945, 10122, 10304, 10475, 11338, 11500, 11678, 11891, 12104, 12317, 12531, 12758, 12985, 13212, 13457, 13727, 13997, 14267, 14537, 14807, 15077, 15347);

DELETE FROM
  team
WHERE
  id IN (1589, 1720, 3842, 4213, 4359, 4527, 4681, 4815, 4963, 5137, 5311, 5453, 5600, 5737, 5900, 6079, 6626, 6808, 6987, 7175, 7368, 7556, 7742, 7932, 8882, 9073, 9257, 9432, 9605, 9772, 9945, 10122, 10304, 10475, 11338, 11500, 11678, 11891, 12104, 12317, 12531, 12758, 12985, 13212, 13457, 13727, 13997, 14267, 14537, 14807, 15077, 15347);

----------------------------------------
-- Hoffman Estates WC

UPDATE
  team
SET
  name_normalized = 'Hoffman Estates WC',
  url_path_slug = NULL
WHERE
  id = 4682;

UPDATE
  tournament_team
SET
  team_id = 4682
WHERE
  id IN (4682, 4816, 4964, 5138, 5454, 5738, 5901, 6080, 6627, 6809, 6988, 7176);

DELETE FROM
  team
WHERE
  id IN (4816, 4964, 5138, 5454, 5738, 5901, 6080, 6627, 6809, 6988, 7176);

----------------------------------------
-- Holy Family School

UPDATE
  team
SET
  name_normalized = 'Holy Family School',
  url_path_slug = NULL
WHERE
  id = 5902;

UPDATE
  tournament_team
SET
  team_id = 5902
WHERE
  id IN (5902, 6081, 6263, 6447);

DELETE FROM
  team
WHERE
  id IN (6081, 6263, 6447);

----------------------------------------
-- Homewood-Flossmoor Cats

UPDATE
  team
SET
  name_normalized = 'Homewood-Flossmoor Cats',
  url_path_slug = NULL
WHERE
  id = 4360;

UPDATE
  tournament_team
SET
  team_id = 4360
WHERE
  id IN (4360, 4528);

DELETE FROM
  team
WHERE
  id IN (4528);

----------------------------------------
-- Hononegah Kids WC

UPDATE
  team
SET
  name_normalized = 'Hononegah Kids WC',
  url_path_slug = NULL
WHERE
  id = 3611;

UPDATE
  tournament_team
SET
  team_id = 3611
WHERE
  id IN (3611, 3843, 3964, 4087, 4214, 4361, 4529, 4683, 4817, 4965, 5139, 5312, 5455, 5601, 5739, 5903, 6082, 6264, 6448, 6628, 6810, 6989, 7177, 7369, 7557);

DELETE FROM
  team
WHERE
  id IN (3843, 3964, 4087, 4214, 4361, 4529, 4683, 4817, 4965, 5139, 5312, 5455, 5601, 5739, 5903, 6082, 6264, 6448, 6628, 6810, 6989, 7177, 7369, 7557);

----------------------------------------
-- Hononegah WC

UPDATE
  team
SET
  name_normalized = 'Hononegah WC',
  url_path_slug = NULL
WHERE
  id = 7743;

UPDATE
  tournament_team
SET
  team_id = 7743
WHERE
  id IN (7743, 7933, 8124, 8311, 8500, 8691, 9258, 9433, 9606, 9773, 9946, 10123, 10305, 10476, 10646, 10829, 11006, 11168, 11339, 11501, 11679, 11892, 12105, 12318, 12532, 12759, 12986, 13213, 13458, 13728, 13998, 14268, 14538, 14808, 15078, 15348);

DELETE FROM
  team
WHERE
  id IN (7933, 8124, 8311, 8500, 8691, 9258, 9433, 9606, 9773, 9946, 10123, 10305, 10476, 10646, 10829, 11006, 11168, 11339, 11501, 11679, 11892, 12105, 12318, 12532, 12759, 12986, 13213, 13458, 13728, 13998, 14268, 14538, 14808, 15078, 15348);

----------------------------------------
-- Hononegah Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Hononegah Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8883;

UPDATE
  tournament_team
SET
  team_id = 8883
WHERE
  id IN (8883, 9074);

DELETE FROM
  team
WHERE
  id IN (9074);

----------------------------------------
-- Hoopeston

UPDATE
  team
SET
  name_normalized = 'Hoopeston',
  url_path_slug = NULL
WHERE
  id = 614;

UPDATE
  tournament_team
SET
  team_id = 614
WHERE
  id IN (614, 1851);

DELETE FROM
  team
WHERE
  id IN (1851);

----------------------------------------
-- Hoopeston Area WC

UPDATE
  team
SET
  name_normalized = 'Hoopeston Area WC',
  url_path_slug = NULL
WHERE
  id = 3844;

UPDATE
  tournament_team
SET
  team_id = 3844
WHERE
  id IN (3844, 3965, 4088, 4215, 4362, 4530, 4684, 4818, 4966, 5140, 5313, 5456, 5602, 5740, 5904, 6083, 6265, 6449, 6629, 6811, 6990, 7178, 7370, 7558, 7744, 7934, 9259, 9434, 9607, 9774, 9947, 10124, 10306, 10477, 11007, 11169, 11340, 11502);

DELETE FROM
  team
WHERE
  id IN (3965, 4088, 4215, 4362, 4530, 4684, 4818, 4966, 5140, 5313, 5456, 5602, 5740, 5904, 6083, 6265, 6449, 6629, 6811, 6990, 7178, 7370, 7558, 7744, 7934, 9259, 9434, 9607, 9774, 9947, 10124, 10306, 10477, 11007, 11169, 11340, 11502);

----------------------------------------
-- Hoopeston Area Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Hoopeston Area Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8884;

UPDATE
  tournament_team
SET
  team_id = 8884
WHERE
  id IN (8884, 9075);

DELETE FROM
  team
WHERE
  id IN (9075);

----------------------------------------
-- Hoopeston Area Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Hoopeston Area Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 11680;

UPDATE
  tournament_team
SET
  team_id = 11680
WHERE
  id IN (11680, 11893, 12106, 12319, 12533, 12760, 12987, 13214, 13459, 13729, 13999, 14269, 14539, 14809, 15079, 15349);

DELETE FROM
  team
WHERE
  id IN (11893, 12106, 12319, 12533, 12760, 12987, 13214, 13459, 13729, 13999, 14269, 14539, 14809, 15079, 15349);

----------------------------------------
-- Hoopeston Intermediate Wrestling

UPDATE
  team
SET
  name_normalized = 'Hoopeston Intermediate Wrestling',
  url_path_slug = NULL
WHERE
  id = 7371;

UPDATE
  tournament_team
SET
  team_id = 7371
WHERE
  id IN (7371, 7559);

DELETE FROM
  team
WHERE
  id IN (7559);

----------------------------------------
-- Hoopeston Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Hoopeston Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 8125;

UPDATE
  tournament_team
SET
  team_id = 8125
WHERE
  id IN (8125, 8312, 8501, 8692);

DELETE FROM
  team
WHERE
  id IN (8312, 8501, 8692);

----------------------------------------
-- Hoopeston-East Lynn

UPDATE
  team
SET
  name_normalized = 'Hoopeston-East Lynn',
  url_path_slug = NULL
WHERE
  id = 1063;

UPDATE
  tournament_team
SET
  team_id = 1063
WHERE
  id IN (1063, 1174);

DELETE FROM
  team
WHERE
  id IN (1174);

----------------------------------------
-- Huntley Jr. Red Raiders

UPDATE
  team
SET
  name_normalized = 'Huntley Jr. Red Raiders',
  url_path_slug = NULL
WHERE
  id = 7372;

UPDATE
  tournament_team
SET
  team_id = 7372
WHERE
  id IN (7372, 7560, 7745, 7935);

DELETE FROM
  team
WHERE
  id IN (7560, 7745, 7935);

----------------------------------------
-- Huntley Jr. Red Raiders WC

UPDATE
  team
SET
  name_normalized = 'Huntley Jr. Red Raiders WC',
  url_path_slug = NULL
WHERE
  id = 8126;

UPDATE
  tournament_team
SET
  team_id = 8126
WHERE
  id IN (8126, 8313);

DELETE FROM
  team
WHERE
  id IN (8313);

----------------------------------------
-- Hurricanes

UPDATE
  team
SET
  name_normalized = 'Hurricanes',
  url_path_slug = NULL
WHERE
  id = 5314;

UPDATE
  tournament_team
SET
  team_id = 5314
WHERE
  id IN (5314, 5457, 5741);

DELETE FROM
  team
WHERE
  id IN (5457, 5741);

----------------------------------------
-- Huskies WC

UPDATE
  team
SET
  name_normalized = 'Huskies WC',
  url_path_slug = NULL
WHERE
  id = 3845;

UPDATE
  tournament_team
SET
  team_id = 3845
WHERE
  id IN (3845, 3966, 4089, 4216, 4363, 4531);

DELETE FROM
  team
WHERE
  id IN (3966, 4089, 4216, 4363, 4531);

----------------------------------------
-- Iguana WC

UPDATE
  team
SET
  name_normalized = 'Iguana WC',
  url_path_slug = NULL
WHERE
  id = 9260;

UPDATE
  tournament_team
SET
  team_id = 9260
WHERE
  id IN (9260, 9435, 9608, 9775, 9948, 10125, 10647, 10830, 11682, 11895, 12108, 12321, 12535, 12762, 12989, 13216, 13461, 13731, 14001, 14271, 14541, 14811, 15081, 15351);

DELETE FROM
  team
WHERE
  id IN (9435, 9608, 9775, 9948, 10125, 10647, 10830, 11682, 11895, 12108, 12321, 12535, 12762, 12989, 13216, 13461, 13731, 14001, 14271, 14541, 14811, 15081, 15351);

----------------------------------------
-- Iguana Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Iguana Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8885;

UPDATE
  tournament_team
SET
  team_id = 8885
WHERE
  id IN (8885, 9076);

DELETE FROM
  team
WHERE
  id IN (9076);

----------------------------------------
-- Illini Bluffs Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Illini Bluffs Kids Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 951;

UPDATE
  tournament_team
SET
  team_id = 951
WHERE
  id IN (951, 1465, 3846, 4090, 4364, 4532, 6991, 7179, 7373, 7561, 7746, 7936, 8127, 8314, 8886, 9077, 9609, 9776);

DELETE FROM
  team
WHERE
  id IN (1465, 3846, 4090, 4364, 4532, 6991, 7179, 7373, 7561, 7746, 7936, 8127, 8314, 8886, 9077, 9609, 9776);

----------------------------------------
-- Illinois Valley Elite WC

UPDATE
  team
SET
  name_normalized = 'Illinois Valley Elite WC',
  url_path_slug = NULL
WHERE
  id = 7374;

UPDATE
  tournament_team
SET
  team_id = 7374
WHERE
  id IN (7374, 7562);

DELETE FROM
  team
WHERE
  id IN (7562);

----------------------------------------
-- Illinois Valley Elite Wrestling

UPDATE
  team
SET
  name_normalized = 'Illinois Valley Elite Wrestling',
  url_path_slug = NULL
WHERE
  id = 7747;

UPDATE
  tournament_team
SET
  team_id = 7747
WHERE
  id IN (7747, 7937, 8128, 8315, 8502, 8693);

DELETE FROM
  team
WHERE
  id IN (7937, 8128, 8315, 8502, 8693);

----------------------------------------
-- Impact WC

UPDATE
  team
SET
  name_normalized = 'Impact WC',
  url_path_slug = NULL
WHERE
  id = 5458;

UPDATE
  tournament_team
SET
  team_id = 5458
WHERE
  id IN (5458, 5603, 5742);

DELETE FROM
  team
WHERE
  id IN (5603, 5742);

----------------------------------------
-- Impact Wrestling

UPDATE
  team
SET
  name_normalized = 'Impact Wrestling',
  url_path_slug = NULL
WHERE
  id = 13462;

UPDATE
  tournament_team
SET
  team_id = 13462
WHERE
  id IN (13462, 13732, 14002, 14272, 14542, 14812, 15082, 15352);

DELETE FROM
  team
WHERE
  id IN (13732, 14002, 14272, 14542, 14812, 15082, 15352);

----------------------------------------
-- Indian Prairie

UPDATE
  team
SET
  name_normalized = 'Indian Prairie',
  url_path_slug = NULL
WHERE
  id = 836;

UPDATE
  tournament_team
SET
  team_id = 836
WHERE
  id IN (836, 952, 1064);

DELETE FROM
  team
WHERE
  id IN (952, 1064);

----------------------------------------
-- Indian Prairie Pioneers

UPDATE
  team
SET
  name_normalized = 'Indian Prairie Pioneers',
  url_path_slug = NULL
WHERE
  id = 1375;

UPDATE
  tournament_team
SET
  team_id = 1375
WHERE
  id IN (1375, 1466, 1592);

DELETE FROM
  team
WHERE
  id IN (1466, 1592);

----------------------------------------
-- Indians Elite

UPDATE
  team
SET
  name_normalized = 'Indians Elite',
  url_path_slug = NULL
WHERE
  id = 6630;

UPDATE
  tournament_team
SET
  team_id = 6630
WHERE
  id IN (6630, 6812, 6992, 7180, 7375, 7563, 7748, 7938, 8129, 8316, 8503, 8694, 8887, 9078);

DELETE FROM
  team
WHERE
  id IN (6812, 6992, 7180, 7375, 7563, 7748, 7938, 8129, 8316, 8503, 8694, 8887, 9078);

----------------------------------------
-- Irish WC

UPDATE
  team
SET
  name_normalized = 'Irish WC',
  url_path_slug = NULL
WHERE
  id = 13463;

UPDATE
  tournament_team
SET
  team_id = 13463
WHERE
  id IN (13463, 13733, 14003, 14273, 14543, 14813, 15083, 15353);

DELETE FROM
  team
WHERE
  id IN (13733, 14003, 14273, 14543, 14813, 15083, 15353);

----------------------------------------
-- Iron Man

UPDATE
  team
SET
  name_normalized = 'Iron Man',
  url_path_slug = NULL
WHERE
  id = 5604;

UPDATE
  tournament_team
SET
  team_id = 5604
WHERE
  id IN (5604, 5743, 5905, 6084, 6266, 6450, 6631, 6813);

DELETE FROM
  team
WHERE
  id IN (5743, 5905, 6084, 6266, 6450, 6631, 6813);

----------------------------------------
-- Isi WC

UPDATE
  team
SET
  name_normalized = 'Isi WC',
  url_path_slug = NULL
WHERE
  id = 11008;

UPDATE
  tournament_team
SET
  team_id = 11008
WHERE
  id IN (11008, 11170, 11341, 11503, 11681, 11894, 12107, 12320, 12534, 12761, 12988, 13215, 13460, 13730, 14000, 14270, 14540, 14810, 15080, 15350);

DELETE FROM
  team
WHERE
  id IN (11170, 11341, 11503, 11681, 11894, 12107, 12320, 12534, 12761, 12988, 13215, 13460, 13730, 14000, 14270, 14540, 14810, 15080, 15350);

----------------------------------------
-- Izzy Style School Of Wrestling

UPDATE
  team
SET
  name_normalized = 'Izzy Style School Of Wrestling',
  url_path_slug = NULL
WHERE
  id = 7376;

UPDATE
  tournament_team
SET
  team_id = 7376
WHERE
  id IN (7376, 7564, 7749, 7939, 8130, 8317);

DELETE FROM
  team
WHERE
  id IN (7564, 7749, 7939, 8130, 8317);

----------------------------------------
-- Izzy Style Wrestling

UPDATE
  team
SET
  name_normalized = 'Izzy Style Wrestling',
  url_path_slug = NULL
WHERE
  id = 8504;

UPDATE
  tournament_team
SET
  team_id = 8504
WHERE
  id IN (8504, 8695, 8888, 9079, 9261, 9436, 9610, 9777, 9949, 10126, 10307, 10478, 10648, 10831, 11009, 11171, 11342, 11504, 11683, 11896, 12109, 12322, 12536, 12763, 12990, 13217, 13464, 13734, 14004, 14274, 14544, 14814, 15084, 15354);

DELETE FROM
  team
WHERE
  id IN (8695, 8888, 9079, 9261, 9436, 9610, 9777, 9949, 10126, 10307, 10478, 10648, 10831, 11009, 11171, 11342, 11504, 11683, 11896, 12109, 12322, 12536, 12763, 12990, 13217, 13464, 13734, 14004, 14274, 14544, 14814, 15084, 15354);

----------------------------------------
-- J.R. Warrior WC

UPDATE
  team
SET
  name_normalized = 'J.R. Warrior WC',
  url_path_slug = NULL
WHERE
  id = 13465;

UPDATE
  tournament_team
SET
  team_id = 13465
WHERE
  id IN (13465, 13735, 14005, 14275, 14545, 14815, 15085, 15355);

DELETE FROM
  team
WHERE
  id IN (13735, 14005, 14275, 14545, 14815, 15085, 15355);

----------------------------------------
-- Jacksonville Area Wrestling

UPDATE
  team
SET
  name_normalized = 'Jacksonville Area Wrestling',
  url_path_slug = NULL
WHERE
  id = 7750;

UPDATE
  tournament_team
SET
  team_id = 7750
WHERE
  id IN (7750, 7940, 8131, 8318, 9262, 9437, 9611, 9778, 9950, 10127, 12537, 12764, 12991, 13218, 13466, 13736, 14006, 14276, 14546, 14816, 15086, 15356);

DELETE FROM
  team
WHERE
  id IN (7940, 8131, 8318, 9262, 9437, 9611, 9778, 9950, 10127, 12537, 12764, 12991, 13218, 13466, 13736, 14006, 14276, 14546, 14816, 15086, 15356);

----------------------------------------
-- Jacksonville Area Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Jacksonville Area Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 6632;

UPDATE
  tournament_team
SET
  team_id = 6632
WHERE
  id IN (6632, 6814, 6993, 7181, 7377, 7565);

DELETE FROM
  team
WHERE
  id IN (6814, 6993, 7181, 7377, 7565);

----------------------------------------
-- Jacksonville WC

UPDATE
  team
SET
  name_normalized = 'Jacksonville WC',
  url_path_slug = NULL
WHERE
  id = 3847;

UPDATE
  tournament_team
SET
  team_id = 3847
WHERE
  id IN (3847, 3967, 4091, 4217, 4365, 4533, 4819, 4967, 5141);

DELETE FROM
  team
WHERE
  id IN (3967, 4091, 4217, 4365, 4533, 4819, 4967, 5141);

----------------------------------------
-- Jacobs Falcons

UPDATE
  team
SET
  name_normalized = 'Jacobs Falcons',
  url_path_slug = NULL
WHERE
  id = 4092;

UPDATE
  tournament_team
SET
  team_id = 4092
WHERE
  id IN (4092, 4218);

DELETE FROM
  team
WHERE
  id IN (4218);

----------------------------------------
-- Jefferson

UPDATE
  team
SET
  name_normalized = 'Jefferson',
  url_path_slug = NULL
WHERE
  id = 838;

UPDATE
  tournament_team
SET
  team_id = 838
WHERE
  id IN (838, 954);

DELETE FROM
  team
WHERE
  id IN (954);

----------------------------------------
-- Jersey Junior Panthers WC

UPDATE
  team
SET
  name_normalized = 'Jersey Junior Panthers WC',
  url_path_slug = NULL
WHERE
  id = 6267;

UPDATE
  tournament_team
SET
  team_id = 6267
WHERE
  id IN (6267, 6451, 6633, 6815);

DELETE FROM
  team
WHERE
  id IN (6451, 6633, 6815);

----------------------------------------
-- Jersey Junior Panthers Wrestling

UPDATE
  team
SET
  name_normalized = 'Jersey Junior Panthers Wrestling',
  url_path_slug = NULL
WHERE
  id = 8505;

UPDATE
  tournament_team
SET
  team_id = 8505
WHERE
  id IN (8505, 8696, 9263, 9438, 10308, 10479, 12538, 12765, 12992, 13219, 13467, 13737, 14007, 14277, 14547, 14817, 15087, 15357);

DELETE FROM
  team
WHERE
  id IN (8696, 9263, 9438, 10308, 10479, 12538, 12765, 12992, 13219, 13467, 13737, 14007, 14277, 14547, 14817, 15087, 15357);

----------------------------------------
-- Jjk Wrestling

UPDATE
  team
SET
  name_normalized = 'Jjk Wrestling',
  url_path_slug = NULL
WHERE
  id = 6634;

UPDATE
  tournament_team
SET
  team_id = 6634
WHERE
  id IN (6634, 6816, 6994, 7182);

DELETE FROM
  team
WHERE
  id IN (6816, 6994, 7182);

----------------------------------------
-- Joliet Area WC

UPDATE
  team
SET
  name_normalized = 'Joliet Area WC',
  url_path_slug = NULL
WHERE
  id = 10309;

UPDATE
  tournament_team
SET
  team_id = 10309
WHERE
  id IN (10309, 10480);

DELETE FROM
  team
WHERE
  id IN (10480);

----------------------------------------
-- Joliet Boys Club

UPDATE
  team
SET
  name_normalized = 'Joliet Boys Club',
  url_path_slug = NULL
WHERE
  id = 5;

UPDATE
  tournament_team
SET
  team_id = 5
WHERE
  id IN (5, 39, 71, 123, 163, 190, 215, 280, 1469, 1594, 1725, 1855, 2132, 2557, 2696, 4366, 4534, 4685, 4820, 4968, 5142, 5315);

DELETE FROM
  team
WHERE
  id IN (39, 71, 123, 163, 190, 215, 280, 1469, 1594, 1725, 1855, 2132, 2557, 2696, 4366, 4534, 4685, 4820, 4968, 5142, 5315);

----------------------------------------
-- Joliet Junior Steelmen

UPDATE
  team
SET
  name_normalized = 'Joliet Junior Steelmen',
  url_path_slug = NULL
WHERE
  id = 5605;

UPDATE
  tournament_team
SET
  team_id = 5605
WHERE
  id IN (5605, 5744, 5906, 6085, 6268, 6452, 6635, 6817);

DELETE FROM
  team
WHERE
  id IN (5744, 5906, 6085, 6268, 6452, 6635, 6817);

----------------------------------------
-- Joliet YMCA

UPDATE
  team
SET
  name_normalized = 'Joliet YMCA',
  url_path_slug = NULL
WHERE
  id = 281;

UPDATE
  tournament_team
SET
  team_id = 281
WHERE
  id IN (281, 380, 839);

DELETE FROM
  team
WHERE
  id IN (380, 839);

----------------------------------------
-- Jordan / Seton Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Jordan / Seton Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1067;

UPDATE
  tournament_team
SET
  team_id = 1067
WHERE
  id IN (1067, 1180, 1856, 2424);

DELETE FROM
  team
WHERE
  id IN (1180, 1856, 2424);

----------------------------------------
-- Jr Maroon WC

UPDATE
  team
SET
  name_normalized = 'Jr Maroon WC',
  url_path_slug = NULL
WHERE
  id = 3850;

UPDATE
  tournament_team
SET
  team_id = 3850
WHERE
  id IN (3850, 4094);

DELETE FROM
  team
WHERE
  id IN (4094);

----------------------------------------
-- Jr Maroons WC

UPDATE
  team
SET
  name_normalized = 'Jr Maroons WC',
  url_path_slug = NULL
WHERE
  id = 12539;

UPDATE
  tournament_team
SET
  team_id = 12539
WHERE
  id IN (12539, 12766, 12993, 13220);

DELETE FROM
  team
WHERE
  id IN (12766, 12993, 13220);

----------------------------------------
-- Jr Rocket Wrestling

UPDATE
  team
SET
  name_normalized = 'Jr Rocket Wrestling',
  url_path_slug = NULL
WHERE
  id = 3851;

UPDATE
  tournament_team
SET
  team_id = 3851
WHERE
  id IN (3851, 3969, 4095, 4220);

DELETE FROM
  team
WHERE
  id IN (3969, 4095, 4220);

----------------------------------------
-- Jr Sentinels

UPDATE
  team
SET
  name_normalized = 'Jr Sentinels',
  url_path_slug = NULL
WHERE
  id = 4096;

UPDATE
  tournament_team
SET
  team_id = 4096
WHERE
  id IN (4096, 4221);

DELETE FROM
  team
WHERE
  id IN (4221);

----------------------------------------
-- Jr. Blazer WC

UPDATE
  team
SET
  name_normalized = 'Jr. Blazer WC',
  url_path_slug = NULL
WHERE
  id = 6995;

UPDATE
  tournament_team
SET
  team_id = 6995
WHERE
  id IN (6995, 7183);

DELETE FROM
  team
WHERE
  id IN (7183);

----------------------------------------
-- Jr. Cougar WC

UPDATE
  team
SET
  name_normalized = 'Jr. Cougar WC',
  url_path_slug = NULL
WHERE
  id = 4367;

UPDATE
  tournament_team
SET
  team_id = 4367
WHERE
  id IN (4367, 4535);

DELETE FROM
  team
WHERE
  id IN (4535);

----------------------------------------
-- Jr. Cougars WC

UPDATE
  team
SET
  name_normalized = 'Jr. Cougars WC',
  url_path_slug = NULL
WHERE
  id = 4969;

UPDATE
  tournament_team
SET
  team_id = 4969
WHERE
  id IN (4969, 5143, 5745);

DELETE FROM
  team
WHERE
  id IN (5143, 5745);

----------------------------------------
-- Jr. Golden Eagles

UPDATE
  team
SET
  name_normalized = 'Jr. Golden Eagles',
  url_path_slug = NULL
WHERE
  id = 3613;

UPDATE
  tournament_team
SET
  team_id = 3613
WHERE
  id IN (3613, 3733, 3849, 3968, 4093, 4219, 4368, 4536, 4686, 4821, 4970, 5144);

DELETE FROM
  team
WHERE
  id IN (3733, 3849, 3968, 4093, 4219, 4368, 4536, 4686, 4821, 4970, 5144);

----------------------------------------
-- Jr. Maroon WC

UPDATE
  team
SET
  name_normalized = 'Jr. Maroon WC',
  url_path_slug = NULL
WHERE
  id = 4369;

UPDATE
  tournament_team
SET
  team_id = 4369
WHERE
  id IN (4369, 4537, 4971, 5145);

DELETE FROM
  team
WHERE
  id IN (4537, 4971, 5145);

----------------------------------------
-- Jr. Panthers Wrestling

UPDATE
  team
SET
  name_normalized = 'Jr. Panthers Wrestling',
  url_path_slug = NULL
WHERE
  id = 4370;

UPDATE
  tournament_team
SET
  team_id = 4370
WHERE
  id IN (4370, 4538, 4687, 4822);

DELETE FROM
  team
WHERE
  id IN (4538, 4687, 4822);

----------------------------------------
-- Jr. Rocket WC

UPDATE
  team
SET
  name_normalized = 'Jr. Rocket WC',
  url_path_slug = NULL
WHERE
  id = 1857;

UPDATE
  tournament_team
SET
  team_id = 1857
WHERE
  id IN (1857, 1995, 2136, 2856, 3736, 4371, 4539, 4823);

DELETE FROM
  team
WHERE
  id IN (1995, 2136, 2856, 3736, 4371, 4539, 4823);

----------------------------------------
-- Jr. Saxons WC

UPDATE
  team
SET
  name_normalized = 'Jr. Saxons WC',
  url_path_slug = NULL
WHERE
  id = 4688;

UPDATE
  tournament_team
SET
  team_id = 4688
WHERE
  id IN (4688, 4824, 4972, 5146);

DELETE FROM
  team
WHERE
  id IN (4824, 4972, 5146);

----------------------------------------
-- Jr. Sentinels

UPDATE
  team
SET
  name_normalized = 'Jr. Sentinels',
  url_path_slug = NULL
WHERE
  id = 4372;

UPDATE
  tournament_team
SET
  team_id = 4372
WHERE
  id IN (4372, 4540, 4973, 5147);

DELETE FROM
  team
WHERE
  id IN (4540, 4973, 5147);

----------------------------------------
-- Junior Bronco WC

UPDATE
  team
SET
  name_normalized = 'Junior Bronco WC',
  url_path_slug = NULL
WHERE
  id = 11010;

UPDATE
  tournament_team
SET
  team_id = 11010
WHERE
  id IN (11010, 11172);

DELETE FROM
  team
WHERE
  id IN (11172);

----------------------------------------
-- Junior Bulldog WC

UPDATE
  team
SET
  name_normalized = 'Junior Bulldog WC',
  url_path_slug = NULL
WHERE
  id = 13468;

UPDATE
  tournament_team
SET
  team_id = 13468
WHERE
  id IN (13468, 13738, 14008, 14278, 14548, 14818, 15088, 15358);

DELETE FROM
  team
WHERE
  id IN (13738, 14008, 14278, 14548, 14818, 15088, 15358);

----------------------------------------
-- Junior Comanche WC

UPDATE
  team
SET
  name_normalized = 'Junior Comanche WC',
  url_path_slug = NULL
WHERE
  id = 9264;

UPDATE
  tournament_team
SET
  team_id = 9264
WHERE
  id IN (9264, 9439, 9612, 9779);

DELETE FROM
  team
WHERE
  id IN (9439, 9612, 9779);

----------------------------------------
-- Junior Cougar WC

UPDATE
  team
SET
  name_normalized = 'Junior Cougar WC',
  url_path_slug = NULL
WHERE
  id = 2857;

UPDATE
  tournament_team
SET
  team_id = 2857
WHERE
  id IN (2857, 7378, 7566, 7751, 7941, 8132, 8319, 8506, 8697, 9265, 9440, 9613, 9780, 9951, 10128, 10310, 10481, 11343, 11505, 13469, 13739, 14009, 14279, 14549, 14819, 15089, 15359);

DELETE FROM
  team
WHERE
  id IN (7378, 7566, 7751, 7941, 8132, 8319, 8506, 8697, 9265, 9440, 9613, 9780, 9951, 10128, 10310, 10481, 11343, 11505, 13469, 13739, 14009, 14279, 14549, 14819, 15089, 15359);

----------------------------------------
-- Junior Cougar Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Junior Cougar Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8889;

UPDATE
  tournament_team
SET
  team_id = 8889
WHERE
  id IN (8889, 9080);

DELETE FROM
  team
WHERE
  id IN (9080);

----------------------------------------
-- Junior Cougars WC

UPDATE
  team
SET
  name_normalized = 'Junior Cougars WC',
  url_path_slug = NULL
WHERE
  id = 6269;

UPDATE
  tournament_team
SET
  team_id = 6269
WHERE
  id IN (6269, 6453, 6636, 6818, 6996, 7184);

DELETE FROM
  team
WHERE
  id IN (6453, 6636, 6818, 6996, 7184);

----------------------------------------
-- Junior Cyclones WC

UPDATE
  team
SET
  name_normalized = 'Junior Cyclones WC',
  url_path_slug = NULL
WHERE
  id = 9952;

UPDATE
  tournament_team
SET
  team_id = 9952
WHERE
  id IN (9952, 10129, 10311, 10482, 11344, 11506, 13470, 13740, 14010, 14280, 14550, 14820, 15090, 15360);

DELETE FROM
  team
WHERE
  id IN (10129, 10311, 10482, 11344, 11506, 13470, 13740, 14010, 14280, 14550, 14820, 15090, 15360);

----------------------------------------
-- Junior Dukes WC

UPDATE
  team
SET
  name_normalized = 'Junior Dukes WC',
  url_path_slug = NULL
WHERE
  id = 7752;

UPDATE
  tournament_team
SET
  team_id = 7752
WHERE
  id IN (7752, 7942);

DELETE FROM
  team
WHERE
  id IN (7942);

----------------------------------------
-- Junior Eagles WC

UPDATE
  team
SET
  name_normalized = 'Junior Eagles WC',
  url_path_slug = NULL
WHERE
  id = 10649;

UPDATE
  tournament_team
SET
  team_id = 10649
WHERE
  id IN (10649, 10832);

DELETE FROM
  team
WHERE
  id IN (10832);

----------------------------------------
-- Junior Gators WC

UPDATE
  team
SET
  name_normalized = 'Junior Gators WC',
  url_path_slug = NULL
WHERE
  id = 5606;

UPDATE
  tournament_team
SET
  team_id = 5606
WHERE
  id IN (5606, 5907, 6086, 6270, 6454);

DELETE FROM
  team
WHERE
  id IN (5907, 6086, 6270, 6454);

----------------------------------------
-- Junior Kahoks

UPDATE
  team
SET
  name_normalized = 'Junior Kahoks',
  url_path_slug = NULL
WHERE
  id = 7753;

UPDATE
  tournament_team
SET
  team_id = 7753
WHERE
  id IN (7753, 7943, 8133, 8320, 8507, 8698, 8890, 9081, 9266, 9441, 9614, 9781, 9953, 10130);

DELETE FROM
  team
WHERE
  id IN (7943, 8133, 8320, 8507, 8698, 8890, 9081, 9266, 9441, 9614, 9781, 9953, 10130);

----------------------------------------
-- Junior Kahoks WC

UPDATE
  team
SET
  name_normalized = 'Junior Kahoks WC',
  url_path_slug = NULL
WHERE
  id = 10650;

UPDATE
  tournament_team
SET
  team_id = 10650
WHERE
  id IN (10650, 10833, 11011, 11173);

DELETE FROM
  team
WHERE
  id IN (10833, 11011, 11173);

----------------------------------------
-- Junior Maroon WC

UPDATE
  team
SET
  name_normalized = 'Junior Maroon WC',
  url_path_slug = NULL
WHERE
  id = 6271;

UPDATE
  tournament_team
SET
  team_id = 6271
WHERE
  id IN (6271, 6455, 6637, 6819, 7379, 7567);

DELETE FROM
  team
WHERE
  id IN (6455, 6637, 6819, 7379, 7567);

----------------------------------------
-- Junior Patriots

UPDATE
  team
SET
  name_normalized = 'Junior Patriots',
  url_path_slug = NULL
WHERE
  id = 6638;

UPDATE
  tournament_team
SET
  team_id = 6638
WHERE
  id IN (6638, 6820);

DELETE FROM
  team
WHERE
  id IN (6820);

----------------------------------------
-- Junior Patriots WC

UPDATE
  team
SET
  name_normalized = 'Junior Patriots WC',
  url_path_slug = NULL
WHERE
  id = 11684;

UPDATE
  tournament_team
SET
  team_id = 11684
WHERE
  id IN (11684, 11897, 12110, 12323, 12540, 12767, 12994, 13221);

DELETE FROM
  team
WHERE
  id IN (11897, 12110, 12323, 12540, 12767, 12994, 13221);

----------------------------------------
-- Junior Patriots Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Junior Patriots Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8891;

UPDATE
  tournament_team
SET
  team_id = 8891
WHERE
  id IN (8891, 9082);

DELETE FROM
  team
WHERE
  id IN (9082);

----------------------------------------
-- Junior Pioneer WC

UPDATE
  team
SET
  name_normalized = 'Junior Pioneer WC',
  url_path_slug = NULL
WHERE
  id = 6639;

UPDATE
  tournament_team
SET
  team_id = 6639
WHERE
  id IN (6639, 6821, 6997, 7185, 7380, 7568, 9267, 9442, 9615, 9782, 9954, 10131, 10312, 10483, 10651, 10834, 11012, 11174, 11685, 11898, 12111, 12324, 13471, 13741, 14011, 14281, 14551, 14821, 15091, 15361);

DELETE FROM
  team
WHERE
  id IN (6821, 6997, 7185, 7380, 7568, 9267, 9442, 9615, 9782, 9954, 10131, 10312, 10483, 10651, 10834, 11012, 11174, 11685, 11898, 12111, 12324, 13471, 13741, 14011, 14281, 14551, 14821, 15091, 15361);

----------------------------------------
-- Junior Pioneer Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Junior Pioneer Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8892;

UPDATE
  tournament_team
SET
  team_id = 8892
WHERE
  id IN (8892, 9083);

DELETE FROM
  team
WHERE
  id IN (9083);

----------------------------------------
-- Junior Pirate WC

UPDATE
  team
SET
  name_normalized = 'Junior Pirate WC',
  url_path_slug = NULL
WHERE
  id = 4097;

UPDATE
  tournament_team
SET
  team_id = 4097
WHERE
  id IN (4097, 4222, 4373, 4541, 4689);

DELETE FROM
  team
WHERE
  id IN (4222, 4373, 4541, 4689);

----------------------------------------
-- Junior Pirates

UPDATE
  team
SET
  name_normalized = 'Junior Pirates',
  url_path_slug = NULL
WHERE
  id = 8893;

UPDATE
  tournament_team
SET
  team_id = 8893
WHERE
  id IN (8893, 9084);

DELETE FROM
  team
WHERE
  id IN (9084);

----------------------------------------
-- Junior Porters WC

UPDATE
  team
SET
  name_normalized = 'Junior Porters WC',
  url_path_slug = NULL
WHERE
  id = 7381;

UPDATE
  tournament_team
SET
  team_id = 7381
WHERE
  id IN (7381, 7569, 7754, 7944, 8508, 8699, 9616, 9783);

DELETE FROM
  team
WHERE
  id IN (7569, 7754, 7944, 8508, 8699, 9616, 9783);

----------------------------------------
-- Junior Raiders West WC

UPDATE
  team
SET
  name_normalized = 'Junior Raiders West WC',
  url_path_slug = NULL
WHERE
  id = 11013;

UPDATE
  tournament_team
SET
  team_id = 11013
WHERE
  id IN (11013, 11175);

DELETE FROM
  team
WHERE
  id IN (11175);

----------------------------------------
-- Junior Rams WC

UPDATE
  team
SET
  name_normalized = 'Junior Rams WC',
  url_path_slug = NULL
WHERE
  id = 7382;

UPDATE
  tournament_team
SET
  team_id = 7382
WHERE
  id IN (7382, 7570);

DELETE FROM
  team
WHERE
  id IN (7570);

----------------------------------------
-- Junior Spartans WC

UPDATE
  team
SET
  name_normalized = 'Junior Spartans WC',
  url_path_slug = NULL
WHERE
  id = 6998;

UPDATE
  tournament_team
SET
  team_id = 6998
WHERE
  id IN (6998, 7186, 7383, 7571, 7755, 7945);

DELETE FROM
  team
WHERE
  id IN (7186, 7383, 7571, 7755, 7945);

----------------------------------------
-- Junior Titan Wrestling

UPDATE
  team
SET
  name_normalized = 'Junior Titan Wrestling',
  url_path_slug = NULL
WHERE
  id = 8509;

UPDATE
  tournament_team
SET
  team_id = 8509
WHERE
  id IN (8509, 8700);

DELETE FROM
  team
WHERE
  id IN (8700);

----------------------------------------
-- Junior Titans Wrestling

UPDATE
  team
SET
  name_normalized = 'Junior Titans Wrestling',
  url_path_slug = NULL
WHERE
  id = 11014;

UPDATE
  tournament_team
SET
  team_id = 11014
WHERE
  id IN (11014, 11176, 11345, 11507, 11686, 11899, 12112, 12325, 12541, 12768, 12995, 13222, 13472, 13742, 14012, 14282, 14552, 14822, 15092, 15362);

DELETE FROM
  team
WHERE
  id IN (11176, 11345, 11507, 11686, 11899, 12112, 12325, 12541, 12768, 12995, 13222, 13472, 13742, 14012, 14282, 14552, 14822, 15092, 15362);

----------------------------------------
-- Junior Vikings

UPDATE
  team
SET
  name_normalized = 'Junior Vikings',
  url_path_slug = NULL
WHERE
  id = 5746;

UPDATE
  tournament_team
SET
  team_id = 5746
WHERE
  id IN (5746, 5908, 6087);

DELETE FROM
  team
WHERE
  id IN (5908, 6087);

----------------------------------------
-- Just Wrestling

UPDATE
  team
SET
  name_normalized = 'Just Wrestling',
  url_path_slug = NULL
WHERE
  id = 10313;

UPDATE
  tournament_team
SET
  team_id = 10313
WHERE
  id IN (10313, 10484);

DELETE FROM
  team
WHERE
  id IN (10484);

----------------------------------------
-- Kane County Grapplers

UPDATE
  team
SET
  name_normalized = 'Kane County Grapplers',
  url_path_slug = NULL
WHERE
  id = 5909;

UPDATE
  tournament_team
SET
  team_id = 5909
WHERE
  id IN (5909, 6088, 6272, 6456, 6640, 6822, 6999, 7187, 7384, 7572, 7756, 7946, 8134, 8321);

DELETE FROM
  team
WHERE
  id IN (6088, 6272, 6456, 6640, 6822, 6999, 7187, 7384, 7572, 7756, 7946, 8134, 8321);

----------------------------------------
-- Kaneland Knights WC

UPDATE
  team
SET
  name_normalized = 'Kaneland Knights WC',
  url_path_slug = NULL
WHERE
  id = 8510;

UPDATE
  tournament_team
SET
  team_id = 8510
WHERE
  id IN (8510, 8701, 10314, 10485, 10652, 10835, 11015, 11177);

DELETE FROM
  team
WHERE
  id IN (8701, 10314, 10485, 10652, 10835, 11015, 11177);

----------------------------------------
-- Kaneland Knights Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Kaneland Knights Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8894;

UPDATE
  tournament_team
SET
  team_id = 8894
WHERE
  id IN (8894, 9085);

DELETE FROM
  team
WHERE
  id IN (9085);

----------------------------------------
-- King Select Wrestling

UPDATE
  team
SET
  name_normalized = 'King Select Wrestling',
  url_path_slug = NULL
WHERE
  id = 7385;

UPDATE
  tournament_team
SET
  team_id = 7385
WHERE
  id IN (7385, 7573, 7757, 7947, 8135, 8322, 8511, 8702, 8895, 9086);

DELETE FROM
  team
WHERE
  id IN (7573, 7757, 7947, 8135, 8322, 8511, 8702, 8895, 9086);

----------------------------------------
-- Kishwaukee WC

UPDATE
  team
SET
  name_normalized = 'Kishwaukee WC',
  url_path_slug = NULL
WHERE
  id = 4374;

UPDATE
  tournament_team
SET
  team_id = 4374
WHERE
  id IN (4374, 4542);

DELETE FROM
  team
WHERE
  id IN (4542);

----------------------------------------
-- Knights WC

UPDATE
  team
SET
  name_normalized = 'Knights WC',
  url_path_slug = NULL
WHERE
  id = 1378;

UPDATE
  tournament_team
SET
  team_id = 1378
WHERE
  id IN (1378, 1996, 5316, 5910, 6089, 7758, 7948, 8136, 8323, 13473, 13743, 14013, 14283, 14553, 14823, 15093, 15363);

DELETE FROM
  team
WHERE
  id IN (1996, 5316, 5910, 6089, 7758, 7948, 8136, 8323, 13473, 13743, 14013, 14283, 14553, 14823, 15093, 15363);

----------------------------------------
-- Knights Wrestling

UPDATE
  team
SET
  name_normalized = 'Knights Wrestling',
  url_path_slug = NULL
WHERE
  id = 4375;

UPDATE
  tournament_team
SET
  team_id = 4375
WHERE
  id IN (4375, 4543, 4690, 4826, 4974, 5148, 5747, 6273, 6457, 6641, 6823, 7386, 7574);

DELETE FROM
  team
WHERE
  id IN (4543, 4690, 4826, 4974, 5148, 5747, 6273, 6457, 6641, 6823, 7386, 7574);

----------------------------------------
-- Knox County Krushers

UPDATE
  team
SET
  name_normalized = 'Knox County Krushers',
  url_path_slug = NULL
WHERE
  id = 6642;

UPDATE
  tournament_team
SET
  team_id = 6642
WHERE
  id IN (6642, 6824, 7000, 7188, 7387, 7575, 7759, 7949, 8896, 9087);

DELETE FROM
  team
WHERE
  id IN (6824, 7000, 7188, 7387, 7575, 7759, 7949, 8896, 9087);

----------------------------------------
-- L-P Crunching Cavs

UPDATE
  team
SET
  name_normalized = 'L-P Crunching Cavs',
  url_path_slug = NULL
WHERE
  id = 2704;

UPDATE
  tournament_team
SET
  team_id = 2704
WHERE
  id IN (2704, 3852, 3971, 4098, 4224, 4376, 4544, 4691, 4827, 4975, 5149, 5462, 5607, 5748, 5911, 6090, 6274, 6458, 6643, 6825, 7001, 7189, 7388, 7576);

DELETE FROM
  team
WHERE
  id IN (3852, 3971, 4098, 4224, 4376, 4544, 4691, 4827, 4975, 5149, 5462, 5607, 5748, 5911, 6090, 6274, 6458, 6643, 6825, 7001, 7189, 7388, 7576);

----------------------------------------
-- Lake County Stallions Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Lake County Stallions Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 13475;

UPDATE
  tournament_team
SET
  team_id = 13475
WHERE
  id IN (13475, 13745, 14015, 14285, 14555, 14825, 15095, 15365);

DELETE FROM
  team
WHERE
  id IN (13745, 14015, 14285, 14555, 14825, 15095, 15365);

----------------------------------------
-- Lake View Jr Wildcats

UPDATE
  team
SET
  name_normalized = 'Lake View Jr Wildcats',
  url_path_slug = NULL
WHERE
  id = 4377;

UPDATE
  tournament_team
SET
  team_id = 4377
WHERE
  id IN (4377, 4545);

DELETE FROM
  team
WHERE
  id IN (4545);

----------------------------------------
-- Lake Villa Lancers

UPDATE
  team
SET
  name_normalized = 'Lake Villa Lancers',
  url_path_slug = NULL
WHERE
  id = 3853;

UPDATE
  tournament_team
SET
  team_id = 3853
WHERE
  id IN (3853, 3972, 4226);

DELETE FROM
  team
WHERE
  id IN (3972, 4226);

----------------------------------------
-- Lake Zurich Cubs WC

UPDATE
  team
SET
  name_normalized = 'Lake Zurich Cubs WC',
  url_path_slug = NULL
WHERE
  id = 9956;

UPDATE
  tournament_team
SET
  team_id = 9956
WHERE
  id IN (9956, 10133, 11688, 11901, 12114, 12327, 12543, 12770, 12997, 13224, 13476, 13746, 14016, 14286, 14556, 14826, 15096, 15366);

DELETE FROM
  team
WHERE
  id IN (10133, 11688, 11901, 12114, 12327, 12543, 12770, 12997, 13224, 13476, 13746, 14016, 14286, 14556, 14826, 15096, 15366);

----------------------------------------
-- Lakeland Predators

UPDATE
  team
SET
  name_normalized = 'Lakeland Predators',
  url_path_slug = NULL
WHERE
  id = 3854;

UPDATE
  tournament_team
SET
  team_id = 3854
WHERE
  id IN (3854, 3973, 4099, 4227, 4378, 4546, 4692, 4976, 5150, 5317, 5463, 5608, 5749, 5912, 6091, 6275, 6459, 6644, 6826, 7002, 7190, 7389, 7577, 7761, 7951);

DELETE FROM
  team
WHERE
  id IN (3973, 4099, 4227, 4378, 4546, 4692, 4976, 5150, 5317, 5463, 5608, 5749, 5912, 6091, 6275, 6459, 6644, 6826, 7002, 7190, 7389, 7577, 7761, 7951);

----------------------------------------
-- Lakeland Predators WC

UPDATE
  team
SET
  name_normalized = 'Lakeland Predators WC',
  url_path_slug = NULL
WHERE
  id = 8138;

UPDATE
  tournament_team
SET
  team_id = 8138
WHERE
  id IN (8138, 8325, 8513, 8704, 8898, 9089, 9269, 9444, 9618, 9785, 9957, 10134, 10654, 10837, 11017, 11179, 11347, 11509);

DELETE FROM
  team
WHERE
  id IN (8325, 8513, 8704, 8898, 9089, 9269, 9444, 9618, 9785, 9957, 10134, 10654, 10837, 11017, 11179, 11347, 11509);

----------------------------------------
-- Lan-Oak P.D. Little Rebels

UPDATE
  team
SET
  name_normalized = 'Lan-Oak P.D. Little Rebels',
  url_path_slug = NULL
WHERE
  id = 4379;

UPDATE
  tournament_team
SET
  team_id = 4379
WHERE
  id IN (4379, 4547);

DELETE FROM
  team
WHERE
  id IN (4547);

----------------------------------------
-- Lan-Oak Park District

UPDATE
  team
SET
  name_normalized = 'Lan-Oak Park District',
  url_path_slug = NULL
WHERE
  id = 1069;

UPDATE
  tournament_team
SET
  team_id = 1069
WHERE
  id IN (1069, 1182);

DELETE FROM
  team
WHERE
  id IN (1182);

----------------------------------------
-- Lancer WC

UPDATE
  team
SET
  name_normalized = 'Lancer WC',
  url_path_slug = NULL
WHERE
  id = 4380;

UPDATE
  tournament_team
SET
  team_id = 4380
WHERE
  id IN (4380, 4548, 4828, 5464, 5609, 5750, 6276, 6460, 6645, 6827, 7003, 7191, 7390, 7578, 8514, 8705);

DELETE FROM
  team
WHERE
  id IN (4548, 4828, 5464, 5609, 5750, 6276, 6460, 6645, 6827, 7003, 7191, 7390, 7578, 8514, 8705);

----------------------------------------
-- Lancer Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lancer Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8899;

UPDATE
  tournament_team
SET
  team_id = 8899
WHERE
  id IN (8899, 9090);

DELETE FROM
  team
WHERE
  id IN (9090);

----------------------------------------
-- Lancers

UPDATE
  team
SET
  name_normalized = 'Lancers',
  url_path_slug = NULL
WHERE
  id = 842;

UPDATE
  tournament_team
SET
  team_id = 842
WHERE
  id IN (842, 958, 1292);

DELETE FROM
  team
WHERE
  id IN (958, 1292);

----------------------------------------
-- Lanphier-Southeast WC

UPDATE
  team
SET
  name_normalized = 'Lanphier-Southeast WC',
  url_path_slug = NULL
WHERE
  id = 959;

UPDATE
  tournament_team
SET
  team_id = 959
WHERE
  id IN (959, 1183);

DELETE FROM
  team
WHERE
  id IN (1183);

----------------------------------------
-- Larkin Jr. Royals WC

UPDATE
  team
SET
  name_normalized = 'Larkin Jr. Royals WC',
  url_path_slug = NULL
WHERE
  id = 4381;

UPDATE
  tournament_team
SET
  team_id = 4381
WHERE
  id IN (4381, 4549);

DELETE FROM
  team
WHERE
  id IN (4549);

----------------------------------------
-- Lasalle Peru Crunching Cavs Youth WC

UPDATE
  team
SET
  name_normalized = 'Lasalle Peru Crunching Cavs Youth WC',
  url_path_slug = NULL
WHERE
  id = 3619;

UPDATE
  tournament_team
SET
  team_id = 3619
WHERE
  id IN (3619, 3739, 9955, 10132, 10315, 10486, 10653, 10836, 11016, 11178, 11346, 11508, 11687, 11900, 12113, 12326, 12542, 12769, 12996, 13223, 13474, 13744, 14014, 14284, 14554, 14824, 15094, 15364);

DELETE FROM
  team
WHERE
  id IN (3739, 9955, 10132, 10315, 10486, 10653, 10836, 11016, 11178, 11346, 11508, 11687, 11900, 12113, 12326, 12542, 12769, 12996, 13223, 13474, 13744, 14014, 14284, 14554, 14824, 15094, 15364);

----------------------------------------
-- Lawrence County Knights WC

UPDATE
  team
SET
  name_normalized = 'Lawrence County Knights WC',
  url_path_slug = NULL
WHERE
  id = 11689;

UPDATE
  tournament_team
SET
  team_id = 11689
WHERE
  id IN (11689, 11902, 12115, 12328, 12544, 12771, 12998, 13225, 13477, 13747, 14017, 14287, 14557, 14827, 15097, 15367);

DELETE FROM
  team
WHERE
  id IN (11902, 12115, 12328, 12544, 12771, 12998, 13225, 13477, 13747, 14017, 14287, 14557, 14827, 15097, 15367);

----------------------------------------
-- Lawrence County WC

UPDATE
  team
SET
  name_normalized = 'Lawrence County WC',
  url_path_slug = NULL
WHERE
  id = 1184;

UPDATE
  tournament_team
SET
  team_id = 1184
WHERE
  id IN (1184, 3855, 4100, 4228, 4382, 4550, 4693, 4829, 4977, 5151, 5465, 6646, 6828, 7004, 7192, 7762, 7952, 8139, 8326, 8515, 8706, 10655, 10838);

DELETE FROM
  team
WHERE
  id IN (3855, 4100, 4228, 4382, 4550, 4693, 4829, 4977, 5151, 5465, 6646, 6828, 7004, 7192, 7762, 7952, 8139, 8326, 8515, 8706, 10655, 10838);

----------------------------------------
-- Lawrence County Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lawrence County Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8900;

UPDATE
  tournament_team
SET
  team_id = 8900
WHERE
  id IN (8900, 9091);

DELETE FROM
  team
WHERE
  id IN (9091);

----------------------------------------
-- Lemont Bears WC

UPDATE
  team
SET
  name_normalized = 'Lemont Bears WC',
  url_path_slug = NULL
WHERE
  id = 961;

UPDATE
  tournament_team
SET
  team_id = 961
WHERE
  id IN (961, 1071, 1185, 1474, 1598, 1863, 2002, 2863, 3621, 3741, 3856, 3974, 4101, 4229, 4383, 4551, 4694, 4830, 4978, 5152, 5318, 5466, 5610, 5751, 5913, 6092, 6277, 6461, 6647, 6829, 7005, 7193, 7391, 7579, 7763, 7953, 8140, 8327, 8516, 8707, 8901, 9092, 9270, 9445, 9619, 9786, 9958, 10135, 10316, 10487, 10656, 10839, 11018, 11180, 11348, 11510, 11690, 11903, 12116, 12329, 12545, 12772, 12999, 13226, 13478, 13748, 14018, 14288, 14558, 14828, 15098, 15368);

DELETE FROM
  team
WHERE
  id IN (1071, 1185, 1474, 1598, 1863, 2002, 2863, 3621, 3741, 3856, 3974, 4101, 4229, 4383, 4551, 4694, 4830, 4978, 5152, 5318, 5466, 5610, 5751, 5913, 6092, 6277, 6461, 6647, 6829, 7005, 7193, 7391, 7579, 7763, 7953, 8140, 8327, 8516, 8707, 8901, 9092, 9270, 9445, 9619, 9786, 9958, 10135, 10316, 10487, 10656, 10839, 11018, 11180, 11348, 11510, 11690, 11903, 12116, 12329, 12545, 12772, 12999, 13226, 13478, 13748, 14018, 14288, 14558, 14828, 15098, 15368);

----------------------------------------
-- Libertyville WC

UPDATE
  team
SET
  name_normalized = 'Libertyville WC',
  url_path_slug = NULL
WHERE
  id = 12546;

UPDATE
  tournament_team
SET
  team_id = 12546
WHERE
  id IN (12546, 12773, 13000, 13227);

DELETE FROM
  team
WHERE
  id IN (12773, 13000, 13227);

----------------------------------------
-- Libertyville Wildcats

UPDATE
  team
SET
  name_normalized = 'Libertyville Wildcats',
  url_path_slug = NULL
WHERE
  id = 6648;

UPDATE
  tournament_team
SET
  team_id = 6648
WHERE
  id IN (6648, 6830);

DELETE FROM
  team
WHERE
  id IN (6830);

----------------------------------------
-- Libertyville Wildcats WC

UPDATE
  team
SET
  name_normalized = 'Libertyville Wildcats WC',
  url_path_slug = NULL
WHERE
  id = 9620;

UPDATE
  tournament_team
SET
  team_id = 9620
WHERE
  id IN (9620, 9787, 10657, 10840);

DELETE FROM
  team
WHERE
  id IN (9787, 10657, 10840);

----------------------------------------
-- Lil Bucs WC

UPDATE
  team
SET
  name_normalized = 'Lil Bucs WC',
  url_path_slug = NULL
WHERE
  id = 7764;

UPDATE
  tournament_team
SET
  team_id = 7764
WHERE
  id IN (7764, 7954, 8141, 8328, 8517, 8708, 9271, 9446);

DELETE FROM
  team
WHERE
  id IN (7954, 8141, 8328, 8517, 8708, 9271, 9446);

----------------------------------------
-- Lil Bucs Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lil Bucs Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8902;

UPDATE
  tournament_team
SET
  team_id = 8902
WHERE
  id IN (8902, 9093);

DELETE FROM
  team
WHERE
  id IN (9093);

----------------------------------------
-- Lil Reaper WC

UPDATE
  team
SET
  name_normalized = 'Lil Reaper WC',
  url_path_slug = NULL
WHERE
  id = 1072;

UPDATE
  tournament_team
SET
  team_id = 1072
WHERE
  id IN (1072, 1475, 1599, 1864, 2003, 7765, 7955, 8142, 8329, 8518, 8709, 10658, 10841, 11691, 11904, 12117, 12330, 12547, 12774, 13001, 13228, 13479, 13749, 14019, 14289, 14559, 14829, 15099, 15369);

DELETE FROM
  team
WHERE
  id IN (1475, 1599, 1864, 2003, 7765, 7955, 8142, 8329, 8518, 8709, 10658, 10841, 11691, 11904, 12117, 12330, 12547, 12774, 13001, 13228, 13479, 13749, 14019, 14289, 14559, 14829, 15099, 15369);

----------------------------------------
-- Lil Storm Youth

UPDATE
  team
SET
  name_normalized = 'Lil Storm Youth',
  url_path_slug = NULL
WHERE
  id = 3857;

UPDATE
  tournament_team
SET
  team_id = 3857
WHERE
  id IN (3857, 4230);

DELETE FROM
  team
WHERE
  id IN (4230);

----------------------------------------
-- Lil' Roughnecks

UPDATE
  team
SET
  name_normalized = 'Lil'' Roughnecks',
  url_path_slug = NULL
WHERE
  id = 4384;

UPDATE
  tournament_team
SET
  team_id = 4384
WHERE
  id IN (4384, 4552, 4695);

DELETE FROM
  team
WHERE
  id IN (4552, 4695);

----------------------------------------
-- Lil' Storm Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Lil'' Storm Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 4385;

UPDATE
  tournament_team
SET
  team_id = 4385
WHERE
  id IN (4385, 4553, 4831, 4979, 5153, 5319, 5467);

DELETE FROM
  team
WHERE
  id IN (4553, 4831, 4979, 5153, 5319, 5467);

----------------------------------------
-- Lil` Coalers WC

UPDATE
  team
SET
  name_normalized = 'Lil` Coalers WC',
  url_path_slug = NULL
WHERE
  id = 11019;

UPDATE
  tournament_team
SET
  team_id = 11019
WHERE
  id IN (11019, 11181, 12548, 12775, 13002, 13229, 13480, 13750, 14020, 14290, 14560, 14830, 15100, 15370);

DELETE FROM
  team
WHERE
  id IN (11181, 12548, 12775, 13002, 13229, 13480, 13750, 14020, 14290, 14560, 14830, 15100, 15370);

----------------------------------------
-- Lil` Storm

UPDATE
  team
SET
  name_normalized = 'Lil` Storm',
  url_path_slug = NULL
WHERE
  id = 6649;

UPDATE
  tournament_team
SET
  team_id = 6649
WHERE
  id IN (6649, 6831, 7006, 7194, 7392, 7580);

DELETE FROM
  team
WHERE
  id IN (6831, 7006, 7194, 7392, 7580);

----------------------------------------
-- Lil` Storm WC

UPDATE
  team
SET
  name_normalized = 'Lil` Storm WC',
  url_path_slug = NULL
WHERE
  id = 7766;

UPDATE
  tournament_team
SET
  team_id = 7766
WHERE
  id IN (7766, 7956);

DELETE FROM
  team
WHERE
  id IN (7956);

----------------------------------------
-- Limestone Crushers WC

UPDATE
  team
SET
  name_normalized = 'Limestone Crushers WC',
  url_path_slug = NULL
WHERE
  id = 6650;

UPDATE
  tournament_team
SET
  team_id = 6650
WHERE
  id IN (6650, 6832);

DELETE FROM
  team
WHERE
  id IN (6832);

----------------------------------------
-- Limestone WC

UPDATE
  team
SET
  name_normalized = 'Limestone WC',
  url_path_slug = NULL
WHERE
  id = 11692;

UPDATE
  tournament_team
SET
  team_id = 11692
WHERE
  id IN (11692, 11905, 12118, 12331, 12549, 12776, 13003, 13230, 13481, 13751, 14021, 14291, 14561, 14831, 15101, 15371);

DELETE FROM
  team
WHERE
  id IN (11905, 12118, 12331, 12549, 12776, 13003, 13230, 13481, 13751, 14021, 14291, 14561, 14831, 15101, 15371);

----------------------------------------
-- Limestone Youth Rocket WC

UPDATE
  team
SET
  name_normalized = 'Limestone Youth Rocket WC',
  url_path_slug = NULL
WHERE
  id = 3858;

UPDATE
  tournament_team
SET
  team_id = 3858
WHERE
  id IN (3858, 3975, 4231, 4980, 5154);

DELETE FROM
  team
WHERE
  id IN (3975, 4231, 4980, 5154);

----------------------------------------
-- Limestone Youth WC

UPDATE
  team
SET
  name_normalized = 'Limestone Youth WC',
  url_path_slug = NULL
WHERE
  id = 4696;

UPDATE
  tournament_team
SET
  team_id = 4696
WHERE
  id IN (4696, 4832, 5611, 5752, 5914, 6093, 7393, 7581);

DELETE FROM
  team
WHERE
  id IN (4832, 5611, 5752, 5914, 6093, 7393, 7581);

----------------------------------------
-- Lincoln Way WC

UPDATE
  team
SET
  name_normalized = 'Lincoln Way WC',
  url_path_slug = NULL
WHERE
  id = 7768;

UPDATE
  tournament_team
SET
  team_id = 7768
WHERE
  id IN (7768, 7958, 8143, 8330, 8519, 8710);

DELETE FROM
  team
WHERE
  id IN (7958, 8143, 8330, 8519, 8710);

----------------------------------------
-- Lincoln Way Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lincoln Way Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8903;

UPDATE
  tournament_team
SET
  team_id = 8903
WHERE
  id IN (8903, 9094);

DELETE FROM
  team
WHERE
  id IN (9094);

----------------------------------------
-- Lincoln Youth WC

UPDATE
  team
SET
  name_normalized = 'Lincoln Youth WC',
  url_path_slug = NULL
WHERE
  id = 10318;

UPDATE
  tournament_team
SET
  team_id = 10318
WHERE
  id IN (10318, 10489, 10660, 10843, 12550, 12777, 13004, 13231);

DELETE FROM
  team
WHERE
  id IN (10489, 10660, 10843, 12550, 12777, 13004, 13231);

----------------------------------------
-- Lincoln Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lincoln Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8904;

UPDATE
  tournament_team
SET
  team_id = 8904
WHERE
  id IN (8904, 9095);

DELETE FROM
  team
WHERE
  id IN (9095);

----------------------------------------
-- Lincoln-Way Vittum Cats

UPDATE
  team
SET
  name_normalized = 'Lincoln-Way Vittum Cats',
  url_path_slug = NULL
WHERE
  id = 9272;

UPDATE
  tournament_team
SET
  team_id = 9272
WHERE
  id IN (9272, 9447);

DELETE FROM
  team
WHERE
  id IN (9447);

----------------------------------------
-- Lincoln-Way WC

UPDATE
  team
SET
  name_normalized = 'Lincoln-Way WC',
  url_path_slug = NULL
WHERE
  id = 4386;

UPDATE
  tournament_team
SET
  team_id = 4386
WHERE
  id IN (4386, 4554, 4697, 4833, 4981, 5155, 5320, 5468, 5612, 5753, 5915, 6094, 6278, 6462, 6651, 6833, 7007, 7195, 7394, 7582, 9621, 9788, 9960, 10137, 10319, 10490, 10661, 10844, 11021, 11183, 11349, 11511, 11693, 11906, 12119, 12332, 12551, 12778, 13005, 13232, 13482, 13752, 14022, 14292, 14562, 14832, 15102, 15372);

DELETE FROM
  team
WHERE
  id IN (4554, 4697, 4833, 4981, 5155, 5320, 5468, 5612, 5753, 5915, 6094, 6278, 6462, 6651, 6833, 7007, 7195, 7394, 7582, 9621, 9788, 9960, 10137, 10319, 10490, 10661, 10844, 11021, 11183, 11349, 11511, 11693, 11906, 12119, 12332, 12551, 12778, 13005, 13232, 13482, 13752, 14022, 14292, 14562, 14832, 15102, 15372);

----------------------------------------
-- Lion`S Den Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Lion`S Den Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 10662;

UPDATE
  tournament_team
SET
  team_id = 10662
WHERE
  id IN (10662, 10845, 11022, 11184, 11350, 11512, 11694, 11907, 12120, 12333, 12552, 12779, 13006, 13233);

DELETE FROM
  team
WHERE
  id IN (10845, 11022, 11184, 11350, 11512, 11694, 11907, 12120, 12333, 12552, 12779, 13006, 13233);

----------------------------------------
-- Lionheart Intense Wrestling

UPDATE
  team
SET
  name_normalized = 'Lionheart Intense Wrestling',
  url_path_slug = NULL
WHERE
  id = 4982;

UPDATE
  tournament_team
SET
  team_id = 4982
WHERE
  id IN (4982, 5156, 5321, 5469, 5613, 5754, 5916, 6095, 6279, 6463, 6652, 6834, 7008, 7196, 7395, 7583, 7769, 7959, 8144, 8331, 8520, 8711, 8905, 9096, 9273, 9448, 9622, 9789, 9961, 10138, 10320, 10491, 10663, 10846, 11023, 11185, 11351, 11513, 11695, 11908, 12121, 12334, 12553, 12780, 13007, 13234, 13483, 13753, 14023, 14293, 14563, 14833, 15103, 15373);

DELETE FROM
  team
WHERE
  id IN (5156, 5321, 5469, 5613, 5754, 5916, 6095, 6279, 6463, 6652, 6834, 7008, 7196, 7395, 7583, 7769, 7959, 8144, 8331, 8520, 8711, 8905, 9096, 9273, 9448, 9622, 9789, 9961, 10138, 10320, 10491, 10663, 10846, 11023, 11185, 11351, 11513, 11695, 11908, 12121, 12334, 12553, 12780, 13007, 13234, 13483, 13753, 14023, 14293, 14563, 14833, 15103, 15373);

----------------------------------------
-- Lions Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Lions Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1073;

UPDATE
  tournament_team
SET
  team_id = 1073
WHERE
  id IN (1073, 1477, 1600, 1732, 1865, 3008, 4387, 4555, 4834, 5322, 5755, 5917, 6096, 6653, 6835, 7009, 7197, 7396, 7584, 7770, 7960, 8145, 8332, 8521, 8712, 8906, 9097, 9274, 9449, 9623, 9790, 9962, 10139, 10321, 10492, 10664, 10847, 11024, 11186, 11352, 11514, 11696, 11909, 12122, 12335, 12554, 12781, 13008, 13235, 13484, 13754, 14024, 14294, 14564, 14834, 15104, 15374);

DELETE FROM
  team
WHERE
  id IN (1477, 1600, 1732, 1865, 3008, 4387, 4555, 4834, 5322, 5755, 5917, 6096, 6653, 6835, 7009, 7197, 7396, 7584, 7770, 7960, 8145, 8332, 8521, 8712, 8906, 9097, 9274, 9449, 9623, 9790, 9962, 10139, 10321, 10492, 10664, 10847, 11024, 11186, 11352, 11514, 11696, 11909, 12122, 12335, 12554, 12781, 13008, 13235, 13484, 13754, 14024, 14294, 14564, 14834, 15104, 15374);

----------------------------------------
-- Lisle WC

UPDATE
  team
SET
  name_normalized = 'Lisle WC',
  url_path_slug = NULL
WHERE
  id = 13485;

UPDATE
  tournament_team
SET
  team_id = 13485
WHERE
  id IN (13485, 13755, 14025, 14295, 14565, 14835, 15105, 15375);

DELETE FROM
  team
WHERE
  id IN (13755, 14025, 14295, 14565, 14835, 15105, 15375);

----------------------------------------
-- Litchfield WC

UPDATE
  team
SET
  name_normalized = 'Litchfield WC',
  url_path_slug = NULL
WHERE
  id = 13486;

UPDATE
  tournament_team
SET
  team_id = 13486
WHERE
  id IN (13486, 13756, 14026, 14296, 14566, 14836, 15106, 15376);

DELETE FROM
  team
WHERE
  id IN (13756, 14026, 14296, 14566, 14836, 15106, 15376);

----------------------------------------
-- Litchfield Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Litchfield Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 3859;

UPDATE
  tournament_team
SET
  team_id = 3859
WHERE
  id IN (3859, 3976);

DELETE FROM
  team
WHERE
  id IN (3976);

----------------------------------------
-- Little Boiler WC

UPDATE
  team
SET
  name_normalized = 'Little Boiler WC',
  url_path_slug = NULL
WHERE
  id = 4102;

UPDATE
  tournament_team
SET
  team_id = 4102
WHERE
  id IN (4102, 4232, 4388, 4556, 4835, 5470, 6280, 6464, 8522, 8713, 9275, 9450);

DELETE FROM
  team
WHERE
  id IN (4232, 4388, 4556, 4835, 5470, 6280, 6464, 8522, 8713, 9275, 9450);

----------------------------------------
-- Little Boilers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Little Boilers Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8907;

UPDATE
  tournament_team
SET
  team_id = 8907
WHERE
  id IN (8907, 9098);

DELETE FROM
  team
WHERE
  id IN (9098);

----------------------------------------
-- Little Celtic WC

UPDATE
  team
SET
  name_normalized = 'Little Celtic WC',
  url_path_slug = NULL
WHERE
  id = 1733;

UPDATE
  tournament_team
SET
  team_id = 1733
WHERE
  id IN (1733, 1867, 2438, 2574, 3009, 3166, 3624, 3743, 3860, 3977, 4103, 4233, 4389, 4557, 4698, 4836, 4983, 5157, 5323, 5471, 5615, 5756, 5918, 6097, 6281, 6465, 6654, 6836, 7010, 7198, 7397, 7585, 7771, 7961, 11697, 11910, 12123, 12336, 12555, 12782, 13009, 13236);

DELETE FROM
  team
WHERE
  id IN (1867, 2438, 2574, 3009, 3166, 3624, 3743, 3860, 3977, 4103, 4233, 4389, 4557, 4698, 4836, 4983, 5157, 5323, 5471, 5615, 5756, 5918, 6097, 6281, 6465, 6654, 6836, 7010, 7198, 7397, 7585, 7771, 7961, 11697, 11910, 12123, 12336, 12555, 12782, 13009, 13236);

----------------------------------------
-- Little Cyclone WC

UPDATE
  team
SET
  name_normalized = 'Little Cyclone WC',
  url_path_slug = NULL
WHERE
  id = 7011;

UPDATE
  tournament_team
SET
  team_id = 7011
WHERE
  id IN (7011, 7199);

DELETE FROM
  team
WHERE
  id IN (7199);

----------------------------------------
-- Little Falcons WC

UPDATE
  team
SET
  name_normalized = 'Little Falcons WC',
  url_path_slug = NULL
WHERE
  id = 5324;

UPDATE
  tournament_team
SET
  team_id = 5324
WHERE
  id IN (5324, 5472, 5616, 5757, 5919, 6098, 6282, 6466, 6655, 6837, 7012, 7200, 7398, 7586, 7772, 7962, 8146, 8333, 10665, 10848, 11025, 11187, 11353, 11515);

DELETE FROM
  team
WHERE
  id IN (5472, 5616, 5757, 5919, 6098, 6282, 6466, 6655, 6837, 7012, 7200, 7398, 7586, 7772, 7962, 8146, 8333, 10665, 10848, 11025, 11187, 11353, 11515);

----------------------------------------
-- Little Huskies Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Little Huskies Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 4985;

UPDATE
  tournament_team
SET
  team_id = 4985
WHERE
  id IN (4985, 5159, 5473, 5617, 5758, 5920, 6099, 6283, 6467, 6656, 6838, 7013, 7201, 7399, 7587, 7773, 7963, 8147, 8334, 8523, 8714, 8908, 9099, 9276, 9451, 9624, 9791, 9963, 10140, 10322, 10493, 10666, 10849, 11026, 11188);

DELETE FROM
  team
WHERE
  id IN (5159, 5473, 5617, 5758, 5920, 6099, 6283, 6467, 6656, 6838, 7013, 7201, 7399, 7587, 7773, 7963, 8147, 8334, 8523, 8714, 8908, 9099, 9276, 9451, 9624, 9791, 9963, 10140, 10322, 10493, 10666, 10849, 11026, 11188);

----------------------------------------
-- Little Indians

UPDATE
  team
SET
  name_normalized = 'Little Indians',
  url_path_slug = NULL
WHERE
  id = 4105;

UPDATE
  tournament_team
SET
  team_id = 4105
WHERE
  id IN (4105, 4986, 5160);

DELETE FROM
  team
WHERE
  id IN (4986, 5160);

----------------------------------------
-- Little Rams Wrestling

UPDATE
  team
SET
  name_normalized = 'Little Rams Wrestling',
  url_path_slug = NULL
WHERE
  id = 8909;

UPDATE
  tournament_team
SET
  team_id = 8909
WHERE
  id IN (8909, 9100, 9277, 9452, 9625, 9792, 9964, 10141);

DELETE FROM
  team
WHERE
  id IN (9100, 9277, 9452, 9625, 9792, 9964, 10141);

----------------------------------------
-- Little Redbird WC

UPDATE
  team
SET
  name_normalized = 'Little Redbird WC',
  url_path_slug = NULL
WHERE
  id = 3861;

UPDATE
  tournament_team
SET
  team_id = 3861
WHERE
  id IN (3861, 3978, 4234, 4390, 4558, 4837, 4987, 5161, 5474, 5618, 5921, 6100, 6284, 6468);

DELETE FROM
  team
WHERE
  id IN (3978, 4234, 4390, 4558, 4837, 4987, 5161, 5474, 5618, 5921, 6100, 6284, 6468);

----------------------------------------
-- Little Redskins WC

UPDATE
  team
SET
  name_normalized = 'Little Redskins WC',
  url_path_slug = NULL
WHERE
  id = 4988;

UPDATE
  tournament_team
SET
  team_id = 4988
WHERE
  id IN (4988, 5162, 5619, 5759, 5922, 6101, 6285, 6469, 6657, 6839, 7014, 7202, 7400, 7588, 7774, 7964, 8148, 8335, 8524, 8715, 9278, 9453);

DELETE FROM
  team
WHERE
  id IN (5162, 5619, 5759, 5922, 6101, 6285, 6469, 6657, 6839, 7014, 7202, 7400, 7588, 7774, 7964, 8148, 8335, 8524, 8715, 9278, 9453);

----------------------------------------
-- Little Redskins Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Little Redskins Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8910;

UPDATE
  tournament_team
SET
  team_id = 8910
WHERE
  id IN (8910, 9101);

DELETE FROM
  team
WHERE
  id IN (9101);

----------------------------------------
-- Little Rockets

UPDATE
  team
SET
  name_normalized = 'Little Rockets',
  url_path_slug = NULL
WHERE
  id = 7015;

UPDATE
  tournament_team
SET
  team_id = 7015
WHERE
  id IN (7015, 7203, 7401, 7589);

DELETE FROM
  team
WHERE
  id IN (7203, 7401, 7589);

----------------------------------------
-- Little Tigers WC

UPDATE
  team
SET
  name_normalized = 'Little Tigers WC',
  url_path_slug = NULL
WHERE
  id = 8525;

UPDATE
  tournament_team
SET
  team_id = 8525
WHERE
  id IN (8525, 8716);

DELETE FROM
  team
WHERE
  id IN (8716);

----------------------------------------
-- Little Viking WC Of H-F

UPDATE
  team
SET
  name_normalized = 'Little Viking WC Of H-F',
  url_path_slug = NULL
WHERE
  id = 4989;

UPDATE
  tournament_team
SET
  team_id = 4989
WHERE
  id IN (4989, 5163);

DELETE FROM
  team
WHERE
  id IN (5163);

----------------------------------------
-- Little Vikings Of H-F

UPDATE
  team
SET
  name_normalized = 'Little Vikings Of H-F',
  url_path_slug = NULL
WHERE
  id = 4701;

UPDATE
  tournament_team
SET
  team_id = 4701
WHERE
  id IN (4701, 4838);

DELETE FROM
  team
WHERE
  id IN (4838);

----------------------------------------
-- Lockport

UPDATE
  team
SET
  name_normalized = 'Lockport',
  url_path_slug = NULL
WHERE
  id = 848;

UPDATE
  tournament_team
SET
  team_id = 848
WHERE
  id IN (848, 1871);

DELETE FROM
  team
WHERE
  id IN (1871);

----------------------------------------
-- Lockport Gators WC

UPDATE
  team
SET
  name_normalized = 'Lockport Gators WC',
  url_path_slug = NULL
WHERE
  id = 4391;

UPDATE
  tournament_team
SET
  team_id = 4391
WHERE
  id IN (4391, 4559, 4990, 5164);

DELETE FROM
  team
WHERE
  id IN (4559, 4990, 5164);

----------------------------------------
-- Lockport Grapplers WC

UPDATE
  team
SET
  name_normalized = 'Lockport Grapplers WC',
  url_path_slug = NULL
WHERE
  id = 962;

UPDATE
  tournament_team
SET
  team_id = 962
WHERE
  id IN (962, 1074, 1187, 1382, 1479, 1604, 1734, 1871);

DELETE FROM
  team
WHERE
  id IN (1074, 1187, 1382, 1479, 1604, 1734, 1871);

----------------------------------------
-- Lockport Jr Porters WC

UPDATE
  team
SET
  name_normalized = 'Lockport Jr Porters WC',
  url_path_slug = NULL
WHERE
  id = 5325;

UPDATE
  tournament_team
SET
  team_id = 5325
WHERE
  id IN (5325, 5476, 5620, 5760);

DELETE FROM
  team
WHERE
  id IN (5476, 5620, 5760);

----------------------------------------
-- Lockport Junior Porters WC

UPDATE
  team
SET
  name_normalized = 'Lockport Junior Porters WC',
  url_path_slug = NULL
WHERE
  id = 9965;

UPDATE
  tournament_team
SET
  team_id = 9965
WHERE
  id IN (9965, 10142, 10323, 10494, 10667, 10850, 11027, 11189, 11354, 11516, 11699, 11912, 12125, 12338, 12557, 12784, 13011, 13238, 13488, 13758, 14028, 14298, 14568, 14838, 15108, 15378);

DELETE FROM
  team
WHERE
  id IN (10142, 10323, 10494, 10667, 10850, 11027, 11189, 11354, 11516, 11699, 11912, 12125, 12338, 12557, 12784, 13011, 13238, 13488, 13758, 14028, 14298, 14568, 14838, 15108, 15378);

----------------------------------------
-- Los Guerreros WC

UPDATE
  team
SET
  name_normalized = 'Los Guerreros WC',
  url_path_slug = NULL
WHERE
  id = 11700;

UPDATE
  tournament_team
SET
  team_id = 11700
WHERE
  id IN (11700, 11913, 12126, 12339);

DELETE FROM
  team
WHERE
  id IN (11913, 12126, 12339);

----------------------------------------
-- Lp Crunching Cavs

UPDATE
  team
SET
  name_normalized = 'Lp Crunching Cavs',
  url_path_slug = NULL
WHERE
  id = 7760;

UPDATE
  tournament_team
SET
  team_id = 7760
WHERE
  id IN (7760, 7950, 8137, 8324, 8512, 8703, 8897, 9088, 9268, 9443, 9617, 9784);

DELETE FROM
  team
WHERE
  id IN (7950, 8137, 8324, 8512, 8703, 8897, 9088, 9268, 9443, 9617, 9784);

----------------------------------------
-- M-S Kids Club

UPDATE
  team
SET
  name_normalized = 'M-S Kids Club',
  url_path_slug = NULL
WHERE
  id = 4991;

UPDATE
  tournament_team
SET
  team_id = 4991
WHERE
  id IN (4991, 5165, 5621);

DELETE FROM
  team
WHERE
  id IN (5165, 5621);

----------------------------------------
-- Macomb Bomber Wrestling

UPDATE
  team
SET
  name_normalized = 'Macomb Bomber Wrestling',
  url_path_slug = NULL
WHERE
  id = 8912;

UPDATE
  tournament_team
SET
  team_id = 8912
WHERE
  id IN (8912, 9103);

DELETE FROM
  team
WHERE
  id IN (9103);

----------------------------------------
-- Macomb Bombers WC

UPDATE
  team
SET
  name_normalized = 'Macomb Bombers WC',
  url_path_slug = NULL
WHERE
  id = 8526;

UPDATE
  tournament_team
SET
  team_id = 8526
WHERE
  id IN (8526, 8717);

DELETE FROM
  team
WHERE
  id IN (8717);

----------------------------------------
-- Macomb Kids Wrestling

UPDATE
  team
SET
  name_normalized = 'Macomb Kids Wrestling',
  url_path_slug = NULL
WHERE
  id = 3862;

UPDATE
  tournament_team
SET
  team_id = 3862
WHERE
  id IN (3862, 4106);

DELETE FROM
  team
WHERE
  id IN (4106);

----------------------------------------
-- Macomb Little Bombers

UPDATE
  team
SET
  name_normalized = 'Macomb Little Bombers',
  url_path_slug = NULL
WHERE
  id = 4392;

UPDATE
  tournament_team
SET
  team_id = 4392
WHERE
  id IN (4392, 4560, 4702, 4992, 5166, 5326, 5622, 5923, 6102);

DELETE FROM
  team
WHERE
  id IN (4560, 4702, 4992, 5166, 5326, 5622, 5923, 6102);

----------------------------------------
-- Macomb Little Bombers Wrestling

UPDATE
  team
SET
  name_normalized = 'Macomb Little Bombers Wrestling',
  url_path_slug = NULL
WHERE
  id = 11355;

UPDATE
  tournament_team
SET
  team_id = 11355
WHERE
  id IN (11355, 11517, 12559, 12786, 13013, 13240, 13490, 13760, 14030, 14300, 14570, 14840, 15110, 15380);

DELETE FROM
  team
WHERE
  id IN (11517, 12559, 12786, 13013, 13240, 13490, 13760, 14030, 14300, 14570, 14840, 15110, 15380);

----------------------------------------
-- Mad Dog Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Mad Dog Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 5327;

UPDATE
  tournament_team
SET
  team_id = 5327
WHERE
  id IN (5327, 5477, 5761, 5924, 6103, 6286, 6470, 6658, 6840, 7016, 7204, 7403, 7591);

DELETE FROM
  team
WHERE
  id IN (5477, 5761, 5924, 6103, 6286, 6470, 6658, 6840, 7016, 7204, 7403, 7591);

----------------------------------------
-- Maddog Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Maddog Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 7777;

UPDATE
  tournament_team
SET
  team_id = 7777
WHERE
  id IN (7777, 7967, 8149, 8336, 8527, 8718, 9279, 9454, 9627, 9794, 9966, 10143, 10324, 10495, 10669, 10852, 11028, 11190, 11356, 11518, 11702, 11915, 12128, 12341, 12560, 12787, 13014, 13241, 13491, 13761, 14031, 14301, 14571, 14841, 15111, 15381);

DELETE FROM
  team
WHERE
  id IN (7967, 8149, 8336, 8527, 8718, 9279, 9454, 9627, 9794, 9966, 10143, 10324, 10495, 10669, 10852, 11028, 11190, 11356, 11518, 11702, 11915, 12128, 12341, 12560, 12787, 13014, 13241, 13491, 13761, 14031, 14301, 14571, 14841, 15111, 15381);

----------------------------------------
-- Maddog Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Maddog Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8913;

UPDATE
  tournament_team
SET
  team_id = 8913
WHERE
  id IN (8913, 9104);

DELETE FROM
  team
WHERE
  id IN (9104);

----------------------------------------
-- Mahomet Seymour Youth WC

UPDATE
  team
SET
  name_normalized = 'Mahomet Seymour Youth WC',
  url_path_slug = NULL
WHERE
  id = 8150;

UPDATE
  tournament_team
SET
  team_id = 8150
WHERE
  id IN (8150, 8337);

DELETE FROM
  team
WHERE
  id IN (8337);

----------------------------------------
-- Mahomet-Seymour Youth WC

UPDATE
  team
SET
  name_normalized = 'Mahomet-Seymour Youth WC',
  url_path_slug = NULL
WHERE
  id = 6287;

UPDATE
  tournament_team
SET
  team_id = 6287
WHERE
  id IN (6287, 6471, 7017, 7205, 7404, 7592);

DELETE FROM
  team
WHERE
  id IN (6471, 7017, 7205, 7404, 7592);

----------------------------------------
-- Maine Eagles

UPDATE
  team
SET
  name_normalized = 'Maine Eagles',
  url_path_slug = NULL
WHERE
  id = 5328;

UPDATE
  tournament_team
SET
  team_id = 5328
WHERE
  id IN (5328, 5478, 6659, 6841, 7018, 7206, 7405, 7593, 7778, 7968, 8151, 8338);

DELETE FROM
  team
WHERE
  id IN (5478, 6659, 6841, 7018, 7206, 7405, 7593, 7778, 7968, 8151, 8338);

----------------------------------------
-- Maine Eagles The Elite WC

UPDATE
  team
SET
  name_normalized = 'Maine Eagles The Elite WC',
  url_path_slug = NULL
WHERE
  id = 6288;

UPDATE
  tournament_team
SET
  team_id = 6288
WHERE
  id IN (6288, 6472);

DELETE FROM
  team
WHERE
  id IN (6472);

----------------------------------------
-- Maine Eagles WC

UPDATE
  team
SET
  name_normalized = 'Maine Eagles WC',
  url_path_slug = NULL
WHERE
  id = 3863;

UPDATE
  tournament_team
SET
  team_id = 3863
WHERE
  id IN (3863, 3979, 4107, 4235, 4393, 4561, 4703, 4840, 4993, 5167, 5623, 5762, 5925, 6104, 8528, 8719, 8914, 9105, 9280, 9455, 9628, 9795, 9967, 10144, 10325, 10496, 10670, 10853, 11029, 11191, 11357, 11519, 11703, 11916, 12129, 12342, 12561, 12788, 13015, 13242, 13492, 13762, 14032, 14302, 14572, 14842, 15112, 15382);

DELETE FROM
  team
WHERE
  id IN (3979, 4107, 4235, 4393, 4561, 4703, 4840, 4993, 5167, 5623, 5762, 5925, 6104, 8528, 8719, 8914, 9105, 9280, 9455, 9628, 9795, 9967, 10144, 10325, 10496, 10670, 10853, 11029, 11191, 11357, 11519, 11703, 11916, 12129, 12342, 12561, 12788, 13015, 13242, 13492, 13762, 14032, 14302, 14572, 14842, 15112, 15382);

----------------------------------------
-- Manteno Jr Panthers

UPDATE
  team
SET
  name_normalized = 'Manteno Jr Panthers',
  url_path_slug = NULL
WHERE
  id = 4236;

UPDATE
  tournament_team
SET
  team_id = 4236
WHERE
  id IN (4236, 4394, 4562, 4704, 4841);

DELETE FROM
  team
WHERE
  id IN (4394, 4562, 4704, 4841);

----------------------------------------
-- Manteno Jr Panthers WC

UPDATE
  team
SET
  name_normalized = 'Manteno Jr Panthers WC',
  url_path_slug = NULL
WHERE
  id = 9281;

UPDATE
  tournament_team
SET
  team_id = 9281
WHERE
  id IN (9281, 9456, 11704, 11917, 12130, 12343);

DELETE FROM
  team
WHERE
  id IN (9456, 11704, 11917, 12130, 12343);

----------------------------------------
-- Marengo Indians Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Marengo Indians Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 5329;

UPDATE
  tournament_team
SET
  team_id = 5329
WHERE
  id IN (5329, 5479, 5624, 5926, 6105);

DELETE FROM
  team
WHERE
  id IN (5479, 5624, 5926, 6105);

----------------------------------------
-- Marengo Shooters WC

UPDATE
  team
SET
  name_normalized = 'Marengo Shooters WC',
  url_path_slug = NULL
WHERE
  id = 5927;

UPDATE
  tournament_team
SET
  team_id = 5927
WHERE
  id IN (5927, 6106);

DELETE FROM
  team
WHERE
  id IN (6106);

----------------------------------------
-- Marengo WC

UPDATE
  team
SET
  name_normalized = 'Marengo WC',
  url_path_slug = NULL
WHERE
  id = 3864;

UPDATE
  tournament_team
SET
  team_id = 3864
WHERE
  id IN (3864, 3980, 4108, 4237, 4395, 4563, 4705, 4842, 4994, 5168);

DELETE FROM
  team
WHERE
  id IN (3980, 4108, 4237, 4395, 4563, 4705, 4842, 4994, 5168);

----------------------------------------
-- Marengo Youth WC

UPDATE
  team
SET
  name_normalized = 'Marengo Youth WC',
  url_path_slug = NULL
WHERE
  id = 9282;

UPDATE
  tournament_team
SET
  team_id = 9282
WHERE
  id IN (9282, 9457, 9629, 9796, 9968, 10145, 10326, 10497, 10671, 10854, 11030, 11192, 11358, 11520, 11705, 11918, 12131, 12344, 12562, 12789, 13016, 13243, 13493, 13763, 14033, 14303, 14573, 14843, 15113, 15383);

DELETE FROM
  team
WHERE
  id IN (9457, 9629, 9796, 9968, 10145, 10326, 10497, 10671, 10854, 11030, 11192, 11358, 11520, 11705, 11918, 12131, 12344, 12562, 12789, 13016, 13243, 13493, 13763, 14033, 14303, 14573, 14843, 15113, 15383);

----------------------------------------
-- Marengo Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Marengo Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 6289;

UPDATE
  tournament_team
SET
  team_id = 6289
WHERE
  id IN (6289, 6473, 6660, 6842, 7019, 7207, 7406, 7594, 7779, 7969, 8152, 8339, 8529, 8720, 8915, 9106);

DELETE FROM
  team
WHERE
  id IN (6473, 6660, 6842, 7019, 7207, 7406, 7594, 7779, 7969, 8152, 8339, 8529, 8720, 8915, 9106);

----------------------------------------
-- Marion

UPDATE
  team
SET
  name_normalized = 'Marion',
  url_path_slug = NULL
WHERE
  id = 1607;

UPDATE
  tournament_team
SET
  team_id = 1607
WHERE
  id IN (1607, 2012);

DELETE FROM
  team
WHERE
  id IN (2012);

----------------------------------------
-- Marion Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Marion Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 8916;

UPDATE
  tournament_team
SET
  team_id = 8916
WHERE
  id IN (8916, 9107);

DELETE FROM
  team
WHERE
  id IN (9107);

----------------------------------------
-- Marshall Red Rush Wrestling

UPDATE
  team
SET
  name_normalized = 'Marshall Red Rush Wrestling',
  url_path_slug = NULL
WHERE
  id = 13494;

UPDATE
  tournament_team
SET
  team_id = 13494
WHERE
  id IN (13494, 13764, 14034, 14304, 14574, 14844, 15114, 15384);

DELETE FROM
  team
WHERE
  id IN (13764, 14034, 14304, 14574, 14844, 15114, 15384);

----------------------------------------
-- Martinez Fox Valley Elite

UPDATE
  team
SET
  name_normalized = 'Martinez Fox Valley Elite',
  url_path_slug = NULL
WHERE
  id = 3865;

UPDATE
  tournament_team
SET
  team_id = 3865
WHERE
  id IN (3865, 3981, 4109, 4238, 4396, 4564, 4706, 4843, 4995, 5169, 5330, 5480, 5625, 5763, 5928, 6107, 6290, 6474, 6661, 6843, 7020, 7208, 7407, 7595, 7780, 7970, 8153, 8340, 8530, 8721, 8917, 9108, 9283, 9458, 9630, 9797, 9969, 10146, 10327, 10498, 11359, 11521, 11706, 11919, 12132, 12345, 12563, 12790, 13017, 13244, 13495, 13765, 14035, 14305, 14575, 14845, 15115, 15385);

DELETE FROM
  team
WHERE
  id IN (3981, 4109, 4238, 4396, 4564, 4706, 4843, 4995, 5169, 5330, 5480, 5625, 5763, 5928, 6107, 6290, 6474, 6661, 6843, 7020, 7208, 7407, 7595, 7780, 7970, 8153, 8340, 8530, 8721, 8917, 9108, 9283, 9458, 9630, 9797, 9969, 10146, 10327, 10498, 11359, 11521, 11706, 11919, 12132, 12345, 12563, 12790, 13017, 13244, 13495, 13765, 14035, 14305, 14575, 14845, 15115, 15385);

----------------------------------------
-- Mat Rat WC

UPDATE
  team
SET
  name_normalized = 'Mat Rat WC',
  url_path_slug = NULL
WHERE
  id = 11031;

UPDATE
  tournament_team
SET
  team_id = 11031
WHERE
  id IN (11031, 11193, 11360, 11522, 11707, 11920, 12133, 12346, 12564, 12791, 13018, 13245, 13496, 13766, 14036, 14306, 14576, 14846, 15116, 15386);

DELETE FROM
  team
WHERE
  id IN (11193, 11360, 11522, 11707, 11920, 12133, 12346, 12564, 12791, 13018, 13245, 13496, 13766, 14036, 14306, 14576, 14846, 15116, 15386);

----------------------------------------
-- Matburns WC

UPDATE
  team
SET
  name_normalized = 'Matburns WC',
  url_path_slug = NULL
WHERE
  id = 965;

UPDATE
  tournament_team
SET
  team_id = 965
WHERE
  id IN (965, 1076, 1189);

DELETE FROM
  team
WHERE
  id IN (1076, 1189);

----------------------------------------
-- Mattoon WC

UPDATE
  team
SET
  name_normalized = 'Mattoon WC',
  url_path_slug = NULL
WHERE
  id = 193;

UPDATE
  tournament_team
SET
  team_id = 193
WHERE
  id IN (193, 853, 966, 1077, 1190, 1383, 1484, 1875, 3628, 3747, 3866, 3982, 4110, 4239, 4397, 4565, 4707, 4844, 4996, 5170, 5481, 5626, 5929, 6108, 6291, 6475, 7021, 7209, 7408, 7596, 8531, 8722, 9284, 9459, 9631, 9798, 9970, 10147, 10328, 10499, 10672, 10855, 11032, 11194, 11361, 11523, 11708, 11921, 12134, 12347, 12565, 12792, 13019, 13246, 13497, 13767, 14037, 14307, 14577, 14847, 15117, 15387);

DELETE FROM
  team
WHERE
  id IN (853, 966, 1077, 1190, 1383, 1484, 1875, 3628, 3747, 3866, 3982, 4110, 4239, 4397, 4565, 4707, 4844, 4996, 5170, 5481, 5626, 5929, 6108, 6291, 6475, 7021, 7209, 7408, 7596, 8531, 8722, 9284, 9459, 9631, 9798, 9970, 10147, 10328, 10499, 10672, 10855, 11032, 11194, 11361, 11523, 11708, 11921, 12134, 12347, 12565, 12792, 13019, 13246, 13497, 13767, 14037, 14307, 14577, 14847, 15117, 15387);

----------------------------------------
-- Maywood Bucs

UPDATE
  team
SET
  name_normalized = 'Maywood Bucs',
  url_path_slug = NULL
WHERE
  id = 7781;

UPDATE
  tournament_team
SET
  team_id = 7781
WHERE
  id IN (7781, 7971, 8154, 8341, 9632, 9799, 9971, 10148);

DELETE FROM
  team
WHERE
  id IN (7971, 8154, 8341, 9632, 9799, 9971, 10148);

----------------------------------------
-- Maywood Bucs/Powerhouse

UPDATE
  team
SET
  name_normalized = 'Maywood Bucs/Powerhouse',
  url_path_slug = NULL
WHERE
  id = 8532;

UPDATE
  tournament_team
SET
  team_id = 8532
WHERE
  id IN (8532, 8723, 8918, 9109, 9285, 9460);

DELETE FROM
  team
WHERE
  id IN (8723, 8918, 9109, 9285, 9460);

----------------------------------------
-- Mc Bulldog WC

UPDATE
  team
SET
  name_normalized = 'Mc Bulldog WC',
  url_path_slug = NULL
WHERE
  id = 7775;

UPDATE
  tournament_team
SET
  team_id = 7775
WHERE
  id IN (7775, 7965, 9626, 9793);

DELETE FROM
  team
WHERE
  id IN (7965, 9626, 9793);

----------------------------------------
-- Mc Bulldog Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Mc Bulldog Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 7402;

UPDATE
  tournament_team
SET
  team_id = 7402
WHERE
  id IN (7402, 7590, 8911, 9102);

DELETE FROM
  team
WHERE
  id IN (7590, 8911, 9102);

----------------------------------------
-- Mc Bulldogs

UPDATE
  team
SET
  name_normalized = 'Mc Bulldogs',
  url_path_slug = NULL
WHERE
  id = 7022;

UPDATE
  tournament_team
SET
  team_id = 7022
WHERE
  id IN (7022, 7210);

DELETE FROM
  team
WHERE
  id IN (7210);

----------------------------------------
-- Mchenry WC

UPDATE
  team
SET
  name_normalized = 'Mchenry WC',
  url_path_slug = NULL
WHERE
  id = 11709;

UPDATE
  tournament_team
SET
  team_id = 11709
WHERE
  id IN (11709, 11922, 12135, 12348, 12566, 12793, 13020, 13247, 13498, 13768, 14038, 14308, 14578, 14848, 15118, 15388);

DELETE FROM
  team
WHERE
  id IN (11922, 12135, 12348, 12566, 12793, 13020, 13247, 13498, 13768, 14038, 14308, 14578, 14848, 15118, 15388);

----------------------------------------
-- Mclean County WC

UPDATE
  team
SET
  name_normalized = 'Mclean County WC',
  url_path_slug = NULL
WHERE
  id = 5627;

UPDATE
  tournament_team
SET
  team_id = 5627
WHERE
  id IN (5627, 5764);

DELETE FROM
  team
WHERE
  id IN (5764);

----------------------------------------
-- Mead Jr. High

UPDATE
  team
SET
  name_normalized = 'Mead Jr. High',
  url_path_slug = NULL
WHERE
  id = 1485;

UPDATE
  tournament_team
SET
  team_id = 1485
WHERE
  id IN (1485, 1610, 1876, 2014);

DELETE FROM
  team
WHERE
  id IN (1610, 1876, 2014);

----------------------------------------
-- Mean Green Machine

UPDATE
  team
SET
  name_normalized = 'Mean Green Machine',
  url_path_slug = NULL
WHERE
  id = 9286;

UPDATE
  tournament_team
SET
  team_id = 9286
WHERE
  id IN (9286, 9461, 9633, 9800);

DELETE FROM
  team
WHERE
  id IN (9461, 9633, 9800);

----------------------------------------
-- Medalist WC

UPDATE
  team
SET
  name_normalized = 'Medalist WC',
  url_path_slug = NULL
WHERE
  id = 5331;

UPDATE
  tournament_team
SET
  team_id = 5331
WHERE
  id IN (5331, 5482);

DELETE FROM
  team
WHERE
  id IN (5482);

----------------------------------------
-- Medinah Lancer WC

UPDATE
  team
SET
  name_normalized = 'Medinah Lancer WC',
  url_path_slug = NULL
WHERE
  id = 1078;

UPDATE
  tournament_team
SET
  team_id = 1078
WHERE
  id IN (1078, 1191);

DELETE FROM
  team
WHERE
  id IN (1191);

----------------------------------------
-- Mendota Mat Masters

UPDATE
  team
SET
  name_normalized = 'Mendota Mat Masters',
  url_path_slug = NULL
WHERE
  id = 4111;

UPDATE
  tournament_team
SET
  team_id = 4111
WHERE
  id IN (4111, 4708, 4845, 4997, 5171, 5483, 5765, 5930, 6109, 6292, 6476, 6662, 6844, 7023, 7211, 7409, 7597, 8155, 8342, 8533, 8724, 8919, 9110, 9287, 9462, 9634, 9801, 10329, 10500, 10673, 10856);

DELETE FROM
  team
WHERE
  id IN (4708, 4845, 4997, 5171, 5483, 5765, 5930, 6109, 6292, 6476, 6662, 6844, 7023, 7211, 7409, 7597, 8155, 8342, 8533, 8724, 8919, 9110, 9287, 9462, 9634, 9801, 10329, 10500, 10673, 10856);

----------------------------------------
-- Mendota WC

UPDATE
  team
SET
  name_normalized = 'Mendota WC',
  url_path_slug = NULL
WHERE
  id = 10330;

UPDATE
  tournament_team
SET
  team_id = 10330
WHERE
  id IN (10330, 10501);

DELETE FROM
  team
WHERE
  id IN (10501);

----------------------------------------
-- Meridian Hawks Youth WC

UPDATE
  team
SET
  name_normalized = 'Meridian Hawks Youth WC',
  url_path_slug = NULL
WHERE
  id = 11710;

UPDATE
  tournament_team
SET
  team_id = 11710
WHERE
  id IN (11710, 11923, 12136, 12349, 12567, 12794, 13021, 13248, 13499, 13769, 14039, 14309, 14579, 14849, 15119, 15389);

DELETE FROM
  team
WHERE
  id IN (11923, 12136, 12349, 12567, 12794, 13021, 13248, 13499, 13769, 14039, 14309, 14579, 14849, 15119, 15389);

----------------------------------------
-- Metamora Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Metamora Kids Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1079;

UPDATE
  tournament_team
SET
  team_id = 1079
WHERE
  id IN (1079, 1192, 1486, 1611, 1877, 2015, 3867, 3983, 4112, 4240, 4398, 4566, 4709, 4846, 4998, 5172, 5484, 5628, 5931, 6110, 6293, 6477, 6663, 6845, 7024, 7212, 7410, 7598, 7782, 7972, 8156, 8343, 8534, 8725, 8920, 9111, 9288, 9463, 9635, 9802, 9972, 10149, 10674, 10857, 11033, 11195, 11711, 11924, 12137, 12350, 12568, 12795, 13022, 13249, 13500, 13770, 14040, 14310, 14580, 14850, 15120, 15390);

DELETE FROM
  team
WHERE
  id IN (1192, 1486, 1611, 1877, 2015, 3867, 3983, 4112, 4240, 4398, 4566, 4709, 4846, 4998, 5172, 5484, 5628, 5931, 6110, 6293, 6477, 6663, 6845, 7024, 7212, 7410, 7598, 7782, 7972, 8156, 8343, 8534, 8725, 8920, 9111, 9288, 9463, 9635, 9802, 9972, 10149, 10674, 10857, 11033, 11195, 11711, 11924, 12137, 12350, 12568, 12795, 13022, 13249, 13500, 13770, 14040, 14310, 14580, 14850, 15120, 15390);

----------------------------------------
-- Metro Stallions

UPDATE
  team
SET
  name_normalized = 'Metro Stallions',
  url_path_slug = NULL
WHERE
  id = 3176;

UPDATE
  tournament_team
SET
  team_id = 3176
WHERE
  id IN (3176, 3868, 3984, 4113);

DELETE FROM
  team
WHERE
  id IN (3868, 3984, 4113);

----------------------------------------
-- Midwest Central Youth WC

UPDATE
  team
SET
  name_normalized = 'Midwest Central Youth WC',
  url_path_slug = NULL
WHERE
  id = 3869;

UPDATE
  tournament_team
SET
  team_id = 3869
WHERE
  id IN (3869, 3985, 4114, 4399, 4567, 4710, 4847, 4999, 5173, 5332, 5629, 5766, 5932, 6111, 6294, 6478, 6664, 6846, 7025, 7213, 7411, 7599, 7783, 7973, 8157, 8344, 8535, 8726, 8921, 9112, 9289, 9464, 9636, 9803, 9973, 10150, 11712, 11925, 12138, 12351, 12569, 12796, 13023, 13250, 13501, 13771, 14041, 14311, 14581, 14851, 15121, 15391);

DELETE FROM
  team
WHERE
  id IN (3985, 4114, 4399, 4567, 4710, 4847, 4999, 5173, 5332, 5629, 5766, 5932, 6111, 6294, 6478, 6664, 6846, 7025, 7213, 7411, 7599, 7783, 7973, 8157, 8344, 8535, 8726, 8921, 9112, 9289, 9464, 9636, 9803, 9973, 10150, 11712, 11925, 12138, 12351, 12569, 12796, 13023, 13250, 13501, 13771, 14041, 14311, 14581, 14851, 15121, 15391);

----------------------------------------
-- Minooka Indians Elite

UPDATE
  team
SET
  name_normalized = 'Minooka Indians Elite',
  url_path_slug = NULL
WHERE
  id = 9290;

UPDATE
  tournament_team
SET
  team_id = 9290
WHERE
  id IN (9290, 9465, 9637, 9804, 9974, 10151);

DELETE FROM
  team
WHERE
  id IN (9465, 9637, 9804, 9974, 10151);

----------------------------------------
-- Minooka Indians Elite Wrestling

UPDATE
  team
SET
  name_normalized = 'Minooka Indians Elite Wrestling',
  url_path_slug = NULL
WHERE
  id = 10331;

UPDATE
  tournament_team
SET
  team_id = 10331
WHERE
  id IN (10331, 10502, 10675, 10858);

DELETE FROM
  team
WHERE
  id IN (10502, 10675, 10858);

----------------------------------------
-- Minooka Little Indians

UPDATE
  team
SET
  name_normalized = 'Minooka Little Indians',
  url_path_slug = NULL
WHERE
  id = 5333;

UPDATE
  tournament_team
SET
  team_id = 5333
WHERE
  id IN (5333, 5485, 5630, 5767, 5933, 6112, 6295, 6479);

DELETE FROM
  team
WHERE
  id IN (5485, 5630, 5767, 5933, 6112, 6295, 6479);

----------------------------------------
-- Moline Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Moline Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1487;

UPDATE
  tournament_team
SET
  team_id = 1487
WHERE
  id IN (1487, 1612, 1879, 2292, 4400, 4568, 4711, 4848, 5000, 5174, 5334, 5486, 5631, 5768, 5934, 6113, 6296, 6480, 6665, 6847, 7026, 7214, 7412, 7600, 7784, 7974, 8158, 8345, 8536, 8727, 8922, 9113, 9291, 9466, 9638, 9805, 9975, 10152, 10332, 10503, 10676, 10859, 11362, 11524, 11713, 11926, 12139, 12352, 12570, 12797, 13024, 13251, 13502, 13772, 14042, 14312, 14582, 14852, 15122, 15392);

DELETE FROM
  team
WHERE
  id IN (1612, 1879, 2292, 4400, 4568, 4711, 4848, 5000, 5174, 5334, 5486, 5631, 5768, 5934, 6113, 6296, 6480, 6665, 6847, 7026, 7214, 7412, 7600, 7784, 7974, 8158, 8345, 8536, 8727, 8922, 9113, 9291, 9466, 9638, 9805, 9975, 10152, 10332, 10503, 10676, 10859, 11362, 11524, 11713, 11926, 12139, 12352, 12570, 12797, 13024, 13251, 13502, 13772, 14042, 14312, 14582, 14852, 15122, 15392);

----------------------------------------
-- Montego Matmen

UPDATE
  team
SET
  name_normalized = 'Montego Matmen',
  url_path_slug = NULL
WHERE
  id = 3632;

UPDATE
  tournament_team
SET
  team_id = 3632
WHERE
  id IN (3632, 3870);

DELETE FROM
  team
WHERE
  id IN (3870);

----------------------------------------
-- Monticello Kids WC

UPDATE
  team
SET
  name_normalized = 'Monticello Kids WC',
  url_path_slug = NULL
WHERE
  id = 6666;

UPDATE
  tournament_team
SET
  team_id = 6666
WHERE
  id IN (6666, 6848);

DELETE FROM
  team
WHERE
  id IN (6848);

----------------------------------------
-- Monticello Youth WC

UPDATE
  team
SET
  name_normalized = 'Monticello Youth WC',
  url_path_slug = NULL
WHERE
  id = 10333;

UPDATE
  tournament_team
SET
  team_id = 10333
WHERE
  id IN (10333, 10504, 12571, 12798, 13025, 13252, 13503, 13773, 14043, 14313, 14583, 14853, 15123, 15393);

DELETE FROM
  team
WHERE
  id IN (10504, 12571, 12798, 13025, 13252, 13503, 13773, 14043, 14313, 14583, 14853, 15123, 15393);

----------------------------------------
-- Morris

UPDATE
  team
SET
  name_normalized = 'Morris',
  url_path_slug = NULL
WHERE
  id = 45;

UPDATE
  tournament_team
SET
  team_id = 45
WHERE
  id IN (45, 194);

DELETE FROM
  team
WHERE
  id IN (194);

----------------------------------------
-- Morris WC

UPDATE
  team
SET
  name_normalized = 'Morris WC',
  url_path_slug = NULL
WHERE
  id = 12572;

UPDATE
  tournament_team
SET
  team_id = 12572
WHERE
  id IN (12572, 12799, 13026, 13253, 13504, 13774, 14044, 14314, 14584, 14854, 15124, 15394);

DELETE FROM
  team
WHERE
  id IN (12799, 13026, 13253, 13504, 13774, 14044, 14314, 14584, 14854, 15124, 15394);

----------------------------------------
-- Morrison Stallions WC

UPDATE
  team
SET
  name_normalized = 'Morrison Stallions WC',
  url_path_slug = NULL
WHERE
  id = 5001;

UPDATE
  tournament_team
SET
  team_id = 5001
WHERE
  id IN (5001, 5175, 5335, 5487, 5632, 5769, 5935, 6114);

DELETE FROM
  team
WHERE
  id IN (5175, 5335, 5487, 5632, 5769, 5935, 6114);

----------------------------------------
-- Morton Little Mustangs WC

UPDATE
  team
SET
  name_normalized = 'Morton Little Mustangs WC',
  url_path_slug = NULL
WHERE
  id = 4401;

UPDATE
  tournament_team
SET
  team_id = 4401
WHERE
  id IN (4401, 4569, 4712, 4849, 5002, 5176, 5336, 5488, 5633, 5770, 5936, 6115, 6297, 6481, 6667, 6849, 7027, 7215, 8159, 8346, 8537, 8728, 8923, 9114, 9292, 9467, 9639, 9806);

DELETE FROM
  team
WHERE
  id IN (4569, 4712, 4849, 5002, 5176, 5336, 5488, 5633, 5770, 5936, 6115, 6297, 6481, 6667, 6849, 7027, 7215, 8159, 8346, 8537, 8728, 8923, 9114, 9292, 9467, 9639, 9806);

----------------------------------------
-- Morton Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Morton Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 969;

UPDATE
  tournament_team
SET
  team_id = 969
WHERE
  id IN (969, 1083, 1195, 1386, 1488, 1613, 1745, 1881, 2020, 2450, 2584, 3750, 3873, 3987, 4115, 4241, 4713, 5003, 5177, 5337, 5634, 5771, 6298, 6482, 7028, 7216, 7413, 7601, 7785, 7975, 8924, 9115, 9976, 10153, 10334, 10505, 11034, 11196, 11714, 11927, 12140, 12353, 12573, 12800, 13027, 13254, 13505, 13775, 14045, 14315, 14585, 14855, 15125, 15395);

DELETE FROM
  team
WHERE
  id IN (1083, 1195, 1386, 1488, 1613, 1745, 1881, 2020, 2450, 2584, 3750, 3873, 3987, 4115, 4241, 4713, 5003, 5177, 5337, 5634, 5771, 6298, 6482, 7028, 7216, 7413, 7601, 7785, 7975, 8924, 9115, 9976, 10153, 10334, 10505, 11034, 11196, 11714, 11927, 12140, 12353, 12573, 12800, 13027, 13254, 13505, 13775, 14045, 14315, 14585, 14855, 15125, 15395);

----------------------------------------
-- Ms Youth WC

UPDATE
  team
SET
  name_normalized = 'Ms Youth WC',
  url_path_slug = NULL
WHERE
  id = 7776;

UPDATE
  tournament_team
SET
  team_id = 7776
WHERE
  id IN (7776, 7966, 10668, 10851, 11701, 11914, 12127, 12340, 12558, 12785, 13012, 13239, 13489, 13759, 14029, 14299, 14569, 14839, 15109, 15379);

DELETE FROM
  team
WHERE
  id IN (7966, 10668, 10851, 11701, 11914, 12127, 12340, 12558, 12785, 13012, 13239, 13489, 13759, 14029, 14299, 14569, 14839, 15109, 15379);

----------------------------------------
-- Mt Vernon Lions

UPDATE
  team
SET
  name_normalized = 'Mt Vernon Lions',
  url_path_slug = NULL
WHERE
  id = 3988;

UPDATE
  tournament_team
SET
  team_id = 3988
WHERE
  id IN (3988, 4116, 4242, 7786, 7976);

DELETE FROM
  team
WHERE
  id IN (4116, 4242, 7786, 7976);

----------------------------------------
-- Mt Vernon Lions WC

UPDATE
  team
SET
  name_normalized = 'Mt Vernon Lions WC',
  url_path_slug = NULL
WHERE
  id = 8160;

UPDATE
  tournament_team
SET
  team_id = 8160
WHERE
  id IN (8160, 8347);

DELETE FROM
  team
WHERE
  id IN (8347);

----------------------------------------
-- Mt. Greenwood

UPDATE
  team
SET
  name_normalized = 'Mt. Greenwood',
  url_path_slug = NULL
WHERE
  id = 295;

UPDATE
  tournament_team
SET
  team_id = 295
WHERE
  id IN (295, 856);

DELETE FROM
  team
WHERE
  id IN (856);

----------------------------------------
-- Mt. Olive Cats Kids Wrestling

UPDATE
  team
SET
  name_normalized = 'Mt. Olive Cats Kids Wrestling',
  url_path_slug = NULL
WHERE
  id = 7787;

UPDATE
  tournament_team
SET
  team_id = 7787
WHERE
  id IN (7787, 7977, 8161, 8348, 8538, 8729, 9293, 9468);

DELETE FROM
  team
WHERE
  id IN (7977, 8161, 8348, 8538, 8729, 9293, 9468);

----------------------------------------
-- Mt. Vernon Lions

UPDATE
  team
SET
  name_normalized = 'Mt. Vernon Lions',
  url_path_slug = NULL
WHERE
  id = 4402;

UPDATE
  tournament_team
SET
  team_id = 4402
WHERE
  id IN (4402, 4570, 5004, 5178, 5339, 5635, 5773, 5937, 6116, 6299, 6483, 6668, 6850, 7029, 7217, 7414, 7602);

DELETE FROM
  team
WHERE
  id IN (4570, 5004, 5178, 5339, 5635, 5773, 5937, 6116, 6299, 6483, 6668, 6850, 7029, 7217, 7414, 7602);

----------------------------------------
-- Mt. Vernon Lions WC

UPDATE
  team
SET
  name_normalized = 'Mt. Vernon Lions WC',
  url_path_slug = NULL
WHERE
  id = 9294;

UPDATE
  tournament_team
SET
  team_id = 9294
WHERE
  id IN (9294, 9469, 9640, 9807, 9977, 10154, 10335, 10506);

DELETE FROM
  team
WHERE
  id IN (9469, 9640, 9807, 9977, 10154, 10335, 10506);

----------------------------------------
-- Mt. Vernon Lions Wrestling

UPDATE
  team
SET
  name_normalized = 'Mt. Vernon Lions Wrestling',
  url_path_slug = NULL
WHERE
  id = 10677;

UPDATE
  tournament_team
SET
  team_id = 10677
WHERE
  id IN (10677, 10860);

DELETE FROM
  team
WHERE
  id IN (10860);

----------------------------------------
-- Mt. Vernon Lions Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Mt. Vernon Lions Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8925;

UPDATE
  tournament_team
SET
  team_id = 8925
WHERE
  id IN (8925, 9116);

DELETE FROM
  team
WHERE
  id IN (9116);

----------------------------------------
-- Mt. Zion Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Mt. Zion Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 970;

UPDATE
  tournament_team
SET
  team_id = 970
WHERE
  id IN (970, 1084, 1196, 1387, 1489, 1614, 1747, 1884, 2022, 2296, 2728, 3635, 3752, 3874, 3989, 4117, 4243, 4403, 4571, 4714, 5005, 5179, 5340, 5636, 5774, 5938, 6117, 6300, 6484, 6669, 6851, 7030, 7218, 8539, 8730, 9641, 9808, 9978, 10155, 10336, 10507, 10678, 10861, 11035, 11197, 11363, 11525, 11715, 11928, 12141, 12354, 12574, 12801, 13028, 13255, 13506, 13776, 14046, 14316, 14586, 14856, 15126, 15396);

DELETE FROM
  team
WHERE
  id IN (1084, 1196, 1387, 1489, 1614, 1747, 1884, 2022, 2296, 2728, 3635, 3752, 3874, 3989, 4117, 4243, 4403, 4571, 4714, 5005, 5179, 5340, 5636, 5774, 5938, 6117, 6300, 6484, 6669, 6851, 7030, 7218, 8539, 8730, 9641, 9808, 9978, 10155, 10336, 10507, 10678, 10861, 11035, 11197, 11363, 11525, 11715, 11928, 12141, 12354, 12574, 12801, 13028, 13255, 13506, 13776, 14046, 14316, 14586, 14856, 15126, 15396);

----------------------------------------
-- Mundelein WC

UPDATE
  team
SET
  name_normalized = 'Mundelein WC',
  url_path_slug = NULL
WHERE
  id = 8162;

UPDATE
  tournament_team
SET
  team_id = 8162
WHERE
  id IN (8162, 8349, 8926, 9117);

DELETE FROM
  team
WHERE
  id IN (8349, 8926, 9117);

----------------------------------------
-- Murphysboro Jr. High

UPDATE
  team
SET
  name_normalized = 'Murphysboro Jr. High',
  url_path_slug = NULL
WHERE
  id = 971;

UPDATE
  tournament_team
SET
  team_id = 971
WHERE
  id IN (971, 1085, 1197, 1388, 1490, 3636, 3753);

DELETE FROM
  team
WHERE
  id IN (1085, 1197, 1388, 1490, 3636, 3753);

----------------------------------------
-- Murphysboro Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Murphysboro Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 3875;

UPDATE
  tournament_team
SET
  team_id = 3875
WHERE
  id IN (3875, 3990, 4118, 4244, 4404, 4572, 4715, 4850, 5006, 5180, 5341, 5489, 5775, 5939, 6118, 6301, 6485, 6670, 6852, 7031, 7219, 7415, 7603, 7788, 7978, 8163, 8350, 8540, 8731, 8927, 9118, 9295, 9470, 9642, 9809, 9979, 10156, 10337, 10508, 10679, 10862, 11036, 11198, 11364, 11526, 11716, 11929, 12142, 12355, 12575, 12802, 13029, 13256, 13507, 13777, 14047, 14317, 14587, 14857, 15127, 15397);

DELETE FROM
  team
WHERE
  id IN (3990, 4118, 4244, 4404, 4572, 4715, 4850, 5006, 5180, 5341, 5489, 5775, 5939, 6118, 6301, 6485, 6670, 6852, 7031, 7219, 7415, 7603, 7788, 7978, 8163, 8350, 8540, 8731, 8927, 9118, 9295, 9470, 9642, 9809, 9979, 10156, 10337, 10508, 10679, 10862, 11036, 11198, 11364, 11526, 11716, 11929, 12142, 12355, 12575, 12802, 13029, 13256, 13507, 13777, 14047, 14317, 14587, 14857, 15127, 15397);

----------------------------------------
-- Mustang WC

UPDATE
  team
SET
  name_normalized = 'Mustang WC',
  url_path_slug = NULL
WHERE
  id = 1308;

UPDATE
  tournament_team
SET
  team_id = 1308
WHERE
  id IN (1308, 1389, 1491, 1616, 1749, 1886, 2024, 2298, 3754, 4405, 4573, 4716, 4851, 5007, 5181, 5342, 5776, 5940, 6119, 6302, 6486, 6671, 6853, 7032, 7220, 7416, 7604, 7789, 7979, 8164, 8351, 8541, 8732, 9643, 9810, 9980, 10157, 10338, 10509, 10680, 10863, 11037, 11199, 11365, 11527, 11717, 11930, 12143, 12356, 12576, 12803, 13030, 13257, 13508, 13778, 14048, 14318, 14588, 14858, 15128, 15398);

DELETE FROM
  team
WHERE
  id IN (1389, 1491, 1616, 1749, 1886, 2024, 2298, 3754, 4405, 4573, 4716, 4851, 5007, 5181, 5342, 5776, 5940, 6119, 6302, 6486, 6671, 6853, 7032, 7220, 7416, 7604, 7789, 7979, 8164, 8351, 8541, 8732, 9643, 9810, 9980, 10157, 10338, 10509, 10680, 10863, 11037, 11199, 11365, 11527, 11717, 11930, 12143, 12356, 12576, 12803, 13030, 13257, 13508, 13778, 14048, 14318, 14588, 14858, 15128, 15398);

----------------------------------------
-- Mustang/Skt WC

UPDATE
  team
SET
  name_normalized = 'Mustang/Skt WC',
  url_path_slug = NULL
WHERE
  id = 8928;

UPDATE
  tournament_team
SET
  team_id = 8928
WHERE
  id IN (8928, 9119, 9296, 9471);

DELETE FROM
  team
WHERE
  id IN (9119, 9296, 9471);

----------------------------------------
-- N.Y.A Jr. Rebels WC

UPDATE
  team
SET
  name_normalized = 'N.Y.A Jr. Rebels WC',
  url_path_slug = NULL
WHERE
  id = 11718;

UPDATE
  tournament_team
SET
  team_id = 11718
WHERE
  id IN (11718, 11931, 12144, 12357, 12577, 12804, 13031, 13258, 13509, 13779, 14049, 14319, 14589, 14859, 15129, 15399);

DELETE FROM
  team
WHERE
  id IN (11931, 12144, 12357, 12577, 12804, 13031, 13258, 13509, 13779, 14049, 14319, 14589, 14859, 15129, 15399);

----------------------------------------
-- Naperville

UPDATE
  team
SET
  name_normalized = 'Naperville',
  url_path_slug = NULL
WHERE
  id = 221;

UPDATE
  tournament_team
SET
  team_id = 221
WHERE
  id IN (221, 399, 860);

DELETE FROM
  team
WHERE
  id IN (399, 860);

----------------------------------------
-- Naperville Eagles

UPDATE
  team
SET
  name_normalized = 'Naperville Eagles',
  url_path_slug = NULL
WHERE
  id = 3876;

UPDATE
  tournament_team
SET
  team_id = 3876
WHERE
  id IN (3876, 3991, 4245);

DELETE FROM
  team
WHERE
  id IN (3991, 4245);

----------------------------------------
-- Naperville Hammer Huskies

UPDATE
  team
SET
  name_normalized = 'Naperville Hammer Huskies',
  url_path_slug = NULL
WHERE
  id = 9644;

UPDATE
  tournament_team
SET
  team_id = 9644
WHERE
  id IN (9644, 9811, 9981, 10158, 10681, 10864);

DELETE FROM
  team
WHERE
  id IN (9811, 9981, 10158, 10681, 10864);

----------------------------------------
-- Naperville Huskie WC

UPDATE
  team
SET
  name_normalized = 'Naperville Huskie WC',
  url_path_slug = NULL
WHERE
  id = 5941;

UPDATE
  tournament_team
SET
  team_id = 5941
WHERE
  id IN (5941, 6120, 6303, 6487);

DELETE FROM
  team
WHERE
  id IN (6120, 6303, 6487);

----------------------------------------
-- Naperville Husky WC

UPDATE
  team
SET
  name_normalized = 'Naperville Husky WC',
  url_path_slug = NULL
WHERE
  id = 7033;

UPDATE
  tournament_team
SET
  team_id = 7033
WHERE
  id IN (7033, 7221, 7790, 7980);

DELETE FROM
  team
WHERE
  id IN (7221, 7790, 7980);

----------------------------------------
-- Naperville Lincoln

UPDATE
  team
SET
  name_normalized = 'Naperville Lincoln',
  url_path_slug = NULL
WHERE
  id = 72;

UPDATE
  tournament_team
SET
  team_id = 72
WHERE
  id IN (72, 166);

DELETE FROM
  team
WHERE
  id IN (166);

----------------------------------------
-- Naperville Patriots

UPDATE
  team
SET
  name_normalized = 'Naperville Patriots',
  url_path_slug = NULL
WHERE
  id = 859;

UPDATE
  tournament_team
SET
  team_id = 859
WHERE
  id IN (859, 1198, 1310, 1390, 1493, 1617);

DELETE FROM
  team
WHERE
  id IN (1198, 1310, 1390, 1493, 1617);

----------------------------------------
-- Naperville WC

UPDATE
  team
SET
  name_normalized = 'Naperville WC',
  url_path_slug = NULL
WHERE
  id = 1087;

UPDATE
  tournament_team
SET
  team_id = 1087
WHERE
  id IN (1087, 2732, 3031, 3992, 4247, 4406, 4574, 4717, 4852, 5008, 5182, 5343, 5490, 5637, 5777, 5942, 6121, 6672, 6854, 7034, 7222, 7417, 7605, 7791, 7981, 8165, 8352, 8542, 8733, 9297, 9472, 9645, 9812, 9982, 10159, 10339, 10510, 10682, 10865, 11038, 11200, 11366, 11528, 11719, 11932, 12145, 12358, 12578, 12805, 13032, 13259, 13510, 13780, 14050, 14320, 14590, 14860, 15130, 15400);

DELETE FROM
  team
WHERE
  id IN (2732, 3031, 3992, 4247, 4406, 4574, 4717, 4852, 5008, 5182, 5343, 5490, 5637, 5777, 5942, 6121, 6672, 6854, 7034, 7222, 7417, 7605, 7791, 7981, 8165, 8352, 8542, 8733, 9297, 9472, 9645, 9812, 9982, 10159, 10339, 10510, 10682, 10865, 11038, 11200, 11366, 11528, 11719, 11932, 12145, 12358, 12578, 12805, 13032, 13259, 13510, 13780, 14050, 14320, 14590, 14860, 15130, 15400);

----------------------------------------
-- Naperville Warhawks

UPDATE
  team
SET
  name_normalized = 'Naperville Warhawks',
  url_path_slug = NULL
WHERE
  id = 861;

UPDATE
  tournament_team
SET
  team_id = 861
WHERE
  id IN (861, 972, 1086, 1199, 4119);

DELETE FROM
  team
WHERE
  id IN (972, 1086, 1199, 4119);

----------------------------------------
-- Naperville Warriors

UPDATE
  team
SET
  name_normalized = 'Naperville Warriors',
  url_path_slug = NULL
WHERE
  id = 973;

UPDATE
  tournament_team
SET
  team_id = 973
WHERE
  id IN (973, 1200, 1494, 2026);

DELETE FROM
  team
WHERE
  id IN (1200, 1494, 2026);

----------------------------------------
-- Naperville Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Naperville Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8929;

UPDATE
  tournament_team
SET
  team_id = 8929
WHERE
  id IN (8929, 9120);

DELETE FROM
  team
WHERE
  id IN (9120);

----------------------------------------
-- New Berlin WC

UPDATE
  team
SET
  name_normalized = 'New Berlin WC',
  url_path_slug = NULL
WHERE
  id = 6304;

UPDATE
  tournament_team
SET
  team_id = 6304
WHERE
  id IN (6304, 6488);

DELETE FROM
  team
WHERE
  id IN (6488);

----------------------------------------
-- New Lenox Lions

UPDATE
  team
SET
  name_normalized = 'New Lenox Lions',
  url_path_slug = NULL
WHERE
  id = 638;

UPDATE
  tournament_team
SET
  team_id = 638
WHERE
  id IN (638, 862, 974);

DELETE FROM
  team
WHERE
  id IN (862, 974);

----------------------------------------
-- New Lenox Oakview

UPDATE
  team
SET
  name_normalized = 'New Lenox Oakview',
  url_path_slug = NULL
WHERE
  id = 7;

UPDATE
  tournament_team
SET
  team_id = 7
WHERE
  id IN (7, 47, 73, 167, 196, 222, 526);

DELETE FROM
  team
WHERE
  id IN (47, 73, 167, 196, 222, 526);

----------------------------------------
-- Niles West Jr Wolves Wrestling

UPDATE
  team
SET
  name_normalized = 'Niles West Jr Wolves Wrestling',
  url_path_slug = NULL
WHERE
  id = 8166;

UPDATE
  tournament_team
SET
  team_id = 8166
WHERE
  id IN (8166, 8353);

DELETE FROM
  team
WHERE
  id IN (8353);

----------------------------------------
-- Nomad Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Nomad Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 11039;

UPDATE
  tournament_team
SET
  team_id = 11039
WHERE
  id IN (11039, 11201, 11367, 11529, 11720, 11933, 12146, 12359, 12579, 12806, 13033, 13260, 13511, 13781, 14051, 14321, 14591, 14861, 15131, 15401);

DELETE FROM
  team
WHERE
  id IN (11201, 11367, 11529, 11720, 11933, 12146, 12359, 12579, 12806, 13033, 13260, 13511, 13781, 14051, 14321, 14591, 14861, 15131, 15401);

----------------------------------------
-- North Boone WC

UPDATE
  team
SET
  name_normalized = 'North Boone WC',
  url_path_slug = NULL
WHERE
  id = 9646;

UPDATE
  tournament_team
SET
  team_id = 9646
WHERE
  id IN (9646, 9813, 11040, 11202, 12580, 12807, 13034, 13261);

DELETE FROM
  team
WHERE
  id IN (9813, 11040, 11202, 12580, 12807, 13034, 13261);

----------------------------------------
-- North Shore Edge

UPDATE
  team
SET
  name_normalized = 'North Shore Edge',
  url_path_slug = NULL
WHERE
  id = 7792;

UPDATE
  tournament_team
SET
  team_id = 7792
WHERE
  id IN (7792, 7982, 8167, 8354, 8543, 8734, 8930, 9121, 9298, 9473, 9647, 9814, 9983, 10160, 10340, 10511);

DELETE FROM
  team
WHERE
  id IN (7982, 8167, 8354, 8543, 8734, 8930, 9121, 9298, 9473, 9647, 9814, 9983, 10160, 10340, 10511);

----------------------------------------
-- Northfield

UPDATE
  team
SET
  name_normalized = 'Northfield',
  url_path_slug = NULL
WHERE
  id = 8;

UPDATE
  tournament_team
SET
  team_id = 8
WHERE
  id IN (8, 49, 74, 137);

DELETE FROM
  team
WHERE
  id IN (49, 74, 137);

----------------------------------------
-- Northshore Gators

UPDATE
  team
SET
  name_normalized = 'Northshore Gators',
  url_path_slug = NULL
WHERE
  id = 4407;

UPDATE
  tournament_team
SET
  team_id = 4407
WHERE
  id IN (4407, 4575);

DELETE FROM
  team
WHERE
  id IN (4575);

----------------------------------------
-- Notre Dame

UPDATE
  team
SET
  name_normalized = 'Notre Dame',
  url_path_slug = NULL
WHERE
  id = 4408;

UPDATE
  tournament_team
SET
  team_id = 4408
WHERE
  id IN (4408, 4576);

DELETE FROM
  team
WHERE
  id IN (4576);

----------------------------------------
-- Notre Dame WC

UPDATE
  team
SET
  name_normalized = 'Notre Dame WC',
  url_path_slug = NULL
WHERE
  id = 4853;

UPDATE
  tournament_team
SET
  team_id = 4853
WHERE
  id IN (4853, 5344, 5491, 7793, 7983, 8168, 8355, 9648, 9815, 9984, 10161, 10341, 10512, 10683, 10866, 11041, 11203, 11368, 11530, 11721, 11934, 12147, 12360, 12581, 12808, 13035, 13262, 13512, 13782, 14052, 14322, 14592, 14862, 15132, 15402);

DELETE FROM
  team
WHERE
  id IN (5344, 5491, 7793, 7983, 8168, 8355, 9648, 9815, 9984, 10161, 10341, 10512, 10683, 10866, 11041, 11203, 11368, 11530, 11721, 11934, 12147, 12360, 12581, 12808, 13035, 13262, 13512, 13782, 14052, 14322, 14592, 14862, 15132, 15402);

----------------------------------------
-- Notre Dame Wrestling

UPDATE
  team
SET
  name_normalized = 'Notre Dame Wrestling',
  url_path_slug = NULL
WHERE
  id = 5009;

UPDATE
  tournament_team
SET
  team_id = 5009
WHERE
  id IN (5009, 5183, 5638, 5778, 5943, 6122, 6305, 6489, 6673, 6855);

DELETE FROM
  team
WHERE
  id IN (5183, 5638, 5778, 5943, 6122, 6305, 6489, 6673, 6855);

----------------------------------------
-- Nwo WC

UPDATE
  team
SET
  name_normalized = 'Nwo WC',
  url_path_slug = NULL
WHERE
  id = 10056;

UPDATE
  tournament_team
SET
  team_id = 10056
WHERE
  id IN (10056, 10233, 10404, 10575, 10758, 10941, 11103, 11265, 11427, 11589, 11802, 12015, 12228, 12441, 12668, 12895, 13122, 13349, 13619, 13889, 14159, 14429, 14699, 14969, 15239, 15509);

DELETE FROM
  team
WHERE
  id IN (10233, 10404, 10575, 10758, 10941, 11103, 11265, 11427, 11589, 11802, 12015, 12228, 12441, 12668, 12895, 13122, 13349, 13619, 13889, 14159, 14429, 14699, 14969, 15239, 15509);

----------------------------------------
-- O'Fallon Little Panthers

UPDATE
  team
SET
  name_normalized = 'O''Fallon Little Panthers',
  url_path_slug = NULL
WHERE
  id = 5010;

UPDATE
  tournament_team
SET
  team_id = 5010
WHERE
  id IN (5010, 5184, 5345, 5492, 5779);

DELETE FROM
  team
WHERE
  id IN (5184, 5345, 5492, 5779);

----------------------------------------
-- OPRF Warhawks

UPDATE
  team
SET
  name_normalized = 'OPRF Warhawks',
  url_path_slug = NULL
WHERE
  id = 1392;

UPDATE
  tournament_team
SET
  team_id = 1392
WHERE
  id IN (1392, 1497);

DELETE FROM
  team
WHERE
  id IN (1497);

----------------------------------------
-- O`Fallon Little Panthers

UPDATE
  team
SET
  name_normalized = 'O`Fallon Little Panthers',
  url_path_slug = NULL
WHERE
  id = 5944;

UPDATE
  tournament_team
SET
  team_id = 5944
WHERE
  id IN (5944, 6123, 6311, 6495, 6680, 6862, 7040, 7228, 7418, 7606, 7794, 7984);

DELETE FROM
  team
WHERE
  id IN (6123, 6311, 6495, 6680, 6862, 7040, 7228, 7418, 7606, 7794, 7984);

----------------------------------------
-- O`Fallon Little Panthers WC

UPDATE
  team
SET
  name_normalized = 'O`Fallon Little Panthers WC',
  url_path_slug = NULL
WHERE
  id = 8169;

UPDATE
  tournament_team
SET
  team_id = 8169
WHERE
  id IN (8169, 8356, 8544, 8735, 9299, 9474, 9649, 9816, 9985, 10162, 10342, 10513, 10684, 10867, 11042, 11204, 11369, 11531, 11722, 11935, 12148, 12361, 12582, 12809, 13036, 13263, 13513, 13783, 14053, 14323, 14593, 14863, 15133, 15403);

DELETE FROM
  team
WHERE
  id IN (8356, 8544, 8735, 9299, 9474, 9649, 9816, 9985, 10162, 10342, 10513, 10684, 10867, 11042, 11204, 11369, 11531, 11722, 11935, 12148, 12361, 12582, 12809, 13036, 13263, 13513, 13783, 14053, 14323, 14593, 14863, 15133, 15403);

----------------------------------------
-- O`Fallon Little Panthers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'O`Fallon Little Panthers Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8931;

UPDATE
  tournament_team
SET
  team_id = 8931
WHERE
  id IN (8931, 9122);

DELETE FROM
  team
WHERE
  id IN (9122);

----------------------------------------
-- Oak Forest

UPDATE
  team
SET
  name_normalized = 'Oak Forest',
  url_path_slug = NULL
WHERE
  id = 75;

UPDATE
  tournament_team
SET
  team_id = 75
WHERE
  id IN (75, 168, 197, 223, 641, 863);

DELETE FROM
  team
WHERE
  id IN (168, 197, 223, 641, 863);

----------------------------------------
-- Oak Forest P. D. Warriors Wrestling

UPDATE
  team
SET
  name_normalized = 'Oak Forest P. D. Warriors Wrestling',
  url_path_slug = NULL
WHERE
  id = 13514;

UPDATE
  tournament_team
SET
  team_id = 13514
WHERE
  id IN (13514, 13784, 14054, 14324, 14594, 14864, 15134, 15404);

DELETE FROM
  team
WHERE
  id IN (13784, 14054, 14324, 14594, 14864, 15134, 15404);

----------------------------------------
-- Oak Forest Park District Warriors

UPDATE
  team
SET
  name_normalized = 'Oak Forest Park District Warriors',
  url_path_slug = NULL
WHERE
  id = 8170;

UPDATE
  tournament_team
SET
  team_id = 8170
WHERE
  id IN (8170, 8357);

DELETE FROM
  team
WHERE
  id IN (8357);

----------------------------------------
-- Oak Forest Warriors

UPDATE
  team
SET
  name_normalized = 'Oak Forest Warriors',
  url_path_slug = NULL
WHERE
  id = 301;

UPDATE
  tournament_team
SET
  team_id = 301
WHERE
  id IN (301, 401, 451, 523, 747, 976, 1092, 1203, 1393, 1496, 1623, 2028, 2302, 4409, 4577, 4854, 5011, 5185, 5346, 5493, 5639, 5780, 5945, 6124, 6306, 6490, 6674, 6856, 7035, 7223, 7419, 7607, 7795, 7985, 8545, 8736, 8932, 9123, 9300, 9475, 9650, 9817, 9986, 10163, 10343, 10514, 10685, 10868);

DELETE FROM
  team
WHERE
  id IN (401, 451, 523, 747, 976, 1092, 1203, 1393, 1496, 1623, 2028, 2302, 4409, 4577, 4854, 5011, 5185, 5346, 5493, 5639, 5780, 5945, 6124, 6306, 6490, 6674, 6856, 7035, 7223, 7419, 7607, 7795, 7985, 8545, 8736, 8932, 9123, 9300, 9475, 9650, 9817, 9986, 10163, 10343, 10514, 10685, 10868);

----------------------------------------
-- Oak Forest Warriors WC

UPDATE
  team
SET
  name_normalized = 'Oak Forest Warriors WC',
  url_path_slug = NULL
WHERE
  id = 11043;

UPDATE
  tournament_team
SET
  team_id = 11043
WHERE
  id IN (11043, 11205, 11370, 11532, 11723, 11936, 12149, 12362, 12583, 12810, 13037, 13264);

DELETE FROM
  team
WHERE
  id IN (11205, 11370, 11532, 11723, 11936, 12149, 12362, 12583, 12810, 13037, 13264);

----------------------------------------
-- Oak Lawn

UPDATE
  team
SET
  name_normalized = 'Oak Lawn',
  url_path_slug = NULL
WHERE
  id = 1889;

UPDATE
  tournament_team
SET
  team_id = 1889
WHERE
  id IN (1889, 2029);

DELETE FROM
  team
WHERE
  id IN (2029);

----------------------------------------
-- Oak Lawn P.D. Wildcats

UPDATE
  team
SET
  name_normalized = 'Oak Lawn P.D. Wildcats',
  url_path_slug = NULL
WHERE
  id = 4410;

UPDATE
  tournament_team
SET
  team_id = 4410
WHERE
  id IN (4410, 4578, 4718, 4855, 5012, 5186, 5347, 5494, 5640, 5781);

DELETE FROM
  team
WHERE
  id IN (4578, 4718, 4855, 5012, 5186, 5347, 5494, 5640, 5781);

----------------------------------------
-- Oak Lawn PD Wildcats

UPDATE
  team
SET
  name_normalized = 'Oak Lawn PD Wildcats',
  url_path_slug = NULL
WHERE
  id = 3877;

UPDATE
  tournament_team
SET
  team_id = 3877
WHERE
  id IN (3877, 3993, 4120);

DELETE FROM
  team
WHERE
  id IN (3993, 4120);

----------------------------------------
-- Oak Lawn Park District

UPDATE
  team
SET
  name_normalized = 'Oak Lawn Park District',
  url_path_slug = NULL
WHERE
  id = 1093;

UPDATE
  tournament_team
SET
  team_id = 1093
WHERE
  id IN (1093, 1204);

DELETE FROM
  team
WHERE
  id IN (1204);

----------------------------------------
-- Oak Lawn Spartan Youth WC Nfp

UPDATE
  team
SET
  name_normalized = 'Oak Lawn Spartan Youth WC Nfp',
  url_path_slug = NULL
WHERE
  id = 6675;

UPDATE
  tournament_team
SET
  team_id = 6675
WHERE
  id IN (6675, 6857, 7036, 7224);

DELETE FROM
  team
WHERE
  id IN (6857, 7036, 7224);

----------------------------------------
-- Oak Park

UPDATE
  team
SET
  name_normalized = 'Oak Park',
  url_path_slug = NULL
WHERE
  id = 643;

UPDATE
  tournament_team
SET
  team_id = 643
WHERE
  id IN (643, 749, 864);

DELETE FROM
  team
WHERE
  id IN (749, 864);

----------------------------------------
-- Oak Park-River Forest WC

UPDATE
  team
SET
  name_normalized = 'Oak Park-River Forest WC',
  url_path_slug = NULL
WHERE
  id = 1091;

UPDATE
  tournament_team
SET
  team_id = 1091
WHERE
  id IN (1091, 1205);

DELETE FROM
  team
WHERE
  id IN (1205);

----------------------------------------
-- Oakwood WC

UPDATE
  team
SET
  name_normalized = 'Oakwood WC',
  url_path_slug = NULL
WHERE
  id = 3878;

UPDATE
  tournament_team
SET
  team_id = 3878
WHERE
  id IN (3878, 3994, 4121, 4249, 4411, 4579, 4719, 4856, 5013, 5187, 5348, 5495, 5641, 5782, 5946, 6125, 6307, 6491, 6676, 6858, 7037, 7225, 7420, 7608);

DELETE FROM
  team
WHERE
  id IN (3994, 4121, 4249, 4411, 4579, 4719, 4856, 5013, 5187, 5348, 5495, 5641, 5782, 5946, 6125, 6307, 6491, 6676, 6858, 7037, 7225, 7420, 7608);

----------------------------------------
-- Oakwood Youth WC

UPDATE
  team
SET
  name_normalized = 'Oakwood Youth WC',
  url_path_slug = NULL
WHERE
  id = 7796;

UPDATE
  tournament_team
SET
  team_id = 7796
WHERE
  id IN (7796, 7986, 8171, 8358, 8546, 8737, 9651, 9818, 9987, 10164, 10686, 10869, 11724, 11937, 12150, 12363, 12584, 12811, 13038, 13265, 13515, 13785, 14055, 14325, 14595, 14865, 15135, 15405);

DELETE FROM
  team
WHERE
  id IN (7986, 8171, 8358, 8546, 8737, 9651, 9818, 9987, 10164, 10686, 10869, 11724, 11937, 12150, 12363, 12584, 12811, 13038, 13265, 13515, 13785, 14055, 14325, 14595, 14865, 15135, 15405);

----------------------------------------
-- Oakwood Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Oakwood Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8933;

UPDATE
  tournament_team
SET
  team_id = 8933
WHERE
  id IN (8933, 9124);

DELETE FROM
  team
WHERE
  id IN (9124);

----------------------------------------
-- Oilers Wrestling

UPDATE
  team
SET
  name_normalized = 'Oilers Wrestling',
  url_path_slug = NULL
WHERE
  id = 6308;

UPDATE
  tournament_team
SET
  team_id = 6308
WHERE
  id IN (6308, 6492, 6677, 6859);

DELETE FROM
  team
WHERE
  id IN (6492, 6677, 6859);

----------------------------------------
-- Olney Cubs WC

UPDATE
  team
SET
  name_normalized = 'Olney Cubs WC',
  url_path_slug = NULL
WHERE
  id = 7797;

UPDATE
  tournament_team
SET
  team_id = 7797
WHERE
  id IN (7797, 7987, 8172, 8359, 8547, 8738, 9301, 9476, 9652, 9819, 9988, 10165, 10344, 10515, 11044, 11206, 11371, 11533, 11725, 11938, 12151, 12364, 12585, 12812, 13039, 13266, 13516, 13786, 14056, 14326, 14596, 14866, 15136, 15406);

DELETE FROM
  team
WHERE
  id IN (7987, 8172, 8359, 8547, 8738, 9301, 9476, 9652, 9819, 9988, 10165, 10344, 10515, 11044, 11206, 11371, 11533, 11725, 11938, 12151, 12364, 12585, 12812, 13039, 13266, 13516, 13786, 14056, 14326, 14596, 14866, 15136, 15406);

----------------------------------------
-- Olympia WC

UPDATE
  team
SET
  name_normalized = 'Olympia WC',
  url_path_slug = NULL
WHERE
  id = 9302;

UPDATE
  tournament_team
SET
  team_id = 9302
WHERE
  id IN (9302, 9477, 10687, 10870, 11045, 11207, 12586, 12813, 13040, 13267, 13517, 13787, 14057, 14327, 14597, 14867, 15137, 15407);

DELETE FROM
  team
WHERE
  id IN (9477, 10687, 10870, 11045, 11207, 12586, 12813, 13040, 13267, 13517, 13787, 14057, 14327, 14597, 14867, 15137, 15407);

----------------------------------------
-- Oprf Little Huskie WC

UPDATE
  team
SET
  name_normalized = 'Oprf Little Huskie WC',
  url_path_slug = NULL
WHERE
  id = 4412;

UPDATE
  tournament_team
SET
  team_id = 4412
WHERE
  id IN (4412, 4580);

DELETE FROM
  team
WHERE
  id IN (4580);

----------------------------------------
-- Orangeville Broncos

UPDATE
  team
SET
  name_normalized = 'Orangeville Broncos',
  url_path_slug = NULL
WHERE
  id = 7421;

UPDATE
  tournament_team
SET
  team_id = 7421
WHERE
  id IN (7421, 7609);

DELETE FROM
  team
WHERE
  id IN (7609);

----------------------------------------
-- Orland Park Pioneers

UPDATE
  team
SET
  name_normalized = 'Orland Park Pioneers',
  url_path_slug = NULL
WHERE
  id = 302;

UPDATE
  tournament_team
SET
  team_id = 302
WHERE
  id IN (302, 644, 865, 978, 1094, 1206, 1315, 1499, 1626, 1758, 2031, 2460, 2886, 3036, 3643, 3760, 3879, 3995, 4122, 4250, 4413, 4581, 4720, 4857, 5014, 5188, 5349, 5496, 5642, 5783, 5947, 6126, 6309, 6493, 6678, 6860, 7038, 7226, 7422, 7610, 7798, 7988, 8173, 8360, 8548, 8739, 8934, 9125, 9303, 9478, 9653, 9820);

DELETE FROM
  team
WHERE
  id IN (644, 865, 978, 1094, 1206, 1315, 1499, 1626, 1758, 2031, 2460, 2886, 3036, 3643, 3760, 3879, 3995, 4122, 4250, 4413, 4581, 4720, 4857, 5014, 5188, 5349, 5496, 5642, 5783, 5947, 6126, 6309, 6493, 6678, 6860, 7038, 7226, 7422, 7610, 7798, 7988, 8173, 8360, 8548, 8739, 8934, 9125, 9303, 9478, 9653, 9820);

----------------------------------------
-- Orland Park Pioneers WC

UPDATE
  team
SET
  name_normalized = 'Orland Park Pioneers WC',
  url_path_slug = NULL
WHERE
  id = 11372;

UPDATE
  tournament_team
SET
  team_id = 11372
WHERE
  id IN (11372, 11534, 11726, 11939, 12152, 12365, 12587, 12814, 13041, 13268, 13518, 13788, 14058, 14328, 14598, 14868, 15138, 15408);

DELETE FROM
  team
WHERE
  id IN (11534, 11726, 11939, 12152, 12365, 12587, 12814, 13041, 13268, 13518, 13788, 14058, 14328, 14598, 14868, 15138, 15408);

----------------------------------------
-- Oswego Cougars

UPDATE
  team
SET
  name_normalized = 'Oswego Cougars',
  url_path_slug = NULL
WHERE
  id = 1394;

UPDATE
  tournament_team
SET
  team_id = 1394
WHERE
  id IN (1394, 1759, 1892, 3880, 3996, 4123, 4251);

DELETE FROM
  team
WHERE
  id IN (1759, 1892, 3880, 3996, 4123, 4251);

----------------------------------------
-- Oswego East Junior Wolves

UPDATE
  team
SET
  name_normalized = 'Oswego East Junior Wolves',
  url_path_slug = NULL
WHERE
  id = 5948;

UPDATE
  tournament_team
SET
  team_id = 5948
WHERE
  id IN (5948, 6127);

DELETE FROM
  team
WHERE
  id IN (6127);

----------------------------------------
-- Oswego East Team No Ego

UPDATE
  team
SET
  name_normalized = 'Oswego East Team No Ego',
  url_path_slug = NULL
WHERE
  id = 6310;

UPDATE
  tournament_team
SET
  team_id = 6310
WHERE
  id IN (6310, 6494, 6679, 6861, 7039, 7227, 7423, 7611);

DELETE FROM
  team
WHERE
  id IN (6494, 6679, 6861, 7039, 7227, 7423, 7611);

----------------------------------------
-- Oswego Panters

UPDATE
  team
SET
  name_normalized = 'Oswego Panters',
  url_path_slug = NULL
WHERE
  id = 5350;

UPDATE
  tournament_team
SET
  team_id = 5350
WHERE
  id IN (5350, 5497);

DELETE FROM
  team
WHERE
  id IN (5497);

----------------------------------------
-- Oswego Panthers

UPDATE
  team
SET
  name_normalized = 'Oswego Panthers',
  url_path_slug = NULL
WHERE
  id = 1501;

UPDATE
  tournament_team
SET
  team_id = 1501
WHERE
  id IN (1501, 1628, 3881, 3997, 4124, 4252, 4414, 4582, 4721, 4858, 5015, 5189, 5643, 5784);

DELETE FROM
  team
WHERE
  id IN (1628, 3881, 3997, 4124, 4252, 4414, 4582, 4721, 4858, 5015, 5189, 5643, 5784);

----------------------------------------
-- Oswego WC

UPDATE
  team
SET
  name_normalized = 'Oswego WC',
  url_path_slug = NULL
WHERE
  id = 10345;

UPDATE
  tournament_team
SET
  team_id = 10345
WHERE
  id IN (10345, 10516, 10688, 10871, 11046, 11208, 11373, 11535, 11727, 11940, 12153, 12366, 12588, 12815, 13042, 13269, 13519, 13789, 14059, 14329, 14599, 14869, 15139, 15409);

DELETE FROM
  team
WHERE
  id IN (10516, 10688, 10871, 11046, 11208, 11373, 11535, 11727, 11940, 12153, 12366, 12588, 12815, 13042, 13269, 13519, 13789, 14059, 14329, 14599, 14869, 15139, 15409);

----------------------------------------
-- Ottawa Crushers Kids WC

UPDATE
  team
SET
  name_normalized = 'Ottawa Crushers Kids WC',
  url_path_slug = NULL
WHERE
  id = 9304;

UPDATE
  tournament_team
SET
  team_id = 9304
WHERE
  id IN (9304, 9479);

DELETE FROM
  team
WHERE
  id IN (9479);

----------------------------------------
-- Ottawa Crushers Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Ottawa Crushers Kids Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8935;

UPDATE
  tournament_team
SET
  team_id = 8935
WHERE
  id IN (8935, 9126);

DELETE FROM
  team
WHERE
  id IN (9126);

----------------------------------------
-- Ottawa Kids WC

UPDATE
  team
SET
  name_normalized = 'Ottawa Kids WC',
  url_path_slug = NULL
WHERE
  id = 7799;

UPDATE
  tournament_team
SET
  team_id = 7799
WHERE
  id IN (7799, 7989);

DELETE FROM
  team
WHERE
  id IN (7989);

----------------------------------------
-- Ottawa Wolfpack WC

UPDATE
  team
SET
  name_normalized = 'Ottawa Wolfpack WC',
  url_path_slug = NULL
WHERE
  id = 11374;

UPDATE
  tournament_team
SET
  team_id = 11374
WHERE
  id IN (11374, 11536, 11728, 11941, 12154, 12367, 12589, 12816, 13043, 13270, 13520, 13790, 14060, 14330, 14600, 14870, 15140, 15410);

DELETE FROM
  team
WHERE
  id IN (11536, 11728, 11941, 12154, 12367, 12589, 12816, 13043, 13270, 13520, 13790, 14060, 14330, 14600, 14870, 15140, 15410);

----------------------------------------
-- Outlaws WC

UPDATE
  team
SET
  name_normalized = 'Outlaws WC',
  url_path_slug = NULL
WHERE
  id = 9989;

UPDATE
  tournament_team
SET
  team_id = 9989
WHERE
  id IN (9989, 10166);

DELETE FROM
  team
WHERE
  id IN (10166);

----------------------------------------
-- P3 Warrior Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'P3 Warrior Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 11729;

UPDATE
  tournament_team
SET
  team_id = 11729
WHERE
  id IN (11729, 11942, 12155, 12368, 12590, 12817, 13044, 13271, 13521, 13791, 14061, 14331, 14601, 14871, 15141, 15411);

DELETE FROM
  team
WHERE
  id IN (11942, 12155, 12368, 12590, 12817, 13044, 13271, 13521, 13791, 14061, 14331, 14601, 14871, 15141, 15411);

----------------------------------------
-- Palatine Cobras WC

UPDATE
  team
SET
  name_normalized = 'Palatine Cobras WC',
  url_path_slug = NULL
WHERE
  id = 9305;

UPDATE
  tournament_team
SET
  team_id = 9305
WHERE
  id IN (9305, 9480, 9654, 9821, 10689, 10872);

DELETE FROM
  team
WHERE
  id IN (9480, 9654, 9821, 10689, 10872);

----------------------------------------
-- Palatine Cobras Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Palatine Cobras Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8936;

UPDATE
  tournament_team
SET
  team_id = 8936
WHERE
  id IN (8936, 9127);

DELETE FROM
  team
WHERE
  id IN (9127);

----------------------------------------
-- Palatine Junior Pirates

UPDATE
  team
SET
  name_normalized = 'Palatine Junior Pirates',
  url_path_slug = NULL
WHERE
  id = 5016;

UPDATE
  tournament_team
SET
  team_id = 5016
WHERE
  id IN (5016, 5190, 6312, 6496, 6681, 6863);

DELETE FROM
  team
WHERE
  id IN (5190, 6312, 6496, 6681, 6863);

----------------------------------------
-- Palatine Panthers

UPDATE
  team
SET
  name_normalized = 'Palatine Panthers',
  url_path_slug = NULL
WHERE
  id = 4859;

UPDATE
  tournament_team
SET
  team_id = 4859
WHERE
  id IN (4859, 5785, 7800, 7990);

DELETE FROM
  team
WHERE
  id IN (5785, 7800, 7990);

----------------------------------------
-- Palatine Panthers WC

UPDATE
  team
SET
  name_normalized = 'Palatine Panthers WC',
  url_path_slug = NULL
WHERE
  id = 4415;

UPDATE
  tournament_team
SET
  team_id = 4415
WHERE
  id IN (4415, 4583, 5017, 5191, 5498, 5949, 6128, 6313, 6497, 6682, 6864, 7041, 7229, 8174, 8361, 8549, 8740);

DELETE FROM
  team
WHERE
  id IN (4583, 5017, 5191, 5498, 5949, 6128, 6313, 6497, 6682, 6864, 7041, 7229, 8174, 8361, 8549, 8740);

----------------------------------------
-- Palos

UPDATE
  team
SET
  name_normalized = 'Palos',
  url_path_slug = NULL
WHERE
  id = 753;

UPDATE
  tournament_team
SET
  team_id = 753
WHERE
  id IN (753, 866);

DELETE FROM
  team
WHERE
  id IN (866);

----------------------------------------
-- Panther Cub Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Panther Cub Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 3882;

UPDATE
  tournament_team
SET
  team_id = 3882
WHERE
  id IN (3882, 3998, 4125, 4253, 4416, 4584, 4722, 4860, 5018, 5192, 5351, 5499, 5787, 5950, 6129, 6314, 6498, 6683, 6865);

DELETE FROM
  team
WHERE
  id IN (3998, 4125, 4253, 4416, 4584, 4722, 4860, 5018, 5192, 5351, 5499, 5787, 5950, 6129, 6314, 6498, 6683, 6865);

----------------------------------------
-- Panther Paw WC

UPDATE
  team
SET
  name_normalized = 'Panther Paw WC',
  url_path_slug = NULL
WHERE
  id = 11731;

UPDATE
  tournament_team
SET
  team_id = 11731
WHERE
  id IN (11731, 11944, 12157, 12370, 13523, 13793, 14063, 14333, 14603, 14873, 15143, 15413);

DELETE FROM
  team
WHERE
  id IN (11944, 12157, 12370, 13523, 13793, 14063, 14333, 14603, 14873, 15143, 15413);

----------------------------------------
-- Panther Powerhouse WC

UPDATE
  team
SET
  name_normalized = 'Panther Powerhouse WC',
  url_path_slug = NULL
WHERE
  id = 5019;

UPDATE
  tournament_team
SET
  team_id = 5019
WHERE
  id IN (5019, 5193, 5352, 5500, 5644, 5788, 6315, 6499, 6684, 6866, 7042, 7230, 7424, 7612, 7801, 7991, 8175, 8362);

DELETE FROM
  team
WHERE
  id IN (5193, 5352, 5500, 5644, 5788, 6315, 6499, 6684, 6866, 7042, 7230, 7424, 7612, 7801, 7991, 8175, 8362);

----------------------------------------
-- Panther Powerhouse Wrestling

UPDATE
  team
SET
  name_normalized = 'Panther Powerhouse Wrestling',
  url_path_slug = NULL
WHERE
  id = 8550;

UPDATE
  tournament_team
SET
  team_id = 8550
WHERE
  id IN (8550, 8741, 9990, 10167, 11732, 11945, 12158, 12371, 12592, 12819, 13046, 13273, 13524, 13794, 14064, 14334, 14604, 14874, 15144, 15414);

DELETE FROM
  team
WHERE
  id IN (8741, 9990, 10167, 11732, 11945, 12158, 12371, 12592, 12819, 13046, 13273, 13524, 13794, 14064, 14334, 14604, 14874, 15144, 15414);

----------------------------------------
-- Panther Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Panther Wrestling Club',
  url_path_slug = 'burbank-panther'
WHERE
  id = 176;

UPDATE
  tournament_team
SET
  team_id = 176
WHERE
  id IN (176, 304, 407, 444, 474, 573, 686, 801, 981, 1095, 1207, 1396, 1504, 1631, 1763, 1894, 2034, 4417, 4585, 4723, 4861, 5020, 5194, 5353, 5501, 9655, 9822, 9991, 10168, 10346, 10517, 10690, 10873, 11047, 11209, 11376, 11538, 11733, 11946, 12159, 12372, 12593, 12820, 13047, 13274, 13525, 13795, 14065, 14335, 14605, 14875, 15145, 15415);

DELETE FROM
  team
WHERE
  id IN (304, 407, 444, 474, 573, 686, 801, 981, 1095, 1207, 1396, 1504, 1631, 1763, 1894, 2034, 4417, 4585, 4723, 4861, 5020, 5194, 5353, 5501, 9655, 9822, 9991, 10168, 10346, 10517, 10690, 10873, 11047, 11209, 11376, 11538, 11733, 11946, 12159, 12372, 12593, 12820, 13047, 13274, 13525, 13795, 14065, 14335, 14605, 14875, 15145, 15415);

----------------------------------------
-- Panther Youth WC

UPDATE
  team
SET
  name_normalized = 'Panther Youth WC',
  url_path_slug = NULL
WHERE
  id = 8551;

UPDATE
  tournament_team
SET
  team_id = 8551
WHERE
  id IN (8551, 8742, 12594, 12821, 13048, 13275);

DELETE FROM
  team
WHERE
  id IN (8742, 12594, 12821, 13048, 13275);

----------------------------------------
-- Patriot Grappling Systems

UPDATE
  team
SET
  name_normalized = 'Patriot Grappling Systems',
  url_path_slug = NULL
WHERE
  id = 8176;

UPDATE
  tournament_team
SET
  team_id = 8176
WHERE
  id IN (8176, 8363);

DELETE FROM
  team
WHERE
  id IN (8363);

----------------------------------------
-- Paxton Youth WC

UPDATE
  team
SET
  name_normalized = 'Paxton Youth WC',
  url_path_slug = NULL
WHERE
  id = 7043;

UPDATE
  tournament_team
SET
  team_id = 7043
WHERE
  id IN (7043, 7231, 7425, 7613, 8177, 8364, 8552, 8743);

DELETE FROM
  team
WHERE
  id IN (7231, 7425, 7613, 8177, 8364, 8552, 8743);

----------------------------------------
-- Paxton Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Paxton Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8937;

UPDATE
  tournament_team
SET
  team_id = 8937
WHERE
  id IN (8937, 9128);

DELETE FROM
  team
WHERE
  id IN (9128);

----------------------------------------
-- Peoria Heights Minutemen

UPDATE
  team
SET
  name_normalized = 'Peoria Heights Minutemen',
  url_path_slug = NULL
WHERE
  id = 5951;

UPDATE
  tournament_team
SET
  team_id = 5951
WHERE
  id IN (5951, 6130, 7044, 7232, 7427, 7615);

DELETE FROM
  team
WHERE
  id IN (6130, 7044, 7232, 7427, 7615);

----------------------------------------
-- Peoria Heights Minutemen Jr Wrestling

UPDATE
  team
SET
  name_normalized = 'Peoria Heights Minutemen Jr Wrestling',
  url_path_slug = NULL
WHERE
  id = 6686;

UPDATE
  tournament_team
SET
  team_id = 6686
WHERE
  id IN (6686, 6868);

DELETE FROM
  team
WHERE
  id IN (6868);

----------------------------------------
-- Peoria Heights Minutemen WC

UPDATE
  team
SET
  name_normalized = 'Peoria Heights Minutemen WC',
  url_path_slug = NULL
WHERE
  id = 7803;

UPDATE
  tournament_team
SET
  team_id = 7803
WHERE
  id IN (7803, 7993, 8179, 8366, 9307, 9482, 9657, 9824, 9993, 10170, 10692, 10875, 11049, 11211, 11378, 11540, 11735, 11948, 12161, 12374, 13527, 13797, 14067, 14337, 14607, 14877, 15147, 15417);

DELETE FROM
  team
WHERE
  id IN (7993, 8179, 8366, 9307, 9482, 9657, 9824, 9993, 10170, 10692, 10875, 11049, 11211, 11378, 11540, 11735, 11948, 12161, 12374, 13527, 13797, 14067, 14337, 14607, 14877, 15147, 15417);

----------------------------------------
-- Peoria Razorbacks

UPDATE
  team
SET
  name_normalized = 'Peoria Razorbacks',
  url_path_slug = NULL
WHERE
  id = 5354;

UPDATE
  tournament_team
SET
  team_id = 5354
WHERE
  id IN (5354, 7804, 7994);

DELETE FROM
  team
WHERE
  id IN (7804, 7994);

----------------------------------------
-- Peoria Razorbacks WC

UPDATE
  team
SET
  name_normalized = 'Peoria Razorbacks WC',
  url_path_slug = NULL
WHERE
  id = 8180;

UPDATE
  tournament_team
SET
  team_id = 8180
WHERE
  id IN (8180, 8367, 8554, 8745, 9308, 9483, 9658, 9825, 9994, 10171, 10348, 10519, 10693, 10876, 11050, 11212, 11379, 11541, 11736, 11949, 12162, 12375, 12596, 12823, 13050, 13277, 13528, 13798, 14068, 14338, 14608, 14878, 15148, 15418);

DELETE FROM
  team
WHERE
  id IN (8367, 8554, 8745, 9308, 9483, 9658, 9825, 9994, 10171, 10348, 10519, 10693, 10876, 11050, 11212, 11379, 11541, 11736, 11949, 12162, 12375, 12596, 12823, 13050, 13277, 13528, 13798, 14068, 14338, 14608, 14878, 15148, 15418);

----------------------------------------
-- Peoria Razorbacks Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Peoria Razorbacks Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8939;

UPDATE
  tournament_team
SET
  team_id = 8939
WHERE
  id IN (8939, 9130);

DELETE FROM
  team
WHERE
  id IN (9130);

----------------------------------------
-- Peoria Razorbacks Youth WC

UPDATE
  team
SET
  name_normalized = 'Peoria Razorbacks Youth WC',
  url_path_slug = NULL
WHERE
  id = 5645;

UPDATE
  tournament_team
SET
  team_id = 5645
WHERE
  id IN (5645, 5789, 5952, 6131, 6316, 6500, 6687, 6869, 7045, 7233, 7428, 7616);

DELETE FROM
  team
WHERE
  id IN (5789, 5952, 6131, 6316, 6500, 6687, 6869, 7045, 7233, 7428, 7616);

----------------------------------------
-- Peoria Wizards WC

UPDATE
  team
SET
  name_normalized = 'Peoria Wizards WC',
  url_path_slug = NULL
WHERE
  id = 10694;

UPDATE
  tournament_team
SET
  team_id = 10694
WHERE
  id IN (10694, 10877, 11051, 11213, 11737, 11950, 12163, 12376, 12597, 12824, 13051, 13278, 13529, 13799, 14069, 14339, 14609, 14879, 15149, 15419);

DELETE FROM
  team
WHERE
  id IN (10877, 11051, 11213, 11737, 11950, 12163, 12376, 12597, 12824, 13051, 13278, 13529, 13799, 14069, 14339, 14609, 14879, 15149, 15419);

----------------------------------------
-- Peotone Little Devils WC

UPDATE
  team
SET
  name_normalized = 'Peotone Little Devils WC',
  url_path_slug = NULL
WHERE
  id = 12598;

UPDATE
  tournament_team
SET
  team_id = 12598
WHERE
  id IN (12598, 12825, 13052, 13279, 13530, 13800, 14070, 14340, 14610, 14880, 15150, 15420);

DELETE FROM
  team
WHERE
  id IN (12825, 13052, 13279, 13530, 13800, 14070, 14340, 14610, 14880, 15150, 15420);

----------------------------------------
-- Petersburg WC

UPDATE
  team
SET
  name_normalized = 'Petersburg WC',
  url_path_slug = NULL
WHERE
  id = 11738;

UPDATE
  tournament_team
SET
  team_id = 11738
WHERE
  id IN (11738, 11951, 12164, 12377, 12599, 12826, 13053, 13280, 13531, 13801, 14071, 14341, 14611, 14881, 15151, 15421);

DELETE FROM
  team
WHERE
  id IN (11951, 12164, 12377, 12599, 12826, 13053, 13280, 13531, 13801, 14071, 14341, 14611, 14881, 15151, 15421);

----------------------------------------
-- Pinckneyville Jr

UPDATE
  team
SET
  name_normalized = 'Pinckneyville Jr',
  url_path_slug = NULL
WHERE
  id = 3999;

UPDATE
  tournament_team
SET
  team_id = 3999
WHERE
  id IN (3999, 4254);

DELETE FROM
  team
WHERE
  id IN (4254);

----------------------------------------
-- Pirates

UPDATE
  team
SET
  name_normalized = 'Pirates',
  url_path_slug = NULL
WHERE
  id = 1764;

UPDATE
  tournament_team
SET
  team_id = 1764
WHERE
  id IN (1764, 2039);

DELETE FROM
  team
WHERE
  id IN (2039);

----------------------------------------
-- Pitbull Wrestling Alliance

UPDATE
  team
SET
  name_normalized = 'Pitbull Wrestling Alliance',
  url_path_slug = NULL
WHERE
  id = 8181;

UPDATE
  tournament_team
SET
  team_id = 8181
WHERE
  id IN (8181, 8368, 8555, 8746, 8940, 9131, 9309, 9484, 9659, 9826, 9995, 10172, 10349, 10520, 10695, 10878);

DELETE FROM
  team
WHERE
  id IN (8368, 8555, 8746, 8940, 9131, 9309, 9484, 9659, 9826, 9995, 10172, 10349, 10520, 10695, 10878);

----------------------------------------
-- Plainfield

UPDATE
  team
SET
  name_normalized = 'Plainfield',
  url_path_slug = NULL
WHERE
  id = 225;

UPDATE
  tournament_team
SET
  team_id = 225
WHERE
  id IN (225, 307, 377, 452, 649, 870);

DELETE FROM
  team
WHERE
  id IN (307, 377, 452, 649, 870);

----------------------------------------
-- Plainfield Indian Trail

UPDATE
  team
SET
  name_normalized = 'Plainfield Indian Trail',
  url_path_slug = NULL
WHERE
  id = 1635;

UPDATE
  tournament_team
SET
  team_id = 1635
WHERE
  id IN (1635, 1899, 2040);

DELETE FROM
  team
WHERE
  id IN (1899, 2040);

----------------------------------------
-- Plainfield Tornadoes WC

UPDATE
  team
SET
  name_normalized = 'Plainfield Tornadoes WC',
  url_path_slug = NULL
WHERE
  id = 5021;

UPDATE
  tournament_team
SET
  team_id = 5021
WHERE
  id IN (5021, 5195, 5355, 5502);

DELETE FROM
  team
WHERE
  id IN (5195, 5355, 5502);

----------------------------------------
-- Plainfield WC

UPDATE
  team
SET
  name_normalized = 'Plainfield WC',
  url_path_slug = NULL
WHERE
  id = 3648;

UPDATE
  tournament_team
SET
  team_id = 3648
WHERE
  id IN (3648, 3884, 4000, 4127, 4255, 4418, 4586, 4724, 4862, 5022, 5196, 5356, 5503, 5646, 5790, 5953, 6132, 6317, 6501, 6688, 6870, 7046, 7234, 7429, 7617, 7805, 7995, 8182, 8369, 8556, 8747, 9996, 10173, 10350, 10521, 10696, 10879);

DELETE FROM
  team
WHERE
  id IN (3884, 4000, 4127, 4255, 4418, 4586, 4724, 4862, 5022, 5196, 5356, 5503, 5646, 5790, 5953, 6132, 6317, 6501, 6688, 6870, 7046, 7234, 7429, 7617, 7805, 7995, 8182, 8369, 8556, 8747, 9996, 10173, 10350, 10521, 10696, 10879);

----------------------------------------
-- Plainfield Wolves WC

UPDATE
  team
SET
  name_normalized = 'Plainfield Wolves WC',
  url_path_slug = NULL
WHERE
  id = 4128;

UPDATE
  tournament_team
SET
  team_id = 4128
WHERE
  id IN (4128, 4256);

DELETE FROM
  team
WHERE
  id IN (4256);

----------------------------------------
-- Plt Prophets WC

UPDATE
  team
SET
  name_normalized = 'Plt Prophets WC',
  url_path_slug = NULL
WHERE
  id = 4257;

UPDATE
  tournament_team
SET
  team_id = 4257
WHERE
  id IN (4257, 4419, 4587);

DELETE FROM
  team
WHERE
  id IN (4419, 4587);

----------------------------------------
-- Polo WC

UPDATE
  team
SET
  name_normalized = 'Polo WC',
  url_path_slug = NULL
WHERE
  id = 3885;

UPDATE
  tournament_team
SET
  team_id = 3885
WHERE
  id IN (3885, 4001, 4129, 4420, 4588, 4725, 4863, 5023, 5197, 5791, 5954, 6133, 8557, 8748, 11052, 11214, 13532, 13802, 14072, 14342, 14612, 14882, 15152, 15422);

DELETE FROM
  team
WHERE
  id IN (4001, 4129, 4420, 4588, 4725, 4863, 5023, 5197, 5791, 5954, 6133, 8557, 8748, 11052, 11214, 13532, 13802, 14072, 14342, 14612, 14882, 15152, 15422);

----------------------------------------
-- Pontiac Pythons

UPDATE
  team
SET
  name_normalized = 'Pontiac Pythons',
  url_path_slug = NULL
WHERE
  id = 3650;

UPDATE
  tournament_team
SET
  team_id = 3650
WHERE
  id IN (3650, 3886, 4002, 4130, 4258, 4421, 4589, 4864, 5024, 5198, 5357, 5504, 5647, 5792, 5955, 6134);

DELETE FROM
  team
WHERE
  id IN (3886, 4002, 4130, 4258, 4421, 4589, 4864, 5024, 5198, 5357, 5504, 5647, 5792, 5955, 6134);

----------------------------------------
-- Pontiac WC

UPDATE
  team
SET
  name_normalized = 'Pontiac WC',
  url_path_slug = NULL
WHERE
  id = 1766;

UPDATE
  tournament_team
SET
  team_id = 1766
WHERE
  id IN (1766, 6318, 6502, 6689, 6871, 7047, 7235, 7430, 7618, 7806, 7996, 8183, 8370, 8558, 8749, 9310, 9485, 9660, 9827, 9997, 10174, 10351, 10522, 10697, 10880, 11053, 11215, 11380, 11542, 12600, 12827, 13054, 13281, 13533, 13803, 14073, 14343, 14613, 14883, 15153, 15423);

DELETE FROM
  team
WHERE
  id IN (6318, 6502, 6689, 6871, 7047, 7235, 7430, 7618, 7806, 7996, 8183, 8370, 8558, 8749, 9310, 9485, 9660, 9827, 9997, 10174, 10351, 10522, 10697, 10880, 11053, 11215, 11380, 11542, 12600, 12827, 13054, 13281, 13533, 13803, 14073, 14343, 14613, 14883, 15153, 15423);

----------------------------------------
-- Pontiac Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Pontiac Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8941;

UPDATE
  tournament_team
SET
  team_id = 8941
WHERE
  id IN (8941, 9132);

DELETE FROM
  team
WHERE
  id IN (9132);

----------------------------------------
-- Pony Express WC

UPDATE
  team
SET
  name_normalized = 'Pony Express WC',
  url_path_slug = NULL
WHERE
  id = 4131;

UPDATE
  tournament_team
SET
  team_id = 4131
WHERE
  id IN (4131, 4422, 4590);

DELETE FROM
  team
WHERE
  id IN (4422, 4590);

----------------------------------------
-- Power House WC

UPDATE
  team
SET
  name_normalized = 'Power House WC',
  url_path_slug = NULL
WHERE
  id = 7807;

UPDATE
  tournament_team
SET
  team_id = 7807
WHERE
  id IN (7807, 7997, 8184, 8371);

DELETE FROM
  team
WHERE
  id IN (7997, 8184, 8371);

----------------------------------------
-- Powerhouse WC

UPDATE
  team
SET
  name_normalized = 'Powerhouse WC',
  url_path_slug = NULL
WHERE
  id = 7431;

UPDATE
  tournament_team
SET
  team_id = 7431
WHERE
  id IN (7431, 7619);

DELETE FROM
  team
WHERE
  id IN (7619);

----------------------------------------
-- Prairie Central Hawks

UPDATE
  team
SET
  name_normalized = 'Prairie Central Hawks',
  url_path_slug = NULL
WHERE
  id = 7808;

UPDATE
  tournament_team
SET
  team_id = 7808
WHERE
  id IN (7808, 7998, 8942, 9133, 9311, 9486, 9661, 9828);

DELETE FROM
  team
WHERE
  id IN (7998, 8942, 9133, 9311, 9486, 9661, 9828);

----------------------------------------
-- Prairie Central Hawks WC

UPDATE
  team
SET
  name_normalized = 'Prairie Central Hawks WC',
  url_path_slug = NULL
WHERE
  id = 8185;

UPDATE
  tournament_team
SET
  team_id = 8185
WHERE
  id IN (8185, 8372, 8559, 8750);

DELETE FROM
  team
WHERE
  id IN (8372, 8559, 8750);

----------------------------------------
-- Prairie Central Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Prairie Central Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 12601;

UPDATE
  tournament_team
SET
  team_id = 12601
WHERE
  id IN (12601, 12828, 13055, 13282, 13534, 13804, 14074, 14344, 14614, 14884, 15154, 15424);

DELETE FROM
  team
WHERE
  id IN (12828, 13055, 13282, 13534, 13804, 14074, 14344, 14614, 14884, 15154, 15424);

----------------------------------------
-- Prairie Central-Chenoa Hawks

UPDATE
  team
SET
  name_normalized = 'Prairie Central-Chenoa Hawks',
  url_path_slug = NULL
WHERE
  id = 5505;

UPDATE
  tournament_team
SET
  team_id = 5505
WHERE
  id IN (5505, 5648, 5956, 6135, 7048, 7236, 7432, 7620);

DELETE FROM
  team
WHERE
  id IN (5648, 5956, 6135, 7048, 7236, 7432, 7620);

----------------------------------------
-- Proviso Power House WC

UPDATE
  team
SET
  name_normalized = 'Proviso Power House WC',
  url_path_slug = NULL
WHERE
  id = 9998;

UPDATE
  tournament_team
SET
  team_id = 9998
WHERE
  id IN (9998, 10175);

DELETE FROM
  team
WHERE
  id IN (10175);

----------------------------------------
-- Proviso Powerhouse WC

UPDATE
  team
SET
  name_normalized = 'Proviso Powerhouse WC',
  url_path_slug = NULL
WHERE
  id = 5025;

UPDATE
  tournament_team
SET
  team_id = 5025
WHERE
  id IN (5025, 5199, 5358, 5506, 5649, 5793, 5957, 6136, 7049, 7237);

DELETE FROM
  team
WHERE
  id IN (5199, 5358, 5506, 5649, 5793, 5957, 6136, 7049, 7237);

----------------------------------------
-- Proviso Township Gladiators WC

UPDATE
  team
SET
  name_normalized = 'Proviso Township Gladiators WC',
  url_path_slug = NULL
WHERE
  id = 11054;

UPDATE
  tournament_team
SET
  team_id = 11054
WHERE
  id IN (11054, 11216, 13535, 13805, 14075, 14345, 14615, 14885, 15155, 15425);

DELETE FROM
  team
WHERE
  id IN (11216, 13535, 13805, 14075, 14345, 14615, 14885, 15155, 15425);

----------------------------------------
-- Psf Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Psf Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 11375;

UPDATE
  tournament_team
SET
  team_id = 11375
WHERE
  id IN (11375, 11537, 11730, 11943, 12156, 12369, 12591, 12818, 13045, 13272, 13522, 13792, 14062, 14332, 14602, 14872, 15142, 15412);

DELETE FROM
  team
WHERE
  id IN (11537, 11730, 11943, 12156, 12369, 12591, 12818, 13045, 13272, 13522, 13792, 14062, 14332, 14602, 14872, 15142, 15412);

----------------------------------------
-- Push Wrestling

UPDATE
  team
SET
  name_normalized = 'Push Wrestling',
  url_path_slug = NULL
WHERE
  id = 10352;

UPDATE
  tournament_team
SET
  team_id = 10352
WHERE
  id IN (10352, 10523, 10698, 10881);

DELETE FROM
  team
WHERE
  id IN (10523, 10698, 10881);

----------------------------------------
-- Quincy Cyclones WC

UPDATE
  team
SET
  name_normalized = 'Quincy Cyclones WC',
  url_path_slug = NULL
WHERE
  id = 9999;

UPDATE
  tournament_team
SET
  team_id = 9999
WHERE
  id IN (9999, 10176, 10353, 10524, 10699, 10882);

DELETE FROM
  team
WHERE
  id IN (10176, 10353, 10524, 10699, 10882);

----------------------------------------
-- Quincy Cyclones Wrestling

UPDATE
  team
SET
  name_normalized = 'Quincy Cyclones Wrestling',
  url_path_slug = NULL
WHERE
  id = 7809;

UPDATE
  tournament_team
SET
  team_id = 7809
WHERE
  id IN (7809, 7999, 8186, 8373, 8560, 8751, 8943, 9134, 9312, 9487, 9662, 9829);

DELETE FROM
  team
WHERE
  id IN (7999, 8186, 8373, 8560, 8751, 8943, 9134, 9312, 9487, 9662, 9829);

----------------------------------------
-- Quincy Little Raiders WC

UPDATE
  team
SET
  name_normalized = 'Quincy Little Raiders WC',
  url_path_slug = NULL
WHERE
  id = 11739;

UPDATE
  tournament_team
SET
  team_id = 11739
WHERE
  id IN (11739, 11952, 12165, 12378, 12602, 12829, 13056, 13283, 13536, 13806, 14076, 14346, 14616, 14886, 15156, 15426);

DELETE FROM
  team
WHERE
  id IN (11952, 12165, 12378, 12602, 12829, 13056, 13283, 13536, 13806, 14076, 14346, 14616, 14886, 15156, 15426);

----------------------------------------
-- Quincy WC

UPDATE
  team
SET
  name_normalized = 'Quincy WC',
  url_path_slug = NULL
WHERE
  id = 4865;

UPDATE
  tournament_team
SET
  team_id = 4865
WHERE
  id IN (4865, 5650, 5958, 6137, 6319, 6503, 6690, 6872, 7050, 7238);

DELETE FROM
  team
WHERE
  id IN (5650, 5958, 6137, 6319, 6503, 6690, 6872, 7050, 7238);

----------------------------------------
-- RTC Wrestling

UPDATE
  team
SET
  name_normalized = 'RTC Wrestling',
  url_path_slug = NULL
WHERE
  id = 11740;

UPDATE
  tournament_team
SET
  team_id = 11740
WHERE
  id IN (11740, 11953, 12166, 12379, 12603, 12830, 13057, 13284);

DELETE FROM
  team
WHERE
  id IN (11953, 12166, 12379, 12603, 12830, 13057, 13284);

----------------------------------------
-- Raider WC

UPDATE
  team
SET
  name_normalized = 'Raider WC',
  url_path_slug = NULL
WHERE
  id = 4423;

UPDATE
  tournament_team
SET
  team_id = 4423
WHERE
  id IN (4423, 4591, 4866);

DELETE FROM
  team
WHERE
  id IN (4591, 4866);

----------------------------------------
-- Rambler / Next Level WC

UPDATE
  team
SET
  name_normalized = 'Rambler / Next Level WC',
  url_path_slug = NULL
WHERE
  id = 7433;

UPDATE
  tournament_team
SET
  team_id = 7433
WHERE
  id IN (7433, 7621);

DELETE FROM
  team
WHERE
  id IN (7621);

----------------------------------------
-- Rambler WC

UPDATE
  team
SET
  name_normalized = 'Rambler WC',
  url_path_slug = NULL
WHERE
  id = 9663;

UPDATE
  tournament_team
SET
  team_id = 9663
WHERE
  id IN (9663, 9830, 10000, 10177);

DELETE FROM
  team
WHERE
  id IN (9830, 10000, 10177);

----------------------------------------
-- Rams WC

UPDATE
  team
SET
  name_normalized = 'Rams WC',
  url_path_slug = NULL
WHERE
  id = 1210;

UPDATE
  tournament_team
SET
  team_id = 1210
WHERE
  id IN (1210, 1904, 3887, 4003, 4132, 4259, 4424, 4592, 4867, 5026, 5200, 5507, 7810, 8000, 10354, 10525, 10701, 10884);

DELETE FROM
  team
WHERE
  id IN (1904, 3887, 4003, 4132, 4259, 4424, 4592, 4867, 5026, 5200, 5507, 7810, 8000, 10354, 10525, 10701, 10884);

----------------------------------------
-- Rams Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rams Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 7051;

UPDATE
  tournament_team
SET
  team_id = 7051
WHERE
  id IN (7051, 7239, 7434, 7622);

DELETE FROM
  team
WHERE
  id IN (7239, 7434, 7622);

----------------------------------------
-- Rantoul Junior Eagles

UPDATE
  team
SET
  name_normalized = 'Rantoul Junior Eagles',
  url_path_slug = NULL
WHERE
  id = 7052;

UPDATE
  tournament_team
SET
  team_id = 7052
WHERE
  id IN (7052, 7240, 7435, 7623, 8561, 8752, 8945, 9136, 10001, 10178);

DELETE FROM
  team
WHERE
  id IN (7240, 7435, 7623, 8561, 8752, 8945, 9136, 10001, 10178);

----------------------------------------
-- Rantoul Junior Wrestling

UPDATE
  team
SET
  name_normalized = 'Rantoul Junior Wrestling',
  url_path_slug = NULL
WHERE
  id = 7811;

UPDATE
  tournament_team
SET
  team_id = 7811
WHERE
  id IN (7811, 8001, 8187, 8374, 10355, 10526);

DELETE FROM
  team
WHERE
  id IN (8001, 8187, 8374, 10355, 10526);

----------------------------------------
-- Rantoul Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Rantoul Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 11056;

UPDATE
  tournament_team
SET
  team_id = 11056
WHERE
  id IN (11056, 11218, 11381, 11543, 11742, 11955, 12168, 12381, 12604, 12831, 13058, 13285, 13538, 13808, 14078, 14348, 14618, 14888, 15158, 15428);

DELETE FROM
  team
WHERE
  id IN (11218, 11381, 11543, 11742, 11955, 12168, 12381, 12604, 12831, 13058, 13285, 13538, 13808, 14078, 14348, 14618, 14888, 15158, 15428);

----------------------------------------
-- Rc Jr. Panthers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rc Jr. Panthers Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8944;

UPDATE
  tournament_team
SET
  team_id = 8944
WHERE
  id IN (8944, 9135);

DELETE FROM
  team
WHERE
  id IN (9135);

----------------------------------------
-- Rebels WC

UPDATE
  team
SET
  name_normalized = 'Rebels WC',
  url_path_slug = NULL
WHERE
  id = 6691;

UPDATE
  tournament_team
SET
  team_id = 6691
WHERE
  id IN (6691, 6873, 7053, 7241, 7436, 7624);

DELETE FROM
  team
WHERE
  id IN (6873, 7053, 7241, 7436, 7624);

----------------------------------------
-- Red Raiders

UPDATE
  team
SET
  name_normalized = 'Red Raiders',
  url_path_slug = NULL
WHERE
  id = 7054;

UPDATE
  tournament_team
SET
  team_id = 7054
WHERE
  id IN (7054, 7242);

DELETE FROM
  team
WHERE
  id IN (7242);

----------------------------------------
-- Red Raiders Wrestling Team

UPDATE
  team
SET
  name_normalized = 'Red Raiders Wrestling Team',
  url_path_slug = NULL
WHERE
  id = 8562;

UPDATE
  tournament_team
SET
  team_id = 8562
WHERE
  id IN (8562, 8753, 8946, 9137, 9313, 9488, 9664, 9831, 10002, 10179, 10356, 10527, 10702, 10885, 11057, 11219, 11382, 11544, 11743, 11956, 12169, 12382, 12605, 12832, 13059, 13286, 13539, 13809, 14079, 14349, 14619, 14889, 15159, 15429);

DELETE FROM
  team
WHERE
  id IN (8753, 8946, 9137, 9313, 9488, 9664, 9831, 10002, 10179, 10356, 10527, 10702, 10885, 11057, 11219, 11382, 11544, 11743, 11956, 12169, 12382, 12605, 12832, 13059, 13286, 13539, 13809, 14079, 14349, 14619, 14889, 15159, 15429);

----------------------------------------
-- Redbird WC

UPDATE
  team
SET
  name_normalized = 'Redbird WC',
  url_path_slug = NULL
WHERE
  id = 985;

UPDATE
  tournament_team
SET
  team_id = 985
WHERE
  id IN (985, 1100, 1509);

DELETE FROM
  team
WHERE
  id IN (1100, 1509);

----------------------------------------
-- Redhawk WC

UPDATE
  team
SET
  name_normalized = 'Redhawk WC',
  url_path_slug = NULL
WHERE
  id = 5959;

UPDATE
  tournament_team
SET
  team_id = 5959
WHERE
  id IN (5959, 6138, 6320, 6504, 6692, 6874, 7055, 7243, 7437, 7625, 7812, 8002, 8188, 8375, 8563, 8754, 9314, 9489);

DELETE FROM
  team
WHERE
  id IN (6138, 6320, 6504, 6692, 6874, 7055, 7243, 7437, 7625, 7812, 8002, 8188, 8375, 8563, 8754, 9314, 9489);

----------------------------------------
-- Redhawk Wc/Panther WC

UPDATE
  team
SET
  name_normalized = 'Redhawk Wc/Panther WC',
  url_path_slug = NULL
WHERE
  id = 9665;

UPDATE
  tournament_team
SET
  team_id = 9665
WHERE
  id IN (9665, 9832);

DELETE FROM
  team
WHERE
  id IN (9832);

----------------------------------------
-- Redhawk Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Redhawk Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8947;

UPDATE
  tournament_team
SET
  team_id = 8947
WHERE
  id IN (8947, 9138);

DELETE FROM
  team
WHERE
  id IN (9138);

----------------------------------------
-- Reed Custer Jr. Panthers WC

UPDATE
  team
SET
  name_normalized = 'Reed Custer Jr. Panthers WC',
  url_path_slug = NULL
WHERE
  id = 10703;

UPDATE
  tournament_team
SET
  team_id = 10703
WHERE
  id IN (10703, 10886, 12606, 12833, 13060, 13287, 13540, 13810, 14080, 14350, 14620, 14890, 15160, 15430);

DELETE FROM
  team
WHERE
  id IN (10886, 12606, 12833, 13060, 13287, 13540, 13810, 14080, 14350, 14620, 14890, 15160, 15430);

----------------------------------------
-- Reed Custer Knights

UPDATE
  team
SET
  name_normalized = 'Reed Custer Knights',
  url_path_slug = NULL
WHERE
  id = 3888;

UPDATE
  tournament_team
SET
  team_id = 3888
WHERE
  id IN (3888, 5027, 5201, 5794);

DELETE FROM
  team
WHERE
  id IN (5027, 5201, 5794);

----------------------------------------
-- Reed Custer Knights WC

UPDATE
  team
SET
  name_normalized = 'Reed Custer Knights WC',
  url_path_slug = NULL
WHERE
  id = 8189;

UPDATE
  tournament_team
SET
  team_id = 8189
WHERE
  id IN (8189, 8376);

DELETE FROM
  team
WHERE
  id IN (8376);

----------------------------------------
-- Renegade WC (Nfp)

UPDATE
  team
SET
  name_normalized = 'Renegade WC (Nfp)',
  url_path_slug = NULL
WHERE
  id = 5960;

UPDATE
  tournament_team
SET
  team_id = 5960
WHERE
  id IN (5960, 6139, 6321, 6505, 6693, 6875, 7056, 7244, 7438, 7626, 7813, 8003, 8190, 8377, 8564, 8755);

DELETE FROM
  team
WHERE
  id IN (6139, 6321, 6505, 6693, 6875, 7056, 7244, 7438, 7626, 7813, 8003, 8190, 8377, 8564, 8755);

----------------------------------------
-- Renegade Wrestling Club (Nfp)

UPDATE
  team
SET
  name_normalized = 'Renegade Wrestling Club (Nfp)',
  url_path_slug = NULL
WHERE
  id = 8948;

UPDATE
  tournament_team
SET
  team_id = 8948
WHERE
  id IN (8948, 9139);

DELETE FROM
  team
WHERE
  id IN (9139);

----------------------------------------
-- Renegades (Stockton)

UPDATE
  team
SET
  name_normalized = 'Renegades (Stockton)',
  url_path_slug = NULL
WHERE
  id = 5961;

UPDATE
  tournament_team
SET
  team_id = 5961
WHERE
  id IN (5961, 6140, 6322, 6506, 6694, 6876, 7057, 7245, 7814, 8004);

DELETE FROM
  team
WHERE
  id IN (6140, 6322, 6506, 6694, 6876, 7057, 7245, 7814, 8004);

----------------------------------------
-- Renegades Elite

UPDATE
  team
SET
  name_normalized = 'Renegades Elite',
  url_path_slug = NULL
WHERE
  id = 7815;

UPDATE
  tournament_team
SET
  team_id = 7815
WHERE
  id IN (7815, 8005);

DELETE FROM
  team
WHERE
  id IN (8005);

----------------------------------------
-- Respect Wrestling

UPDATE
  team
SET
  name_normalized = 'Respect Wrestling',
  url_path_slug = NULL
WHERE
  id = 10003;

UPDATE
  tournament_team
SET
  team_id = 10003
WHERE
  id IN (10003, 10180, 10357, 10528, 10704, 10887);

DELETE FROM
  team
WHERE
  id IN (10180, 10357, 10528, 10704, 10887);

----------------------------------------
-- Rich Rattlers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rich Rattlers Wrestling Club',
  url_path_slug = 'rich-rattlers'
WHERE
  id = 4425;

UPDATE
  tournament_team
SET
  team_id = 4425
WHERE
  id IN (4425, 4593, 4868, 5028, 5202, 5359, 5651, 5796, 5962, 6141, 6695, 6877, 7058, 7246);

DELETE FROM
  team
WHERE
  id IN (4593, 4868, 5028, 5202, 5359, 5651, 5796, 5962, 6141, 6695, 6877, 7058, 7246);

----------------------------------------
-- Rich Township

UPDATE
  team
SET
  name_normalized = 'Rich Township',
  url_path_slug = NULL
WHERE
  id = 453;

UPDATE
  tournament_team
SET
  team_id = 453
WHERE
  id IN (453, 653, 874);

DELETE FROM
  team
WHERE
  id IN (653, 874);

----------------------------------------
-- Rich Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rich Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 986;

UPDATE
  tournament_team
SET
  team_id = 986
WHERE
  id IN (986, 1101, 1212, 1398, 1510, 1638, 1769, 2046);

DELETE FROM
  team
WHERE
  id IN (1101, 1212, 1398, 1510, 1638, 1769, 2046);

----------------------------------------
-- Richmond Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Richmond Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 4260;

UPDATE
  tournament_team
SET
  team_id = 4260
WHERE
  id IN (4260, 5029, 5203, 5360, 5508, 5652, 5797, 5963, 6142, 6323, 6507, 6696, 6878, 7059, 7247, 7439, 7627, 7816, 8006, 8191, 8378, 9315, 9490, 9666, 9833, 10004, 10181, 10705, 10888, 11058, 11220, 11383, 11545, 11744, 11957, 12170, 12383);

DELETE FROM
  team
WHERE
  id IN (5029, 5203, 5360, 5508, 5652, 5797, 5963, 6142, 6323, 6507, 6696, 6878, 7059, 7247, 7439, 7627, 7816, 8006, 8191, 8378, 9315, 9490, 9666, 9833, 10004, 10181, 10705, 10888, 11058, 11220, 11383, 11545, 11744, 11957, 12170, 12383);

----------------------------------------
-- Riot Room Wrestling

UPDATE
  team
SET
  name_normalized = 'Riot Room Wrestling',
  url_path_slug = NULL
WHERE
  id = 11384;

UPDATE
  tournament_team
SET
  team_id = 11384
WHERE
  id IN (11384, 11546, 11745, 11958, 12171, 12384, 12607, 12834, 13061, 13288, 13541, 13811, 14081, 14351, 14621, 14891, 15161, 15431);

DELETE FROM
  team
WHERE
  id IN (11546, 11745, 11958, 12171, 12384, 12607, 12834, 13061, 13288, 13541, 13811, 14081, 14351, 14621, 14891, 15161, 15431);

----------------------------------------
-- River Bend WC

UPDATE
  team
SET
  name_normalized = 'River Bend WC',
  url_path_slug = NULL
WHERE
  id = 8565;

UPDATE
  tournament_team
SET
  team_id = 8565
WHERE
  id IN (8565, 8756, 11059, 11221, 11385, 11547, 11746, 11959, 12172, 12385, 12608, 12835, 13062, 13289, 13542, 13812, 14082, 14352, 14622, 14892, 15162, 15432);

DELETE FROM
  team
WHERE
  id IN (8756, 11059, 11221, 11385, 11547, 11746, 11959, 12172, 12385, 12608, 12835, 13062, 13289, 13542, 13812, 14082, 14352, 14622, 14892, 15162, 15432);

----------------------------------------
-- River Bend Wrestling Club

UPDATE
  team
SET
  name_normalized = 'River Bend Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8949;

UPDATE
  tournament_team
SET
  team_id = 8949
WHERE
  id IN (8949, 9140);

DELETE FROM
  team
WHERE
  id IN (9140);

----------------------------------------
-- Riverbend WC

UPDATE
  team
SET
  name_normalized = 'Riverbend WC',
  url_path_slug = NULL
WHERE
  id = 3054;

UPDATE
  tournament_team
SET
  team_id = 3054
WHERE
  id IN (3054, 3889, 4004, 4133, 4261, 4426, 4594, 4726, 4869, 5030, 5204, 5361, 5509, 5653, 5798, 6697, 6879, 7060, 7248, 7817, 8007, 8192, 8379);

DELETE FROM
  team
WHERE
  id IN (3889, 4004, 4133, 4261, 4426, 4594, 4726, 4869, 5030, 5204, 5361, 5509, 5653, 5798, 6697, 6879, 7060, 7248, 7817, 8007, 8192, 8379);

----------------------------------------
-- Riverdale Elite

UPDATE
  team
SET
  name_normalized = 'Riverdale Elite',
  url_path_slug = NULL
WHERE
  id = 10005;

UPDATE
  tournament_team
SET
  team_id = 10005
WHERE
  id IN (10005, 10182);

DELETE FROM
  team
WHERE
  id IN (10182);

----------------------------------------
-- Riverdale Junior Rams Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Riverdale Junior Rams Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1400;

UPDATE
  tournament_team
SET
  team_id = 1400
WHERE
  id IN (1400, 1639, 1911, 2470, 2608, 3890, 4005, 4262, 4727, 4870, 5031, 5205, 5362, 5510, 5654, 5964, 6143, 6324, 6508, 6698, 6880, 7061, 7249, 13543, 13813, 14083, 14353, 14623, 14893, 15163, 15433);

DELETE FROM
  team
WHERE
  id IN (1639, 1911, 2470, 2608, 3890, 4005, 4262, 4727, 4870, 5031, 5205, 5362, 5510, 5654, 5964, 6143, 6324, 6508, 6698, 6880, 7061, 7249, 13543, 13813, 14083, 14353, 14623, 14893, 15163, 15433);

----------------------------------------
-- Riverton Hawks Wrestling

UPDATE
  team
SET
  name_normalized = 'Riverton Hawks Wrestling',
  url_path_slug = NULL
WHERE
  id = 7062;

UPDATE
  tournament_team
SET
  team_id = 7062
WHERE
  id IN (7062, 7250, 7440, 7628);

DELETE FROM
  team
WHERE
  id IN (7250, 7440, 7628);

----------------------------------------
-- Riverton Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Riverton Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 7818;

UPDATE
  tournament_team
SET
  team_id = 7818
WHERE
  id IN (7818, 8008, 8193, 8380, 8566, 8757, 8950, 9141, 9316, 9491);

DELETE FROM
  team
WHERE
  id IN (8008, 8193, 8380, 8566, 8757, 8950, 9141, 9316, 9491);

----------------------------------------
-- Riverton-Williamsville Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Riverton-Williamsville Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 10706;

UPDATE
  tournament_team
SET
  team_id = 10706
WHERE
  id IN (10706, 10889);

DELETE FROM
  team
WHERE
  id IN (10889);

----------------------------------------
-- Riverton/Williamsville Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Riverton/Williamsville Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 9667;

UPDATE
  tournament_team
SET
  team_id = 9667
WHERE
  id IN (9667, 9834, 10006, 10183, 10358, 10529);

DELETE FROM
  team
WHERE
  id IN (9834, 10006, 10183, 10358, 10529);

----------------------------------------
-- Road Warriors

UPDATE
  team
SET
  name_normalized = 'Road Warriors',
  url_path_slug = NULL
WHERE
  id = 5032;

UPDATE
  tournament_team
SET
  team_id = 5032
WHERE
  id IN (5032, 5206, 5511, 5655, 5965, 6144, 6325, 6509);

DELETE FROM
  team
WHERE
  id IN (5206, 5511, 5655, 5965, 6144, 6325, 6509);

----------------------------------------
-- Road Warriors WC

UPDATE
  team
SET
  name_normalized = 'Road Warriors WC',
  url_path_slug = NULL
WHERE
  id = 8567;

UPDATE
  tournament_team
SET
  team_id = 8567
WHERE
  id IN (8567, 8758, 10007, 10184, 10707, 10890, 11386, 11548);

DELETE FROM
  team
WHERE
  id IN (8758, 10007, 10184, 10707, 10890, 11386, 11548);

----------------------------------------
-- Robbins Park District Jayhawks

UPDATE
  team
SET
  name_normalized = 'Robbins Park District Jayhawks',
  url_path_slug = NULL
WHERE
  id = 10708;

UPDATE
  tournament_team
SET
  team_id = 10708
WHERE
  id IN (10708, 10891);

DELETE FROM
  team
WHERE
  id IN (10891);

----------------------------------------
-- Rochelle Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rochelle Wrestling Club',
  url_path_slug = 'rochelle'
WHERE
  id = 875;

UPDATE
  tournament_team
SET
  team_id = 875
WHERE
  id IN (875, 3891, 4006, 4263, 4871, 5033, 5207, 5363, 7063, 7251, 7819, 8009, 8194, 8381, 8568, 8759, 8951, 9142, 9668, 9835, 10008, 10185, 10359, 10530, 10709, 10892, 11060, 11222, 11387, 11549, 11747, 11960, 12173, 12386, 12609, 12836, 13063, 13290, 13544, 13814, 14084, 14354, 14624, 14894, 15164, 15434);

DELETE FROM
  team
WHERE
  id IN (3891, 4006, 4263, 4871, 5033, 5207, 5363, 7063, 7251, 7819, 8009, 8194, 8381, 8568, 8759, 8951, 9142, 9668, 9835, 10008, 10185, 10359, 10530, 10709, 10892, 11060, 11222, 11387, 11549, 11747, 11960, 12173, 12386, 12609, 12836, 13063, 13290, 13544, 13814, 14084, 14354, 14624, 14894, 15164, 15434);

----------------------------------------
-- Rochester WC

UPDATE
  team
SET
  name_normalized = 'Rochester WC',
  url_path_slug = NULL
WHERE
  id = 5364;

UPDATE
  tournament_team
SET
  team_id = 5364
WHERE
  id IN (5364, 5656, 5799, 5966, 6145, 6326, 6510, 6699, 6881, 7064, 7252, 8195, 8382, 8569, 8760, 9669, 9836, 10360, 10531);

DELETE FROM
  team
WHERE
  id IN (5656, 5799, 5966, 6145, 6326, 6510, 6699, 6881, 7064, 7252, 8195, 8382, 8569, 8760, 9669, 9836, 10360, 10531);

----------------------------------------
-- Rock Falls Junior Rocket Wrestling

UPDATE
  team
SET
  name_normalized = 'Rock Falls Junior Rocket Wrestling',
  url_path_slug = NULL
WHERE
  id = 3892;

UPDATE
  tournament_team
SET
  team_id = 3892
WHERE
  id IN (3892, 10710, 10893);

DELETE FROM
  team
WHERE
  id IN (10710, 10893);

----------------------------------------
-- Rock Falls Rippin

UPDATE
  team
SET
  name_normalized = 'Rock Falls Rippin',
  url_path_slug = NULL
WHERE
  id = 4134;

UPDATE
  tournament_team
SET
  team_id = 4134
WHERE
  id IN (4134, 4264);

DELETE FROM
  team
WHERE
  id IN (4264);

----------------------------------------
-- Rock Island

UPDATE
  team
SET
  name_normalized = 'Rock Island',
  url_path_slug = NULL
WHERE
  id = 876;

UPDATE
  tournament_team
SET
  team_id = 876
WHERE
  id IN (876, 1640);

DELETE FROM
  team
WHERE
  id IN (1640);

----------------------------------------
-- Rock Island Jr. Rocks

UPDATE
  team
SET
  name_normalized = 'Rock Island Jr. Rocks',
  url_path_slug = NULL
WHERE
  id = 4427;

UPDATE
  tournament_team
SET
  team_id = 4427
WHERE
  id IN (4427, 4595);

DELETE FROM
  team
WHERE
  id IN (4595);

----------------------------------------
-- Rock Island WC

UPDATE
  team
SET
  name_normalized = 'Rock Island WC',
  url_path_slug = NULL
WHERE
  id = 4872;

UPDATE
  tournament_team
SET
  team_id = 4872
WHERE
  id IN (4872, 5034, 5208, 5512, 5657, 5800, 5967, 6146, 6327, 6511, 6700, 6882, 7065, 7253, 7441, 7629, 7820, 8010, 8196, 8383, 8570, 8761, 9317, 9492, 9670, 9837, 11388, 11550, 11748, 11961, 12174, 12387, 13545, 13815, 14085, 14355, 14625, 14895, 15165, 15435);

DELETE FROM
  team
WHERE
  id IN (5034, 5208, 5512, 5657, 5800, 5967, 6146, 6327, 6511, 6700, 6882, 7065, 7253, 7441, 7629, 7820, 8010, 8196, 8383, 8570, 8761, 9317, 9492, 9670, 9837, 11388, 11550, 11748, 11961, 12174, 12387, 13545, 13815, 14085, 14355, 14625, 14895, 15165, 15435);

----------------------------------------
-- Rock Island Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rock Island Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8952;

UPDATE
  tournament_team
SET
  team_id = 8952
WHERE
  id IN (8952, 9143);

DELETE FROM
  team
WHERE
  id IN (9143);

----------------------------------------
-- Rocket Fuel WC

UPDATE
  team
SET
  name_normalized = 'Rocket Fuel WC',
  url_path_slug = NULL
WHERE
  id = 7066;

UPDATE
  tournament_team
SET
  team_id = 7066
WHERE
  id IN (7066, 7254, 7821, 8011, 8571, 8762, 12610, 12837, 13064, 13291, 13546, 13816, 14086, 14356, 14626, 14896, 15166, 15436);

DELETE FROM
  team
WHERE
  id IN (7254, 7821, 8011, 8571, 8762, 12610, 12837, 13064, 13291, 13546, 13816, 14086, 14356, 14626, 14896, 15166, 15436);

----------------------------------------
-- Rocket Fuel Wc-Rock Ridge

UPDATE
  team
SET
  name_normalized = 'Rocket Fuel Wc-Rock Ridge',
  url_path_slug = NULL
WHERE
  id = 5968;

UPDATE
  tournament_team
SET
  team_id = 5968
WHERE
  id IN (5968, 6147, 6328, 6512);

DELETE FROM
  team
WHERE
  id IN (6147, 6328, 6512);

----------------------------------------
-- Rocket Fuel Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rocket Fuel Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8953;

UPDATE
  tournament_team
SET
  team_id = 8953
WHERE
  id IN (8953, 9144);

DELETE FROM
  team
WHERE
  id IN (9144);

----------------------------------------
-- Rockets WC

UPDATE
  team
SET
  name_normalized = 'Rockets WC',
  url_path_slug = NULL
WHERE
  id = 10711;

UPDATE
  tournament_team
SET
  team_id = 10711
WHERE
  id IN (10711, 10894, 11749, 11962, 12175, 12388, 12611, 12838, 13065, 13292, 13547, 13817, 14087, 14357, 14627, 14897, 15167, 15437);

DELETE FROM
  team
WHERE
  id IN (10894, 11749, 11962, 12175, 12388, 12611, 12838, 13065, 13292, 13547, 13817, 14087, 14357, 14627, 14897, 15167, 15437);

----------------------------------------
-- Rockets Youth WC

UPDATE
  team
SET
  name_normalized = 'Rockets Youth WC',
  url_path_slug = NULL
WHERE
  id = 8572;

UPDATE
  tournament_team
SET
  team_id = 8572
WHERE
  id IN (8572, 8763, 9318, 9493);

DELETE FROM
  team
WHERE
  id IN (8763, 9318, 9493);

----------------------------------------
-- Rockford Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Rockford Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 10;

UPDATE
  tournament_team
SET
  team_id = 10
WHERE
  id IN (10, 1774, 1913, 2050, 2176, 2472, 2610, 3773, 3893, 4007, 4135, 4265, 4428, 4596, 4729, 5036, 5210, 5514, 5802, 5969, 6148, 6329, 6513, 7067, 7255, 7442, 7630, 7822, 8012, 8197, 8384, 8573, 8764, 8954, 9145, 9319, 9494, 11061, 11223, 12612, 12839, 13066, 13293, 13548, 13818, 14088, 14358, 14628, 14898, 15168, 15438);

DELETE FROM
  team
WHERE
  id IN (1774, 1913, 2050, 2176, 2472, 2610, 3773, 3893, 4007, 4135, 4265, 4428, 4596, 4729, 5036, 5210, 5514, 5802, 5969, 6148, 6329, 6513, 7067, 7255, 7442, 7630, 7822, 8012, 8197, 8384, 8573, 8764, 8954, 9145, 9319, 9494, 11061, 11223, 12612, 12839, 13066, 13293, 13548, 13818, 14088, 14358, 14628, 14898, 15168, 15438);

----------------------------------------
-- Rockridge Jr. High

UPDATE
  team
SET
  name_normalized = 'Rockridge Jr. High',
  url_path_slug = NULL
WHERE
  id = 989;

UPDATE
  tournament_team
SET
  team_id = 989
WHERE
  id IN (989, 1103, 1213);

DELETE FROM
  team
WHERE
  id IN (1103, 1213);

----------------------------------------
-- Rockridge Rocket Fuel WC

UPDATE
  team
SET
  name_normalized = 'Rockridge Rocket Fuel WC',
  url_path_slug = NULL
WHERE
  id = 7443;

UPDATE
  tournament_team
SET
  team_id = 7443
WHERE
  id IN (7443, 7631);

DELETE FROM
  team
WHERE
  id IN (7631);

----------------------------------------
-- Rockridge WC

UPDATE
  team
SET
  name_normalized = 'Rockridge WC',
  url_path_slug = NULL
WHERE
  id = 5035;

UPDATE
  tournament_team
SET
  team_id = 5035
WHERE
  id IN (5035, 5209, 5513, 5801);

DELETE FROM
  team
WHERE
  id IN (5209, 5513, 5801);

----------------------------------------
-- Rogue Wrestling

UPDATE
  team
SET
  name_normalized = 'Rogue Wrestling',
  url_path_slug = NULL
WHERE
  id = 9671;

UPDATE
  tournament_team
SET
  team_id = 9671
WHERE
  id IN (9671, 9838, 10009, 10186);

DELETE FROM
  team
WHERE
  id IN (9838, 10009, 10186);

----------------------------------------
-- Romeoville WC

UPDATE
  team
SET
  name_normalized = 'Romeoville WC',
  url_path_slug = NULL
WHERE
  id = 5658;

UPDATE
  tournament_team
SET
  team_id = 5658
WHERE
  id IN (5658, 5803, 5970, 6149);

DELETE FROM
  team
WHERE
  id IN (5803, 5970, 6149);

----------------------------------------
-- Romeoville Westview

UPDATE
  team
SET
  name_normalized = 'Romeoville Westview',
  url_path_slug = NULL
WHERE
  id = 67;

UPDATE
  tournament_team
SET
  team_id = 67
WHERE
  id IN (67, 76);

DELETE FROM
  team
WHERE
  id IN (76);

----------------------------------------
-- Romeoville Youth WC

UPDATE
  team
SET
  name_normalized = 'Romeoville Youth WC',
  url_path_slug = NULL
WHERE
  id = 6330;

UPDATE
  tournament_team
SET
  team_id = 6330
WHERE
  id IN (6330, 6514, 6701, 6883);

DELETE FROM
  team
WHERE
  id IN (6514, 6701, 6883);

----------------------------------------
-- Rosemont Cobras

UPDATE
  team
SET
  name_normalized = 'Rosemont Cobras',
  url_path_slug = NULL
WHERE
  id = 539;

UPDATE
  tournament_team
SET
  team_id = 539
WHERE
  id IN (539, 879, 990, 1214, 1401, 1775, 1914, 2051, 2177, 2473, 2611);

DELETE FROM
  team
WHERE
  id IN (879, 990, 1214, 1401, 1775, 1914, 2051, 2177, 2473, 2611);

----------------------------------------
-- Roughnecks WC

UPDATE
  team
SET
  name_normalized = 'Roughnecks WC',
  url_path_slug = NULL
WHERE
  id = 10010;

UPDATE
  tournament_team
SET
  team_id = 10010
WHERE
  id IN (10010, 10187, 10361, 10532, 10712, 10895, 11750, 11963, 12176, 12389, 13549, 13819, 14089, 14359, 14629, 14899, 15169, 15439);

DELETE FROM
  team
WHERE
  id IN (10187, 10361, 10532, 10712, 10895, 11750, 11963, 12176, 12389, 13549, 13819, 14089, 14359, 14629, 14899, 15169, 15439);

----------------------------------------
-- Round Lake Area Park District

UPDATE
  team
SET
  name_normalized = 'Round Lake Area Park District',
  url_path_slug = NULL
WHERE
  id = 991;

UPDATE
  tournament_team
SET
  team_id = 991
WHERE
  id IN (991, 1105, 1402);

DELETE FROM
  team
WHERE
  id IN (1105, 1402);

----------------------------------------
-- Round Lake Bad Boyz

UPDATE
  team
SET
  name_normalized = 'Round Lake Bad Boyz',
  url_path_slug = NULL
WHERE
  id = 4429;

UPDATE
  tournament_team
SET
  team_id = 4429
WHERE
  id IN (4429, 4597);

DELETE FROM
  team
WHERE
  id IN (4597);

----------------------------------------
-- Round Lake Jr. Panthers WC

UPDATE
  team
SET
  name_normalized = 'Round Lake Jr. Panthers WC',
  url_path_slug = NULL
WHERE
  id = 11751;

UPDATE
  tournament_team
SET
  team_id = 11751
WHERE
  id IN (11751, 11964, 12177, 12390, 12613, 12840, 13067, 13294, 13550, 13820, 14090, 14360, 14630, 14900, 15170, 15440);

DELETE FROM
  team
WHERE
  id IN (11964, 12177, 12390, 12613, 12840, 13067, 13294, 13550, 13820, 14090, 14360, 14630, 14900, 15170, 15440);

----------------------------------------
-- Round Lake Spartans

UPDATE
  team
SET
  name_normalized = 'Round Lake Spartans',
  url_path_slug = NULL
WHERE
  id = 880;

UPDATE
  tournament_team
SET
  team_id = 880
WHERE
  id IN (880, 1513, 1643, 2052, 2327);

DELETE FROM
  team
WHERE
  id IN (1513, 1643, 2052, 2327);

----------------------------------------
-- Round Lake Titans

UPDATE
  team
SET
  name_normalized = 'Round Lake Titans',
  url_path_slug = NULL
WHERE
  id = 7823;

UPDATE
  tournament_team
SET
  team_id = 7823
WHERE
  id IN (7823, 8013, 8198, 8385);

DELETE FROM
  team
WHERE
  id IN (8013, 8198, 8385);

----------------------------------------
-- Roxana Kids Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Roxana Kids Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 3655;

UPDATE
  tournament_team
SET
  team_id = 3655
WHERE
  id IN (3655, 3775, 3894, 4008, 4136, 4266, 4430, 4598, 4730, 4873, 5037, 5211, 5365, 5515, 5971, 6150, 6331, 6515, 6702, 6884, 7068, 7256, 7444, 7632, 7824, 8014, 8199, 8386, 10011, 10188, 11752, 11965, 12178, 12391, 12614, 12841, 13068, 13295, 13551, 13821, 14091, 14361, 14631, 14901, 15171, 15441);

DELETE FROM
  team
WHERE
  id IN (3775, 3894, 4008, 4136, 4266, 4430, 4598, 4730, 4873, 5037, 5211, 5365, 5515, 5971, 6150, 6331, 6515, 6702, 6884, 7068, 7256, 7444, 7632, 7824, 8014, 8199, 8386, 10011, 10188, 11752, 11965, 12178, 12391, 12614, 12841, 13068, 13295, 13551, 13821, 14091, 14361, 14631, 14901, 15171, 15441);

----------------------------------------
-- Roxana Outlaws

UPDATE
  team
SET
  name_normalized = 'Roxana Outlaws',
  url_path_slug = NULL
WHERE
  id = 8955;

UPDATE
  tournament_team
SET
  team_id = 8955
WHERE
  id IN (8955, 9146, 9320, 9495, 9672, 9839);

DELETE FROM
  team
WHERE
  id IN (9146, 9320, 9495, 9672, 9839);

----------------------------------------
-- Roxana Roughnecks

UPDATE
  team
SET
  name_normalized = 'Roxana Roughnecks',
  url_path_slug = NULL
WHERE
  id = 1106;

UPDATE
  tournament_team
SET
  team_id = 1106
WHERE
  id IN (1106, 1217);

DELETE FROM
  team
WHERE
  id IN (1217);

----------------------------------------
-- Roxana Vikings

UPDATE
  team
SET
  name_normalized = 'Roxana Vikings',
  url_path_slug = NULL
WHERE
  id = 1107;

UPDATE
  tournament_team
SET
  team_id = 1107
WHERE
  id IN (1107, 1216);

DELETE FROM
  team
WHERE
  id IN (1216);

----------------------------------------
-- Rwc

UPDATE
  team
SET
  name_normalized = 'Rwc',
  url_path_slug = NULL
WHERE
  id = 10700;

UPDATE
  tournament_team
SET
  team_id = 10700
WHERE
  id IN (10700, 10883, 11055, 11217, 11741, 11954, 12167, 12380, 13537, 13807, 14077, 14347, 14617, 14887, 15157, 15427);

DELETE FROM
  team
WHERE
  id IN (10883, 11055, 11217, 11741, 11954, 12167, 12380, 13537, 13807, 14077, 14347, 14617, 14887, 15157, 15427);

----------------------------------------
-- SCN Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'SCN Youth Wrestling Club',
  url_path_slug = 'scn'
WHERE
  id = 4876;

UPDATE
  tournament_team
SET
  team_id = 4876
WHERE
  id IN (4876, 5040, 5214, 5368, 5519, 5660, 5806, 5972, 6151, 6334, 6518, 6705, 6887, 7070, 7258, 7445, 7633, 7825, 8015, 8200, 8387, 8574, 8765, 8956, 9147, 9321, 9496, 9673, 9840, 10012, 10189, 10362, 10533, 10713, 10896, 11062, 11224, 11389, 11551, 11753, 11966, 12179, 12392, 12615, 12842, 13069, 13296, 13552, 13822, 14092, 14362, 14632, 14902, 15172, 15442);

DELETE FROM
  team
WHERE
  id IN (5040, 5214, 5368, 5519, 5660, 5806, 5972, 6151, 6334, 6518, 6705, 6887, 7070, 7258, 7445, 7633, 7825, 8015, 8200, 8387, 8574, 8765, 8956, 9147, 9321, 9496, 9673, 9840, 10012, 10189, 10362, 10533, 10713, 10896, 11062, 11224, 11389, 11551, 11753, 11966, 12179, 12392, 12615, 12842, 13069, 13296, 13552, 13822, 14092, 14362, 14632, 14902, 15172, 15442);

----------------------------------------
-- SJO Spartan WC

UPDATE
  team
SET
  name_normalized = 'SJO Spartan WC',
  url_path_slug = NULL
WHERE
  id = 4436;

UPDATE
  tournament_team
SET
  team_id = 4436
WHERE
  id IN (4436, 4604);

DELETE FROM
  team
WHERE
  id IN (4604);

----------------------------------------
-- SJO Spartan Youth WC

UPDATE
  team
SET
  name_normalized = 'SJO Spartan Youth WC',
  url_path_slug = NULL
WHERE
  id = 5046;

UPDATE
  tournament_team
SET
  team_id = 5046
WHERE
  id IN (5046, 5220, 5664, 5810, 5973, 6152, 6339, 6523, 6709, 6891, 7076, 7264, 7446, 7634);

DELETE FROM
  team
WHERE
  id IN (5220, 5664, 5810, 5973, 6152, 6339, 6523, 6709, 6891, 7076, 7264, 7446, 7634);

----------------------------------------
-- SJO Spartan Ywc

UPDATE
  team
SET
  name_normalized = 'SJO Spartan Ywc',
  url_path_slug = NULL
WHERE
  id = 13553;

UPDATE
  tournament_team
SET
  team_id = 13553
WHERE
  id IN (13553, 13823, 14093, 14363, 14633, 14903, 15173, 15443);

DELETE FROM
  team
WHERE
  id IN (13823, 14093, 14363, 14633, 14903, 15173, 15443);

----------------------------------------
-- SJO Spartans Youth WC

UPDATE
  team
SET
  name_normalized = 'SJO Spartans Youth WC',
  url_path_slug = NULL
WHERE
  id = 5372;

UPDATE
  tournament_team
SET
  team_id = 5372
WHERE
  id IN (5372, 5523, 8201, 8388);

DELETE FROM
  team
WHERE
  id IN (5523, 8201, 8388);

----------------------------------------
-- SJO Youth WC

UPDATE
  team
SET
  name_normalized = 'SJO Youth WC',
  url_path_slug = NULL
WHERE
  id = 4736;

UPDATE
  tournament_team
SET
  team_id = 4736
WHERE
  id IN (4736, 4878, 7826, 8016, 11063, 11225, 11390, 11552, 11754, 11967, 12180, 12393, 12616, 12843, 13070, 13297);

DELETE FROM
  team
WHERE
  id IN (4878, 7826, 8016, 11063, 11225, 11390, 11552, 11754, 11967, 12180, 12393, 12616, 12843, 13070, 13297);

----------------------------------------
-- SJO Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'SJO Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 3900;

UPDATE
  tournament_team
SET
  team_id = 3900
WHERE
  id IN (3900, 4013, 4271);

DELETE FROM
  team
WHERE
  id IN (4013, 4271);

----------------------------------------
-- Saber WC

UPDATE
  team
SET
  name_normalized = 'Saber WC',
  url_path_slug = NULL
WHERE
  id = 8202;

UPDATE
  tournament_team
SET
  team_id = 8202
WHERE
  id IN (8202, 8389, 8575, 8766, 9323, 9498, 9674, 9841, 10013, 10190, 10364, 10535, 10715, 10898, 11392, 11554, 11756, 11969, 12182, 12395, 13555, 13825, 14095, 14365, 14635, 14905, 15175, 15445);

DELETE FROM
  team
WHERE
  id IN (8389, 8575, 8766, 9323, 9498, 9674, 9841, 10013, 10190, 10364, 10535, 10715, 10898, 11392, 11554, 11756, 11969, 12182, 12395, 13555, 13825, 14095, 14365, 14635, 14905, 15175, 15445);

----------------------------------------
-- Saber Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Saber Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 7447;

UPDATE
  tournament_team
SET
  team_id = 7447
WHERE
  id IN (7447, 7635, 7827, 8017, 8957, 9148);

DELETE FROM
  team
WHERE
  id IN (7635, 7827, 8017, 8957, 9148);

----------------------------------------
-- Salem WC

UPDATE
  team
SET
  name_normalized = 'Salem WC',
  url_path_slug = NULL
WHERE
  id = 11757;

UPDATE
  tournament_team
SET
  team_id = 11757
WHERE
  id IN (11757, 11970, 12183, 12396, 12618, 12845, 13072, 13299, 13556, 13826, 14096, 14366, 14636, 14906, 15176, 15446);

DELETE FROM
  team
WHERE
  id IN (11970, 12183, 12396, 12618, 12845, 13072, 13299, 13556, 13826, 14096, 14366, 14636, 14906, 15176, 15446);

----------------------------------------
-- Sandwich WC

UPDATE
  team
SET
  name_normalized = 'Sandwich WC',
  url_path_slug = NULL
WHERE
  id = 3895;

UPDATE
  tournament_team
SET
  team_id = 3895
WHERE
  id IN (3895, 11758, 11971, 12184, 12397, 12619, 12846, 13073, 13300, 13557, 13827, 14097, 14367, 14637, 14907, 15177, 15447);

DELETE FROM
  team
WHERE
  id IN (11758, 11971, 12184, 12397, 12619, 12846, 13073, 13300, 13557, 13827, 14097, 14367, 14637, 14907, 15177, 15447);

----------------------------------------
-- Sauk Valley Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Sauk Valley Wrestling Club',
  url_path_slug = 'sauk-valley'
WHERE
  id = 5366;

UPDATE
  tournament_team
SET
  team_id = 5366
WHERE
  id IN (5366, 5516, 5659, 5804, 5974, 6153, 6332, 6516, 6703, 6885, 7069, 7257, 7448, 7636, 7828, 8018, 8203, 8390, 8576, 8767, 8958, 9149, 9324, 9499, 9675, 9842, 10014, 10191, 10365, 10536, 10716, 10899, 11065, 11227, 11393, 11555, 11759, 11972, 12185, 12398, 12620, 12847, 13074, 13301, 13558, 13828, 14098, 14368, 14638, 14908, 15178, 15448);

DELETE FROM
  team
WHERE
  id IN (5516, 5659, 5804, 5974, 6153, 6332, 6516, 6703, 6885, 7069, 7257, 7448, 7636, 7828, 8018, 8203, 8390, 8576, 8767, 8958, 9149, 9324, 9499, 9675, 9842, 10014, 10191, 10365, 10536, 10716, 10899, 11065, 11227, 11393, 11555, 11759, 11972, 12185, 12398, 12620, 12847, 13074, 13301, 13558, 13828, 14098, 14368, 14638, 14908, 15178, 15448);

----------------------------------------
-- Saukee Youth WC

UPDATE
  team
SET
  name_normalized = 'Saukee Youth WC',
  url_path_slug = NULL
WHERE
  id = 3896;

UPDATE
  tournament_team
SET
  team_id = 3896
WHERE
  id IN (3896, 4009, 4137, 4267, 4431, 4599, 4731, 4874, 5038, 5212, 5367, 5517, 5805, 5975, 6154, 6333, 6517, 6704, 6886, 7449, 7637, 7829, 8019, 9325, 9500, 10366, 10537, 10717, 10900, 13559, 13829, 14099, 14369, 14639, 14909, 15179, 15449);

DELETE FROM
  team
WHERE
  id IN (4009, 4137, 4267, 4431, 4599, 4731, 4874, 5038, 5212, 5367, 5517, 5805, 5975, 6154, 6333, 6517, 6704, 6886, 7449, 7637, 7829, 8019, 9325, 9500, 10366, 10537, 10717, 10900, 13559, 13829, 14099, 14369, 14639, 14909, 15179, 15449);

----------------------------------------
-- Saukee Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Saukee Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8959;

UPDATE
  tournament_team
SET
  team_id = 8959
WHERE
  id IN (8959, 9150);

DELETE FROM
  team
WHERE
  id IN (9150);

----------------------------------------
-- Savanna

UPDATE
  team
SET
  name_normalized = 'Savanna',
  url_path_slug = 'savanna'
WHERE
  id = 227;

UPDATE
  tournament_team
SET
  team_id = 227
WHERE
  id IN (227, 3657, 4138, 4432, 4600, 4732, 4875, 5039, 5213, 5518);

DELETE FROM
  team
WHERE
  id IN (3657, 4138, 4432, 4600, 4732, 4875, 5039, 5213, 5518);

----------------------------------------
-- Schaumburg Jr. Saxons

UPDATE
  team
SET
  name_normalized = 'Schaumburg Jr. Saxons',
  url_path_slug = NULL
WHERE
  id = 4433;

UPDATE
  tournament_team
SET
  team_id = 4433
WHERE
  id IN (4433, 4601);

DELETE FROM
  team
WHERE
  id IN (4601);

----------------------------------------
-- Scorpion WC

UPDATE
  team
SET
  name_normalized = 'Scorpion WC',
  url_path_slug = NULL
WHERE
  id = 8577;

UPDATE
  tournament_team
SET
  team_id = 8577
WHERE
  id IN (8577, 8768, 9326, 9501, 9676, 9843, 10015, 10192, 10367, 10538, 10718, 10901, 11066, 11228, 11394, 11556, 11760, 11973, 12186, 12399, 12621, 12848, 13075, 13302);

DELETE FROM
  team
WHERE
  id IN (8768, 9326, 9501, 9676, 9843, 10015, 10192, 10367, 10538, 10718, 10901, 11066, 11228, 11394, 11556, 11760, 11973, 12186, 12399, 12621, 12848, 13075, 13302);

----------------------------------------
-- Scorpion Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Scorpion Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8960;

UPDATE
  tournament_team
SET
  team_id = 8960
WHERE
  id IN (8960, 9151);

DELETE FROM
  team
WHERE
  id IN (9151);

----------------------------------------
-- Seneca Irish Cadets

UPDATE
  team
SET
  name_normalized = 'Seneca Irish Cadets',
  url_path_slug = NULL
WHERE
  id = 4877;

UPDATE
  tournament_team
SET
  team_id = 4877
WHERE
  id IN (4877, 5041, 5215, 5520, 5807, 5976, 6155, 6335, 6519, 6706, 6888, 7071, 7259, 7450, 7638, 7830, 8020);

DELETE FROM
  team
WHERE
  id IN (5041, 5215, 5520, 5807, 5976, 6155, 6335, 6519, 6706, 6888, 7071, 7259, 7450, 7638, 7830, 8020);

----------------------------------------
-- Seneca Irish Cadets WC

UPDATE
  team
SET
  name_normalized = 'Seneca Irish Cadets WC',
  url_path_slug = NULL
WHERE
  id = 8204;

UPDATE
  tournament_team
SET
  team_id = 8204
WHERE
  id IN (8204, 8391, 8578, 8769);

DELETE FROM
  team
WHERE
  id IN (8391, 8578, 8769);

----------------------------------------
-- Shadow Wolves Wrestling

UPDATE
  team
SET
  name_normalized = 'Shadow Wolves Wrestling',
  url_path_slug = NULL
WHERE
  id = 13560;

UPDATE
  tournament_team
SET
  team_id = 13560
WHERE
  id IN (13560, 13830, 14100, 14370, 14640, 14910, 15180, 15450);

DELETE FROM
  team
WHERE
  id IN (13830, 14100, 14370, 14640, 14910, 15180, 15450);

----------------------------------------
-- Shamrock WC

UPDATE
  team
SET
  name_normalized = 'Shamrock WC',
  url_path_slug = NULL
WHERE
  id = 4733;

UPDATE
  tournament_team
SET
  team_id = 4733
WHERE
  id IN (4733, 5042, 5216, 5369, 5521, 5661, 5808, 5977, 6156, 6336, 6520, 6707, 6889, 7072, 7260, 7451, 7639, 7831, 8021, 8579, 8770);

DELETE FROM
  team
WHERE
  id IN (5042, 5216, 5369, 5521, 5661, 5808, 5977, 6156, 6336, 6520, 6707, 6889, 7072, 7260, 7451, 7639, 7831, 8021, 8579, 8770);

----------------------------------------
-- Shamrock Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Shamrock Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8961;

UPDATE
  tournament_team
SET
  team_id = 8961
WHERE
  id IN (8961, 9152);

DELETE FROM
  team
WHERE
  id IN (9152);

----------------------------------------
-- Sharks WC

UPDATE
  team
SET
  name_normalized = 'Sharks WC',
  url_path_slug = NULL
WHERE
  id = 3897;

UPDATE
  tournament_team
SET
  team_id = 3897
WHERE
  id IN (3897, 4010, 4268, 4434, 4602, 5662, 5978, 6157, 7073, 7261, 7452, 7640, 7832, 8022, 8205, 8392, 8580, 8771, 9327, 9502, 13561, 13831, 14101, 14371, 14641, 14911, 15181, 15451);

DELETE FROM
  team
WHERE
  id IN (4010, 4268, 4434, 4602, 5662, 5978, 6157, 7073, 7261, 7452, 7640, 7832, 8022, 8205, 8392, 8580, 8771, 9327, 9502, 13561, 13831, 14101, 14371, 14641, 14911, 15181, 15451);

----------------------------------------
-- Sharks Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Sharks Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8962;

UPDATE
  tournament_team
SET
  team_id = 8962
WHERE
  id IN (8962, 9153);

DELETE FROM
  team
WHERE
  id IN (9153);

----------------------------------------
-- Shelbyville Jr Rams

UPDATE
  team
SET
  name_normalized = 'Shelbyville Jr Rams',
  url_path_slug = NULL
WHERE
  id = 3898;

UPDATE
  tournament_team
SET
  team_id = 3898
WHERE
  id IN (3898, 4011, 4139);

DELETE FROM
  team
WHERE
  id IN (4011, 4139);

----------------------------------------
-- Shelbyville Jr Rams Wrestling

UPDATE
  team
SET
  name_normalized = 'Shelbyville Jr Rams Wrestling',
  url_path_slug = NULL
WHERE
  id = 5043;

UPDATE
  tournament_team
SET
  team_id = 5043
WHERE
  id IN (5043, 5217, 5663, 5809, 5979, 6158, 6337, 6521, 6708, 6890, 7074, 7262, 7453, 7641);

DELETE FROM
  team
WHERE
  id IN (5217, 5663, 5809, 5979, 6158, 6337, 6521, 6708, 6890, 7074, 7262, 7453, 7641);

----------------------------------------
-- Shelbyville Jr. Rams WC

UPDATE
  team
SET
  name_normalized = 'Shelbyville Jr. Rams WC',
  url_path_slug = NULL
WHERE
  id = 4734;

UPDATE
  tournament_team
SET
  team_id = 4734
WHERE
  id IN (4734, 5370, 5522);

DELETE FROM
  team
WHERE
  id IN (5370, 5522);

----------------------------------------
-- Shelbyville Jr. Wrestling Rams

UPDATE
  team
SET
  name_normalized = 'Shelbyville Jr. Wrestling Rams',
  url_path_slug = NULL
WHERE
  id = 12622;

UPDATE
  tournament_team
SET
  team_id = 12622
WHERE
  id IN (12622, 12849, 13076, 13303, 13562, 13832, 14102, 14372, 14642, 14912, 15182, 15452);

DELETE FROM
  team
WHERE
  id IN (12849, 13076, 13303, 13562, 13832, 14102, 14372, 14642, 14912, 15182, 15452);

----------------------------------------
-- Shelbyville Junior Wrestling Rams

UPDATE
  team
SET
  name_normalized = 'Shelbyville Junior Wrestling Rams',
  url_path_slug = NULL
WHERE
  id = 7833;

UPDATE
  tournament_team
SET
  team_id = 7833
WHERE
  id IN (7833, 8023);

DELETE FROM
  team
WHERE
  id IN (8023);

----------------------------------------
-- Shepard Wrestling

UPDATE
  team
SET
  name_normalized = 'Shepard Wrestling',
  url_path_slug = NULL
WHERE
  id = 4435;

UPDATE
  tournament_team
SET
  team_id = 4435
WHERE
  id IN (4435, 4603);

DELETE FROM
  team
WHERE
  id IN (4603);

----------------------------------------
-- Sherman-Williamsville WC

UPDATE
  team
SET
  name_normalized = 'Sherman-Williamsville WC',
  url_path_slug = NULL
WHERE
  id = 7834;

UPDATE
  tournament_team
SET
  team_id = 7834
WHERE
  id IN (7834, 8024, 8206, 8393);

DELETE FROM
  team
WHERE
  id IN (8024, 8206, 8393);

----------------------------------------
-- Sherrard Jr WC

UPDATE
  team
SET
  name_normalized = 'Sherrard Jr WC',
  url_path_slug = NULL
WHERE
  id = 3899;

UPDATE
  tournament_team
SET
  team_id = 3899
WHERE
  id IN (3899, 4012, 4140, 4270, 5044, 5218);

DELETE FROM
  team
WHERE
  id IN (4012, 4140, 4270, 5044, 5218);

----------------------------------------
-- Sherrard Jr. Tigers WC

UPDATE
  team
SET
  name_normalized = 'Sherrard Jr. Tigers WC',
  url_path_slug = NULL
WHERE
  id = 11761;

UPDATE
  tournament_team
SET
  team_id = 11761
WHERE
  id IN (11761, 11974, 12187, 12400, 12623, 12850, 13077, 13304, 13563, 13833, 14103, 14373, 14643, 14913, 15183, 15453);

DELETE FROM
  team
WHERE
  id IN (11974, 12187, 12400, 12623, 12850, 13077, 13304, 13563, 13833, 14103, 14373, 14643, 14913, 15183, 15453);

----------------------------------------
-- Shooters Elite WC

UPDATE
  team
SET
  name_normalized = 'Shooters Elite WC',
  url_path_slug = NULL
WHERE
  id = 6338;

UPDATE
  tournament_team
SET
  team_id = 6338
WHERE
  id IN (6338, 6522, 7075, 7263);

DELETE FROM
  team
WHERE
  id IN (6522, 7075, 7263);

----------------------------------------
-- Silver & Black Attack

UPDATE
  team
SET
  name_normalized = 'Silver & Black Attack',
  url_path_slug = NULL
WHERE
  id = 4735;

UPDATE
  tournament_team
SET
  team_id = 4735
WHERE
  id IN (4735, 5045, 5219);

DELETE FROM
  team
WHERE
  id IN (5045, 5219);

----------------------------------------
-- Siwa

UPDATE
  team
SET
  name_normalized = 'Siwa',
  url_path_slug = NULL
WHERE
  id = 9322;

UPDATE
  tournament_team
SET
  team_id = 9322
WHERE
  id IN (9322, 9497);

DELETE FROM
  team
WHERE
  id IN (9497);

----------------------------------------
-- Soldier Wrestling Allstar Training

UPDATE
  team
SET
  name_normalized = 'Soldier Wrestling Allstar Training',
  url_path_slug = NULL
WHERE
  id = 7454;

UPDATE
  tournament_team
SET
  team_id = 7454
WHERE
  id IN (7454, 7642, 7835, 8025);

DELETE FROM
  team
WHERE
  id IN (7642, 7835, 8025);

----------------------------------------
-- Somonauk WC

UPDATE
  team
SET
  name_normalized = 'Somonauk WC',
  url_path_slug = NULL
WHERE
  id = 6340;

UPDATE
  tournament_team
SET
  team_id = 6340
WHERE
  id IN (6340, 6524);

DELETE FROM
  team
WHERE
  id IN (6524);

----------------------------------------
-- Somonauk Wrestlers Club

UPDATE
  team
SET
  name_normalized = 'Somonauk Wrestlers Club',
  url_path_slug = NULL
WHERE
  id = 9328;

UPDATE
  tournament_team
SET
  team_id = 9328
WHERE
  id IN (9328, 9503, 10016, 10193);

DELETE FROM
  team
WHERE
  id IN (9503, 10016, 10193);

----------------------------------------
-- Sons Of Thunder

UPDATE
  team
SET
  name_normalized = 'Sons Of Thunder',
  url_path_slug = NULL
WHERE
  id = 5373;

UPDATE
  tournament_team
SET
  team_id = 5373
WHERE
  id IN (5373, 5524, 5665, 5811, 5980, 6159);

DELETE FROM
  team
WHERE
  id IN (5524, 5665, 5811, 5980, 6159);

----------------------------------------
-- Sot-C

UPDATE
  team
SET
  name_normalized = 'Sot-C',
  url_path_slug = NULL
WHERE
  id = 10363;

UPDATE
  tournament_team
SET
  team_id = 10363
WHERE
  id IN (10363, 10534, 10714, 10897, 11064, 11226, 11391, 11553, 11755, 11968, 12181, 12394, 12617, 12844, 13071, 13298, 13554, 13824, 14094, 14364, 14634, 14904, 15174, 15444);

DELETE FROM
  team
WHERE
  id IN (10534, 10714, 10897, 11064, 11226, 11391, 11553, 11755, 11968, 12181, 12394, 12617, 12844, 13071, 13298, 13554, 13824, 14094, 14364, 14634, 14904, 15174, 15444);

----------------------------------------
-- Southern Illinois Bulldogs WC

UPDATE
  team
SET
  name_normalized = 'Southern Illinois Bulldogs WC',
  url_path_slug = NULL
WHERE
  id = 11395;

UPDATE
  tournament_team
SET
  team_id = 11395
WHERE
  id IN (11395, 11557, 11762, 11975, 12188, 12401, 12624, 12851, 13078, 13305, 13564, 13834, 14104, 14374, 14644, 14914, 15184, 15454);

DELETE FROM
  team
WHERE
  id IN (11557, 11762, 11975, 12188, 12401, 12624, 12851, 13078, 13305, 13564, 13834, 14104, 14374, 14644, 14914, 15184, 15454);

----------------------------------------
-- Southern Illinois Eagles

UPDATE
  team
SET
  name_normalized = 'Southern Illinois Eagles',
  url_path_slug = NULL
WHERE
  id = 3780;

UPDATE
  tournament_team
SET
  team_id = 3780
WHERE
  id IN (3780, 3901, 4014, 4142);

DELETE FROM
  team
WHERE
  id IN (3901, 4014, 4142);

----------------------------------------
-- Southern Illinois Eagles

UPDATE
  team
SET
  name_normalized = 'Southern Illinois Eagles',
  url_path_slug = NULL
WHERE
  id = 4437;

UPDATE
  tournament_team
SET
  team_id = 4437
WHERE
  id IN (4437, 4605, 4737, 4879);

DELETE FROM
  team
WHERE
  id IN (4605, 4737, 4879);

----------------------------------------
-- Southern Illinois United Thundercats

UPDATE
  team
SET
  name_normalized = 'Southern Illinois United Thundercats',
  url_path_slug = NULL
WHERE
  id = 5812;

UPDATE
  tournament_team
SET
  team_id = 5812
WHERE
  id IN (5812, 5981, 6160);

DELETE FROM
  team
WHERE
  id IN (5981, 6160);

----------------------------------------
-- Southern Kings

UPDATE
  team
SET
  name_normalized = 'Southern Kings',
  url_path_slug = NULL
WHERE
  id = 8963;

UPDATE
  tournament_team
SET
  team_id = 8963
WHERE
  id IN (8963, 9154);

DELETE FROM
  team
WHERE
  id IN (9154);

----------------------------------------
-- Southside Comets

UPDATE
  team
SET
  name_normalized = 'Southside Comets',
  url_path_slug = NULL
WHERE
  id = 7077;

UPDATE
  tournament_team
SET
  team_id = 7077
WHERE
  id IN (7077, 7265, 7455, 7643);

DELETE FROM
  team
WHERE
  id IN (7265, 7455, 7643);

----------------------------------------
-- Southside Outlaws WC

UPDATE
  team
SET
  name_normalized = 'Southside Outlaws WC',
  url_path_slug = NULL
WHERE
  id = 10368;

UPDATE
  tournament_team
SET
  team_id = 10368
WHERE
  id IN (10368, 10539, 10719, 10902, 11067, 11229, 11396, 11558, 11763, 11976, 12189, 12402, 12625, 12852, 13079, 13306, 13565, 13835, 14105, 14375, 14645, 14915, 15185, 15455);

DELETE FROM
  team
WHERE
  id IN (10539, 10719, 10902, 11067, 11229, 11396, 11558, 11763, 11976, 12189, 12402, 12625, 12852, 13079, 13306, 13565, 13835, 14105, 14375, 14645, 14915, 15185, 15455);

----------------------------------------
-- Southside Stallions

UPDATE
  team
SET
  name_normalized = 'Southside Stallions',
  url_path_slug = NULL
WHERE
  id = 6710;

UPDATE
  tournament_team
SET
  team_id = 6710
WHERE
  id IN (6710, 6892, 7078, 7266, 7456, 7644, 7836, 8026, 8207, 8394, 8581, 8772);

DELETE FROM
  team
WHERE
  id IN (6892, 7078, 7266, 7456, 7644, 7836, 8026, 8207, 8394, 8581, 8772);

----------------------------------------
-- Southside Stallions School Of Wrestling

UPDATE
  team
SET
  name_normalized = 'Southside Stallions School Of Wrestling',
  url_path_slug = NULL
WHERE
  id = 8964;

UPDATE
  tournament_team
SET
  team_id = 8964
WHERE
  id IN (8964, 9155);

DELETE FROM
  team
WHERE
  id IN (9155);

----------------------------------------
-- Southside WC

UPDATE
  team
SET
  name_normalized = 'Southside WC',
  url_path_slug = NULL
WHERE
  id = 3902;

UPDATE
  tournament_team
SET
  team_id = 3902
WHERE
  id IN (3902, 4015, 4143, 4273);

DELETE FROM
  team
WHERE
  id IN (4015, 4143, 4273);

----------------------------------------
-- Sparta Junior Bulldogs

UPDATE
  team
SET
  name_normalized = 'Sparta Junior Bulldogs',
  url_path_slug = NULL
WHERE
  id = 8582;

UPDATE
  tournament_team
SET
  team_id = 8582
WHERE
  id IN (8582, 8773, 8965, 9156, 9329, 9504, 10017, 10194, 10720, 10903);

DELETE FROM
  team
WHERE
  id IN (8773, 8965, 9156, 9329, 9504, 10017, 10194, 10720, 10903);

----------------------------------------
-- Sparta Junior Bulldogs WC

UPDATE
  team
SET
  name_normalized = 'Sparta Junior Bulldogs WC',
  url_path_slug = NULL
WHERE
  id = 11068;

UPDATE
  tournament_team
SET
  team_id = 11068
WHERE
  id IN (11068, 11230, 11397, 11559, 11764, 11977, 12190, 12403, 12626, 12853, 13080, 13307, 13566, 13836, 14106, 14376, 14646, 14916, 15186, 15456);

DELETE FROM
  team
WHERE
  id IN (11230, 11397, 11559, 11764, 11977, 12190, 12403, 12626, 12853, 13080, 13307, 13566, 13836, 14106, 14376, 14646, 14916, 15186, 15456);

----------------------------------------
-- Spartan Elite WC

UPDATE
  team
SET
  name_normalized = 'Spartan Elite WC',
  url_path_slug = NULL
WHERE
  id = 8208;

UPDATE
  tournament_team
SET
  team_id = 8208
WHERE
  id IN (8208, 8395);

DELETE FROM
  team
WHERE
  id IN (8395);

----------------------------------------
-- Spartan WC

UPDATE
  team
SET
  name_normalized = 'Spartan WC',
  url_path_slug = NULL
WHERE
  id = 10721;

UPDATE
  tournament_team
SET
  team_id = 10721
WHERE
  id IN (10721, 10904, 11069, 11231, 12627, 12854, 13081, 13308, 13567, 13837, 14107, 14377, 14647, 14917, 15187, 15457);

DELETE FROM
  team
WHERE
  id IN (10904, 11069, 11231, 12627, 12854, 13081, 13308, 13567, 13837, 14107, 14377, 14647, 14917, 15187, 15457);

----------------------------------------
-- Spider WC

UPDATE
  team
SET
  name_normalized = 'Spider WC',
  url_path_slug = NULL
WHERE
  id = 5374;

UPDATE
  tournament_team
SET
  team_id = 5374
WHERE
  id IN (5374, 5666);

DELETE FROM
  team
WHERE
  id IN (5666);

----------------------------------------
-- Springfield Capital Kids WC

UPDATE
  team
SET
  name_normalized = 'Springfield Capital Kids WC',
  url_path_slug = NULL
WHERE
  id = 5982;

UPDATE
  tournament_team
SET
  team_id = 5982
WHERE
  id IN (5982, 6161, 6341, 6525, 6711, 6893, 7079, 7267, 7457, 7645);

DELETE FROM
  team
WHERE
  id IN (6161, 6341, 6525, 6711, 6893, 7079, 7267, 7457, 7645);

----------------------------------------
-- Springfield Capital Kids Wrestling

UPDATE
  team
SET
  name_normalized = 'Springfield Capital Kids Wrestling',
  url_path_slug = NULL
WHERE
  id = 5375;

UPDATE
  tournament_team
SET
  team_id = 5375
WHERE
  id IN (5375, 5525, 5813);

DELETE FROM
  team
WHERE
  id IN (5525, 5813);

----------------------------------------
-- Springfield Capitals

UPDATE
  team
SET
  name_normalized = 'Springfield Capitals',
  url_path_slug = NULL
WHERE
  id = 3781;

UPDATE
  tournament_team
SET
  team_id = 3781
WHERE
  id IN (3781, 3903, 4016, 4274, 4438, 4606, 4738, 4880, 5047, 5221);

DELETE FROM
  team
WHERE
  id IN (3903, 4016, 4274, 4438, 4606, 4738, 4880, 5047, 5221);

----------------------------------------
-- Springfield Capitals Kids WC

UPDATE
  team
SET
  name_normalized = 'Springfield Capitals Kids WC',
  url_path_slug = NULL
WHERE
  id = 4144;

UPDATE
  tournament_team
SET
  team_id = 4144
WHERE
  id IN (4144, 4274, 7837, 8027, 8209, 8396, 8583, 8774, 8966, 9157, 9330, 9505, 9677, 9844);

DELETE FROM
  team
WHERE
  id IN (4274, 7837, 8027, 8209, 8396, 8583, 8774, 8966, 9157, 9330, 9505, 9677, 9844);

----------------------------------------
-- Springs Elite

UPDATE
  team
SET
  name_normalized = 'Springs Elite',
  url_path_slug = NULL
WHERE
  id = 10369;

UPDATE
  tournament_team
SET
  team_id = 10369
WHERE
  id IN (10369, 10540, 10722, 10905);

DELETE FROM
  team
WHERE
  id IN (10540, 10722, 10905);

----------------------------------------
-- Springs Elite WC

UPDATE
  team
SET
  name_normalized = 'Springs Elite WC',
  url_path_slug = NULL
WHERE
  id = 11070;

UPDATE
  tournament_team
SET
  team_id = 11070
WHERE
  id IN (11070, 11232, 11398, 11560, 11765, 11978, 12191, 12404, 12628, 12855, 13082, 13309, 13568, 13838, 14108, 14378, 14648, 14918, 15188, 15458);

DELETE FROM
  team
WHERE
  id IN (11232, 11398, 11560, 11765, 11978, 12191, 12404, 12628, 12855, 13082, 13309, 13568, 13838, 14108, 14378, 14648, 14918, 15188, 15458);

----------------------------------------
-- St. Barnabas / Christ The King

UPDATE
  team
SET
  name_normalized = 'St. Barnabas / Christ The King',
  url_path_slug = NULL
WHERE
  id = 1516;

UPDATE
  tournament_team
SET
  team_id = 1516
WHERE
  id IN (1516, 1778, 1919, 2056, 2478, 2615);

DELETE FROM
  team
WHERE
  id IN (1778, 1919, 2056, 2478, 2615);

----------------------------------------
-- St. Bede Wrestling Club

UPDATE
  team
SET
  name_normalized = 'St. Bede Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1108;

UPDATE
  tournament_team
SET
  team_id = 1108
WHERE
  id IN (1108, 3904, 4017, 4275);

DELETE FROM
  team
WHERE
  id IN (3904, 4017, 4275);

----------------------------------------
-- St. Charles

UPDATE
  team
SET
  name_normalized = 'St. Charles',
  url_path_slug = NULL
WHERE
  id = 884;

UPDATE
  tournament_team
SET
  team_id = 884
WHERE
  id IN (884, 1647, 2057);

DELETE FROM
  team
WHERE
  id IN (1647, 2057);

----------------------------------------
-- St. Charles East WC

UPDATE
  team
SET
  name_normalized = 'St. Charles East WC',
  url_path_slug = NULL
WHERE
  id = 5048;

UPDATE
  tournament_team
SET
  team_id = 5048
WHERE
  id IN (5048, 5222, 5526, 5814, 5983, 6162, 6712, 6894, 7080, 7268, 7458, 7646, 7838, 8028, 8210, 8397, 8584, 8775);

DELETE FROM
  team
WHERE
  id IN (5222, 5526, 5814, 5983, 6162, 6712, 6894, 7080, 7268, 7458, 7646, 7838, 8028, 8210, 8397, 8584, 8775);

----------------------------------------
-- St. Charles WC

UPDATE
  team
SET
  name_normalized = 'St. Charles WC',
  url_path_slug = NULL
WHERE
  id = 1109;

UPDATE
  tournament_team
SET
  team_id = 1109
WHERE
  id IN (1109, 1219, 1517, 1779, 4439, 4607, 4739, 4881, 8967, 9158, 9331, 9506, 9678, 9845, 10370, 10541, 10723, 10906, 11071, 11233, 11399, 11561, 11766, 11979, 12192, 12405, 12629, 12856, 13083, 13310, 13569, 13839, 14109, 14379, 14649, 14919, 15189, 15459);

DELETE FROM
  team
WHERE
  id IN (1219, 1517, 1779, 4439, 4607, 4739, 4881, 8967, 9158, 9331, 9506, 9678, 9845, 10370, 10541, 10723, 10906, 11071, 11233, 11399, 11561, 11766, 11979, 12192, 12405, 12629, 12856, 13083, 13310, 13569, 13839, 14109, 14379, 14649, 14919, 15189, 15459);

----------------------------------------
-- St. Joseph Odgen Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'St. Joseph Odgen Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8968;

UPDATE
  tournament_team
SET
  team_id = 8968
WHERE
  id IN (8968, 9159);

DELETE FROM
  team
WHERE
  id IN (9159);

----------------------------------------
-- St. Joseph Ogden Youth WC

UPDATE
  team
SET
  name_normalized = 'St. Joseph Ogden Youth WC',
  url_path_slug = NULL
WHERE
  id = 9332;

UPDATE
  tournament_team
SET
  team_id = 9332
WHERE
  id IN (9332, 9507, 9679, 9846, 10018, 10195, 10724, 10907);

DELETE FROM
  team
WHERE
  id IN (9507, 9679, 9846, 10018, 10195, 10724, 10907);

----------------------------------------
-- St. Tarcissus Raiders WC

UPDATE
  team
SET
  name_normalized = 'St. Tarcissus Raiders WC',
  url_path_slug = NULL
WHERE
  id = 995;

UPDATE
  tournament_team
SET
  team_id = 995
WHERE
  id IN (995, 1110, 1220, 1518, 1648, 4740, 5049, 5223, 5376, 5527, 5984, 6163, 6342, 6526, 6713, 6895, 7081, 7269, 8585, 8776, 8969, 9160, 9333, 9508);

DELETE FROM
  team
WHERE
  id IN (1110, 1220, 1518, 1648, 4740, 5049, 5223, 5376, 5527, 5984, 6163, 6342, 6526, 6713, 6895, 7081, 7269, 8585, 8776, 8969, 9160, 9333, 9508);

----------------------------------------
-- Stallions Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Stallions Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 12630;

UPDATE
  tournament_team
SET
  team_id = 12630
WHERE
  id IN (12630, 12857, 13084, 13311);

DELETE FROM
  team
WHERE
  id IN (12857, 13084, 13311);

----------------------------------------
-- Stampede WC

UPDATE
  team
SET
  name_normalized = 'Stampede WC',
  url_path_slug = NULL
WHERE
  id = 11072;

UPDATE
  tournament_team
SET
  team_id = 11072
WHERE
  id IN (11072, 11234, 11400, 11562);

DELETE FROM
  team
WHERE
  id IN (11234, 11400, 11562);

----------------------------------------
-- Stateline Stingers WC

UPDATE
  team
SET
  name_normalized = 'Stateline Stingers WC',
  url_path_slug = NULL
WHERE
  id = 12631;

UPDATE
  tournament_team
SET
  team_id = 12631
WHERE
  id IN (12631, 12858, 13085, 13312, 13570, 13840, 14110, 14380, 14650, 14920, 15190, 15460);

DELETE FROM
  team
WHERE
  id IN (12858, 13085, 13312, 13570, 13840, 14110, 14380, 14650, 14920, 15190, 15460);

----------------------------------------
-- Stateline Wildcats

UPDATE
  team
SET
  name_normalized = 'Stateline Wildcats',
  url_path_slug = NULL
WHERE
  id = 3905;

UPDATE
  tournament_team
SET
  team_id = 3905
WHERE
  id IN (3905, 4018, 4145, 4276);

DELETE FROM
  team
WHERE
  id IN (4018, 4145, 4276);

----------------------------------------
-- Sterling Challand

UPDATE
  team
SET
  name_normalized = 'Sterling Challand',
  url_path_slug = NULL
WHERE
  id = 11;

UPDATE
  tournament_team
SET
  team_id = 11
WHERE
  id IN (11, 62, 77, 148, 454);

DELETE FROM
  team
WHERE
  id IN (62, 77, 148, 454);

----------------------------------------
-- Sterling Newman Junior Comets

UPDATE
  team
SET
  name_normalized = 'Sterling Newman Junior Comets',
  url_path_slug = 'sterling-newman'
WHERE
  id = 663;

UPDATE
  tournament_team
SET
  team_id = 663
WHERE
  id IN (663, 776, 887, 975, 1088, 1202, 1312, 1391, 1495, 1620, 1753, 2300, 2455, 2591, 2733, 3188, 3330, 3439, 3664, 3783, 3907, 4021, 4146, 4277, 4440, 4608, 5050, 5224);

DELETE FROM
  team
WHERE
  id IN (776, 887, 975, 1088, 1202, 1312, 1391, 1495, 1620, 1753, 2300, 2455, 2591, 2733, 3188, 3330, 3439, 3664, 3783, 3907, 4021, 4146, 4277, 4440, 4608, 5050, 5224);

----------------------------------------
-- Stillman Valley WC

UPDATE
  team
SET
  name_normalized = 'Stillman Valley WC',
  url_path_slug = NULL
WHERE
  id = 3908;

UPDATE
  tournament_team
SET
  team_id = 3908
WHERE
  id IN (3908, 4022, 4147, 4278, 4441, 4609, 4741, 4882, 5051, 5225, 5528, 5815, 6343, 6527, 6714, 6896, 7082, 7270, 7459, 7647, 7839, 8029, 8211, 8398, 8586, 8777, 9334, 9509, 9680, 9847, 10019, 10196, 10371, 10542, 11767, 11980, 12193, 12406, 12632, 12859, 13086, 13313, 13571, 13841, 14111, 14381, 14651, 14921, 15191, 15461);

DELETE FROM
  team
WHERE
  id IN (4022, 4147, 4278, 4441, 4609, 4741, 4882, 5051, 5225, 5528, 5815, 6343, 6527, 6714, 6896, 7082, 7270, 7459, 7647, 7839, 8029, 8211, 8398, 8586, 8777, 9334, 9509, 9680, 9847, 10019, 10196, 10371, 10542, 11767, 11980, 12193, 12406, 12632, 12859, 13086, 13313, 13571, 13841, 14111, 14381, 14651, 14921, 15191, 15461);

----------------------------------------
-- Stillman Valley Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Stillman Valley Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8970;

UPDATE
  tournament_team
SET
  team_id = 8970
WHERE
  id IN (8970, 9161);

DELETE FROM
  team
WHERE
  id IN (9161);

----------------------------------------
-- Stingers WC

UPDATE
  team
SET
  name_normalized = 'Stingers WC',
  url_path_slug = NULL
WHERE
  id = 11073;

UPDATE
  tournament_team
SET
  team_id = 11073
WHERE
  id IN (11073, 11235);

DELETE FROM
  team
WHERE
  id IN (11235);

----------------------------------------
-- Stockton Renegades

UPDATE
  team
SET
  name_normalized = 'Stockton Renegades',
  url_path_slug = NULL
WHERE
  id = 5667;

UPDATE
  tournament_team
SET
  team_id = 5667
WHERE
  id IN (5667, 8212, 8399, 8587, 8778, 8971, 9162, 9335, 9510, 9681, 9848, 10020, 10197, 10372, 10543, 10725, 10908);

DELETE FROM
  team
WHERE
  id IN (8212, 8399, 8587, 8778, 8971, 9162, 9335, 9510, 9681, 9848, 10020, 10197, 10372, 10543, 10725, 10908);

----------------------------------------
-- Stockton Renegades WC

UPDATE
  team
SET
  name_normalized = 'Stockton Renegades WC',
  url_path_slug = NULL
WHERE
  id = 11074;

UPDATE
  tournament_team
SET
  team_id = 11074
WHERE
  id IN (11074, 11236, 11401, 11563, 11768, 11981, 12194, 12407, 12633, 12860, 13087, 13314, 13572, 13842, 14112, 14382, 14652, 14922, 15192, 15462);

DELETE FROM
  team
WHERE
  id IN (11236, 11401, 11563, 11768, 11981, 12194, 12407, 12633, 12860, 13087, 13314, 13572, 13842, 14112, 14382, 14652, 14922, 15192, 15462);

----------------------------------------
-- Storm Youth WC

UPDATE
  team
SET
  name_normalized = 'Storm Youth WC',
  url_path_slug = NULL
WHERE
  id = 8588;

UPDATE
  tournament_team
SET
  team_id = 8588
WHERE
  id IN (8588, 8779, 9336, 9511, 9682, 9849, 10021, 10198, 10373, 10544, 10726, 10909, 11075, 11237, 11402, 11564, 11769, 11982, 12195, 12408, 12634, 12861, 13088, 13315, 13573, 13843, 14113, 14383, 14653, 14923, 15193, 15463);

DELETE FROM
  team
WHERE
  id IN (8779, 9336, 9511, 9682, 9849, 10021, 10198, 10373, 10544, 10726, 10909, 11075, 11237, 11402, 11564, 11769, 11982, 12195, 12408, 12634, 12861, 13088, 13315, 13573, 13843, 14113, 14383, 14653, 14923, 15193, 15463);

----------------------------------------
-- Storm Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Storm Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8972;

UPDATE
  tournament_team
SET
  team_id = 8972
WHERE
  id IN (8972, 9163);

DELETE FROM
  team
WHERE
  id IN (9163);

----------------------------------------
-- Streator Bulldog WC

UPDATE
  team
SET
  name_normalized = 'Streator Bulldog WC',
  url_path_slug = NULL
WHERE
  id = 5985;

UPDATE
  tournament_team
SET
  team_id = 5985
WHERE
  id IN (5985, 6164);

DELETE FROM
  team
WHERE
  id IN (6164);

----------------------------------------
-- Streator WC

UPDATE
  team
SET
  name_normalized = 'Streator WC',
  url_path_slug = NULL
WHERE
  id = 7460;

UPDATE
  tournament_team
SET
  team_id = 7460
WHERE
  id IN (7460, 7648, 7840, 8030, 8213, 8400, 10374, 10545, 12635, 12862, 13089, 13316, 13574, 13844, 14114, 14384, 14654, 14924, 15194, 15464);

DELETE FROM
  team
WHERE
  id IN (7648, 7840, 8030, 8213, 8400, 10374, 10545, 12635, 12862, 13089, 13316, 13574, 13844, 14114, 14384, 14654, 14924, 15194, 15464);

----------------------------------------
-- Sudden Victory Athletic Center

UPDATE
  team
SET
  name_normalized = 'Sudden Victory Athletic Center',
  url_path_slug = NULL
WHERE
  id = 10022;

UPDATE
  tournament_team
SET
  team_id = 10022
WHERE
  id IN (10022, 10199);

DELETE FROM
  team
WHERE
  id IN (10199);

----------------------------------------
-- Super Soldiers WC

UPDATE
  team
SET
  name_normalized = 'Super Soldiers WC',
  url_path_slug = NULL
WHERE
  id = 9337;

UPDATE
  tournament_team
SET
  team_id = 9337
WHERE
  id IN (9337, 9512, 9683, 9850, 10023, 10200, 10375, 10546, 10727, 10910, 11076, 11238);

DELETE FROM
  team
WHERE
  id IN (9512, 9683, 9850, 10023, 10200, 10375, 10546, 10727, 10910, 11076, 11238);

----------------------------------------
-- Suplex WC

UPDATE
  team
SET
  name_normalized = 'Suplex WC',
  url_path_slug = NULL
WHERE
  id = 11770;

UPDATE
  tournament_team
SET
  team_id = 11770
WHERE
  id IN (11770, 11983, 12196, 12409, 12636, 12863, 13090, 13317);

DELETE FROM
  team
WHERE
  id IN (11983, 12196, 12409, 12636, 12863, 13090, 13317);

----------------------------------------
-- Sycamore Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Sycamore Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 889;

UPDATE
  tournament_team
SET
  team_id = 889
WHERE
  id IN (889, 1112, 1221, 1520, 1650, 1923, 4442, 4610, 4742, 4883, 5052, 5226, 5529, 5668, 5816, 5986, 6165, 6344, 6528, 6715, 6897, 7083, 7271, 7461, 7649, 7841, 8031, 8214, 8401, 8589, 8780, 8973, 9164, 9338, 9513, 9684, 9851, 10024, 10201, 10376, 10547, 10728, 10911, 11077, 11239, 11403, 11565, 11771, 11984, 12197, 12410, 12637, 12864, 13091, 13318, 13575, 13845, 14115, 14385, 14655, 14925, 15195, 15465);

DELETE FROM
  team
WHERE
  id IN (1112, 1221, 1520, 1650, 1923, 4442, 4610, 4742, 4883, 5052, 5226, 5529, 5668, 5816, 5986, 6165, 6344, 6528, 6715, 6897, 7083, 7271, 7461, 7649, 7841, 8031, 8214, 8401, 8589, 8780, 8973, 9164, 9338, 9513, 9684, 9851, 10024, 10201, 10376, 10547, 10728, 10911, 11077, 11239, 11403, 11565, 11771, 11984, 12197, 12410, 12637, 12864, 13091, 13318, 13575, 13845, 14115, 14385, 14655, 14925, 15195, 15465);

----------------------------------------
-- T-Bird / Raider Wrestling

UPDATE
  team
SET
  name_normalized = 'T-Bird / Raider Wrestling',
  url_path_slug = NULL
WHERE
  id = 4443;

UPDATE
  tournament_team
SET
  team_id = 4443
WHERE
  id IN (4443, 4611, 4743, 4884, 5053, 5227);

DELETE FROM
  team
WHERE
  id IN (4611, 4743, 4884, 5053, 5227);

----------------------------------------
-- Takedown WC

UPDATE
  team
SET
  name_normalized = 'Takedown WC',
  url_path_slug = NULL
WHERE
  id = 4885;

UPDATE
  tournament_team
SET
  team_id = 4885
WHERE
  id IN (4885, 5054, 5228, 5377);

DELETE FROM
  team
WHERE
  id IN (5054, 5228, 5377);

----------------------------------------
-- Taylorville Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Taylorville Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 998;

UPDATE
  tournament_team
SET
  team_id = 998
WHERE
  id IN (998, 1404, 1521, 3909, 4148, 4279, 4444, 4612, 4744, 5055, 5229, 5378, 5669, 5817, 5987, 6166, 6345, 6529, 6716, 6898, 7084, 7272, 7462, 7650, 7842, 8032, 8215, 8402, 8590, 8781, 8976, 9167, 11774, 11987, 12200, 12413, 12640, 12867, 13094, 13321, 13577, 13847, 14117, 14387, 14657, 14927, 15197, 15467);

DELETE FROM
  team
WHERE
  id IN (1404, 1521, 3909, 4148, 4279, 4444, 4612, 4744, 5055, 5229, 5378, 5669, 5817, 5987, 6166, 6345, 6529, 6716, 6898, 7084, 7272, 7462, 7650, 7842, 8032, 8215, 8402, 8590, 8781, 8976, 9167, 11774, 11987, 12200, 12413, 12640, 12867, 13094, 13321, 13577, 13847, 14117, 14387, 14657, 14927, 15197, 15467);

----------------------------------------
-- Tazewell County Kidz WC

UPDATE
  team
SET
  name_normalized = 'Tazewell County Kidz WC',
  url_path_slug = NULL
WHERE
  id = 3910;

UPDATE
  tournament_team
SET
  team_id = 3910
WHERE
  id IN (3910, 4023, 4445, 4613);

DELETE FROM
  team
WHERE
  id IN (4023, 4445, 4613);

----------------------------------------
-- Team 1006

UPDATE
  team
SET
  name_normalized = 'Team 1006',
  url_path_slug = NULL
WHERE
  id = 8591;

UPDATE
  tournament_team
SET
  team_id = 8591
WHERE
  id IN (8591, 8782, 8977, 9168);

DELETE FROM
  team
WHERE
  id IN (8782, 8977, 9168);

----------------------------------------
-- Team 1006 Wrestling

UPDATE
  team
SET
  name_normalized = 'Team 1006 Wrestling',
  url_path_slug = NULL
WHERE
  id = 9341;

UPDATE
  tournament_team
SET
  team_id = 9341
WHERE
  id IN (9341, 9516, 9687, 9854, 10027, 10204, 10379, 10550, 10730, 10913, 11079, 11241);

DELETE FROM
  team
WHERE
  id IN (9516, 9687, 9854, 10027, 10204, 10379, 10550, 10730, 10913, 11079, 11241);

----------------------------------------
-- Team 312

UPDATE
  team
SET
  name_normalized = 'Team 312',
  url_path_slug = NULL
WHERE
  id = 9342;

UPDATE
  tournament_team
SET
  team_id = 9342
WHERE
  id IN (9342, 9517, 9688, 9855, 10028, 10205, 10380, 10551, 10731, 10914);

DELETE FROM
  team
WHERE
  id IN (9517, 9688, 9855, 10028, 10205, 10380, 10551, 10731, 10914);

----------------------------------------
-- Team 312 WC

UPDATE
  team
SET
  name_normalized = 'Team 312 WC',
  url_path_slug = NULL
WHERE
  id = 11080;

UPDATE
  tournament_team
SET
  team_id = 11080
WHERE
  id IN (11080, 11242, 11405, 11567, 11775, 11988, 12201, 12414, 13578, 13848, 14118, 14388, 14658, 14928, 15198, 15468);

DELETE FROM
  team
WHERE
  id IN (11242, 11405, 11567, 11775, 11988, 12201, 12414, 13578, 13848, 14118, 14388, 14658, 14928, 15198, 15468);

----------------------------------------
-- Team 360 Gatorstyle Wrestling

UPDATE
  team
SET
  name_normalized = 'Team 360 Gatorstyle Wrestling',
  url_path_slug = NULL
WHERE
  id = 8978;

UPDATE
  tournament_team
SET
  team_id = 8978
WHERE
  id IN (8978, 9169);

DELETE FROM
  team
WHERE
  id IN (9169);

----------------------------------------
-- Team C Squared

UPDATE
  team
SET
  name_normalized = 'Team C Squared',
  url_path_slug = NULL
WHERE
  id = 8216;

UPDATE
  tournament_team
SET
  team_id = 8216
WHERE
  id IN (8216, 8403);

DELETE FROM
  team
WHERE
  id IN (8403);

----------------------------------------
-- Team Cuff Wrestling Tigers

UPDATE
  team
SET
  name_normalized = 'Team Cuff Wrestling Tigers',
  url_path_slug = NULL
WHERE
  id = 10029;

UPDATE
  tournament_team
SET
  team_id = 10029
WHERE
  id IN (10029, 10206);

DELETE FROM
  team
WHERE
  id IN (10206);

----------------------------------------
-- Team El1Te Wrestling

UPDATE
  team
SET
  name_normalized = 'Team El1Te Wrestling',
  url_path_slug = NULL
WHERE
  id = 11081;

UPDATE
  tournament_team
SET
  team_id = 11081
WHERE
  id IN (11081, 11243, 11406, 11568, 11776, 11989, 12202, 12415, 13579, 13849, 14119, 14389, 14659, 14929, 15199, 15469);

DELETE FROM
  team
WHERE
  id IN (11243, 11406, 11568, 11776, 11989, 12202, 12415, 13579, 13849, 14119, 14389, 14659, 14929, 15199, 15469);

----------------------------------------
-- Team Mascoutah

UPDATE
  team
SET
  name_normalized = 'Team Mascoutah',
  url_path_slug = NULL
WHERE
  id = 7463;

UPDATE
  tournament_team
SET
  team_id = 7463
WHERE
  id IN (7463, 7651);

DELETE FROM
  team
WHERE
  id IN (7651);

----------------------------------------
-- Team Mascoutah WC

UPDATE
  team
SET
  name_normalized = 'Team Mascoutah WC',
  url_path_slug = NULL
WHERE
  id = 9343;

UPDATE
  tournament_team
SET
  team_id = 9343
WHERE
  id IN (9343, 9518, 9689, 9856, 10381, 10552, 11082, 11244, 11407, 11569, 11777, 11990, 12203, 12416, 12641, 12868, 13095, 13322, 13580, 13850, 14120, 14390, 14660, 14930, 15200, 15470);

DELETE FROM
  team
WHERE
  id IN (9518, 9689, 9856, 10381, 10552, 11082, 11244, 11407, 11569, 11777, 11990, 12203, 12416, 12641, 12868, 13095, 13322, 13580, 13850, 14120, 14390, 14660, 14930, 15200, 15470);

----------------------------------------
-- Team Mascoutah Wrestling

UPDATE
  team
SET
  name_normalized = 'Team Mascoutah Wrestling',
  url_path_slug = NULL
WHERE
  id = 8979;

UPDATE
  tournament_team
SET
  team_id = 8979
WHERE
  id IN (8979, 9170);

DELETE FROM
  team
WHERE
  id IN (9170);

----------------------------------------
-- Team No Ego WC

UPDATE
  team
SET
  name_normalized = 'Team No Ego WC',
  url_path_slug = NULL
WHERE
  id = 7843;

UPDATE
  tournament_team
SET
  team_id = 7843
WHERE
  id IN (7843, 8033, 8217, 8404, 8592, 8783, 9344, 9519, 9690, 9857, 10030, 10207);

DELETE FROM
  team
WHERE
  id IN (8033, 8217, 8404, 8592, 8783, 9344, 9519, 9690, 9857, 10030, 10207);

----------------------------------------
-- Team One

UPDATE
  team
SET
  name_normalized = 'Team One',
  url_path_slug = NULL
WHERE
  id = 6346;

UPDATE
  tournament_team
SET
  team_id = 6346
WHERE
  id IN (6346, 6530);

DELETE FROM
  team
WHERE
  id IN (6530);

----------------------------------------
-- Team Piasa WC

UPDATE
  team
SET
  name_normalized = 'Team Piasa WC',
  url_path_slug = NULL
WHERE
  id = 11778;

UPDATE
  tournament_team
SET
  team_id = 11778
WHERE
  id IN (11778, 11991, 12204, 12417, 13581, 13851, 14121, 14391, 14661, 14931, 15201, 15471);

DELETE FROM
  team
WHERE
  id IN (11991, 12204, 12417, 13581, 13851, 14121, 14391, 14661, 14931, 15201, 15471);

----------------------------------------
-- Team Poeta

UPDATE
  team
SET
  name_normalized = 'Team Poeta',
  url_path_slug = NULL
WHERE
  id = 10732;

UPDATE
  tournament_team
SET
  team_id = 10732
WHERE
  id IN (10732, 10915, 11083, 11245);

DELETE FROM
  team
WHERE
  id IN (10915, 11083, 11245);

----------------------------------------
-- Team Topnotch Wrestling

UPDATE
  team
SET
  name_normalized = 'Team Topnotch Wrestling',
  url_path_slug = NULL
WHERE
  id = 9345;

UPDATE
  tournament_team
SET
  team_id = 9345
WHERE
  id IN (9345, 9520);

DELETE FROM
  team
WHERE
  id IN (9520);

----------------------------------------
-- Team West Wolves

UPDATE
  team
SET
  name_normalized = 'Team West Wolves',
  url_path_slug = NULL
WHERE
  id = 4446;

UPDATE
  tournament_team
SET
  team_id = 4446
WHERE
  id IN (4446, 4614, 5379, 5530, 5818, 5988, 6167, 6347, 6531, 6717, 6899, 7085, 7273, 7464, 7652, 7844, 8034);

DELETE FROM
  team
WHERE
  id IN (4614, 5379, 5530, 5818, 5988, 6167, 6347, 6531, 6717, 6899, 7085, 7273, 7464, 7652, 7844, 8034);

----------------------------------------
-- Team Xpress

UPDATE
  team
SET
  name_normalized = 'Team Xpress',
  url_path_slug = NULL
WHERE
  id = 4447;

UPDATE
  tournament_team
SET
  team_id = 4447
WHERE
  id IN (4447, 4615);

DELETE FROM
  team
WHERE
  id IN (4615);

----------------------------------------
-- The Island City Misfits Elite Wrestling

UPDATE
  team
SET
  name_normalized = 'The Island City Misfits Elite Wrestling',
  url_path_slug = NULL
WHERE
  id = 13582;

UPDATE
  tournament_team
SET
  team_id = 13582
WHERE
  id IN (13582, 13852, 14122, 14392, 14662, 14932, 15202, 15472);

DELETE FROM
  team
WHERE
  id IN (13852, 14122, 14392, 14662, 14932, 15202, 15472);

----------------------------------------
-- The Law

UPDATE
  team
SET
  name_normalized = 'The Law',
  url_path_slug = NULL
WHERE
  id = 7086;

UPDATE
  tournament_team
SET
  team_id = 7086
WHERE
  id IN (7086, 7274, 7465, 7653, 7845, 8035, 8218, 8405, 8593, 8784, 8980, 9171, 9346, 9521, 9691, 9858, 10031, 10208, 10382, 10553, 10733, 10916);

DELETE FROM
  team
WHERE
  id IN (7274, 7465, 7653, 7845, 8035, 8218, 8405, 8593, 8784, 8980, 9171, 9346, 9521, 9691, 9858, 10031, 10208, 10382, 10553, 10733, 10916);

----------------------------------------
-- The Law WC

UPDATE
  team
SET
  name_normalized = 'The Law WC',
  url_path_slug = NULL
WHERE
  id = 11084;

UPDATE
  tournament_team
SET
  team_id = 11084
WHERE
  id IN (11084, 11246, 11408, 11570, 11779, 11992, 12205, 12418, 12642, 12869, 13096, 13323, 13583, 13853, 14123, 14393, 14663, 14933, 15203, 15473);

DELETE FROM
  team
WHERE
  id IN (11246, 11408, 11570, 11779, 11992, 12205, 12418, 12642, 12869, 13096, 13323, 13583, 13853, 14123, 14393, 14663, 14933, 15203, 15473);

----------------------------------------
-- The Wrestling Factory

UPDATE
  team
SET
  name_normalized = 'The Wrestling Factory',
  url_path_slug = NULL
WHERE
  id = 8594;

UPDATE
  tournament_team
SET
  team_id = 8594
WHERE
  id IN (8594, 8785, 8981, 9172, 9347, 9522, 9692, 9859, 10032, 10209, 10383, 10554);

DELETE FROM
  team
WHERE
  id IN (8785, 8981, 9172, 9347, 9522, 9692, 9859, 10032, 10209, 10383, 10554);

----------------------------------------
-- Thornwood Kids WC

UPDATE
  team
SET
  name_normalized = 'Thornwood Kids WC',
  url_path_slug = NULL
WHERE
  id = 1405;

UPDATE
  tournament_team
SET
  team_id = 1405
WHERE
  id IN (1405, 1522);

DELETE FROM
  team
WHERE
  id IN (1522);

----------------------------------------
-- Thunder WC

UPDATE
  team
SET
  name_normalized = 'Thunder WC',
  url_path_slug = NULL
WHERE
  id = 11780;

UPDATE
  tournament_team
SET
  team_id = 11780
WHERE
  id IN (11780, 11993, 12206, 12419, 12643, 12870, 13097, 13324, 13584, 13854, 14124, 14394, 14664, 14934, 15204, 15474);

DELETE FROM
  team
WHERE
  id IN (11993, 12206, 12419, 12643, 12870, 13097, 13324, 13584, 13854, 14124, 14394, 14664, 14934, 15204, 15474);

----------------------------------------
-- Thundercat Wrestling

UPDATE
  team
SET
  name_normalized = 'Thundercat Wrestling',
  url_path_slug = NULL
WHERE
  id = 9348;

UPDATE
  tournament_team
SET
  team_id = 9348
WHERE
  id IN (9348, 9523);

DELETE FROM
  team
WHERE
  id IN (9523);

----------------------------------------
-- Thundercat Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Thundercat Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 6348;

UPDATE
  tournament_team
SET
  team_id = 6348
WHERE
  id IN (6348, 6532);

DELETE FROM
  team
WHERE
  id IN (6532);

----------------------------------------
-- Thundercats

UPDATE
  team
SET
  name_normalized = 'Thundercats',
  url_path_slug = NULL
WHERE
  id = 9693;

UPDATE
  tournament_team
SET
  team_id = 9693
WHERE
  id IN (9693, 9860, 10033, 10210);

DELETE FROM
  team
WHERE
  id IN (9860, 10033, 10210);

----------------------------------------
-- Tiger Elite Wrestling Inc

UPDATE
  team
SET
  name_normalized = 'Tiger Elite Wrestling Inc',
  url_path_slug = NULL
WHERE
  id = 12644;

UPDATE
  tournament_team
SET
  team_id = 12644
WHERE
  id IN (12644, 12871, 13098, 13325);

DELETE FROM
  team
WHERE
  id IN (12871, 13098, 13325);

----------------------------------------
-- Tiger Elite Wrestling Inc.

UPDATE
  team
SET
  name_normalized = 'Tiger Elite Wrestling Inc.',
  url_path_slug = NULL
WHERE
  id = 13585;

UPDATE
  tournament_team
SET
  team_id = 13585
WHERE
  id IN (13585, 13855, 14125, 14395, 14665, 14935, 15205, 15475);

DELETE FROM
  team
WHERE
  id IN (13855, 14125, 14395, 14665, 14935, 15205, 15475);

----------------------------------------
-- Tiger Town Tanglers Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Tiger Town Tanglers Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1115;

UPDATE
  tournament_team
SET
  team_id = 1115
WHERE
  id IN (1115, 1523, 1651, 1781, 2061, 3786, 3912, 4026, 4150, 4281, 4449, 4617, 4887, 5057, 5231, 5381, 5532, 5671, 5820, 5990, 6169, 6350, 6534, 7087, 7275, 7466, 7654, 7846, 8036, 8219, 8406, 8595, 8786, 8982, 9173, 9349, 9524, 9694, 9861, 10034, 10211, 10384, 10555, 10734, 10917, 11409, 11571, 11781, 11994, 12207, 12420, 12645, 12872, 13099, 13326, 13586, 13856, 14126, 14396, 14666, 14936, 15206, 15476);

DELETE FROM
  team
WHERE
  id IN (1523, 1651, 1781, 2061, 3786, 3912, 4026, 4150, 4281, 4449, 4617, 4887, 5057, 5231, 5381, 5532, 5671, 5820, 5990, 6169, 6350, 6534, 7087, 7275, 7466, 7654, 7846, 8036, 8219, 8406, 8595, 8786, 8982, 9173, 9349, 9524, 9694, 9861, 10034, 10211, 10384, 10555, 10734, 10917, 11409, 11571, 11781, 11994, 12207, 12420, 12645, 12872, 13099, 13326, 13586, 13856, 14126, 14396, 14666, 14936, 15206, 15476);

----------------------------------------
-- Tiger WC

UPDATE
  team
SET
  name_normalized = 'Tiger WC',
  url_path_slug = NULL
WHERE
  id = 3911;

UPDATE
  tournament_team
SET
  team_id = 3911
WHERE
  id IN (3911, 4025, 4149, 4280, 4448, 4616, 4745, 4886, 5056, 5230, 5380, 5531, 5670, 5819, 5989, 6168, 6349, 6533, 6718, 6900, 8596, 8787);

DELETE FROM
  team
WHERE
  id IN (4025, 4149, 4280, 4448, 4616, 4745, 4886, 5056, 5230, 5380, 5531, 5670, 5819, 5989, 6168, 6349, 6533, 6718, 6900, 8596, 8787);

----------------------------------------
-- Tiger Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Tiger Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8983;

UPDATE
  tournament_team
SET
  team_id = 8983
WHERE
  id IN (8983, 9174);

DELETE FROM
  team
WHERE
  id IN (9174);

----------------------------------------
-- Timber Wolves WC

UPDATE
  team
SET
  name_normalized = 'Timber Wolves WC',
  url_path_slug = NULL
WHERE
  id = 7847;

UPDATE
  tournament_team
SET
  team_id = 7847
WHERE
  id IN (7847, 8037, 8220, 8407, 10385, 10556, 10735, 10918, 11085, 11247, 11782, 11995, 12208, 12421, 13587, 13857, 14127, 14397, 14667, 14937, 15207, 15477);

DELETE FROM
  team
WHERE
  id IN (8037, 8220, 8407, 10385, 10556, 10735, 10918, 11085, 11247, 11782, 11995, 12208, 12421, 13587, 13857, 14127, 14397, 14667, 14937, 15207, 15477);

----------------------------------------
-- Timberwolves WC

UPDATE
  team
SET
  name_normalized = 'Timberwolves WC',
  url_path_slug = NULL
WHERE
  id = 5991;

UPDATE
  tournament_team
SET
  team_id = 5991
WHERE
  id IN (5991, 6170, 6719, 6901);

DELETE FROM
  team
WHERE
  id IN (6170, 6719, 6901);

----------------------------------------
-- Tinley Park Bulldogs Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Tinley Park Bulldogs Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 149;

UPDATE
  tournament_team
SET
  team_id = 149
WHERE
  id IN (149, 169, 199, 550, 666, 778, 890, 999, 1116, 1222, 1406, 1524, 1652, 1782, 1926, 2062, 2189, 2339, 2485, 2624, 2918, 3230, 3335, 3667, 3787, 3913, 4027, 4151, 4282, 4450, 4618, 4746, 4888, 5058, 5232, 5382, 5533, 5672, 5821, 5992, 6171, 6351, 6535, 6720, 6902, 7088, 7276, 7467, 7655, 7848, 8038, 8221, 8408, 8597, 8788, 8984, 9175, 9350, 9525, 9695, 9862, 10035, 10212, 10386, 10557, 10736, 10919, 11086, 11248, 11410, 11572, 11783, 11996, 12209, 12422, 12646, 12873, 13100, 13327, 13588, 13858, 14128, 14398, 14668, 14938, 15208, 15478);

DELETE FROM
  team
WHERE
  id IN (169, 199, 550, 666, 778, 890, 999, 1116, 1222, 1406, 1524, 1652, 1782, 1926, 2062, 2189, 2339, 2485, 2624, 2918, 3230, 3335, 3667, 3787, 3913, 4027, 4151, 4282, 4450, 4618, 4746, 4888, 5058, 5232, 5382, 5533, 5672, 5821, 5992, 6171, 6351, 6535, 6720, 6902, 7088, 7276, 7467, 7655, 7848, 8038, 8221, 8408, 8597, 8788, 8984, 9175, 9350, 9525, 9695, 9862, 10035, 10212, 10386, 10557, 10736, 10919, 11086, 11248, 11410, 11572, 11783, 11996, 12209, 12422, 12646, 12873, 13100, 13327, 13588, 13858, 14128, 14398, 14668, 14938, 15208, 15478);

----------------------------------------
-- Titan 2028 WC

UPDATE
  team
SET
  name_normalized = 'Titan 2028 WC',
  url_path_slug = NULL
WHERE
  id = 10737;

UPDATE
  tournament_team
SET
  team_id = 10737
WHERE
  id IN (10737, 10920);

DELETE FROM
  team
WHERE
  id IN (10920);

----------------------------------------
-- Titan Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Titan Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 6721;

UPDATE
  tournament_team
SET
  team_id = 6721
WHERE
  id IN (6721, 6903, 7089, 7277);

DELETE FROM
  team
WHERE
  id IN (6903, 7089, 7277);

----------------------------------------
-- Tj Trained

UPDATE
  team
SET
  name_normalized = 'Tj Trained',
  url_path_slug = NULL
WHERE
  id = 8974;

UPDATE
  tournament_team
SET
  team_id = 8974
WHERE
  id IN (8974, 9165, 9339, 9514, 9685, 9852);

DELETE FROM
  team
WHERE
  id IN (9165, 9339, 9514, 9685, 9852);

----------------------------------------
-- Tj Trained Wrestling

UPDATE
  team
SET
  name_normalized = 'Tj Trained Wrestling',
  url_path_slug = NULL
WHERE
  id = 10025;

UPDATE
  tournament_team
SET
  team_id = 10025
WHERE
  id IN (10025, 10202, 10377, 10548, 10729, 10912, 11078, 11240, 11404, 11566, 11772, 11985, 12198, 12411, 12638, 12865, 13092, 13319, 13576, 13846, 14116, 14386, 14656, 14926, 15196, 15466);

DELETE FROM
  team
WHERE
  id IN (10202, 10377, 10548, 10729, 10912, 11078, 11240, 11404, 11566, 11772, 11985, 12198, 12411, 12638, 12865, 13092, 13319, 13576, 13846, 14116, 14386, 14656, 14926, 15196, 15466);

----------------------------------------
-- Tj Trained-Canton

UPDATE
  team
SET
  name_normalized = 'Tj Trained-Canton',
  url_path_slug = NULL
WHERE
  id = 11773;

UPDATE
  tournament_team
SET
  team_id = 11773
WHERE
  id IN (11773, 11986, 12199, 12412);

DELETE FROM
  team
WHERE
  id IN (11986, 12199, 12412);

----------------------------------------
-- Tj Trained-Decatur

UPDATE
  team
SET
  name_normalized = 'Tj Trained-Decatur',
  url_path_slug = NULL
WHERE
  id = 8975;

UPDATE
  tournament_team
SET
  team_id = 8975
WHERE
  id IN (8975, 9166);

DELETE FROM
  team
WHERE
  id IN (9166);

----------------------------------------
-- Tj Trained-Springfield

UPDATE
  team
SET
  name_normalized = 'Tj Trained-Springfield',
  url_path_slug = NULL
WHERE
  id = 9340;

UPDATE
  tournament_team
SET
  team_id = 9340
WHERE
  id IN (9340, 9515, 9686, 9853, 10026, 10203, 10378, 10549);

DELETE FROM
  team
WHERE
  id IN (9515, 9686, 9853, 10026, 10203, 10378, 10549);

----------------------------------------
-- Tj Trained-Springfield Wrestling

UPDATE
  team
SET
  name_normalized = 'Tj Trained-Springfield Wrestling',
  url_path_slug = NULL
WHERE
  id = 12639;

UPDATE
  tournament_team
SET
  team_id = 12639
WHERE
  id IN (12639, 12866, 13093, 13320);

DELETE FROM
  team
WHERE
  id IN (12866, 13093, 13320);

----------------------------------------
-- Tony Capriola Sr. WC

UPDATE
  team
SET
  name_normalized = 'Tony Capriola Sr. WC',
  url_path_slug = NULL
WHERE
  id = 13589;

UPDATE
  tournament_team
SET
  team_id = 13589
WHERE
  id IN (13589, 13859, 14129, 14399, 14669, 14939, 15209, 15479);

DELETE FROM
  team
WHERE
  id IN (13859, 14129, 14399, 14669, 14939, 15209, 15479);

----------------------------------------
-- Top Dog WC

UPDATE
  team
SET
  name_normalized = 'Top Dog WC',
  url_path_slug = NULL
WHERE
  id = 13590;

UPDATE
  tournament_team
SET
  team_id = 13590
WHERE
  id IN (13590, 13860, 14130, 14400, 14670, 14940, 15210, 15480);

DELETE FROM
  team
WHERE
  id IN (13860, 14130, 14400, 14670, 14940, 15210, 15480);

----------------------------------------
-- Toss Em Up Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Toss Em Up Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 10037;

UPDATE
  tournament_team
SET
  team_id = 10037
WHERE
  id IN (10037, 10214, 10388, 10559, 10739, 10922, 11087, 11249, 11411, 11573, 11784, 11997, 12210, 12423, 12647, 12874, 13101, 13328, 13591, 13861, 14131, 14401, 14671, 14941, 15211, 15481);

DELETE FROM
  team
WHERE
  id IN (10214, 10388, 10559, 10739, 10922, 11087, 11249, 11411, 11573, 11784, 11997, 12210, 12423, 12647, 12874, 13101, 13328, 13591, 13861, 14131, 14401, 14671, 14941, 15211, 15481);

----------------------------------------
-- Trailblazers

UPDATE
  team
SET
  name_normalized = 'Trailblazers',
  url_path_slug = NULL
WHERE
  id = 1654;

UPDATE
  tournament_team
SET
  team_id = 1654
WHERE
  id IN (1654, 1928);

DELETE FROM
  team
WHERE
  id IN (1928);

----------------------------------------
-- Trevian WC

UPDATE
  team
SET
  name_normalized = 'Trevian WC',
  url_path_slug = NULL
WHERE
  id = 4889;

UPDATE
  tournament_team
SET
  team_id = 4889
WHERE
  id IN (4889, 5060, 5234, 5673, 5823, 6353, 6537, 7091, 7279, 7469, 7657);

DELETE FROM
  team
WHERE
  id IN (5060, 5234, 5673, 5823, 6353, 6537, 7091, 7279, 7469, 7657);

----------------------------------------
-- Tri-City Braves

UPDATE
  team
SET
  name_normalized = 'Tri-City Braves',
  url_path_slug = NULL
WHERE
  id = 1526;

UPDATE
  tournament_team
SET
  team_id = 1526
WHERE
  id IN (1526, 1655, 1929, 3914, 4028, 4283, 5061, 5235);

DELETE FROM
  team
WHERE
  id IN (1655, 1929, 3914, 4028, 4283, 5061, 5235);

----------------------------------------
-- Tri-Valley WC

UPDATE
  team
SET
  name_normalized = 'Tri-Valley WC',
  url_path_slug = NULL
WHERE
  id = 5384;

UPDATE
  tournament_team
SET
  team_id = 5384
WHERE
  id IN (5384, 5535, 5824);

DELETE FROM
  team
WHERE
  id IN (5535, 5824);

----------------------------------------
-- Triad Knights

UPDATE
  team
SET
  name_normalized = 'Triad Knights',
  url_path_slug = NULL
WHERE
  id = 4452;

UPDATE
  tournament_team
SET
  team_id = 4452
WHERE
  id IN (4452, 4620, 4748, 4890, 5062, 5236);

DELETE FROM
  team
WHERE
  id IN (4620, 4748, 4890, 5062, 5236);

----------------------------------------
-- Triad Knights WC

UPDATE
  team
SET
  name_normalized = 'Triad Knights WC',
  url_path_slug = NULL
WHERE
  id = 10389;

UPDATE
  tournament_team
SET
  team_id = 10389
WHERE
  id IN (10389, 10560, 10740, 10923, 11088, 11250, 11412, 11574, 13592, 13862, 14132, 14402, 14672, 14942, 15212, 15482);

DELETE FROM
  team
WHERE
  id IN (10560, 10740, 10923, 11088, 11250, 11412, 11574, 13592, 13862, 14132, 14402, 14672, 14942, 15212, 15482);

----------------------------------------
-- Triad Little Knights

UPDATE
  team
SET
  name_normalized = 'Triad Little Knights',
  url_path_slug = NULL
WHERE
  id = 5385;

UPDATE
  tournament_team
SET
  team_id = 5385
WHERE
  id IN (5385, 5536, 5674, 5825, 5994, 6173, 6354, 6538, 6723, 6905, 7092, 7280, 7470, 7658, 7849, 8039);

DELETE FROM
  team
WHERE
  id IN (5536, 5674, 5825, 5994, 6173, 6354, 6538, 6723, 6905, 7092, 7280, 7470, 7658, 7849, 8039);

----------------------------------------
-- Triad Little Knights WC

UPDATE
  team
SET
  name_normalized = 'Triad Little Knights WC',
  url_path_slug = NULL
WHERE
  id = 8223;

UPDATE
  tournament_team
SET
  team_id = 8223
WHERE
  id IN (8223, 8410, 8599, 8790, 9351, 9526, 9697, 9864, 10038, 10215);

DELETE FROM
  team
WHERE
  id IN (8410, 8599, 8790, 9351, 9526, 9697, 9864, 10038, 10215);

----------------------------------------
-- Triad Little Knights Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Triad Little Knights Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8985;

UPDATE
  tournament_team
SET
  team_id = 8985
WHERE
  id IN (8985, 9176);

DELETE FROM
  team
WHERE
  id IN (9176);

----------------------------------------
-- Tribe WC

UPDATE
  team
SET
  name_normalized = 'Tribe WC',
  url_path_slug = NULL
WHERE
  id = 10741;

UPDATE
  tournament_team
SET
  team_id = 10741
WHERE
  id IN (10741, 10924, 11089, 11251, 11413, 11575, 11785, 11998, 12211, 12424, 12648, 12875, 13102, 13329);

DELETE FROM
  team
WHERE
  id IN (10924, 11089, 11251, 11413, 11575, 11785, 11998, 12211, 12424, 12648, 12875, 13102, 13329);

----------------------------------------
-- Trico WC

UPDATE
  team
SET
  name_normalized = 'Trico WC',
  url_path_slug = NULL
WHERE
  id = 10039;

UPDATE
  tournament_team
SET
  team_id = 10039
WHERE
  id IN (10039, 10216, 10390, 10561, 11090, 11252, 11414, 11576, 11786, 11999, 12212, 12425, 12649, 12876, 13103, 13330, 13593, 13863, 14133, 14403, 14673, 14943, 15213, 15483);

DELETE FROM
  team
WHERE
  id IN (10216, 10390, 10561, 11090, 11252, 11414, 11576, 11786, 11999, 12212, 12425, 12649, 12876, 13103, 13330, 13593, 13863, 14133, 14403, 14673, 14943, 15213, 15483);

----------------------------------------
-- Triple D WC

UPDATE
  team
SET
  name_normalized = 'Triple D WC',
  url_path_slug = NULL
WHERE
  id = 6355;

UPDATE
  tournament_team
SET
  team_id = 6355
WHERE
  id IN (6355, 6539);

DELETE FROM
  team
WHERE
  id IN (6539);

----------------------------------------
-- Triumph `The Art Of Wrestling`

UPDATE
  team
SET
  name_normalized = 'Triumph `The Art Of Wrestling`',
  url_path_slug = NULL
WHERE
  id = 7471;

UPDATE
  tournament_team
SET
  team_id = 7471
WHERE
  id IN (7471, 7659, 7850, 8040);

DELETE FROM
  team
WHERE
  id IN (7659, 7850, 8040);

----------------------------------------
-- Turk WC

UPDATE
  team
SET
  name_normalized = 'Turk WC',
  url_path_slug = NULL
WHERE
  id = 1118;

UPDATE
  tournament_team
SET
  team_id = 1118
WHERE
  id IN (1118, 1224);

DELETE FROM
  team
WHERE
  id IN (1224);

----------------------------------------
-- Twin City WC

UPDATE
  team
SET
  name_normalized = 'Twin City WC',
  url_path_slug = NULL
WHERE
  id = 5063;

UPDATE
  tournament_team
SET
  team_id = 5063
WHERE
  id IN (5063, 5237);

DELETE FROM
  team
WHERE
  id IN (5237);

----------------------------------------
-- United Southern Allstars

UPDATE
  team
SET
  name_normalized = 'United Southern Allstars',
  url_path_slug = NULL
WHERE
  id = 6724;

UPDATE
  tournament_team
SET
  team_id = 6724
WHERE
  id IN (6724, 6906, 7093, 7281);

DELETE FROM
  team
WHERE
  id IN (6906, 7093, 7281);

----------------------------------------
-- United Township WC

UPDATE
  team
SET
  name_normalized = 'United Township WC',
  url_path_slug = NULL
WHERE
  id = 4749;

UPDATE
  tournament_team
SET
  team_id = 4749
WHERE
  id IN (4749, 4891, 5064, 5238, 5537);

DELETE FROM
  team
WHERE
  id IN (4891, 5064, 5238, 5537);

----------------------------------------
-- Unity Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Unity Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 1000;

UPDATE
  tournament_team
SET
  team_id = 1000
WHERE
  id IN (1000, 1225, 1529, 1656, 3788, 3915, 4030, 4152, 4453, 4621, 4892, 5065, 5239, 5386, 5538, 5675, 5826, 5995, 6174, 7851, 8041, 8224, 8411, 8600, 8791, 8986, 9177, 9352, 9527, 9699, 9866, 10041, 10218, 10392, 10563, 10742, 10925, 11091, 11253, 11415, 11577, 11787, 12000, 12213, 12426, 12650, 12877, 13104, 13331, 13594, 13864, 14134, 14404, 14674, 14944, 15214, 15484);

DELETE FROM
  team
WHERE
  id IN (1225, 1529, 1656, 3788, 3915, 4030, 4152, 4453, 4621, 4892, 5065, 5239, 5386, 5538, 5675, 5826, 5995, 6174, 7851, 8041, 8224, 8411, 8600, 8791, 8986, 9177, 9352, 9527, 9699, 9866, 10041, 10218, 10392, 10563, 10742, 10925, 11091, 11253, 11415, 11577, 11787, 12000, 12213, 12426, 12650, 12877, 13104, 13331, 13594, 13864, 14134, 14404, 14674, 14944, 15214, 15484);

----------------------------------------
-- Up Town WC

UPDATE
  team
SET
  name_normalized = 'Up Town WC',
  url_path_slug = NULL
WHERE
  id = 9698;

UPDATE
  tournament_team
SET
  team_id = 9698
WHERE
  id IN (9698, 9865, 10040, 10217, 10391, 10562);

DELETE FROM
  team
WHERE
  id IN (9865, 10040, 10217, 10391, 10562);

----------------------------------------
-- Urbana Kids Club

UPDATE
  team
SET
  name_normalized = 'Urbana Kids Club',
  url_path_slug = NULL
WHERE
  id = 1001;

UPDATE
  tournament_team
SET
  team_id = 1001
WHERE
  id IN (1001, 1226, 1530, 1657, 1787, 3789);

DELETE FROM
  team
WHERE
  id IN (1226, 1530, 1657, 1787, 3789);

----------------------------------------
-- Urbana Tigers WC

UPDATE
  team
SET
  name_normalized = 'Urbana Tigers WC',
  url_path_slug = NULL
WHERE
  id = 9353;

UPDATE
  tournament_team
SET
  team_id = 9353
WHERE
  id IN (9353, 9528, 9700, 9867, 10042, 10219, 10393, 10564, 10743, 10926, 11092, 11254, 11416, 11578, 11788, 12001, 12214, 12427, 12651, 12878, 13105, 13332, 13595, 13865, 14135, 14405, 14675, 14945, 15215, 15485);

DELETE FROM
  team
WHERE
  id IN (9528, 9700, 9867, 10042, 10219, 10393, 10564, 10743, 10926, 11092, 11254, 11416, 11578, 11788, 12001, 12214, 12427, 12651, 12878, 13105, 13332, 13595, 13865, 14135, 14405, 14675, 14945, 15215, 15485);

----------------------------------------
-- Vandal Kings

UPDATE
  team
SET
  name_normalized = 'Vandal Kings',
  url_path_slug = NULL
WHERE
  id = 10744;

UPDATE
  tournament_team
SET
  team_id = 10744
WHERE
  id IN (10744, 10927);

DELETE FROM
  team
WHERE
  id IN (10927);

----------------------------------------
-- Vandalia Jr Vandals

UPDATE
  team
SET
  name_normalized = 'Vandalia Jr Vandals',
  url_path_slug = NULL
WHERE
  id = 7472;

UPDATE
  tournament_team
SET
  team_id = 7472
WHERE
  id IN (7472, 7660);

DELETE FROM
  team
WHERE
  id IN (7660);

----------------------------------------
-- Vandalia Jr WC

UPDATE
  team
SET
  name_normalized = 'Vandalia Jr WC',
  url_path_slug = NULL
WHERE
  id = 8225;

UPDATE
  tournament_team
SET
  team_id = 8225
WHERE
  id IN (8225, 8412, 8601, 8792, 9354, 9529, 9701, 9868, 10043, 10220, 10394, 10565, 10745, 10928, 11093, 11255, 11417, 11579, 11789, 12002, 12215, 12428, 12652, 12879, 13106, 13333, 13596, 13866, 14136, 14406, 14676, 14946, 15216, 15486);

DELETE FROM
  team
WHERE
  id IN (8412, 8601, 8792, 9354, 9529, 9701, 9868, 10043, 10220, 10394, 10565, 10745, 10928, 11093, 11255, 11417, 11579, 11789, 12002, 12215, 12428, 12652, 12879, 13106, 13333, 13596, 13866, 14136, 14406, 14676, 14946, 15216, 15486);

----------------------------------------
-- Vandalia Jr Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Vandalia Jr Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8987;

UPDATE
  tournament_team
SET
  team_id = 8987
WHERE
  id IN (8987, 9178);

DELETE FROM
  team
WHERE
  id IN (9178);

----------------------------------------
-- Vandalia Jr. Wrestling

UPDATE
  team
SET
  name_normalized = 'Vandalia Jr. Wrestling',
  url_path_slug = NULL
WHERE
  id = 3670;

UPDATE
  tournament_team
SET
  team_id = 3670
WHERE
  id IN (3670, 3791, 3916, 4031, 4153, 4284, 4454, 4622, 4750, 4893, 5066, 5240, 5387, 5539, 5676, 5996, 6175, 6356, 6540, 6725, 6907, 7094, 7282);

DELETE FROM
  team
WHERE
  id IN (3791, 3916, 4031, 4153, 4284, 4454, 4622, 4750, 4893, 5066, 5240, 5387, 5539, 5676, 5996, 6175, 6356, 6540, 6725, 6907, 7094, 7282);

----------------------------------------
-- Vandalia Kings

UPDATE
  team
SET
  name_normalized = 'Vandalia Kings',
  url_path_slug = NULL
WHERE
  id = 8602;

UPDATE
  tournament_team
SET
  team_id = 8602
WHERE
  id IN (8602, 8793, 8988, 9179);

DELETE FROM
  team
WHERE
  id IN (8793, 8988, 9179);

----------------------------------------
-- Vermilion Valley Elite

UPDATE
  team
SET
  name_normalized = 'Vermilion Valley Elite',
  url_path_slug = NULL
WHERE
  id = 5677;

UPDATE
  tournament_team
SET
  team_id = 5677
WHERE
  id IN (5677, 5997, 6176);

DELETE FROM
  team
WHERE
  id IN (5997, 6176);

----------------------------------------
-- Victory Elite Wrestling

UPDATE
  team
SET
  name_normalized = 'Victory Elite Wrestling',
  url_path_slug = NULL
WHERE
  id = 13597;

UPDATE
  tournament_team
SET
  team_id = 13597
WHERE
  id IN (13597, 13867, 14137, 14407, 14677, 14947, 15217, 15487);

DELETE FROM
  team
WHERE
  id IN (13867, 14137, 14407, 14677, 14947, 15217, 15487);

----------------------------------------
-- Viking Gold WC

UPDATE
  team
SET
  name_normalized = 'Viking Gold WC',
  url_path_slug = NULL
WHERE
  id = 6357;

UPDATE
  tournament_team
SET
  team_id = 6357
WHERE
  id IN (6357, 6541);

DELETE FROM
  team
WHERE
  id IN (6541);

----------------------------------------
-- Villa Park Young Warriors WC

UPDATE
  team
SET
  name_normalized = 'Villa Park Young Warriors WC',
  url_path_slug = NULL
WHERE
  id = 3918;

UPDATE
  tournament_team
SET
  team_id = 3918
WHERE
  id IN (3918, 4033, 4286, 5679, 5999, 6178, 6359, 6543, 6727, 6909, 7096, 7284, 7474, 7662, 7852, 8042, 8226, 8413, 9355, 9530, 10746, 10929, 12653, 12880, 13107, 13334, 13598, 13868, 14138, 14408, 14678, 14948, 15218, 15488);

DELETE FROM
  team
WHERE
  id IN (4033, 4286, 5679, 5999, 6178, 6359, 6543, 6727, 6909, 7096, 7284, 7474, 7662, 7852, 8042, 8226, 8413, 9355, 9530, 10746, 10929, 12653, 12880, 13107, 13334, 13598, 13868, 14138, 14408, 14678, 14948, 15218, 15488);

----------------------------------------
-- Villa-Lombard Cougars

UPDATE
  team
SET
  name_normalized = 'Villa-Lombard Cougars',
  url_path_slug = NULL
WHERE
  id = 1002;

UPDATE
  tournament_team
SET
  team_id = 1002
WHERE
  id IN (1002, 1121, 1227, 1342, 1408, 1531, 1658, 1790, 1936, 2069, 2196, 2346, 2492, 2631, 3234, 3671, 3792, 3917, 4032, 4154, 4285, 4455, 4623, 4751, 4894, 5067, 5241, 5388, 5540, 5678, 5827, 5998, 6177, 6358, 6542, 6726, 6908, 7095, 7283, 7473, 7661, 7853, 8043, 8227, 8414, 8603, 8794, 8989, 9180, 9356, 9531, 9702, 9869, 10044, 10221, 10395, 10566, 10747, 10930);

DELETE FROM
  team
WHERE
  id IN (1121, 1227, 1342, 1408, 1531, 1658, 1790, 1936, 2069, 2196, 2346, 2492, 2631, 3234, 3671, 3792, 3917, 4032, 4154, 4285, 4455, 4623, 4751, 4894, 5067, 5241, 5388, 5540, 5678, 5827, 5998, 6177, 6358, 6542, 6726, 6908, 7095, 7283, 7473, 7661, 7853, 8043, 8227, 8414, 8603, 8794, 8989, 9180, 9356, 9531, 9702, 9869, 10044, 10221, 10395, 10566, 10747, 10930);

----------------------------------------
-- Vittum Cats

UPDATE
  team
SET
  name_normalized = 'Vittum Cats',
  url_path_slug = NULL
WHERE
  id = 895;

UPDATE
  tournament_team
SET
  team_id = 895
WHERE
  id IN (895, 1003, 1122, 1228, 1343, 1409, 1532, 1659, 1791, 1937, 2197, 2347, 2778, 2925, 3235, 3673, 3794, 3919, 4034, 4155, 4287, 4456, 4624, 4752, 4895, 5068, 5242, 5389, 5541, 5680, 5828, 6000, 6179, 6360, 6544, 6728, 6910, 7097, 7285, 7475, 7663, 7854, 8044, 8228, 8415, 8604, 8795, 8990, 9181, 10748, 10931, 11094, 11256, 11418, 11580, 11790, 12003, 12216, 12429, 12654, 12881, 13108, 13335, 13599, 13869, 14139, 14409, 14679, 14949, 15219, 15489);

DELETE FROM
  team
WHERE
  id IN (1003, 1122, 1228, 1343, 1409, 1532, 1659, 1791, 1937, 2197, 2347, 2778, 2925, 3235, 3673, 3794, 3919, 4034, 4155, 4287, 4456, 4624, 4752, 4895, 5068, 5242, 5389, 5541, 5680, 5828, 6000, 6179, 6360, 6544, 6728, 6910, 7097, 7285, 7475, 7663, 7854, 8044, 8228, 8415, 8604, 8795, 8990, 9181, 10748, 10931, 11094, 11256, 11418, 11580, 11790, 12003, 12216, 12429, 12654, 12881, 13108, 13335, 13599, 13869, 14139, 14409, 14679, 14949, 15219, 15489);

----------------------------------------
-- Vittum Cats Of SKT

UPDATE
  team
SET
  name_normalized = 'Vittum Cats Of SKT',
  url_path_slug = NULL
WHERE
  id = 10045;

UPDATE
  tournament_team
SET
  team_id = 10045
WHERE
  id IN (10045, 10222, 10396, 10567);

DELETE FROM
  team
WHERE
  id IN (10222, 10396, 10567);

----------------------------------------
-- Vittum Vikings

UPDATE
  team
SET
  name_normalized = 'Vittum Vikings',
  url_path_slug = NULL
WHERE
  id = 670;

UPDATE
  tournament_team
SET
  team_id = 670
WHERE
  id IN (670, 782);

DELETE FROM
  team
WHERE
  id IN (782);

----------------------------------------
-- Vortex WC

UPDATE
  team
SET
  name_normalized = 'Vortex WC',
  url_path_slug = NULL
WHERE
  id = 13600;

UPDATE
  tournament_team
SET
  team_id = 13600
WHERE
  id IN (13600, 13870, 14140, 14410, 14680, 14950, 15220, 15490);

DELETE FROM
  team
WHERE
  id IN (13870, 14140, 14410, 14680, 14950, 15220, 15490);

----------------------------------------
-- Warhawks

UPDATE
  team
SET
  name_normalized = 'Warhawks',
  url_path_slug = NULL
WHERE
  id = 1660;

UPDATE
  tournament_team
SET
  team_id = 1660
WHERE
  id IN (1660, 1793, 2072);

DELETE FROM
  team
WHERE
  id IN (1793, 2072);

----------------------------------------
-- Warren County Brawlers

UPDATE
  team
SET
  name_normalized = 'Warren County Brawlers',
  url_path_slug = NULL
WHERE
  id = 7477;

UPDATE
  tournament_team
SET
  team_id = 7477
WHERE
  id IN (7477, 7665);

DELETE FROM
  team
WHERE
  id IN (7665);

----------------------------------------
-- Warren County WC

UPDATE
  team
SET
  name_normalized = 'Warren County WC',
  url_path_slug = NULL
WHERE
  id = 5390;

UPDATE
  tournament_team
SET
  team_id = 5390
WHERE
  id IN (5390, 6361, 6545, 7098, 7286);

DELETE FROM
  team
WHERE
  id IN (6361, 6545, 7098, 7286);

----------------------------------------
-- Warrensburg WC

UPDATE
  team
SET
  name_normalized = 'Warrensburg WC',
  url_path_slug = NULL
WHERE
  id = 4156;

UPDATE
  tournament_team
SET
  team_id = 4156
WHERE
  id IN (4156, 4288, 4457, 4625, 4753, 4896, 5069, 5243, 5542, 5681, 6002, 6181, 6362, 6546, 6729, 6911);

DELETE FROM
  team
WHERE
  id IN (4288, 4457, 4625, 4753, 4896, 5069, 5243, 5542, 5681, 6002, 6181, 6362, 6546, 6729, 6911);

----------------------------------------
-- Warrensburg-Latham Jr Cardinals WC

UPDATE
  team
SET
  name_normalized = 'Warrensburg-Latham Jr Cardinals WC',
  url_path_slug = NULL
WHERE
  id = 8230;

UPDATE
  tournament_team
SET
  team_id = 8230
WHERE
  id IN (8230, 8417, 8991, 9182);

DELETE FROM
  team
WHERE
  id IN (8417, 8991, 9182);

----------------------------------------
-- Warrensburg-Latham Jr. Cardinals WC

UPDATE
  team
SET
  name_normalized = 'Warrensburg-Latham Jr. Cardinals WC',
  url_path_slug = NULL
WHERE
  id = 12655;

UPDATE
  tournament_team
SET
  team_id = 12655
WHERE
  id IN (12655, 12882, 13109, 13336, 13601, 13871, 14141, 14411, 14681, 14951, 15221, 15491);

DELETE FROM
  team
WHERE
  id IN (12882, 13109, 13336, 13601, 13871, 14141, 14411, 14681, 14951, 15221, 15491);

----------------------------------------
-- Warrior Monks

UPDATE
  team
SET
  name_normalized = 'Warrior Monks',
  url_path_slug = NULL
WHERE
  id = 6730;

UPDATE
  tournament_team
SET
  team_id = 6730
WHERE
  id IN (6730, 6912, 7099, 7287, 7478, 7666, 8606, 8797, 8992, 9183);

DELETE FROM
  team
WHERE
  id IN (6912, 7099, 7287, 7478, 7666, 8606, 8797, 8992, 9183);

----------------------------------------
-- Warrior Wrestling

UPDATE
  team
SET
  name_normalized = 'Warrior Wrestling',
  url_path_slug = NULL
WHERE
  id = 8993;

UPDATE
  tournament_team
SET
  team_id = 8993
WHERE
  id IN (8993, 9184, 9357, 9532, 11419, 11581);

DELETE FROM
  team
WHERE
  id IN (9184, 9357, 9532, 11419, 11581);

----------------------------------------
-- Warriors Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Warriors Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 12656;

UPDATE
  tournament_team
SET
  team_id = 12656
WHERE
  id IN (12656, 12883, 13110, 13337, 13602, 13872, 14142, 14412, 14682, 14952, 15222, 15492);

DELETE FROM
  team
WHERE
  id IN (12883, 13110, 13337, 13602, 13872, 14142, 14412, 14682, 14952, 15222, 15492);

----------------------------------------
-- Washington Jr. Panthers

UPDATE
  team
SET
  name_normalized = 'Washington Jr. Panthers',
  url_path_slug = NULL
WHERE
  id = 3920;

UPDATE
  tournament_team
SET
  team_id = 3920
WHERE
  id IN (3920, 4035, 4289, 5391, 5682, 6363, 6547);

DELETE FROM
  team
WHERE
  id IN (4035, 4289, 5391, 5682, 6363, 6547);

----------------------------------------
-- Waterloo Bulldog Wrestling

UPDATE
  team
SET
  name_normalized = 'Waterloo Bulldog Wrestling',
  url_path_slug = NULL
WHERE
  id = 10046;

UPDATE
  tournament_team
SET
  team_id = 10046
WHERE
  id IN (10046, 10223, 10749, 10932, 11095, 11257);

DELETE FROM
  team
WHERE
  id IN (10223, 10749, 10932, 11095, 11257);

----------------------------------------
-- Waubonsie WC

UPDATE
  team
SET
  name_normalized = 'Waubonsie WC',
  url_path_slug = NULL
WHERE
  id = 4458;

UPDATE
  tournament_team
SET
  team_id = 4458
WHERE
  id IN (4458, 4626, 4754, 4897, 5070, 5244, 5543, 5829, 6003, 6182, 6364, 6548, 6731, 6913, 7100, 7288, 7479, 7667, 7856, 8046, 8231, 8418, 8607, 8798);

DELETE FROM
  team
WHERE
  id IN (4626, 4754, 4897, 5070, 5244, 5543, 5829, 6003, 6182, 6364, 6548, 6731, 6913, 7100, 7288, 7479, 7667, 7856, 8046, 8231, 8418, 8607, 8798);

----------------------------------------
-- Waubonsie Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Waubonsie Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8994;

UPDATE
  tournament_team
SET
  team_id = 8994
WHERE
  id IN (8994, 9185);

DELETE FROM
  team
WHERE
  id IN (9185);

----------------------------------------
-- Waukegan

UPDATE
  team
SET
  name_normalized = 'Waukegan',
  url_path_slug = NULL
WHERE
  id = 1940;

UPDATE
  tournament_team
SET
  team_id = 1940
WHERE
  id IN (1940, 2073);

DELETE FROM
  team
WHERE
  id IN (2073);

----------------------------------------
-- Waukegan Takedown

UPDATE
  team
SET
  name_normalized = 'Waukegan Takedown',
  url_path_slug = NULL
WHERE
  id = 4459;

UPDATE
  tournament_team
SET
  team_id = 4459
WHERE
  id IN (4459, 4627);

DELETE FROM
  team
WHERE
  id IN (4627);

----------------------------------------
-- Waukegan Youth WC

UPDATE
  team
SET
  name_normalized = 'Waukegan Youth WC',
  url_path_slug = NULL
WHERE
  id = 5683;

UPDATE
  tournament_team
SET
  team_id = 5683
WHERE
  id IN (5683, 6004, 6183, 6365, 6549, 6732, 6914, 7101, 7289, 7480, 7668, 7857, 8047, 8232, 8419, 8608, 8799, 9358, 9533, 9703, 9870);

DELETE FROM
  team
WHERE
  id IN (6004, 6183, 6365, 6549, 6732, 6914, 7101, 7289, 7480, 7668, 7857, 8047, 8232, 8419, 8608, 8799, 9358, 9533, 9703, 9870);

----------------------------------------
-- Waukegan Youth Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Waukegan Youth Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8995;

UPDATE
  tournament_team
SET
  team_id = 8995
WHERE
  id IN (8995, 9186);

DELETE FROM
  team
WHERE
  id IN (9186);

----------------------------------------
-- West Carroll WC

UPDATE
  team
SET
  name_normalized = 'West Carroll WC',
  url_path_slug = NULL
WHERE
  id = 6366;

UPDATE
  tournament_team
SET
  team_id = 6366
WHERE
  id IN (6366, 6550, 6733, 6915, 7102, 7290, 8233, 8420, 8609, 8800);

DELETE FROM
  team
WHERE
  id IN (6550, 6733, 6915, 7102, 7290, 8233, 8420, 8609, 8800);

----------------------------------------
-- West Carroll Wrestling Club

UPDATE
  team
SET
  name_normalized = 'West Carroll Wrestling Club',
  url_path_slug = NULL
WHERE
  id = 8996;

UPDATE
  tournament_team
SET
  team_id = 8996
WHERE
  id IN (8996, 9187);

DELETE FROM
  team
WHERE
  id IN (9187);

----------------------------------------
-- West Chicago

UPDATE
  team
SET
  name_normalized = 'West Chicago',
  url_path_slug = NULL
WHERE
  id = 78;

UPDATE
  tournament_team
SET
  team_id = 78
WHERE
  id IN (78, 152, 170, 200, 231, 557, 896);

DELETE FROM
  team
WHERE
  id IN (152, 170, 200, 231, 557, 896);

----------------------------------------
-- West Chicago Junior Wildcats

UPDATE
  team
SET
  name_normalized = 'West Chicago Junior Wildcats',
  url_path_slug = NULL
WHERE
  id = 6005;

UPDATE
  tournament_team
SET
  team_id = 6005
WHERE
  id IN (6005, 6184, 6367, 6551, 6734, 6916);

DELETE FROM
  team
WHERE
  id IN (6184, 6367, 6551, 6734, 6916);

----------------------------------------
-- West Chicago Park District

UPDATE
  team
SET
  name_normalized = 'West Chicago Park District',
  url_path_slug = NULL
WHERE
  id = 1662;

UPDATE
  tournament_team
SET
  team_id = 1662
WHERE
  id IN (1662, 4159);

DELETE FROM
  team
WHERE
  id IN (4159);

----------------------------------------
-- West Frankfort Jr WC

UPDATE
  team
SET
  name_normalized = 'West Frankfort Jr WC',
  url_path_slug = NULL
WHERE
  id = 3921;

UPDATE
  tournament_team
SET
  team_id = 3921
WHERE
  id IN (3921, 4036, 4160, 4292);

DELETE FROM
  team
WHERE
  id IN (4036, 4160, 4292);

----------------------------------------
-- West Frankfort Jr Wrestling

UPDATE
  team
SET
  name_normalized = 'West Frankfort Jr Wrestling',
  url_path_slug = NULL
WHERE
  id = 6006;

UPDATE
  tournament_team
SET
  team_id = 6006
WHERE
  id IN (6006, 6185, 6368, 6552, 6735, 6917, 7103, 7291);

DELETE FROM
  team
WHERE
  id IN (6185, 6368, 6552, 6735, 6917, 7103, 7291);

----------------------------------------
-- West Frankfort Jr. Redbirds

UPDATE
  team
SET
  name_normalized = 'West Frankfort Jr. Redbirds',
  url_path_slug = NULL
WHERE
  id = 8610;

UPDATE
  tournament_team
SET
  team_id = 8610
WHERE
  id IN (8610, 8801, 8997, 9188, 9359, 9534, 10397, 10568);

DELETE FROM
  team
WHERE
  id IN (8801, 8997, 9188, 9359, 9534, 10397, 10568);

----------------------------------------
-- West Frankfort Jr. Redbirds WC

UPDATE
  team
SET
  name_normalized = 'West Frankfort Jr. Redbirds WC',
  url_path_slug = NULL
WHERE
  id = 11420;

UPDATE
  tournament_team
SET
  team_id = 11420
WHERE
  id IN (11420, 11582, 11791, 12004, 12217, 12430, 12657, 12884, 13111, 13338, 13603, 13873, 14143, 14413, 14683, 14953, 15223, 15493);

DELETE FROM
  team
WHERE
  id IN (11582, 11791, 12004, 12217, 12430, 12657, 12884, 13111, 13338, 13603, 13873, 14143, 14413, 14683, 14953, 15223, 15493);

----------------------------------------
-- West Frankfort Jr. WC

UPDATE
  team
SET
  name_normalized = 'West Frankfort Jr. WC',
  url_path_slug = NULL
WHERE
  id = 4460;

UPDATE
  tournament_team
SET
  team_id = 4460
WHERE
  id IN (4460, 4628, 4898, 5071, 5245);

DELETE FROM
  team
WHERE
  id IN (4628, 4898, 5071, 5245);

----------------------------------------
-- West Hancock Titan WC

UPDATE
  team
SET
  name_normalized = 'West Hancock Titan WC',
  url_path_slug = NULL
WHERE
  id = 6007;

UPDATE
  tournament_team
SET
  team_id = 6007
WHERE
  id IN (6007, 6186, 6369, 6553, 6736, 6918, 7104, 7292, 7481, 7669);

DELETE FROM
  team
WHERE
  id IN (6186, 6369, 6553, 6736, 6918, 7104, 7292, 7481, 7669);

----------------------------------------
-- West Hancock WC

UPDATE
  team
SET
  name_normalized = 'West Hancock WC',
  url_path_slug = NULL
WHERE
  id = 5392;

UPDATE
  tournament_team
SET
  team_id = 5392
WHERE
  id IN (5392, 5830, 7858, 8048, 8234, 8421, 8611, 8802, 10750, 10933, 11096, 11258);

DELETE FROM
  team
WHERE
  id IN (5830, 7858, 8048, 8234, 8421, 8611, 8802, 10750, 10933, 11096, 11258);

----------------------------------------
-- West Leyden

UPDATE
  team
SET
  name_normalized = 'West Leyden',
  url_path_slug = NULL
WHERE
  id = 439;

UPDATE
  tournament_team
SET
  team_id = 439
WHERE
  id IN (439, 455);

DELETE FROM
  team
WHERE
  id IN (455);

----------------------------------------
-- West Suburban Girls Wrestling Club

UPDATE
  team
SET
  name_normalized = 'West Suburban Girls Wrestling Club',
  url_path_slug = 'wsgw'
WHERE
  id = 11792;

UPDATE
  tournament_team
SET
  team_id = 11792
WHERE
  id IN (11792, 12005, 12218, 12431, 12658, 12885, 13112, 13339, 13604, 13874, 14144, 14414, 14684, 14954, 15224, 15494, 16180, 16316, 16455, 16593);

DELETE FROM
  team
WHERE
  id IN (12005, 12218, 12431, 12658, 12885, 13112, 13339, 13604, 13874, 14144, 14414, 14684, 14954, 15224, 15494, 16180, 16316, 16455, 16593);

----------------------------------------
-- Western Illinois All-Stars

UPDATE
  team
SET
  name_normalized = 'Western Illinois All-Stars',
  url_path_slug = NULL
WHERE
  id = 8235;

UPDATE
  tournament_team
SET
  team_id = 8235
WHERE
  id IN (8235, 8422, 8612, 8803);

DELETE FROM
  team
WHERE
  id IN (8422, 8612, 8803);

----------------------------------------
-- Westville Jr. High

UPDATE
  team
SET
  name_normalized = 'Westville Jr. High',
  url_path_slug = NULL
WHERE
  id = 1124;

UPDATE
  tournament_team
SET
  team_id = 1124
WHERE
  id IN (1124, 1229, 1412, 1537, 1664);

DELETE FROM
  team
WHERE
  id IN (1229, 1412, 1537, 1664);

----------------------------------------
-- Westville Youth Wrestling

UPDATE
  team
SET
  name_normalized = 'Westville Youth Wrestling',
  url_path_slug = NULL
WHERE
  id = 3922;

UPDATE
  tournament_team
SET
  team_id = 3922
WHERE
  id IN (3922, 4037, 4161, 4293, 4461, 4629, 4756, 4899, 5072, 5246, 5393, 5544, 5684, 5831, 6008, 6187, 6370, 6554, 6737, 6919, 7105, 7293, 7482, 7670, 7859, 8049, 8236, 8423, 8613, 8804, 12659, 12886, 13113, 13340, 13605, 13875, 14145, 14415, 14685, 14955, 15225, 15495);

DELETE FROM
  team
WHERE
  id IN (4037, 4161, 4293, 4461, 4629, 4756, 4899, 5072, 5246, 5393, 5544, 5684, 5831, 6008, 6187, 6370, 6554, 6737, 6919, 7105, 7293, 7482, 7670, 7859, 8049, 8236, 8423, 8613, 8804, 12659, 12886, 13113, 13340, 13605, 13875, 14145, 14415, 14685, 14955, 15225, 15495);

----------------------------------------
-- Wheaton

UPDATE
  team
SET
  name_normalized = 'Wheaton',
  url_path_slug = NULL
WHERE
  id = 171;

UPDATE
  tournament_team
SET
  team_id = 171
WHERE
  id IN (171, 201);

DELETE FROM
  team
WHERE
  id IN (201);

----------------------------------------
-- Wheaton Bulldogs

UPDATE
  team
SET
  name_normalized = 'Wheaton Bulldogs',
  url_path_slug = NULL
WHERE
  id = 2784;

UPDATE
  tournament_team
SET
  team_id = 2784
WHERE
  id IN (2784, 3084, 3678, 3923, 4038, 4294, 4462, 4630);

DELETE FROM
  team
WHERE
  id IN (3084, 3678, 3923, 4038, 4294, 4462, 4630);

----------------------------------------
-- Wheaton Falcons

UPDATE
  team
SET
  name_normalized = 'Wheaton Falcons',
  url_path_slug = NULL
WHERE
  id = 898;

UPDATE
  tournament_team
SET
  team_id = 898
WHERE
  id IN (898, 1008, 1125);

DELETE FROM
  team
WHERE
  id IN (1008, 1125);

----------------------------------------
-- Wheaton Franklin

UPDATE
  team
SET
  name_normalized = 'Wheaton Franklin',
  url_path_slug = NULL
WHERE
  id = 232;

UPDATE
  tournament_team
SET
  team_id = 232
WHERE
  id IN (232, 333, 456);

DELETE FROM
  team
WHERE
  id IN (333, 456);

----------------------------------------
-- Wheaton Franklin WC

UPDATE
  team
SET
  name_normalized = 'Wheaton Franklin WC',
  url_path_slug = NULL
WHERE
  id = 4463;

UPDATE
  tournament_team
SET
  team_id = 4463
WHERE
  id IN (4463, 4631, 4757, 4900);

DELETE FROM
  team
WHERE
  id IN (4631, 4757, 4900);

----------------------------------------
-- Wheaton Monroe Eagles

UPDATE
  team
SET
  name_normalized = 'Wheaton Monroe Eagles',
  url_path_slug = NULL
WHERE
  id = 3799;

UPDATE
  tournament_team
SET
  team_id = 3799
WHERE
  id IN (3799, 3924, 4039, 4162, 4295, 4464, 4632, 4758, 4901, 5073, 5247);

DELETE FROM
  team
WHERE
  id IN (3924, 4039, 4162, 4295, 4464, 4632, 4758, 4901, 5073, 5247);

----------------------------------------
-- Wheaton Tiger WC

UPDATE
  team
SET
  name_normalized = 'Wheaton Tiger WC',
  url_path_slug = NULL
WHERE
  id = 4759;

UPDATE
  tournament_team
SET
  team_id = 4759
WHERE
  id IN (4759, 4902, 5074, 5248, 5394, 5545, 5685, 5832, 11097, 11259, 11421, 11583);

DELETE FROM
  team
WHERE
  id IN (4902, 5074, 5248, 5394, 5545, 5685, 5832, 11097, 11259, 11421, 11583);

----------------------------------------
-- Wheaton WC

UPDATE
  team
SET
  name_normalized = 'Wheaton WC',
  url_path_slug = NULL
WHERE
  id = 4296;

UPDATE
  tournament_team
SET
  team_id = 4296
WHERE
  id IN (4296, 11793, 12006, 12219, 12432, 12660, 12887, 13114, 13341, 13606, 13876, 14146, 14416, 14686, 14956, 15226, 15496);

DELETE FROM
  team
WHERE
  id IN (11793, 12006, 12219, 12432, 12660, 12887, 13114, 13341, 13606, 13876, 14146, 14416, 14686, 14956, 15226, 15496);

----------------------------------------
-- Wheaton Warrenville

UPDATE
  team
SET
  name_normalized = 'Wheaton Warrenville',
  url_path_slug = NULL
WHERE
  id = 1413;

UPDATE
  tournament_team
SET
  team_id = 1413
WHERE
  id IN (1413, 1665, 1797);

DELETE FROM
  team
WHERE
  id IN (1665, 1797);

----------------------------------------
-- Wheaton Warrenville Tiger WC

UPDATE
  team
SET
  name_normalized = 'Wheaton Warrenville Tiger WC',
  url_path_slug = NULL
WHERE
  id = 9360;

UPDATE
  tournament_team
SET
  team_id = 9360
WHERE
  id IN (9360, 9535, 9704, 9871, 10751, 10934);

DELETE FROM
  team
WHERE
  id IN (9535, 9704, 9871, 10751, 10934);

----------------------------------------
-- Wheeling Wildcat WC

UPDATE
  team
SET
  name_normalized = 'Wheeling Wildcat WC',
  url_path_slug = NULL
WHERE
  id = 10752;

UPDATE
  tournament_team
SET
  team_id = 10752
WHERE
  id IN (10752, 10935, 11794, 12007, 12220, 12433, 13607, 13877, 14147, 14417, 14687, 14957, 15227, 15497);

DELETE FROM
  team
WHERE
  id IN (10935, 11794, 12007, 12220, 12433, 13607, 13877, 14147, 14417, 14687, 14957, 15227, 15497);

----------------------------------------
-- Whips WC

UPDATE
  team
SET
  name_normalized = 'Whips WC',
  url_path_slug = NULL
WHERE
  id = 10398;

UPDATE
  tournament_team
SET
  team_id = 10398
WHERE
  id IN (10398, 10569, 11098, 11260);

DELETE FROM
  team
WHERE
  id IN (10569, 11098, 11260);

----------------------------------------
-- Wildcats Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Wildcats Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 12661;

UPDATE
  tournament_team
SET
  team_id = 12661
WHERE
  id IN (12661, 12888, 13115, 13342, 13608, 13878, 14148, 14418, 14688, 14958, 15228, 15498);

DELETE FROM
  team
WHERE
  id IN (12888, 13115, 13342, 13608, 13878, 14148, 14418, 14688, 14958, 15228, 15498);

----------------------------------------
-- Will County Warriors

UPDATE
  team
SET
  name_normalized = 'Will County Warriors',
  url_path_slug = NULL
WHERE
  id = 7106;

UPDATE
  tournament_team
SET
  team_id = 7106
WHERE
  id IN (7106, 7294, 7483, 7671, 7860, 8050, 8237, 8424, 8614, 8805, 8998, 9189, 9361, 9536, 9705, 9872, 10753, 10936);

DELETE FROM
  team
WHERE
  id IN (7294, 7483, 7671, 7860, 8050, 8237, 8424, 8614, 8805, 8998, 9189, 9361, 9536, 9705, 9872, 10753, 10936);

----------------------------------------
-- Will County Warriors WC

UPDATE
  team
SET
  name_normalized = 'Will County Warriors WC',
  url_path_slug = NULL
WHERE
  id = 11422;

UPDATE
  tournament_team
SET
  team_id = 11422
WHERE
  id IN (11422, 11584, 11795, 12008, 12221, 12434, 12662, 12889, 13116, 13343, 13609, 13879, 14149, 14419, 14689, 14959, 15229, 15499);

DELETE FROM
  team
WHERE
  id IN (11584, 11795, 12008, 12221, 12434, 12662, 12889, 13116, 13343, 13609, 13879, 14149, 14419, 14689, 14959, 15229, 15499);

----------------------------------------
-- Williamsville WC

UPDATE
  team
SET
  name_normalized = 'Williamsville WC',
  url_path_slug = NULL
WHERE
  id = 11099;

UPDATE
  tournament_team
SET
  team_id = 11099
WHERE
  id IN (11099, 11261, 11423, 11585, 11796, 12009, 12222, 12435, 13610, 13880, 14150, 14420, 14690, 14960, 15230, 15500);

DELETE FROM
  team
WHERE
  id IN (11261, 11423, 11585, 11796, 12009, 12222, 12435, 13610, 13880, 14150, 14420, 14690, 14960, 15230, 15500);

----------------------------------------
-- Wilmington WC

UPDATE
  team
SET
  name_normalized = 'Wilmington WC',
  url_path_slug = NULL
WHERE
  id = 4760;

UPDATE
  tournament_team
SET
  team_id = 4760
WHERE
  id IN (4760, 11797, 12010, 12223, 12436, 12663, 12890, 13117, 13344, 13611, 13881, 14151, 14421, 14691, 14961, 15231, 15501);

DELETE FROM
  team
WHERE
  id IN (11797, 12010, 12223, 12436, 12663, 12890, 13117, 13344, 13611, 13881, 14151, 14421, 14691, 14961, 15231, 15501);

----------------------------------------
-- Windy City Wrestling Of Notre Dame

UPDATE
  team
SET
  name_normalized = 'Windy City Wrestling Of Notre Dame',
  url_path_slug = NULL
WHERE
  id = 8615;

UPDATE
  tournament_team
SET
  team_id = 8615
WHERE
  id IN (8615, 8806, 8999, 9190, 9362, 9537);

DELETE FROM
  team
WHERE
  id IN (8806, 8999, 9190, 9362, 9537);

----------------------------------------
-- Wolfpak WC

UPDATE
  team
SET
  name_normalized = 'Wolfpak WC',
  url_path_slug = NULL
WHERE
  id = 3681;

UPDATE
  tournament_team
SET
  team_id = 3681
WHERE
  id IN (3681, 3800, 3925, 4040, 4163, 4297, 4465, 4633, 4761, 4903, 5075, 5249, 5395, 5546, 5686, 5833, 6009, 6188, 6371, 6555, 7107, 7295, 8238, 8425, 8616, 8807, 9363, 9538, 10047, 10224, 10399, 10570, 11100, 11262, 11424, 11586, 11798, 12011, 12224, 12437, 12664, 12891, 13118, 13345, 13612, 13882, 14152, 14422, 14692, 14962, 15232, 15502);

DELETE FROM
  team
WHERE
  id IN (3800, 3925, 4040, 4163, 4297, 4465, 4633, 4761, 4903, 5075, 5249, 5395, 5546, 5686, 5833, 6009, 6188, 6371, 6555, 7107, 7295, 8238, 8425, 8616, 8807, 9363, 9538, 10047, 10224, 10399, 10570, 11100, 11262, 11424, 11586, 11798, 12011, 12224, 12437, 12664, 12891, 13118, 13345, 13612, 13882, 14152, 14422, 14692, 14962, 15232, 15502);

----------------------------------------
-- Wolves WC

UPDATE
  team
SET
  name_normalized = 'Wolves WC',
  url_path_slug = NULL
WHERE
  id = 6010;

UPDATE
  tournament_team
SET
  team_id = 6010
WHERE
  id IN (6010, 6189, 6372, 6556, 6738, 6920, 7108, 7296, 7484, 7672, 9364, 9539, 9706, 9873, 10048, 10225, 10400, 10571, 10754, 10937, 11799, 12012, 12225, 12438, 12665, 12892, 13119, 13346, 13613, 13883, 14153, 14423, 14693, 14963, 15233, 15503);

DELETE FROM
  team
WHERE
  id IN (6189, 6372, 6556, 6738, 6920, 7108, 7296, 7484, 7672, 9364, 9539, 9706, 9873, 10048, 10225, 10400, 10571, 10754, 10937, 11799, 12012, 12225, 12438, 12665, 12892, 13119, 13346, 13613, 13883, 14153, 14423, 14693, 14963, 15233, 15503);

----------------------------------------
-- Wolves WC Inc

UPDATE
  team
SET
  name_normalized = 'Wolves WC Inc',
  url_path_slug = NULL
WHERE
  id = 8617;

UPDATE
  tournament_team
SET
  team_id = 8617
WHERE
  id IN (8617, 8808);

DELETE FROM
  team
WHERE
  id IN (8808);

----------------------------------------
-- Wolves WC Inc.

UPDATE
  team
SET
  name_normalized = 'Wolves WC Inc.',
  url_path_slug = NULL
WHERE
  id = 8239;

UPDATE
  tournament_team
SET
  team_id = 8239
WHERE
  id IN (8239, 8426);

DELETE FROM
  team
WHERE
  id IN (8426);

----------------------------------------
-- Wolves Wrestling Club Inc.

UPDATE
  team
SET
  name_normalized = 'Wolves Wrestling Club Inc.',
  url_path_slug = NULL
WHERE
  id = 7861;

UPDATE
  tournament_team
SET
  team_id = 7861
WHERE
  id IN (7861, 8051, 9000, 9191);

DELETE FROM
  team
WHERE
  id IN (8051, 9000, 9191);

----------------------------------------
-- Woodstock Cardinals

UPDATE
  team
SET
  name_normalized = 'Woodstock Cardinals',
  url_path_slug = NULL
WHERE
  id = 1232;

UPDATE
  tournament_team
SET
  team_id = 1232
WHERE
  id IN (1232, 1414);

DELETE FROM
  team
WHERE
  id IN (1414);

----------------------------------------
-- Woodstock Cyclones

UPDATE
  team
SET
  name_normalized = 'Woodstock Cyclones',
  url_path_slug = NULL
WHERE
  id = 9707;

UPDATE
  tournament_team
SET
  team_id = 9707
WHERE
  id IN (9707, 9874);

DELETE FROM
  team
WHERE
  id IN (9874);

----------------------------------------
-- Woodstock Cyclones WC

UPDATE
  team
SET
  name_normalized = 'Woodstock Cyclones WC',
  url_path_slug = NULL
WHERE
  id = 8618;

UPDATE
  tournament_team
SET
  team_id = 8618
WHERE
  id IN (8618, 8809, 12666, 12893, 13120, 13347, 13614, 13884, 14154, 14424, 14694, 14964, 15234, 15504);

DELETE FROM
  team
WHERE
  id IN (8809, 12666, 12893, 13120, 13347, 13614, 13884, 14154, 14424, 14694, 14964, 15234, 15504);

----------------------------------------
-- Wrestling 365

UPDATE
  team
SET
  name_normalized = 'Wrestling 365',
  url_path_slug = NULL
WHERE
  id = 9365;

UPDATE
  tournament_team
SET
  team_id = 9365
WHERE
  id IN (9365, 9540, 9708, 9875, 10049, 10226);

DELETE FROM
  team
WHERE
  id IN (9540, 9708, 9875, 10049, 10226);

----------------------------------------
-- Wrestling Factory

UPDATE
  team
SET
  name_normalized = 'Wrestling Factory',
  url_path_slug = NULL
WHERE
  id = 3926;

UPDATE
  tournament_team
SET
  team_id = 3926
WHERE
  id IN (3926, 4041, 4164, 4298, 4466, 4634, 4762, 4904, 5076, 5250, 5396, 5547, 5687, 5834, 6011, 6190, 6373, 6557, 6739, 6921, 7109, 7297, 7485, 7673, 7862, 8052, 8240, 8427);

DELETE FROM
  team
WHERE
  id IN (4041, 4164, 4298, 4466, 4634, 4762, 4904, 5076, 5250, 5396, 5547, 5687, 5834, 6011, 6190, 6373, 6557, 6739, 6921, 7109, 7297, 7485, 7673, 7862, 8052, 8240, 8427);

----------------------------------------
-- Wrestling Inc

UPDATE
  team
SET
  name_normalized = 'Wrestling Inc',
  url_path_slug = NULL
WHERE
  id = 10050;

UPDATE
  tournament_team
SET
  team_id = 10050
WHERE
  id IN (10050, 10227, 10401, 10572, 10755, 10938);

DELETE FROM
  team
WHERE
  id IN (10227, 10401, 10572, 10755, 10938);

----------------------------------------
-- Wrestling Wolverines

UPDATE
  team
SET
  name_normalized = 'Wrestling Wolverines',
  url_path_slug = NULL
WHERE
  id = 7863;

UPDATE
  tournament_team
SET
  team_id = 7863
WHERE
  id IN (7863, 8053);

DELETE FROM
  team
WHERE
  id IN (8053);

----------------------------------------
-- Ww Tiger WC

UPDATE
  team
SET
  name_normalized = 'Ww Tiger WC',
  url_path_slug = NULL
WHERE
  id = 6001;

UPDATE
  tournament_team
SET
  team_id = 6001
WHERE
  id IN (6001, 6180, 6374, 6558, 6740, 6922, 7110, 7298, 7476, 7664);

DELETE FROM
  team
WHERE
  id IN (6180, 6374, 6558, 6740, 6922, 7110, 7298, 7476, 7664);

----------------------------------------
-- Ww Tigers WC

UPDATE
  team
SET
  name_normalized = 'Ww Tigers WC',
  url_path_slug = NULL
WHERE
  id = 7855;

UPDATE
  tournament_team
SET
  team_id = 7855
WHERE
  id IN (7855, 8045, 8229, 8416, 8605, 8796);

DELETE FROM
  team
WHERE
  id IN (8045, 8229, 8416, 8605, 8796);

----------------------------------------
-- Xtreme Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Xtreme Wrestling Club',
  url_path_slug = 'xtreme-wc'
WHERE
  id = 5397;

UPDATE
  tournament_team
SET
  team_id = 5397
WHERE
  id IN (5397, 5548, 5688, 6012, 6191, 6375, 6559, 6741, 6923, 7111, 7299, 7486, 7674, 7864, 8054, 8241, 8428, 8619, 8810, 9001, 9192, 9366, 9541, 9709, 9876, 10051, 10228, 10402, 10573, 10756, 10939, 11101, 11263, 11425, 11587, 11800, 12013, 12226, 12439, 13615, 13885, 14155, 14425, 14695, 14965, 15235, 15505);

DELETE FROM
  team
WHERE
  id IN (5548, 5688, 6012, 6191, 6375, 6559, 6741, 6923, 7111, 7299, 7486, 7674, 7864, 8054, 8241, 8428, 8619, 8810, 9001, 9192, 9366, 9541, 9709, 9876, 10051, 10228, 10402, 10573, 10756, 10939, 11101, 11263, 11425, 11587, 11800, 12013, 12226, 12439, 13615, 13885, 14155, 14425, 14695, 14965, 15235, 15505);

----------------------------------------
-- Yorkville Wrestling Club

UPDATE
  team
SET
  name_normalized = 'Yorkville Wrestling Club',
  url_path_slug = 'yorkville'
WHERE
  id = 1127;

UPDATE
  tournament_team
SET
  team_id = 1127
WHERE
  id IN (1127, 1233, 1540, 1667, 1798, 1942, 2077, 2934, 3683, 3801, 3927, 4042, 4165, 4299, 4467, 4635, 4763, 4905, 5077, 5251, 5398, 5549, 5689, 5835, 6013, 6192, 6376, 6560, 6742, 6924, 7112, 7300, 7487, 7675, 7865, 8055, 8242, 8429, 8620, 8811, 9002, 9193, 9367, 9542, 9710, 9877, 10052, 10229, 10403, 10574, 10757, 10940, 11102, 11264, 11426, 11588, 11801, 12014, 12227, 12440, 12667, 12894, 13121, 13348, 13616, 13886, 14156, 14426, 14696, 14966, 15236, 15506);

DELETE FROM
  team
WHERE
  id IN (1233, 1540, 1667, 1798, 1942, 2077, 2934, 3683, 3801, 3927, 4042, 4165, 4299, 4467, 4635, 4763, 4905, 5077, 5251, 5398, 5549, 5689, 5835, 6013, 6192, 6376, 6560, 6742, 6924, 7112, 7300, 7487, 7675, 7865, 8055, 8242, 8429, 8620, 8811, 9002, 9193, 9367, 9542, 9710, 9877, 10052, 10229, 10403, 10574, 10757, 10940, 11102, 11264, 11426, 11588, 11801, 12014, 12227, 12440, 12667, 12894, 13121, 13348, 13616, 13886, 14156, 14426, 14696, 14966, 15236, 15506);

----------------------------------------
-- Young Champions

UPDATE
  team
SET
  name_normalized = 'Young Champions',
  url_path_slug = NULL
WHERE
  id = 4166;

UPDATE
  tournament_team
SET
  team_id = 4166
WHERE
  id IN (4166, 5078, 5252, 5399);

DELETE FROM
  team
WHERE
  id IN (5078, 5252, 5399);

----------------------------------------
-- Young Guns

UPDATE
  team
SET
  name_normalized = 'Young Guns',
  url_path_slug = NULL
WHERE
  id = 9711;

UPDATE
  tournament_team
SET
  team_id = 9711
WHERE
  id IN (9711, 9878, 10053, 10230);

DELETE FROM
  team
WHERE
  id IN (9878, 10053, 10230);

----------------------------------------
-- Young Guns Wrestling

UPDATE
  team
SET
  name_normalized = 'Young Guns Wrestling',
  url_path_slug = NULL
WHERE
  id = 9368;

UPDATE
  tournament_team
SET
  team_id = 9368
WHERE
  id IN (9368, 9543);

DELETE FROM
  team
WHERE
  id IN (9543);

----------------------------------------
-- Young Warrior Wrestling Academy

UPDATE
  team
SET
  name_normalized = 'Young Warrior Wrestling Academy',
  url_path_slug = NULL
WHERE
  id = 13617;

UPDATE
  tournament_team
SET
  team_id = 13617
WHERE
  id IN (13617, 13887, 14157, 14427, 14697, 14967, 15237, 15507);

DELETE FROM
  team
WHERE
  id IN (13887, 14157, 14427, 14697, 14967, 15237, 15507);

----------------------------------------
-- Youth Razorbacks

UPDATE
  team
SET
  name_normalized = 'Youth Razorbacks',
  url_path_slug = NULL
WHERE
  id = 1010;

UPDATE
  tournament_team
SET
  team_id = 1010
WHERE
  id IN (1010, 1099);

DELETE FROM
  team
WHERE
  id IN (1099);

----------------------------------------
-- Z'S Team Pena

UPDATE
  team
SET
  name_normalized = 'Z''S Team Pena',
  url_path_slug = NULL
WHERE
  id = 7488;

UPDATE
  tournament_team
SET
  team_id = 7488
WHERE
  id IN (7488, 7676, 7866, 8056, 8243, 8430, 8621, 8812, 9003, 9194, 9369, 9544, 9712, 9879, 10054, 10231);

DELETE FROM
  team
WHERE
  id IN (7676, 7866, 8056, 8243, 8430, 8621, 8812, 9003, 9194, 9369, 9544, 9712, 9879, 10054, 10231);

----------------------------------------
-- Zee-Bee Stingers WC

UPDATE
  team
SET
  name_normalized = 'Zee-Bee Stingers WC',
  url_path_slug = 'zee-bee'
WHERE
  id = 4906;

UPDATE
  tournament_team
SET
  team_id = 4906
WHERE
  id IN (4906, 5079, 5253, 6014, 6193, 6377, 6561, 6743, 6925, 7113, 7301, 7489, 7677, 7867, 8057, 8244, 8431, 8622, 8813, 9004, 9195, 9370, 9545, 10055, 10232);

DELETE FROM
  team
WHERE
  id IN (5079, 5253, 6014, 6193, 6377, 6561, 6743, 6925, 7113, 7301, 7489, 7677, 7867, 8057, 8244, 8431, 8622, 8813, 9004, 9195, 9370, 9545, 10055, 10232);

----------------------------------------
-- Zero Fox Wrestling

UPDATE
  team
SET
  name_normalized = 'Zero Fox Wrestling',
  url_path_slug = 'zero-fox'
WHERE
  id = 13618;

UPDATE
  tournament_team
SET
  team_id = 13618
WHERE
  id IN (13618, 13888, 14158, 14428, 14698, 14968, 15238, 15508);

DELETE FROM
  team
WHERE
  id IN (13888, 14158, 14428, 14698, 14968, 15238, 15508);
