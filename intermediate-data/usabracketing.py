...
_NAME_EXCEPTIONS_2026: dict[tuple[str, str], bracket_utils.Competitor] = {
    (
...
def main_tmp() -> None:
    ...
    for bracket_key, bracket_matches in parsed_matches.items():
        _fix_bracket_top_bottom(bracket_matches)
        raw_matches = bracket_matches.values()
        cleaned_matches = bracket_utils.clean_raw_matches(
            raw_matches, _NAME_EXCEPTIONS_2026
        )

        division, weight = bracket_key
        weight_class = bracket_utils.WeightClass(
            division=division, weight=weight, matches=cleaned_matches
        )
    ...
    path = root / "intermediate-data" / "extracted.2026.json"
