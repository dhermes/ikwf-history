...
NAME_EXCEPTIONS: dict[tuple[str, str], Competitor] = {
    ("ANTHONY DEGANI JR", "CRY"): Competitor(
    ...
    # NOTE: This also fixes a typo on BJ's name (BENARD -> BERNARD)
    ("BENARD FUTRELL II", "HAE"): Competitor(
    ...
    ("JERRY BEMIS III", "OAL"): Competitor(
    ...
    ("NATHAN ST. CLAIR", "HIN"): Competitor(
        first_name="NATHAN", last_name="ST. CLAIR", suffix=None, team="HIN"
    ),
    ("TOM REEDY JR", "MOL"): Competitor(
...
def extract_bracket(weight: int, divison: Literal["senior", "novice"]) -> list[Match]:
    ...
    with open(HERE / "2002" / divison / filename) as file_obj:
        html = file_obj.read()
