...
def _get_matches_for_scores(connection: sqlite3.Connection):
    matches_for_scores_sql = _get_sql("_matches-for-scores.sql")
...
def _parse_scores(result: str, top_win: bool) -> tuple[int, int] | None:
...
def main():
    with sqlite3.connect(HERE / "ikwf.sqlite") as connection:
        ...
        matches = _get_matches_for_scores(connection)

        for match in matches:
            match_id = match["id"]
            scores = _parse_scores(match["result"], match["top_win"])
            if scores is None:
                print(f"  ({match_id}, NULL, NULL),")
            else:
                top_score, bottom_score = scores
                print(f"  ({match_id}, {top_score}, {bottom_score}),")
